...
gpgcheck=0" >> /etc/yum.repos.d/local-daos-group.repo
    ...
    # All of this below is a workaround to not yet having 8.5.2111 images
    # but this should be fleshed out and supported so that we can do testing
    # before we have images available
    # provisionNodes() needs to fall-back and install the version without the
    # point release if it fails to install an image with a point release
    # below and dnf upgrade will take care of the rest
    # ugly hack.  we should find a better way to do this:
    if [[ ${STAGE_NAME:-} != Test\ CentOS\ 8.*\ RPMs ]]; then
        ...
        # hack because we cannot actually use EL8-version: 8.5.2111 yet because it
        # borks provisionNodes()
        version=8.5.2111
        if [ -n "$version" ]; then
