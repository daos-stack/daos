...
retry_cmd 300 clush -B -S -l root -w "$NODESTRING" --copy ci_key* --dest=/tmp/

retry_cmd 300 clush -B -S -l root -w "$NODESTRING" --copy utils/cq/requirements.txt --dest=/tmp/cq-req.txt

retry_cmd 300 clush -B -S -l root -w "$NODESTRING" --copy requirements.txt --dest=/tmp/
