...
sudo dnf install -y "${fused_pkg}" || echo "${fused_pkg} not available"
sudo dnf install -y "${pmdk_pkg}" || echo "${pmdk_pkg} not available"
sudo dnf install -y "${isal_pkg}" || echo "${isal_pkg} not available"
sudo dnf install -y "${isal_crypto_pkg}" || echo "${isal_crypto_pkg} not available"
popd || exit 1
