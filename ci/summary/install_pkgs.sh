...
#     local distro="$4"
#     local repo_url="${ARTIFACTS_URL:-${JENKINS_URL}job/}"daos-stack/job/"$repo"/job/"${branch//\//%252F}"/"$build_number"/artifact/artifacts/$distro-bullseye/
...
add_inst_repo "daos" "${BRANCH_NAME}" "${BUILD_NUMBER}" "${distro}" "true"
