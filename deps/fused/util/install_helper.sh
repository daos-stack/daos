...
if [ "${udevrulesdir}" != "" ]; then
    install -D -m 644 "${MESON_SOURCE_ROOT}/util/udev.rules" \
        "${DESTDIR}${udevrulesdir}/99-fused.rules"
fi

if [ "$initscriptdir" != "" ]; then
    install -D -m 755 "${MESON_SOURCE_ROOT}/util/init_script" \
            "${DESTDIR}${initscriptdir}/fused"

    if test -x /usr/sbin/update-rc.d && test -z "${DESTDIR}"; then
        /usr/sbin/update-rc.d fused start 34 S . start 41 0 6 . || /bin/true
    else
        echo "== FURTHER ACTION REQUIRED =="
        echo "Make sure that your init system will start the ${DESTDIR}${initscriptdir}/init.d/fused init script"
    fi
fi
