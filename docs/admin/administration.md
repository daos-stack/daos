...
| engine\_suicide| INFO\_ONLY| NOTICE| excluded rank suicide detected| Indicates that a DAOS engine rank has performed a self-exclusion (suicide) due to detecting an unrecoverable condition. The rank is automatically restarted by the control plane. | An engine detected an internal condition requiring restart (e.g., unresponsive state, critical error). The control plane monitors for this event and automatically restarts the affected rank. |
