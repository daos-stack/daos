...
|CRT\_EVENT\_DELAY|Delay in seconds before handling each CaRT event. INTEGER. Default to 0 s. A longer delay enables batching of successive CaRT events, leading to fewer pool map changes when multiple engines become unavailable at around the same time.|
