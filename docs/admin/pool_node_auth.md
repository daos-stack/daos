...
- A **node certificate** (`node:<hostname>`) is meant for exactly one
  client machine. The server checks that the certificate's name matches
  the machine name the client's agent reports.
- A **tenant certificate** (`tenant:<name>`) is shared by a group of
  machines. Any node holding the tenant certificate and key may
  connect.

The machine name comes from the client's agent and is not verified. A
node certificate guards against misdeployment, not against a
compromised host; revoking the certificate identity does. Short
hostnames must be unique across the tenants sharing a pool.
...
refuse a staged copy and rejects `--replace` for `--tenant`. To rotate
a tenant, issue under a new name, deploy to every holder, then revoke
the old name. Revoking a name invalidates every certificate issued
under it, including one issued a moment earlier.
...
| Server checks machine name | yes, as reported by the agent | no |
...
Within 30 days of an expiry, or before a certificate's validity
starts, the check passes and marks the line `WARN`:
...
certificate](#reissuing-a-certificate)). A revocation is dated
`pool_cert_max_clock_skew` ahead of the server clock; certificates
dated before it are refused. `Handles evicted: 0` on a node revocation
usually means a misspelled name — revoking an unknown identity succeeds.
...
- Client certificate: `issue --replace` (node); for a tenant, issue
  under a new name, deploy, then revoke the old name.
...
| Parameter | Default | Description |
|---|---|---|
| `pool_cert_max_clock_skew` | `5m` | Clock tolerance for proof-of-possession freshness and certificate NotBefore; revocations are dated this far ahead. Raise (e.g. `10m`) for deployments with poor clock synchronization. Values require a unit suffix. |
