...
Below example, where system was not shutdown properly and that set the mode as dirty on the VOS pool file.
#### pmempool check command ran on the system where the shutdown was not done cleanly
*  -v: More verbose.
```
...
1
```
#### pmempool check repair command ran on the system
*  -v: More verbose.
*  -r: repair the pool.
...
0
```
#### pmempool check command ran on system after repair
```
