...
* DAOS _admin_ nodes must have the same version
...
* The `MAJOR.MINOR` version of DAOS _client_ nodes can differ from
  ...
  For example, clients at version 2.8 can communicate with servers
  at versions 2.6 and 2.8.
  Similarly, servers at version 2.8 can communicate with clients
  at version 2.6 and 2.8.

!!! attention
    TODO: For the `MAJOR` version change from 2.8 to 3.0, are we
    maintaining "N+1" interoperability? Or is the version change from
    2 to 3 breaking interoperability between 2.8 and 3.0?
...
* `global_version=x` was introduced in DAOS 3.0.

!!! attention
    TODO: determine the latest pool global version for 3.0
