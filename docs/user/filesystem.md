...
libioil intercepts streaming IO functions, fread, fwrite, etc. It issues IO request via dfs API immediately for each call. libpil4dfs only intercepts low level read/write. fread, fwrite, fprintf etc. are not explicitly intercepted. The default streaming IO buffer size is 8KB (may vary on different OSes). The low level API read() is called by streaming IO API in case read buffer is empty/depleted. write() is called when the write buffer is full or fclose() is called. Consequently, the number of IO interception reported by libioil and libpil4dfs could be different.
