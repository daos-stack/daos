...
The host hardware must support virtualized device access, and it must be enabled in the system BIOS.
...
HugePages allocated could be checked with the following command:

```bash
$ sysctl vm.nr_hugepages
```

The default size of a huge page, the number of available HugePages, etc. could be found with the
following command:
