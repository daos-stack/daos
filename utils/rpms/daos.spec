...
# this library is only generated by the DAOS build if hdf5 is installed
lib = ""
if [ -e %{_libdir}/libdaos_serialize.so ]; then
    $lib = %{_libdir}/libdaos_serialize.so
fi
echo $lib


#cp %{buildroot}/%{_libdir}/libdaos_serialize.so %{_libdir}/libdaos_serialize.so
s -l %{buildroot}/%{_libdir}/
