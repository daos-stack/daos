...
- On the client side: `ibping -c <count> -C <Card ID> -L <Card Server LID>`
- On the client side: `ibping -c <count> -C <Card ID> -G <Card Server GID>`
- All these infos could be retrieved with `ibstat` and  `ibv_devinfo` and `ibv_devices`
- An helper script `ibping_test.sh` is available in `src/utils/debug/scripts/ibping_test.sh` to
  check the connectivity between several servers and clients.

To test RDMA CM connection:
- client: `ucmatose -s <server ip>`
- server: `ucmatose -b <server ip>`
- An helper script `ibping-test.sh` is available in `src/utils/debug/scripts/ibping-test.sh` to
  check the connectivity between several servers and clients.
- An helper script `ucmatose-test.sh` is available in `src/utils/debug/scripts/ucmatose-test.sh` to
  check the connectivity between several servers and clients.

To test RDMA data transfer with `rping`:
- An helper script `rping-test.sh` is available in `src/utils/debug/scripts/rping-test.sh` to
  check the connectivity between several servers and clients.

To test IB Verbs RC (Reliable Connection) with `ibv_rc_pingpong`:
...
- An helper script `ibv_rc_pingpong-test.sh` is available in
  `src/utils/debug/scripts/ibv_rc_pingpong-test.sh` to check the connectivity between several servers
  and clients.
...
- An helper script `mercury-test.sh` is available in `src/utils/debug/scripts/mercury-test.sh` to
  check the connectivity between several servers and clients.
