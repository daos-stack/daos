...
#
# Usage:
#   finalize-daos-dev.sh [--activate] [-j|--jobs N] [daos-make.sh flags...]
#
# Examples:
#   finalize-daos-dev.sh --force --deps      # full (re)build, isolated (was install-daos.sh)
#   finalize-daos-dev.sh                     # incremental rebuild, isolated (was update-daos.sh)
#   finalize-daos-dev.sh --activate --force --deps  # full rebuild AND make it the live ticket
#
...

{
	...
	EOF
} | ssh "$BUILD_NODE" bash -l -s |& tee "$CWD/finalize-daos-dev.log"
