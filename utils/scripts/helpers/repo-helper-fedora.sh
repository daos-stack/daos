...
disable_repos () {
    ...
    shift
    local save_repos
    IFS=" " read -r -a save_repos <<< "${*:-} daos_ci-fedora${archive}-artifactory"
...
if [ -n "$REPO_FILE_URL" ]; then
    ...
    curl -k --noproxy '*' -sSf -o "daos_ci-fedora${archive}-artifactory.repo"        \
         "{$REPO_FILE_URL}daos_ci-fedora${archive}-artifactory.repo"
