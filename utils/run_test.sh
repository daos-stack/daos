...
if [ -d "/mnt/daos" ]; then
    ...
    if [ -z "$RUN_TEST_VALGRIND" ]; then
        ...
        COMP="UTEST_common"
        BTREE_SIZE=20000
        run_test src/common/tests/btree.sh perf -s ${BTREE_SIZE}
        run_test src/common/tests/btree.sh perf direct -s ${BTREE_SIZE}
        run_test src/common/tests/btree.sh perf ukey -s ${BTREE_SIZE}
        run_test src/common/tests/btree.sh dyn perf -s ${BTREE_SIZE}
        run_test src/common/tests/btree.sh dyn perf ukey -s ${BTREE_SIZE}
    else
