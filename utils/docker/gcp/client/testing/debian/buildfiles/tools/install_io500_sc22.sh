...
+PFIND_HASH=dfs_find
 ...
 INSTALL_DIR=\$PWD
 BIN=\$INSTALL_DIR/bin
@@ -55,11 +55,16 @@ function git_co {
 function get_ior {
   echo "Getting IOR and mdtest"
   git_co https://github.com/hpc/ior.git ior \$IOR_HASH
+  sed -i 's/dfs_cont_create_with_label(poh, o->cont, NULL, NULL, &coh, NULL)/-1/g' "\$BUILD"/ior/src/aiori-DFS.c
+  sed -i 's/dfs_cont_create(poh, co_uuid, NULL, &coh, NULL)/-1/g' "\$BUILD"/ior/src/aiori-DFS.c
+  echo "Printing path of BUILD"
+  echo "\$BUILD"
+  ls "\$BUILD"
 }
...
main() {
  ...
  cat "${MY_IO500_PATH}/prepare.sh"
  echo "find result for debug_setup.sh: "
  find / -name "debug_setup.sh" 2>/dev/null
  echo "contents of ${MY_DAOS_INSTALL_PATH}/include: "
  ls ${MY_DAOS_INSTALL_PATH}/include
  ls ${MY_DAOS_INSTALL_PATH}/include/gurt
