...
that must be present on each host (where the number of pmem devices usually
matches the number of NUMA nodes on each host).
This will define the number of IO servers that will be configured per host.
