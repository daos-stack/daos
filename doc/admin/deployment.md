...
[generate command options]
      ...
      -c, --net-class=[best-available|ethernet|infiniband] Network class
                       preferred, defaults to best available (default:
                       best-available)
```
...
that must be present on each host (and must match the number and NUMA affinity
of matching network interfaces).
This will equal the number of IO servers that will be configured per host.
...
per pmem device (usually matches the number of NUMA nodes on each host).
...
'best-available' will attempt to choose the most performant sets of interfaces
and supported provider that match the number and NUMA affinity of pmem devices.
