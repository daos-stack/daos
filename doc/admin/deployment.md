...
install/etc/daos_server.yaml or pass the location to daos_server on
...
running sudo daos_server prep-nvme -p 4096 -u bob. This will perform
the required setup in order for daos_server to be run by user "bob" who
...
src/control/mgmt/init/setup_spdk.sh (script is installed as
install/share/setup_spdk.sh).
...
prior knowledge of daos_server binary location):

    linuxuser ALL=/home/linuxuser/projects/daos_m/install/bin/daos_server
    prep-nvme*
...
reboot and *daos_server* will exit on completion of the task.
...
invocation of *daos_server* and must be run as root to display all the
...
To display the supported OFI provider, use the following command:
    ...
    # /scratch/standan/daos_m/opt/ofi/bin/fi_info -l
    psm2:
        version: 1.7
    ofi_rxm:
        version: 1.0
...
config_types.go), if not present in the configuration.
...
CRT_PHY_ADDR_STR set, the user os environment will be used instead of
...
variables will be added as specified in the env_vars list within the
...
for the path specified through the -f option of the daos_server command
...
Force different port number to bind daos_server to, this will also be
...
ofi+psm2 for Omni-Path, ofi+verbs;ofi_rxm for Infiniband/RoCE and
finally
...
When bdev_class is set to nvme, bdev_list is the list of unique NVMe
...
When bdev_class is set to file, bdev_list is the list of file paths
...
To start the DAOS server, run:
    ...
    orterun -np &lt;num_servers&gt; --hostfile ${hostfile}
    --enable-recovery --report-uri ${urifile} daos_server
...
allow the daos_server to run with root priviledged on each storage
nodes.
...
suite can be executed:
    ...
    orterun -np <num_clients> --hostfile ${hostfile} --ompi-server file:${urifile} ./daos_test
...
[^3]: https://www.open-mpi.org/faq/?category=running#mpirun-hostfile
