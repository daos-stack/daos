...
communications. A set of self-signed certificates for a given
DAOS system may be generated by running the `gen_certificates.sh` script
provided with the DAOS software if there is not an existing TLS certificate
infrastructure.
...
Self-Signed CA created in ./daosCA
...
...
transport_config:
  ...
  # Name of server according to its certificate
  server_name: server
```
