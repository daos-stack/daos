		  ...
		  const uuid_t pool_uuid)
{
	...
	return daos_control_pool_set_prop(dmg_ctx, NULL, (uuid_t *)pool_uuid, (char *)prop_name,
					  (char *)prop_value);
}
		...
		daos_prop_t *prop, d_rank_list_t *svc, uuid_t uuid)
{
...

	{
		...
		rc = daos_control_pool_create(dmg_ctx, &args);
...
dmg_storage_device_list(const char *dmg_config_file, int *ndisks, device_list *devices)
{
	...
	return daos_control_storage_device_list(dmg_ctx, NULL, ndisks, devices);
}
...
dmg_storage_set_nvme_fault(const char *dmg_config_file, char *host, const uuid_t uuid)
{
	...
	return daos_control_storage_set_nvme_fault(dmg_ctx, host, (uuid_t *)uuid);
}
				...
				size_t stats_len, const uuid_t uuid)
{
	...
	return daos_control_storage_query_device_health(dmg_ctx, host, key, stats, (int)stats_len,
							(uuid_t *)uuid);
}
			...
			const char *subsystems)
{
	...
	return daos_control_server_set_logmasks(dmg_ctx, NULL, (char *)masks, (char *)streams,
						(char *)subsystems);
}
