...
static int
check_query_options_cmp(const void *p1, const void *p2)
{
	const int	*opt1 = p1;
	const int	*opt2 = p2;
...
parse_check_query_report(struct json_object *obj, struct daos_check_report_info *dcri)
{
	...
	/*
	 * The options in the output for "dmg -j check query" are not sorted, but "dmg check repair"
	 * uses the sorted option from "dmg check query". Here, we will sort the options for further
	 * usage for subsequent "dmg check repair". See DAOS-13974.
	 */
	qsort(dcri->dcri_options, dcri->dcri_option_nr, sizeof(dcri->dcri_options[0]),
	      check_query_options_cmp);
