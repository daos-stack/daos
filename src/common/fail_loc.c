...
{
	daos_fail_loc_set(0);
	D_DEBUG(DB_ANY, "*** fail_loc=" DF_X64, daos_fail_loc);
}
...
daos_fail_loc_set(uint64_t fail_loc)
{
	...
	if (attr_in.fa_id >= DAOS_FAIL_MAX_GROUP) {
		D_ERROR("invalid fail_loc " DF_X64 " fa_id: %u", fail_loc, attr_in.fa_id);
		return;
	}
...
daos_shard_fail_value(uint16_t *shards, int nr)
{
	...
	if (nr == 0 || nr > 4) {
		D_ERROR("ignore nr %d, should within [1, 4]", nr);
		return fail_val;
	}
...
daos_fail_init(void)
{
	...
	if (rc != 0) {
		DL_ERROR(rc, "Failed to set fault attr");
		d_fault_inject_fini();
	}
	rc = 0;
