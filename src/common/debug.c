...
DAOS_FOREACH_DB(DD_INSTANTIATE_DB, DAOS_FOREACH_DB)
DAOS_FOREACH_LOG_FAC(DD_INSTANTIATE_FAC, DAOS_FOREACH_DB)
...
debug_fini_locked(void)
{
	...
	rc = DD_REGISTER_DB(DAOS_FOREACH_DB);
...
daos_debug_init(char *logfile)
{
	...
	rc = DD_REGISTER_FAC(DAOS_FOREACH_LOG_FAC);
	if (rc != 0) /* Just print a message but no need to fail */
