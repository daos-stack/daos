...
typedef struct _Mgmt__CheckLeaderReq Mgmt__CheckLeaderReq;
typedef struct _Mgmt__CheckLeaderResp Mgmt__CheckLeaderResp;
...
#define MGMT__CHECK_QUERY_RESP__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mgmt__check_query_resp__descriptor) \
    , 0, CHK__CHECK_INST_STATUS__CIS_INIT, CHK__CHECK_SCAN_PHASE__CSP_PREPARE, NULL, NULL, 0,NULL, 0,NULL, 0 }
...
typedef enum {
  MGMT__CHECK_LEADER_REQ__REQ__NOT_SET = 0,
  MGMT__CHECK_LEADER_REQ__REQ_START_REQ = 2,
  MGMT__CHECK_LEADER_REQ__REQ_STOP_REQ = 3,
  MGMT__CHECK_LEADER_REQ__REQ_SET_POLICY_REQ = 4,
  MGMT__CHECK_LEADER_REQ__REQ_QUERY_REQ = 5
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(MGMT__CHECK_LEADER_REQ__REQ)
} Mgmt__CheckLeaderReq__ReqCase;

/*
 * CheckLeaderReq contains the request to be forwarded to the check leader.
 */
struct  _Mgmt__CheckLeaderReq
{
  ProtobufCMessage base;
  uint32_t drpc_method;
  Mgmt__CheckLeaderReq__ReqCase req_case;
  union {
    Mgmt__CheckStartReq *start_req;
    Mgmt__CheckStopReq *stop_req;
    Mgmt__CheckSetPolicyReq *set_policy_req;
    Mgmt__CheckQueryReq *query_req;
  };
};
#define MGMT__CHECK_LEADER_REQ__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mgmt__check_leader_req__descriptor) \
    , 0, MGMT__CHECK_LEADER_REQ__REQ__NOT_SET, {0} }


typedef enum {
  MGMT__CHECK_LEADER_RESP__RESP__NOT_SET = 0,
  MGMT__CHECK_LEADER_RESP__RESP_DAOS_RESP = 2,
  MGMT__CHECK_LEADER_RESP__RESP_START_RESP = 3,
  MGMT__CHECK_LEADER_RESP__RESP_STOP_RESP = 4,
  MGMT__CHECK_LEADER_RESP__RESP_QUERY_RESP = 5
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(MGMT__CHECK_LEADER_RESP__RESP)
} Mgmt__CheckLeaderResp__RespCase;

/*
 * CheckLeaderResp contains the status of the request and the response generated by the check leader.
 */
struct  _Mgmt__CheckLeaderResp
{
  ProtobufCMessage base;
  uint32_t drpc_status;
  Mgmt__CheckLeaderResp__RespCase resp_case;
  union {
    Mgmt__DaosResp *daos_resp;
    Mgmt__CheckStartResp *start_resp;
    Mgmt__CheckStopResp *stop_resp;
    Mgmt__CheckQueryResp *query_resp;
  };
};
#define MGMT__CHECK_LEADER_RESP__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mgmt__check_leader_resp__descriptor) \
    , 0, MGMT__CHECK_LEADER_RESP__RESP__NOT_SET, {0} }
...
typedef void (*Mgmt__CheckActResp_Closure)
                 (const Mgmt__CheckActResp *message,
                  void *closure_data);
typedef void (*Mgmt__CheckLeaderReq_Closure)
                 (const Mgmt__CheckLeaderReq *message,
                  void *closure_data);
typedef void (*Mgmt__CheckLeaderResp_Closure)
                 (const Mgmt__CheckLeaderResp *message,
                  void *closure_data);
...
extern const ProtobufCMessageDescriptor mgmt__check_leader_req__descriptor;
extern const ProtobufCMessageDescriptor mgmt__check_leader_resp__descriptor;
