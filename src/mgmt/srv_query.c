...
bs_state_query(void *arg)
{
	...
	D_ASSERT(info != NULL);
	D_DEBUG(DB_MGMT, "BIO blobstore state query on xs:%d, tgt:%d", info->dmi_xs_id,
		info->dmi_tgt_id);

	bxc = info->dmi_nvme_ctxt;
	if (bxc == NULL) {
		D_ERROR("BIO NVMe context not initialized for xs:%d, tgt:%d", info->dmi_xs_id,
			info->dmi_tgt_id);
		return;
	}
	...
	if (rc)
		DL_ERROR(rc, "Blobstore query on dev:" DF_UUID " failed",
			 DP_UUID(bs_arg->bs_arg_uuid));
}
...
int ds_mgmt_get_bs_state(uuid_t bs_uuid, int *bs_state)
{
	...
	if (!uuid_is_null(bs_uuid)) {
		...
		if (rc != 0) {
			D_ERROR("Blobstore UUID:" DF_UUID " not found", DP_UUID(bs_uuid));
			return rc;
		}
...
bio_health_query(void *arg)
{
	...
	D_ASSERT(info != NULL);
	D_DEBUG(DB_MGMT, "BIO health stats query on xs:%d, tgt:%d", info->dmi_xs_id,
		info->dmi_tgt_id);
...
ds_mgmt_bio_health_query(struct mgmt_bio_health *mbh, uuid_t dev_uuid)
{
	...
	if (rc != 0) {
		D_ERROR("Device UUID:" DF_UUID " not found", DP_UUID(dev_uuid));
		return rc;
	}
	if (dev_info->sdi_tgts == NULL) {
		D_ERROR("No targets mapped to device");
	...
	D_DEBUG(DB_MGMT, "Querying BIO Health Data for dev:" DF_UUID, DP_UUID(dev_uuid));
...
bio_storage_dev_manage_led(void *arg)
{
	...
	if (bxc == NULL) {
		D_ERROR("BIO NVMe context not initialized for xs:%d, tgt:%d", mod_info->dmi_xs_id,
			mod_info->dmi_tgt_id);
		return -DER_INVAL;
	}
...
bio_faulty_state_set(void *arg)
{
	...
	D_ASSERT(info != NULL);
	D_DEBUG(DB_MGMT, "BIO health state set on xs:%d, tgt:%d", info->dmi_xs_id,
		info->dmi_tgt_id);
...
ds_mgmt_dev_set_faulty(uuid_t dev_uuid, Ctl__DevManageResp *resp)
{
	...
	D_DEBUG(DB_MGMT, "Setting FAULTY SMD device state for dev:" DF_UUID, DP_UUID(dev_uuid));
	...
	rc = dss_ult_execute(bio_storage_dev_manage_led, &led_info, NULL, NULL,
			     init_xs_type(), 0, 0);
	if (rc != 0) {
		D_ERROR("FAULT LED state not set on device:" DF_UUID, DP_UUID(dev_uuid));
		if (rc == -DER_NOSYS) {
			led_state = CTL__LED_STATE__NA;
			/* Reset rc for non-VMD case */
			rc = 0;
...
ds_mgmt_dev_replace(uuid_t old_dev_uuid, uuid_t new_dev_uuid, Ctl__DevManageResp *resp)
{
	...
	D_DEBUG(DB_MGMT, "Replacing device:" DF_UUID " with device:" DF_UUID, DP_UUID(old_dev_uuid),
		DP_UUID(new_dev_uuid));
