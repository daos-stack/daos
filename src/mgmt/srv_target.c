...
static int
ds_mgmt_cleanup_newborns(uuid_t uuid, unsigned int flags)
{
	struct tgt_kill_arg arg;
	uuid_copy(arg.pool_uuid, uuid);
	arg.flags = flags;
	return dss_thread_collective(tgt_kill_pool, &arg, 0);
}
...
ds_mgmt_tgt_setup(void)
{
	...
	if (bio_nvme_configured(SMD_DEV_TYPE_META)) {
		struct recreate_tgts_arg recreate_arg = {
		    .bind_cpu_fn         = dss_bind_to_xstream_cpuset,
		    .cleanup_newborns_fn = ds_mgmt_cleanup_newborns,
		    .newborns_path       = newborns_path,
		    .vos_storage_path    = dss_storage_path,
		};
