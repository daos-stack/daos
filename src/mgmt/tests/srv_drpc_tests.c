...
test_drpc_pool_set_prop_byteval_oversize(void **state)
{
	...
	/* Allocate a real buffer that is one byte larger than the cap so
	 * protobuf packing reads valid memory; the cap is enforced inside
	 * daos_prop_entry_set_byteval, not at the wire layer. */
