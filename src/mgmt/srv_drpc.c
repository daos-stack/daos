...
ds_mgmt_drpc_check_set_policy(Drpc__Call *drpc_req, Drpc__Response *drpc_resp)
{
	...
	struct drpc_alloc        alloc = PROTO_ALLOCATOR_INIT(alloc);
	Mgmt__CheckSetPolicyResp resp  = MGMT__CHECK_SET_POLICY_RESP__INIT;
	int                      rc    = 0;
	size_t                   len;
	...
	resp.status = rc;
	len         = mgmt__check_set_policy_resp__get_packed_size(&resp);
	D_ALLOC(body, len);
	if (body == NULL) {
		D_ERROR("Failed to allocate response body (set policy for check)\n");
		drpc_resp->status = DRPC__STATUS__FAILED_MARSHAL;
	} else {
		mgmt__check_set_policy_resp__pack(&resp, body);
		drpc_resp->body.len  = len;
		drpc_resp->body.data = body;
	}
