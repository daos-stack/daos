...
tc_add_req_to_inprog(struct pc_inprogress *pc_inprog, crt_rpc_t *tc_req)
{
	...
	CRT_INIT_LIST_HEAD(&tc_req_item->ptc_link);
	...
	crt_list_add_tail(&tc_req_item->ptc_link,
...
td_add_req_to_pc_inprog(struct pc_inprogress *pc_inprog, crt_rpc_t *td_req)
{
	...
	/* init the pc_req item */
	CRT_INIT_LIST_HEAD(&td_req_item->ptd_link);
	...
	pthread_mutex_lock(&pc_inprog->pc_req_mutex);
	crt_list_add_tail(&td_req_item->ptd_link,
...
pc_inprog_create(struct pc_inprogress **pc_inprog, crt_rpc_t *rpc_req)
{
	...
	CRT_INIT_LIST_HEAD(&pc_inp_item->pc_tc_list);
	...
	CRT_INIT_LIST_HEAD(&pc_inp_item->pc_td_list);
...
pc_inprog_destroy(struct pc_inprogress *pc_inprog)
{
	...
	crt_list_for_each_entry_safe(tc, tc_next, &pc_inprog->pc_tc_list,
				      ptc_link) {
		crt_list_del_init(&tc->ptc_link);
	...
	crt_list_for_each_entry_safe(td, td_next, &pc_inprog->pc_td_list,
				      ptd_link) {
		crt_list_del_init(&td->ptd_link);
		 ...
		 struct dmg_pool_create_in *pc_in2)
{
	...
	if (strcmp(pc_in1->pc_grp, pc_in2->pc_grp) != 0)
		return false;
	if (!crt_rank_list_identical(pc_in1->pc_tgts, pc_in2->pc_tgts, true))
		return false;
...
tgt_create_cb(const struct crt_cb_info *cb_info)
{
	...
	if (rc)
		D_ERROR(DF_UUID": RPC error while creating tgt on rank %d: "
			"%d\n", CP_UUID(pc_inprog->pc_pool_uuid),
	...
	if (rc != 0 || tc_out->tc_rc != 0) {
		...
		crt_list_for_each_entry_safe(tc, tc_next,
					      &pc_inprog->pc_tc_list,
					      ptc_link) {
		...
		D_DEBUG(DF_MGMT, DF_UUID": tgt "DF_UUID" created on rank %d\n",
			CP_UUID(pc_inprog->pc_pool_uuid),
			CP_UUID(tc_out->tc_tgt_uuid), tc_req->cr_ep.ep_rank);
		...
		if (pc_in->pc_tgts == NULL) {
			...
			found = crt_rank_list_find(pc_in->pc_tgts,
...
svc_create_fail:
		D_ERROR(DF_UUID": pool svc setup failed with %d\n",
			CP_UUID(pc_inprog->pc_pool_uuid), rc);
	}
	...
	crt_list_for_each_entry_safe(tc, tc_next, &pc_inprog->pc_tc_list,
		...
		pc_inprog->pc_td_num++;
		opc = DAOS_RPC_OPCODE(DMG_TGT_DESTROY, DAOS_MGMT_MODULE, 1);
		rc = crt_req_create(dss_get_module_info()->dmi_ctx, svr_ep,
				    opc, &td_req);
		if (rc != 0) {
			D_ERROR("crt_req_create(DMG_TGT_DESTROY) failed, "
				"rc: %d.\n", rc);
			pc_inprog->pc_td_ack_num++;
			pc_inprog->pc_td_fail_num++;
			/* decref corresponds to the addref in
		...
		if (rc != 0) {
			D_ERROR("crt_req_send(DMG_TGT_DESTROY) failed, "
				"rc: %d.\n", rc);
			pc_inprog->pc_td_ack_num++;
			pc_inprog->pc_td_fail_num++;
			continue;
...
int
dmgs_hdlr_pool_create(crt_rpc_t *rpc_req)
{
	struct dmg_pool_create_in	*pc_in;
	crt_endpoint_t			svr_ep;
	struct dmg_pool_create_out	*pc_out;
	...
	pc_out->pc_svc = NULL;
	if (pc_in->pc_tgts)
		crt_rank_list_sort(pc_in->pc_tgts);
	...
	D_ALLOC(pc_out->pc_svc->rl_ranks,
		pc_in->pc_svc_nr * sizeof(crt_rank_t));
	...
	svr_ep.ep_grp = NULL;
	svr_ep.ep_tag = 0;
	opc = DAOS_RPC_OPCODE(DMG_TGT_CREATE, DAOS_MGMT_MODULE, 1);
	for (i = 0; i < pc_inprog->pc_tc_num; i++) {
		if (pc_in->pc_tgts == NULL)
			svr_ep.ep_rank = i;
		else
		...
		rc = crt_req_create(dss_get_module_info()->dmi_ctx, svr_ep,
				    opc, &tc_req);
		if (rc != 0) {
			D_ERROR("crt_req_create(DMG_TGT_CREATE) failed, "
				"rc: %d.\n", rc);
		...
		if (rc != 0) {
			D_ERROR("crt_req_send(DMG_TGT_CREATE) failed, "
				"rc: %d.\n", rc);
			pc_inprog->pc_tc_ack_num = pc_inprog->pc_tc_num - i;
			pc_inprog->pc_tc_fail_num = pc_inprog->pc_tc_num - i;
			D_GOTO(out, rc);
...
pd_inprog_create(struct pd_inprogress **pd_inprog, crt_rpc_t *rpc_req)
{
	...
	CRT_INIT_LIST_HEAD(&pd_inp_item->pd_td_list);
...
pd_inprog_destroy(struct pd_inprogress *pd_inprog)
{
	...
	/* cleanup tgt-destroy req list */
	crt_list_for_each_entry_safe(td, td_next, &pd_inprog->pd_td_list,
				      ptd_link) {
		crt_list_del_init(&td->ptd_link);
...
pd_tgt_destroy_cb(const struct crt_cb_info *cb_info)
{
	...
	if (pd_out->pd_rc == 0)
		D_DEBUG(DF_MGMT, "Destroying pool "DF_UUID" succeed.\n",
			CP_UUID(pd_in->pd_pool_uuid));
	else
		D_ERROR("Destroying pool "DF_UUID"failed, rc: %d.\n",
			CP_UUID(pd_in->pd_pool_uuid), pd_out->pd_rc);
...
int
dmgs_hdlr_pool_destroy(crt_rpc_t *rpc_req)
{
	struct dmg_pool_destroy_in	*pd_in;
	crt_endpoint_t			svr_ep;
	struct dmg_pool_destroy_out	*pd_out;
