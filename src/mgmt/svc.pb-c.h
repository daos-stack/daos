...
struct  Mgmt__JoinResp
{
  ...
  uint32_t map_version;
  /*
   * DAOS version that system supported.
   */
  uint32_t daos_version;
};
#define MGMT__JOIN_RESP__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mgmt__join_resp__descriptor) \
    , 0, 0, MGMT__JOIN_RESP__STATE__IN, (char *)protobuf_c_empty_string, 0, 0, 0 }
...
struct  Mgmt__GetAttachInfoResp
{
  ...
  /*
   * Authentication flavors allowed by the server
   */
  size_t n_valid_auth_flavors;
  uint32_t *valid_auth_flavors;
};
#define MGMT__GET_ATTACH_INFO_RESP__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mgmt__get_attach_info_resp__descriptor) \
    , 0, 0,NULL, 0,NULL, NULL, 0, (char *)protobuf_c_empty_string, 0,NULL, 0,NULL, NULL, 0,NULL, 0,NULL }
