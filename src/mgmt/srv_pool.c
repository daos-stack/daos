...
ds_mgmt_pool_svc_create(uuid_t pool_uuid, unsigned int uid, unsigned int gid,
			...
			const char *group, daos_rank_list_t *ranks,
...
ds_mgmt_hdlr_pool_create(crt_rpc_t *rpc_req)
{
	...
	daos_rank_t			*tc_out_ranks;
	uuid_t				*tc_out_uuids;
	crt_group_t			*grp = NULL;
	char				id[DAOS_UUID_STR_SIZE];
	daos_rank_list_t		*rank_list;
	daos_rank_list_t		tmp_rank_list = {0};
	daos_rank_t			ranks_array[TMP_RANKS_ARRAY_SIZE];
...
free:
	...
	if (tgt_uuids != NULL)
		D_FREE(tgt_uuids, ranks_size * sizeof(*tgt_uuids));
