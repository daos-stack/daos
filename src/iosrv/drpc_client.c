...
notify_ras_event(Mgmt__RASEvent *evt)
{
	...
	D_DEBUG(DB_MGMT, "sending RAS event\n");
...
ds_notify_pool_svc_update(uuid_t pool_uuid, d_rank_list_t *svc)
{
	...
	evt.id = "pool_svc_ranks_update";
	evt.severity = (uint32_t)RAS_SEV_INFO;
	evt.type = (uint32_t)RAS_TYPE_STATE_CHANGE;
	evt.msg = "List of pool service replica ranks has been updated.";
	...
	if (evt.hostname == NULL) {
		...
		D_GOTO(out_timestamp, rc = -DER_NOMEM);
	}
	(void)uname(&uts);
	...
	D_DEBUG(DB_MGMT, "&&& RAS EVENT id: [%s] ts: [%s] host: [%s] sev: [%s]"
		...
		evt.id, evt.timestamp, evt.hostname, ras_event_sev2str(evt.severity),
		ras_event_type2str(evt.type), DP_UUID(evt.pool_svc_info->pool_uuid),
		evt.msg);
