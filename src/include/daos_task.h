...
#define DAOS_OPC_LIST 				\
	...
	XY(DAOS_OPC_POOL_CONNECT)		\
	XY(DAOS_OPC_POOL_DISCONNECT)		\
	XY(DAOS_OPC_POOL_EXCLUDE)		\
	XY(DAOS_OPC_POOL_EXCLUDE_OUT)		\
	XY(DAOS_OPC_POOL_ADD)			\
	XY(DAOS_OPC_POOL_QUERY)			\
	XY(DAOS_OPC_POOL_QUERY_INFO)		\
	XY(DAOS_OPC_POOL_LIST_ATTR)		\
	XY(DAOS_OPC_POOL_GET_ATTR)		\
	XY(DAOS_OPC_POOL_SET_ATTR)		\
	XY(DAOS_OPC_POOL_DEL_ATTR)		\
	XY(DAOS_OPC_POOL_STOP_SVC)		\
	XY(DAOS_OPC_POOL_LIST_CONT)		\
	XY(DAOS_OPC_CONT_CREATE)		\
	XY(DAOS_OPC_CONT_OPEN)			\
	XY(DAOS_OPC_CONT_CLOSE)			\
	XY(DAOS_OPC_CONT_DESTROY)		\
	XY(DAOS_OPC_CONT_QUERY)			\
	XY(DAOS_OPC_CONT_SET_PROP)		\
	XY(DAOS_OPC_CONT_UPDATE_ACL)		\
	XY(DAOS_OPC_CONT_DELETE_ACL)		\
	XY(DAOS_OPC_CONT_AGGREGATE)		\
	XY(DAOS_OPC_CONT_ROLLBACK)		\
	XY(DAOS_OPC_CONT_SUBSCRIBE)		\
	XY(DAOS_OPC_CONT_LIST_ATTR)		\
	XY(DAOS_OPC_CONT_GET_ATTR)		\
	XY(DAOS_OPC_CONT_SET_ATTR)		\
	XY(DAOS_OPC_CONT_DEL_ATTR)		\
	XY(DAOS_OPC_CONT_ALLOC_OIDS)		\
	XY(DAOS_OPC_CONT_LIST_SNAP)		\
	XY(DAOS_OPC_CONT_CREATE_SNAP)		\
	XY(DAOS_OPC_CONT_DESTROY_SNAP)		\
	XY(DAOS_OPC_TX_OPEN)			\
	XY(DAOS_OPC_TX_COMMIT)			\
	XY(DAOS_OPC_TX_ABORT)			\
	XY(DAOS_OPC_TX_OPEN_SNAP)		\
	XY(DAOS_OPC_TX_CLOSE)			\
	XY(DAOS_OPC_TX_RESTART)			\
	XY(DAOS_OPC_GAP_0)	/* GAP */	\
	XY(DAOS_OPC_OBJ_QUERY_CLASS)		\
	XY(DAOS_OPC_OBJ_LIST_CLASS)		\
	XY(DAOS_OPC_OBJ_OPEN)			\
	XY(DAOS_OPC_OBJ_CLOSE)			\
	XY(DAOS_OPC_OBJ_PUNCH)			\
	XY(DAOS_OPC_OBJ_PUNCH_DKEYS)		\
	XY(DAOS_OPC_OBJ_PUNCH_AKEYS)		\
	XY(DAOS_OPC_OBJ_QUERY)			\
	XY(DAOS_OPC_OBJ_QUERY_KEY)		\
	XY(DAOS_OPC_OBJ_SYNC)			\
	XY(DAOS_OPC_OBJ_FETCH)			\
	XY(DAOS_OPC_OBJ_UPDATE)			\
	XY(DAOS_OPC_OBJ_LIST_DKEY)		\
	XY(DAOS_OPC_OBJ_LIST_AKEY)		\
	XY(DAOS_OPC_OBJ_LIST_RECX)		\
	XY(DAOS_OPC_OBJ_LIST_OBJ)		\
	XY(DAOS_OPC_ARRAY_CREATE)		\
	XY(DAOS_OPC_ARRAY_OPEN)			\
	XY(DAOS_OPC_ARRAY_CLOSE)		\
	XY(DAOS_OPC_ARRAY_DESTROY)		\
	XY(DAOS_OPC_ARRAY_READ)			\
	XY(DAOS_OPC_ARRAY_WRITE)		\
	XY(DAOS_OPC_ARRAY_PUNCH)		\
	XY(DAOS_OPC_ARRAY_GET_SIZE)		\
	XY(DAOS_OPC_ARRAY_SET_SIZE)		\
	XY(DAOS_OPC_ARRAY_STAT)			\
	XY(DAOS_OPC_KV_OPEN)			\
	XY(DAOS_OPC_KV_CLOSE)			\
	XY(DAOS_OPC_KV_DESTROY)			\
	XY(DAOS_OPC_KV_GET)			\
	XY(DAOS_OPC_KV_PUT)			\
	XY(DAOS_OPC_KV_REMOVE)			\
	XY(DAOS_OPC_KV_LIST)			\
	XY(DAOS_OPC_POOL_FILTER_CONT)		\
	XY(DAOS_OPC_OBJ_KEY2ANCHOR)		\
	XY(DAOS_OPC_CONT_SNAP_OIT_CREATE)	\
	XY(DAOS_OPC_CONT_SNAP_OIT_DESTROY)	\
	XY(DAOS_OPC_PIPELINE_RUN)
