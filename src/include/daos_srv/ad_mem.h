...
int
ad_tx_assign(struct ad_tx *tx, void *addr, daos_size_t size, uint64_t val);
...
/**
 * memset a storage region, save the operation for redo and old memory content for undo.
 */
int
ad_tx_move(struct ad_tx *tx, void *dst, void *src, daos_size_t size);

/** setbit in the bitmap, save the operation for redo and the old bit for undo */
int
ad_tx_setbit(struct ad_tx *tx, void *bmap, int pos, int nbits);

/** clear bit in the bitmap, save the operation for redo and  the old bit for undo */
int
ad_tx_clrbit(struct ad_tx *tx, void *bmap, int pos, int nbits);
