...
D_CASSERT(sizeof(struct dtx_entry) ==
	  offsetof(struct dtx_entry, dte_mbs) + sizeof(struct dtx_memberships *));
...
#define VOS_COND_FETCH_MASK       (VOS_OF_COND_AKEY_FETCH | VOS_OF_COND_DKEY_FETCH)

/** Mask for akey conditionals passed to to the update */
#define VOS_COND_AKEY_UPDATE_MASK (VOS_OF_COND_AKEY_UPDATE | VOS_OF_COND_AKEY_INSERT)

/** Mask for dkey conditionals passed to to the update */
#define VOS_COND_DKEY_UPDATE_MASK (VOS_OF_COND_DKEY_UPDATE | VOS_OF_COND_DKEY_INSERT)

/** Mask for any conditionals passed to to the update */
#define VOS_COND_UPDATE_MASK      (VOS_COND_DKEY_UPDATE_MASK | VOS_COND_AKEY_UPDATE_MASK)

/** Mask for if the update has any conditional update */
#define VOS_COND_UPDATE_OP_MASK   (VOS_OF_COND_DKEY_UPDATE | VOS_OF_COND_AKEY_UPDATE)
...
D_CASSERT((VOS_USE_TIMESTAMPS &
	   (VOS_GET_MAX | VOS_GET_MIN | VOS_GET_DKEY | VOS_GET_AKEY | VOS_GET_RECX)) == 0);
 ...
 */
typedef int (*vos_iter_filter_cb_t)(daos_handle_t ih, vos_iter_desc_t *desc, void *cb_arg,
				    unsigned int *acts);
 ...
 */
typedef int (*vos_iter_cb_t)(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
