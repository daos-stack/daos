...
#define D_VECTOR_SEGMENT_RAW_CAPACITY (4096 - sizeof(D_VECTOR_SEGMENT_HEADER))
...
D_CASSERT(sizeof(struct d_vector_segment) == 4096);
...
d_vector_segment_alloc(d_vector_t *dv)
{
	...
	dvs->dvs_capacity   = dv->dv_capacity;
	return dvs;
}
...
d_vector_init(size_t entry_size, d_vector_t *dv)
{
	...
	dv->dv_capacity   = D_VECTOR_SEGMENT_RAW_CAPACITY / entry_size;
