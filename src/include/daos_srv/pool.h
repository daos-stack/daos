...
struct ds_pool {
	...
	uint32_t              sp_stopping : 1, sp_fetch_hdls : 1, sp_need_discard : 1;
...
int
ds_pool_svc_upgrade(uuid_t pool_uuid, d_rank_list_t *ranks);
int
ds_pool_failed_add(uuid_t uuid, int rc);
void
ds_pool_failed_remove(uuid_t uuid);
int
ds_pool_failed_lookup(uuid_t uuid);
...
int
ds_pool_get_tgt_idx_by_state(const uuid_t pool_uuid, unsigned int status, int **tgts,
...
ds_pool_svc_check_evict(uuid_t pool_uuid, d_rank_list_t *ranks, uuid_t *handles, size_t n_handles,
			uint32_t destroy, uint32_t force, char *machine, uint32_t *count);
...
int
ds_pool_tgt_discard(uuid_t pool_uuid, uint64_t epoch);
