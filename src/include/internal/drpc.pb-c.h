...
typedef enum _Drpc__Status {
	/*
	 * The method executed and provided a response payload, if needed. Otherwise, the method
	 * simply succeeded.
	 */
	 ...
	 */
	DRPC__STATUS__FAILED_MARSHAL = 7 PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(DRPC__STATUS)
} Drpc__Status;
...
struct _Drpc__Response {
	...
	int64_t             sequence;
	/*
	 * High-level status of the RPC. If SUCCESS, method-specific status may be included in the
	 * body.
	 */
	Drpc__Status        status;
	/*
...
/* Drpc__Call methods */
void
drpc__call__init(Drpc__Call *message);
size_t
drpc__call__get_packed_size(const Drpc__Call *message);
size_t
drpc__call__pack(const Drpc__Call *message, uint8_t *out);
size_t
drpc__call__pack_to_buffer(const Drpc__Call *message, ProtobufCBuffer *buffer);
Drpc__Call *
drpc__call__unpack(ProtobufCAllocator *allocator, size_t len, const uint8_t *data);
void
drpc__call__free_unpacked(Drpc__Call *message, ProtobufCAllocator *allocator);
/* Drpc__Response methods */
void
drpc__response__init(Drpc__Response *message);
size_t
drpc__response__get_packed_size(const Drpc__Response *message);
size_t
drpc__response__pack(const Drpc__Response *message, uint8_t *out);
size_t
drpc__response__pack_to_buffer(const Drpc__Response *message, ProtobufCBuffer *buffer);
Drpc__Response *
drpc__response__unpack(ProtobufCAllocator *allocator, size_t len, const uint8_t *data);
void
drpc__response__free_unpacked(Drpc__Response *message, ProtobufCAllocator *allocator);
/* --- per-message closures --- */

typedef void (*Drpc__Call_Closure)(const Drpc__Call *message, void *closure_data);
typedef void (*Drpc__Response_Closure)(const Drpc__Response *message, void *closure_data);
