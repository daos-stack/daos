...
int
ds_cont_svc_init(struct cont_svc **svcp, const uuid_t pool_uuid, uint64_t id, struct ds_rsvc *rsvc);
void
...
int
ds_cont_svc_set_prop(uuid_t pool_uuid, uuid_t cont_uuid, d_rank_list_t *ranks, daos_prop_t *prop);
int
ds_cont_list(uuid_t pool_uuid, struct daos_pool_cont_info **conts, uint64_t *ncont);
int
ds_cont_filter(uuid_t pool_uuid, daos_pool_cont_filter_t *filt, struct daos_pool_cont_info2 **conts,
	       uint64_t *ncont);
int
...
ds_cont_tgt_open(uuid_t pool_uuid, uuid_t cont_hdl_uuid, uuid_t cont_uuid, uint64_t flags,
		 uint64_t sec_capas, uint32_t status_pm_ver);
/*
...
struct ds_cont_child {
	...
	uint32_t              sc_dtx_resyncing : 1, sc_dtx_reindex : 1, sc_dtx_reindex_abort : 1,
	    sc_props_fetched : 1, sc_stopping : 1, sc_vos_agg_active : 1, sc_ec_agg_active : 1,
	    sc_scrubbing : 1, sc_discarding : 1;
...
typedef int (*cont_aggregate_cb_t)(struct ds_cont_child *cont, daos_epoch_range_t *epr,
...
int
ds_cont_close_by_pool_hdls(uuid_t pool_uuid, uuid_t *pool_hdls, int n_pool_hdls, crt_context_t ctx);
int
...
int
ds_cont_get_props(struct cont_props *cont_props, uuid_t pool_uuid, uuid_t cont_uuid);
...
int
ds_cont_child_open_create(uuid_t pool_uuid, uuid_t cont_uuid, struct ds_cont_child **cont);
...
int
ds_cont_fetch_snaps(struct ds_iv_ns *ns, uuid_t cont_uuid, uint64_t **snapshots, int *snap_count);

/** revoke all cached snapshot epochs */
int
ds_cont_revoke_snaps(struct ds_iv_ns *ns, uuid_t cont_uuid, unsigned int shortcut,
...
int
dsc_cont_open(daos_handle_t poh, uuid_t cont_uuid, uuid_t cont_hdl_uuid, unsigned int flags,
	      daos_handle_t *coh);
int
...
int
ds_cont_ec_eph_insert(struct ds_pool *pool, uuid_t cont_uuid, int tgt_idx, uint64_t **epoch_p);
int
