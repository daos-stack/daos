...
sgl_test_forward(d_sg_list_t *sgl, struct daos_sgl_idx *sgl_idx, uint64_t bytes)
{
	...
	if (sgl_idx->iov_offset + bytes > sgl->sg_iovs[sgl_idx->iov_idx].iov_len) {
		sgl_idx->iov_idx++;
		sgl_idx->iov_offset = 0;
	}
...
#define DP_IOM(m)  (m)->iom_nr, DP_RECX((m)->iom_recx_lo), DP_RECX((m)->iom_recx_hi)
...
int
daos_array_sort(void *array, unsigned int len, bool unique, daos_sort_ops_t *ops);
int
daos_array_find(void *array, unsigned int len, uint64_t key, daos_sort_ops_t *ops);
int
daos_array_find_le(void *array, unsigned int len, uint64_t key, daos_sort_ops_t *ops);
int
daos_array_find_ge(void *array, unsigned int len, uint64_t key, daos_sort_ops_t *ops);

void
daos_array_shuffle(void *arr, unsigned int len, daos_sort_ops_t *ops);

int
daos_sgls_copy_ptr(d_sg_list_t *dst, int dst_nr, d_sg_list_t *src, int src_nr);
int
daos_sgls_copy_data_out(d_sg_list_t *dst, int dst_nr, d_sg_list_t *src, int src_nr);
int
 ...
 */
bool
daos_sgl_get_bytes(d_sg_list_t *sgl, bool check_buf, struct daos_sgl_idx *idx, size_t buf_len_req,
...
daos_sgl_processor(d_sg_list_t *sgl, bool check_buf, struct daos_sgl_idx *idx,
		   size_t requested_bytes, daos_sgl_process_cb process_cb, void *cb_args);
...
enum { DAOS_FAIL_UNIT_TEST_GROUP = 1, DAOS_FAIL_SYS_TEST_GROUP = 2, DAOS_FAIL_MAX_GROUP };
...
#define DAOS_FAIL_GROUP_GET(fail_loc)        ((fail_loc & DAOS_FAIL_GROUP_MASK) >> DAOS_FAIL_GROUP_SHIFT)
...
#define DAOS_SHARD_OBJ_UPDATE_TIMEOUT_SINGLE (DAOS_FAIL_UNIT_TEST_GROUP_LOC | 0x07)
...
#define DAOS_POOL_FAIL_MAP_REFRESH_SERIOUSLY (DAOS_FAIL_UNIT_TEST_GROUP_LOC | 0x73)
...
static inline int
__is_po2(unsigned long long val)
{
	return !(val & (val - 1));
}
...
#define DAOS_RECX_COVERED(recx_1, recx_2)                                                          \
	(((recx_1).rx_idx >= (recx_2).rx_idx) && (DAOS_RECX_END(recx_1) <= DAOS_RECX_END(recx_2)))
...
crt_init_options_t *
daos_crt_init_opt_get(bool server, int crt_nr);

int
crt_proc_struct_dtx_id(crt_proc_t proc, crt_proc_op_t proc_op, struct dtx_id *dti);
int
crt_proc_daos_prop_t(crt_proc_t proc, crt_proc_op_t proc_op, daos_prop_t **data);
int
crt_proc_struct_daos_acl(crt_proc_t proc, crt_proc_op_t proc_op, struct daos_acl **data);
...
int
daos_prop_entry_copy(struct daos_prop_entry *entry, struct daos_prop_entry *entry_dup);
daos_recx_t *
...
daos_parse_ctype(const char *string, daos_cont_layout_t *type)
{
	...
	else if (strcasecmp(string, "DATABASE") == 0 || strcasecmp(string, "DB") == 0)
		*type = DAOS_PROP_CO_LAYOUT_DATABASE;
	else if (strcasecmp(string, "ROOT") == 0 || strcasecmp(string, "RNTuple") == 0)
		*type = DAOS_PROP_CO_LAYOUT_ROOT;
	else if (strcasecmp(string, "SEISMIC") == 0 || strcasecmp(string, "DSG") == 0)
		*type = DAOS_PROP_CO_LAYOUT_SEISMIC;
	else if (strcasecmp(string, "METEO") == 0 || strcasecmp(string, "FDB") == 0)
