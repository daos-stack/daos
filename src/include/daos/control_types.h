...
struct daos_control_pool_create_args {
	...
	daos_prop_t   *prop;
	/**
	 * [in]  svc->rl_nr is the requested number of service replicas
	 *       (equivalent to dmg --nsvc); 0 lets the server choose.
	 * [out] svc->rl_nr is the actual replica count; svc->rl_ranks[0..rl_nr)
	 *       is the service rank list.
	 */
	d_rank_list_t *svc;
