...
daos_obj_is_srank(daos_obj_id_t oid)
{
	...
	return oc == DAOS_OC_R3S_SPEC_RANK || oc == DAOS_OC_R1S_SPEC_RANK ||
	       oc == DAOS_OC_R2S_SPEC_RANK || oc == DAOS_OC_EC_K2P1_SPEC_RANK_L32K ||
...
daos_oclass_id_t
daos_obj_get_oclass(daos_handle_t coh, daos_ofeat_t ofeats, daos_oclass_hints_t hints,
		    uint32_t args);
...
static inline void
daos_obj_set_oid(daos_obj_id_t *oid, daos_ofeat_t ofeats, daos_oclass_id_t cid, uint32_t args)
{
	uint64_t hdr;
...
daos_oid_is_oit(daos_obj_id_t oid)
{
	...
	return (oc == DAOS_OC_OIT_RF0) || (oc == DAOS_OC_OIT_RF1) || (oc == DAOS_OC_OIT_RF2) ||
	       (oc == DAOS_OC_OIT_RF3) || (oc == DAOS_OC_OIT_RF4);
}
...
daos_obj_generate_oid_by_rf(daos_handle_t poh, uint64_t rf_factor, daos_obj_id_t *oid,
			    daos_ofeat_t ofeats, daos_oclass_id_t cid, daos_oclass_hints_t hints,
			    uint32_t args);
...
daos_recx_ep_list_dump(struct daos_recx_ep_list *lists, unsigned int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		D_PRINT("daos_recx_ep_list[%d], nr %d,total %d,ep_valid %d:\n", i, list->re_nr,
			list->re_total, list->re_ep_valid);
		for (j = 0; j < list->re_nr; j++) {
			recx_ep = &list->re_items[j];
			D_PRINT("[[" DF_U64 "," DF_U64 "], " DF_X64 "]  ", recx_ep->re_recx.rx_idx,
				recx_ep->re_recx.rx_nr, recx_ep->re_ep);
		}
		D_PRINT("\n");
	}
}
