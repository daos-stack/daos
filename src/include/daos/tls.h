...
struct daos_thread_local_storage *dss_tls_get(void);
struct daos_thread_local_storage *dc_tls_get(unsigned int tag);

int ds_tls_key_create(void);
 ...
 */
static inline void *
daos_module_key_get(struct daos_thread_local_storage *dtls,
		    struct daos_module_key *key)
{
...
void daos_register_key(struct daos_module_key *key);
void daos_unregister_key(struct daos_module_key *key);
struct daos_thread_local_storage * dc_tls_init(int tag, uint32_t pid);
void dc_tls_fini(void);
struct daos_thread_local_storage * dss_tls_init(int tag, int xs_id, int tgt_id);
void dss_tls_fini(struct daos_thread_local_storage *dtls);
