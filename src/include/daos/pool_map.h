...
#define DP_TARGET(t) t->ta_comp.co_id, t->ta_comp.co_rank, t->ta_comp.co_index, t->ta_comp.co_status
...
gen_pool_buf(struct pool_map *map, struct pool_buf **map_buf_out, int map_version, int ndomains,
	     int nnodes, int ntargets, const uint32_t *domains, uuid_t target_uuids[],
	     const d_rank_list_t *target_addrs, uuid_t **uuids_out, uint32_t dss_tgt_nr);
