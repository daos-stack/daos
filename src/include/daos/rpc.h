...
static const struct daos_protocol_table daos_rpc_protocol_tables[] = {
    {/* Latest protocol */
     ...
     .versions = {[DAOS_VOS_MODULE]      = DAOS_VOS_VERSION,
		  [DAOS_MGMT_MODULE]     = DAOS_MGMT_VERSION,
		  [DAOS_POOL_MODULE]     = DAOS_POOL_VERSION,
		  [DAOS_CONT_MODULE]     = DAOS_CONT_VERSION,
		  [DAOS_OBJ_MODULE]      = DAOS_OBJ_VERSION,
		  [DAOS_REBUILD_MODULE]  = DAOS_REBUILD_VERSION,
		  [DAOS_RSVC_MODULE]     = DAOS_RSVC_VERSION,
		  [DAOS_RDB_MODULE]      = DAOS_RDB_VERSION,
		  [DAOS_RDBT_MODULE]     = DAOS_RDBT_VERSION,
		  [DAOS_SEC_MODULE]      = DAOS_SEC_VERSION,
		  [DAOS_DTX_MODULE]      = DAOS_DTX_VERSION,
		  [DAOS_PIPELINE_MODULE] = DAOS_PIPELINE_VERSION,
		  [DAOS_CHK_MODULE]      = DAOS_CHK_VERSION}},
    /* Please update DAOS_VERSION_PROTOCOL - 1 table when rolling upgrade is supported.
    {
	.protocol = DAOS_VERSION_PROTOCAL - 1;
	.versions = {
		[DAOS_VOS_MODULE]	= DAOS_VOS_VERSION,
		[DAOS_MGMT_MODULE]	= DAOS_MGMT_VERSION,
		[DAOS_POOL_MODULE]	= DAOS_POOL_VERSION,
		[DAOS_CONT_MODULE]	= DAOS_CONT_VERSION,
		[DAOS_OBJ_MODULE]	= DAOS_OBJ_VERSION,
