...
sgl_move_forward(d_sg_list_t *sgl, struct daos_sgl_idx *sgl_idx, uint64_t bytes)
{
	...
	D_DEBUG(DB_TRACE,
		"Moving sgl index formward by %lu bytes."
		"Idx: " DF_SGL_IDX "\n",
...
#define DF_KEY      "[%d] '%.*s'"
#define DP_KEY(key) (int)(key)->iov_len, (int)(key)->iov_len, daos_key2str(key)
#endif
...
static inline int
daos_gettime_coarse(uint64_t *time)
{
	struct timespec now;
	int             rc;
...
#define daos_iov_left(sgl, iov_idx, iov_off) ((sgl)->sg_iovs[iov_idx].iov_len - (iov_off))
...
{
	return err == -DER_HG || err == -DER_ADDRSTR_GEN || err == -DER_PMIX || err == -DER_UNREG ||
	       err == -DER_UNREACH || err == -DER_CANCELED || err == -DER_NOREPLY ||
	       err == -DER_OOG;
}
