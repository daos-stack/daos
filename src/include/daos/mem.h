...
struct umem_cache {
	...
	/** Waitqueue for umem_cache_reserve() callers */
	void			*ca_reserve_wq;
...
/** Reserve one free memory pages for the potential non-evictable page growing. It's usually
 *  called before starting a transaction, and the transaction might need to alloc space from
 *  empty non-evictable zone which are not mapped yet.
 *
 * \param[in]	store	The umem store
 *
 * \return 0 on success
