...
#define D_ALLOC_CORE_NZ(ptr, size, count)                                                          \
	do {                                                                                       \
		...
		D_CHECK_ALLOC(malloc, true, ptr, #ptr, size, count, #count, 0);                    \
	} while (0)

#define D_STRNDUP(ptr, s, n)                                                                       \
	do {                                                                                       \
		(ptr) = d_strndup(s, n);                                                           \
		D_CHECK_ALLOC(strndup, true, ptr, #ptr, strnlen(s, n) + 1, 0, #ptr, 0);            \
	} while (0)
...
#define D_STRNDUP_S(ptr, s)                                                                        \
	do {                                                                                       \
		...
		D_CHECK_ALLOC(strndup, true, ptr, #ptr, sizeof(s), 0, #ptr, 0);                    \
	} while (0)
...
#define D_ASPRINTF(ptr, ...)                                                                       \
	do {                                                                                       \
		...
		D_CHECK_ALLOC(asprintf, _rc != -1, ptr, #ptr, _rc + 1, 0, #ptr, (ptr) = NULL);     \
	} while (0)
...
#define D_ALIGNED_ALLOC(ptr, alignment, size)                                                      \
	do {                                                                                       \
		(ptr) = (__typeof__(ptr))d_aligned_alloc(alignment, size);                         \
...
#define D_REALLOC_COMMON(newptr, oldptr, oldsize, size, cnt)                                       \
	do {                                                                                       \
		...
		if ((newptr) != NULL) {                                                            \
			if (_cnt <= 1)                                                             \
				D_DEBUG(DB_MEM,                                                    \
					"realloc '" #newptr "':%zu at %p old '" #oldptr            \
					"':%zu at %p.\n",                                          \
					_esz, (newptr), _oldsz, (oldptr));                         \
			else                                                                       \
				D_DEBUG(DB_MEM,                                                    \
					"realloc '" #newptr "':%zu * '" #cnt                       \
					"':%zu at %p old '" #oldptr "':%zu at %p.\n",              \
					_esz, _cnt, (newptr), _oldsz, (oldptr));                   \
			(oldptr) = NULL;                                                           \
			if (_oldsz < _sz)                                                          \
				memset((char *)(newptr) + _oldsz, 0, _sz - _oldsz);                \
			break;                                                                     \
		}                                                                                  \
		if (_cnt <= 1)                                                                     \
...
#define D_REALLOC(newptr, oldptr, oldsize, size) D_REALLOC_COMMON(newptr, oldptr, oldsize, size, 1)

#define D_REALLOC_ARRAY(newptr, oldptr, oldcount, count)                                           \
	D_REALLOC_COMMON(newptr, oldptr, (oldcount) * sizeof(*(oldptr)), sizeof(*(oldptr)), count)

#define D_REALLOC_NZ(newptr, oldptr, size) D_REALLOC_COMMON(newptr, oldptr, size, size, 1)

#define D_REALLOC_ARRAY_NZ(newptr, oldptr, count)                                                  \
	D_REALLOC_COMMON(newptr, oldptr, (count) * sizeof(*(oldptr)), sizeof(*(oldptr)), count)
...
#define D_REALLOC_Z(newptr, oldptr, size) D_REALLOC_COMMON(newptr, oldptr, 0, size, 1)
...
int
d_rank_list_merge(d_rank_list_t *src_set, d_rank_list_t *merge_set);
d_rank_list_t *
...
void
d_getenv_int(const char *env, unsigned int *int_val);
int
