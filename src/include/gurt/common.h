 ...
 * \return error code
 */
#define D_VEC_DECLARE_INIT(name, type) int name##_init(struct name *vec, uint32_t cap)
 ...
 * \param[in]	vec	vector
 */
#define D_VEC_DECLARE_FINI(name, type) void name##_fini(struct name *vec)
 ...
 * \return error code
 */
#define D_VEC_DECLARE_APPEND(name, type, elem) int name##_append(struct name *vec, type elem)
...
#define D_VEC_DECLARE_DELETE_AT(name, type)                                                        \
	int name##_delete_at(struct name *vec, int i, unsigned int flags)
 ...
 *
 *   <\a name>_init(vec, cap)
 *   <\a name>_fini(vec)
 *   <\a name>_append(vec, <\a elem>)
 *   <\a name>_delete_at(vec, i, flags)
 *
 * If \a elem is elem, the append function takes a \a type elem; if \a elem is
 * *elem, the append function takes a \a type *elem.
 *
 * \param[in]	name	of the vector type
 * \param[in]	type	of elements
 * \param[in]	elem	parameter form for <\a name>_append
 */
...
D_VEC_DECLARE(d_vec_pointers, void *, elem)
...
D_VEC_DECLARE(d_vec_ref_tracker_rec, struct d_ref_tracker_rec, *elem);

typedef uint64_t (*d_ref_tracker_cb_get_time_t)(void);
typedef uint64_t (*d_ref_tracker_cb_get_thread_t)(void);
...
struct d_ref_tracker {
	...
	d_ref_tracker_cb_get_time_t   rft_get_time;
	d_ref_tracker_cb_get_thread_t rft_get_thread;
};

#ifdef DAOS_WITH_REF_TRACKER
void d_ref_tracker_init(struct d_ref_tracker *tracker, d_ref_tracker_cb_get_time_t get_time,
...
void d_ref_tracker_set_user(struct d_ref_tracker *tracker, void *addr, int user);
#else
#define d_ref_tracker_init(tracker, get_time, get_thread)		do {} while (0)
...
#define d_ref_tracker_set_user(tracker, addr, user)			do {} while (0)
#endif
