...
dtx_tls_init(int tags, int xs_id, int tgt_id)
{
	...
	rc = d_tm_add_metric(
	    &tls->dt_dtx_leader_total, D_TM_GAUGE, "total number of leader dtx in cache", "entry",
	    "mem/dtx/dtx_leader_handle_%u/tgt_%u", sizeof(struct dtx_leader_handle), tgt_id);
	if (rc != DER_SUCCESS)
	...
	rc = d_tm_add_metric(&tls->dt_dtx_entry_total, D_TM_GAUGE,
			     ...
			     "mem/dtx/dtx_entry_%u/tgt_%u", sizeof(struct dtx_entry), tgt_id);
	if (rc != DER_SUCCESS)
