...
/**
 * Anytime when the server (re)start, the dtx_guard_epoch will be set as
 ...
 */
static daos_epoch_t dtx_guard_epoch;

daos_epoch_t
dtx_get_guard_epoch(void)
{
	return dtx_guard_epoch;
}

static void
dtx_set_guard_epoch(void)
{
	/* XXX: It is a temporary solution, just set the dtx_guard_epoch
	 *	as current server local HLC when (re-)start plus the hlc
	 *	epsilon. If the hlc epsilon is maintained well, then the
	 *	dtx_guard_epoch may be a bit higher than the theoretical
	 *	system level highest HLC at that time.
	 */
	dtx_guard_epoch = crt_hlc_get() + crt_hlc_epsilon_get();
}
...
dtx_handler(crt_rpc_t *rpc)
{
	...
	case DTX_CHECK:
		 ...
		 */
		if (din->di_epoch != 0 &&
		    din->di_epoch <= dtx_get_guard_epoch())
