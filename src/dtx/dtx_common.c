		    ...
		    dtx_agg_cb_t agg_cb, int allow_failure, void *func_arg)
{
	...
	if (sub_cnt > dtx_disp_step_length) {
		dlh->dlh_forward_cnt = dtx_disp_step_length;
		dlh->dlh_agg_cb = NULL;
	} else {
...
exec:
	...
	if (sub_cnt > 0) {
		...
		D_INFO("More dispatch sub-requests for "
		       DF_DTI", normal %u, delay %u, idx %u, cnt %d, allow_failure %d\n",
	...
	D_INFO("Delay dispatch sub-requests for "
	       DF_DTI", normal %u, delay %u, cnt %d, allow_failure %d\n",
	       DP_DTI(&dlh->dlh_handle.dth_xid), dlh->dlh_normal_sub_cnt,
	       dlh->dlh_delay_sub_cnt, dlh->dlh_forward_cnt, allow_failure);
