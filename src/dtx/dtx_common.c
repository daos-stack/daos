...
dtx_reindex_ult(void *arg)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": starting DTX reindex ULT on xstream %d\n",
	...
	while (!cont->sc_dtx_reindex_abort && !dss_xstream_exiting(dmi->dmi_xstream)) {
		...
		if (rc > 0) {
			D_DEBUG(DB_DSMS, DF_CONT": DTX reindex done\n",
	...
	D_DEBUG(DB_DSMS, DF_CONT": stopping DTX reindex ULT on stream %d\n",
		DP_CONT(NULL, cont->sc_uuid), dmi->dmi_tgt_id);
