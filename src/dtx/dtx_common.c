		 ...
		 struct dtx_memberships *mbs, struct dtx_leader_handle **p_dlh)
{
	...
	if (rc != 0) {
		...
		*p_dlh = dlh;
		d_tm_inc_counter(tls->dt_dtx_leader_total, 1);
	}
...
dtx_entry_put(struct dtx_entry *dte)
{
	if (--(dte->dte_refs) == 0) {
		...
		d_tm_dec_counter(tls->dt_dtx_entry_total, 1);
		D_FREE(dte);
	}
}
...
dtx_leader_end(struct dtx_leader_handle *dlh, struct ds_cont_hdl *coh, int result)
{
	...
	dte->dte_refs = 1;
	dte->dte_mbs = mbs;
	d_tm_inc_counter(tls->dt_dtx_entry_total, 1);
...
out:
	...
	D_FREE(dth->dth_oid_array);
	D_FREE(dlh);
	d_tm_dec_counter(tls->dt_dtx_leader_total, 1);
