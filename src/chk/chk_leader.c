		...
		if (notify) {
			...
			if (rc != 0)
				D_ERROR(DF_LEADER" failed to notify the engines that "
					"the pool "DF_UUID" got failure: "DF_RC"\n",
...
out:
	if (rc != 0) {
		D_ERROR(DF_LEADER" failed to build pool service clues for "DF_UUID": "DF_RC"\n",
...
out:
	if (rc != 0) {
		D_ERROR(DF_LEADER" failed to destroy other pool service replicas for "DF_UUID
			": "DF_RC"\n", DP_LEADER(ins), DP_UUID(cpr->cpr_uuid), DP_RC(rc));
...
report:
	...
	D_CDEBUG(result != 0 || rc != 0, DLOG_ERR, DLOG_INFO,
		 DF_LEADER" detects dangling pool "DF_UUID", action %u, seq "
		 DF_X64", handle_rc %d, report_rc %d, decision %d\n",
		 DP_LEADER(ins), DP_UUID(uuid), act, seq, result, rc, decision);
	...
	default:
		D_ERROR(DF_LEADER" got invalid decision %d for dangling pool "
			DF_UUID" with seq "DF_X64". Ignore the inconsistency.\n",
...
report:
	...
	D_CDEBUG(result != 0 || rc != 0, DLOG_ERR, DLOG_INFO,
		 DF_LEADER" detects orphan pool "DF_UUID", action %u, seq "
		 DF_X64", advice %d, handle_rc %d, report_rc %d, decision %d\n", DP_LEADER(ins),
		 DP_UUID(cpr->cpr_uuid), act, seq, cpr->cpr_advice, result, rc, decision);
...
report:
	...
	D_CDEBUG(result != 0 || rc != 0, DLOG_ERR, DLOG_INFO,
		 DF_LEADER" detects corrupted pool "DF_UUID", action %u, seq "
		 DF_X64", advice %d, handle_rc %d, report_rc %d, decision %d\n", DP_LEADER(ins),
		 DP_UUID(cpr->cpr_uuid), act, seq, cpr->cpr_advice, result, rc, decision);
...
#if 0
	/* Wait Liwei's patch to support DS_RSVC_DICTATE. */
	rc = ds_rsvc_dist_start(DS_RSVC_CLASS_POOL, &psid, ranks,
				cpr->cpr_healthy ? DS_RSVC_START : DS_RSVC_DICTATE,
				false /* bootstrap */, 0 /* size */);
#else
	rc = ds_rsvc_dist_start(DS_RSVC_CLASS_POOL, &psid, cpr->cpr_uuid,
				ranks, false /* create */, false /* bootstrap */, 0 /* size */);
#endif

out:
	d_rank_list_free(ranks);
	if (rc != 0) {
		D_ERROR(DF_LEADER" failed to start pool service for "DF_UUID", skip it: "DF_RC"\n",
			DP_LEADER(ins), DP_UUID(cpr->cpr_uuid), DP_RC(rc));
...
chk_leader_handle_pools_list(struct chk_sched_args *csa)
{
	...
	for (i = 0; i < clp_nr; i++) {
		...
		D_ERROR("Failed to verify pool "DF_UUID" existence with %s: "DF_RC"\n",
