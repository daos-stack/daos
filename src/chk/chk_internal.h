...
typedef int (*chk_co_rpc_cb_t)(void *args, uint64_t gen, uint32_t rank, int result,
...
uint32_t chk_ins_merge_status(uint32_t status_a, uint32_t status_b, uint64_t *gen_a,
			      uint64_t *gen_b);
...
int chk_engine_query(uint64_t gen, int pool_nr, uuid_t pools[], uint32_t *ins_status,
		     uint32_t *shard_nr, struct chk_query_pool_shard **shards, uint64_t *l_gen);
