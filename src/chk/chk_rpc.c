		 ...
		 uuid_t iv_uuid, chk_co_rpc_cb_t start_cb, void *args)
{
	...
	rc = start_cb(args, 0, 0 /* unused rank */, 0 /* unused result */,
	...
	for (i = 0, cmp_rank = cso->cso_cmp_ranks.ca_arrays; i < cso->cso_cmp_ranks.ca_count;
	     i++, cmp_rank++) {
		rc = start_cb(args, 0, *cmp_rank, 1, NULL /* unused data */, 0 /* unused nr */);
		if (rc != 0)
			goto out;
	}
		...
		chk_co_rpc_cb_t stop_cb, void *args)
{
	...
	for (i = 0, rank = cso->cso_ranks.ca_arrays; i < cso->cso_ranks.ca_count; i++, rank++) {
		rc = stop_cb(args, 0, *rank, 1, &cso->cso_flags, 0 /* unused nr */);
		if (rc != 0)
			goto out;
	}
		 ...
		 chk_co_rpc_cb_t query_cb, void *args)
{
	...
	rc = query_cb(args, cqo->cqo_gen, cqo->cqo_ins_status /* reuse rank as ins_stauts */,
		      cqo->cqo_status, cqo->cqo_shards.ca_arrays, cqo->cqo_shards.ca_count);
...
chk_cont_list_remote(struct ds_pool *pool, uint64_t gen, chk_co_rpc_cb_t list_cb, void *args)
{
	...
	rc = list_cb(args, 0, 0 /* unused rank */, cclo->cclo_status,
