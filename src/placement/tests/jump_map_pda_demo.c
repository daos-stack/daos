...
pda_layout_demo(void **state)
{
	...
	print_message("press enter to show layout of OC_S128 object, PDA 128 ...\n");
	getchar();
	assert_placement_success_pda(pl_map, OC_S128, 128, tgts_per_grp);

	print_message("press enter to show layout of OC_S128 object, PDA 1 ...\n");
	getchar();
	assert_placement_success_pda(pl_map, OC_S128, 1, tgts_per_grp);
...
static const struct CMUnitTest pda_demo_tests[] = {
	/* Standard configurations */
	T("PDA layout demo", pda_layout_demo),
};

int
pda_demo_run(bool verbose)
{
	int rc = 0;

	g_verbose = verbose;

	rc += cmocka_run_group_tests_name("Jump Map Placement PDA demo", pda_demo_tests,
					  NULL, NULL);
