...
generate_oids(struct test_ctx *ctx)
{
	...
	for (i = 0; i < ctx->num_oids; i++) {
		ctx->oids[i].oid.omd_id.hi = 0;
		ctx->oids[i].oid.omd_id.lo = ((uint64_t)rand() << 32) | i;
		ctx->oids[i].oid.omd_ver   = pool_map_get_version(ctx->pool_map);
		ctx->oids[i].oid.omd_pda   = 1;
		daos_obj_set_oid_by_class(&ctx->oids[i].oid.omd_id, 0, ctx->oclass, 0);
	}
}
...
retry:
		...
		diff[i].nr = pl_obj_find_rebuild(ctx->pl_map, PLT_LAYOUT_VERSION, &ctx->oids[i].oid,
						 NULL, ver, spare_tgts, shard_ids, buf_size);
