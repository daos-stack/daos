...
fail_shard_during_reintegration(void **state)
{
	...
	jtc_init_with_layout(&ctx, 4, 2, 8, OC_RP_3GX, g_verbose);
	...
	grp_size = reint_layout->ol_grp_size;
	assert_success(jtc_create_layout(&ctx));
	for (i = 0; i < reint_layout->ol_grp_nr; i++) {
		int remain = 0;

		for (j = 0; j < reint_layout->ol_grp_size; j++) {
			struct pl_obj_shard	*shard;
			struct pl_obj_shard	*new_shard;

			shard = &reint_layout->ol_shards[i * grp_size + j];
			new_shard = &ctx.layout->ol_shards[i * grp_size + j];
			if (shard->po_target == new_shard->po_target &&
			    !new_shard->po_rebuilding && !new_shard->po_reintegrating)
				remain++;
		}
		assert_true(remain >= 2);
	}
