		    ...
		    uint32_t nodes_per_domain, uint32_t vos_per_target)
{
	...
	for (i = 0; i < BENCHMARK_COUNT; i++)
		pl_obj_place(pl_map, 0, &obj_table[i], 0, -1, NULL, &layout_table[i]);
	...
	if (vtune_loop) {
		...
		while (1)
			for (i = 0; i < BENCHMARK_COUNT; i++)
				pl_obj_place(pl_map, 0, &obj_table[i], 0, -1, NULL,
					     &layout_table[i]);
	}
		      ...
		      double *percent_moved)
{
	...
	for (obj_idx = 0; obj_idx < test_entries; obj_idx++)
		pl_obj_place(iter_pl_map, 0, &obj_table[obj_idx], 0, -1,
			    ...
			    uint32_t nodes_per_domain, uint32_t vos_per_target)
{
	...
	for (type_idx = 0; type_idx < num_map_types - 1; type_idx++) {
		...
		/* Initial placement */
		for (obj_idx = 0; obj_idx < test_entries; obj_idx++)
			pl_obj_place(initial_pl_map, 0, &obj_table[obj_idx], 0, -1, NULL,
