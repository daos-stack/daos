...
static int
jump_map_obj_generate_extending_layout(struct pl_jump_map *jmap, struct jm_obj_placement *jmop,
				       uint32_t layout_version, struct daos_obj_md *md,
				       uint32_t rebuild_version, struct pl_obj_layout **layout_pp)
{
	...
	rc = obj_layout_alloc_and_get(jmap, layout_version, jmop, md, allow_status,
				      rebuild_version, &layout, NULL, NULL);
	if (rc != 0) {
	...
	rc = obj_layout_alloc_and_get(jmap, layout_version, jmop, md, allow_status,
				      rebuild_version, &extend_layout, NULL, NULL);
	if (rc != 0) {
		D_ERROR(DF_OID" get_layout_alloc failed, rc "DF_RC"\n",
			DP_OID(md->omd_id), DP_RC(rc));
		D_GOTO(out, rc);
	...
	obj_layout_dump(md->omd_id, extend_layout);
		   ...
		   struct pl_obj_layout **layout_pp)
{
	...
	if (unlikely(is_extending || is_adding_new) && rebuild_version != 0 &&
		...
		pl_obj_layout_free(layout);
		layout = NULL;
		rc = jump_map_obj_generate_extending_layout(jmap, &jmop, layout_version, md,
