...
get_target(struct pool_domain *curr_dom, struct pool_target **target,
	   ...
	   uint32_t grp_size, uint32_t allow_status, pool_comp_type_t fdom_lvl,
	   bool remap)
{
	int                     range_set;
	...
	while (current != remap_list) {
		...
		if (spare_avail) {
			...
			get_target(root, &spare_tgt, crc(key, rebuild_key), dom_used, dom_occupied,
				   dgu->dgu_used, tgts_used, shard_id, jmop->jmop_grp_size,
				   allow_status, jmop->jmop_fdom_lvl, true);
