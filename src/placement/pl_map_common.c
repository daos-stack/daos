		...
		unsigned int *remap_flags)
{
	...
	if (gen_mode == PRE_REBUILD) {
		...
		if (status & (PO_COMP_ST_DOWN | PO_COMP_ST_DRAIN)) {
			...
			if (comp->co_flags & PO_COMPF_DOWN2UP) {
				...
				if (comp->co_fseq <= 1) { /* impossible? */
					status = PO_COMP_ST_NEW;
				} else {
					flags |= PL_SHARD_HAS_PEER;
					status = PO_COMP_ST_DOWNOUT;
				}
			}
		}
		available = (status & PO_COMP_ST_UPIN);
		goto out;
	}
	...
	if (status & (PO_COMP_ST_DOWN | PO_COMP_ST_DRAIN)) {
		...
		if (comp->co_in_ver > allow_version) {
			...
			flags = PL_SHARD_REBUILDING | PL_SHARD_REINTEGRATING | PL_SHARD_HAS_PEER;
		}
	}
	 ...
	 * be remapped anymore.
	 */
	available = (status & (PO_COMP_ST_UPIN | PO_COMP_ST_DRAIN));
out:
	if (remap_flags)
		*remap_flags |= flags;
...
bool
need_remap_comp(struct pool_component *comp, uint32_t allow_version, enum layout_gen_mode gen_mode,
		unsigned int *remap_flags)
{
	return !is_comp_avaible(comp, allow_version, gen_mode, remap_flags);
}
...
next_fail:
	if (spare_avail) {
		...
		layout_set_shard_flags(layout, f_shard->fs_shard_idx, gen_mode,
				       f_shard->fs_remap_flags);
	} else {
		l_shard->po_shard = -1;
		l_shard->po_target = -1;
