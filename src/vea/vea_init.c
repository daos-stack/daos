...
load_bitmap_entry(daos_handle_t ih, d_iov_t *key, d_iov_t *val, void *arg)
{
	...
	rc = bitmap_entry_insert(vsi, vfb, false, NULL, 0);
...
load_space_info(struct vea_space_info *vsi)
{
	...
	*/
	...
	if (rc && rc != -DER_NONEXIST) {
		...
		rc = dbtree_upsert(vsi->vsi_md_bitmap_btr, BTR_PROBE_EQ,
				   DAOS_INTENT_UPDATE, &key, &val, &val_out);
		if (rc)
			return rc;

		df = (struct vea_hint_df *)val_out.iov_buf;
	} else {
