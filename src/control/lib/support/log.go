...
const (
	YYYYMMDD        = "2006-01-23"
	HHMMSS          = "15:04:05.678"
	YYYYMMDD_HHMMSS = "2006-01-23 15:04:05.678"
)
...
func cpLinesFromLog(log logging.Logger, srcFile string, destFile string, opts ...CollectLogsParams) error {
	...
	if opts[0].LogCmd == "EngineLog" {
		...
		actStartTime, _ = time.Parse(YYYYMMDD_HHMMSS, actStartTime.Format(YYYYMMDD_HHMMSS))
		actEndTime, _ = time.Parse(YYYYMMDD_HHMMSS, actEndTime.Format(YYYYMMDD_HHMMSS))
		...
		for scanner.Scan() {
			...
			expLogTime, _ := time.Parse(YYYYMMDD_HHMMSS, expDateTime)
