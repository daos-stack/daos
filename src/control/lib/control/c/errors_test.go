...
func TestControlC_StatusFromMsg(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"DER_SUCCESS(0) rejected":   {msg: "unexpected: " + daos.Success.Error(), expHit: false},
		"DER_SUCCESS(0) skipped in favor of later failure token": {
			msg:    "got " + daos.Success.Error() + " but wanted: " + daos.Busy.Error(),
			expSt:  daos.Busy,
			expHit: true,
		},
	} {
...
func TestControlC_ErrorToRC(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"typed fault: rpc timeout": {
			err:   &fault.Fault{Code: code.ClientRpcTimeout, Description: "request timed out"},
			expRC: int(daos.TimedOut),
		},
		"typed fault: connection refused": {
			err:   &fault.Fault{Code: code.ClientConnectionRefused},
			expRC: int(daos.Unreachable),
		},
		"typed fault: checker invalid member states is retryable": {
			err:   checker.FaultIncorrectMemberStates(true, "0-1", "joined"),
			expRC: int(daos.TryAgain),
		},
		"typed fault: checker not enabled": {
			err:   checker.FaultCheckerNotEnabled,
			expRC: int(daos.NotInit),
		},
		"typed fault: checker enabled": {
			err:   checker.FaultCheckerEnabled,
			expRC: int(daos.Busy),
		},
		"typed fault: wrapped still maps": {
			err:   fmt.Errorf("outer: %w", &fault.Fault{Code: code.ClientConnectionTimedOut}),
			expRC: int(daos.TimedOut),
		},
		"DER_SUCCESS token in error text must not read as success": {
			err:   errors.New("unexpected status: " + daos.Success.Error()),
