...
// validatePoolCreateArgs rejects malformed input before the RPC: nil args,
// nil pool_uuid_out (no output target), a non-NULL *svc_out (would leak),
// and the both-sizes-zero case. Errors wrap daos.InvalidInput so errorToRC
// collapses them to a single rc.
func validatePoolCreateArgs(args *C.struct_daos_control_pool_create_args, svcOut **C.d_rank_list_t, poolUUIDOut *C.uuid_t) error {
	if args == nil {
		return fmt.Errorf("args is nil: %w", daos.InvalidInput)
	}
	if poolUUIDOut == nil {
		return fmt.Errorf("pool_uuid_out is nil: %w", daos.InvalidInput)
	}
	if svcOut != nil && *svcOut != nil {
		return fmt.Errorf("*svc_out must be NULL on input: %w", daos.InvalidInput)
	}
	if args.dcpa_scm_size == 0 && args.dcpa_nvme_size == 0 {
...
// daos_control_pool_create creates a pool described by args. On success the
// new pool's UUID is written to poolUUIDOut, and if svcOut is non-NULL,
// *svcOut receives the service replica list (allocated with d_rank_list_alloc;
// caller releases with d_rank_list_free). *svcOut must be NULL on input and
// remains NULL if the server returns no replicas.
//
//export daos_control_pool_create
func daos_control_pool_create(handle C.uintptr_t, args *C.struct_daos_control_pool_create_args, svcOut **C.d_rank_list_t, poolUUIDOut *C.uuid_t) C.int {
	return withContext(handle, func(ctx *ctrlContext) error {
		if err := validatePoolCreateArgs(args, svcOut, poolUUIDOut); err != nil {
			ctx.log.Errorf("PoolCreate: %s", err)
			return err
		}

		goProps, err := propsFromC(args.dcpa_prop)
		if err != nil {
			return err
		}

		userName, err := uidToUsername(uint32(args.dcpa_uid))
		if err != nil {
			ctx.log.Errorf("PoolCreate: %s", err)
			return err
		}
		groupName, err := gidToGroupname(uint32(args.dcpa_gid))
		if err != nil {
			ctx.log.Errorf("PoolCreate: %s", err)
			return err
		}
		...
		req := &control.PoolCreateReq{
			User:       userName,
			UserGroup:  groupName,
			NumSvcReps: uint32(args.dcpa_nsvc),
		}
		req.SetSystem(goString(args.dcpa_grp))

		if goTgts := rankListFromC(args.dcpa_tgts); len(goTgts) > 0 {
			req.Ranks = goTgts
		}

		req.TierBytes = []uint64{uint64(args.dcpa_scm_size), uint64(args.dcpa_nvme_size)}
		req.Properties = goProps

		// ACL and owner/owner-group entries are handled outside the
		// property registry; apply them after the uid/gid defaults so
		// that prop entries win when present.
		if err := poolCreateExtrasFromC(args.dcpa_prop, req); err != nil {
			ctx.log.Errorf("PoolCreate: %s", err)
			return err
		}
		...
		if err != nil {
			ctx.log.Errorf("PoolCreate failed: %s", summarizeRPCErr(err))
			return err
		}
		...
		copyUUIDToC(respUUID, poolUUIDOut)

		if svcOut != nil && len(resp.SvcReps) > 0 {
			svc := C.d_rank_list_alloc(C.uint32_t(len(resp.SvcReps)))
			if svc == nil {
			...
			copyRankListToC(svcRanks, svc, len(resp.SvcReps))
			*svcOut = svc
		}
