...
func validatePoolCreateArgs(args *C.struct_daos_control_pool_create_args) error {
	...
	if args.svc != nil {
		return fmt.Errorf("svc is an output field and must be NULL: %w", daos.InvalidInput)
	}
...
func daos_control_pool_create(handle C.uintptr_t, args *C.struct_daos_control_pool_create_args) C.int {
	return withContext(handle, func(ctx *ctrlContext) error {
		...
		if len(resp.SvcReps) > 0 {
			args.svc = C.d_rank_list_alloc(C.uint32_t(len(resp.SvcReps)))
			if args.svc == nil {
			...
			copyRankListToC(svcRanks, args.svc, len(resp.SvcReps))
		}
