...
// Helpers for populating a daos_prop_t in test code. cgo can't write into the
// dpe_type/dpe_val/dpe_str union directly, so we do it in C.
static void set_prop_entry_val(daos_prop_t *prop, uint32_t idx, uint32_t type,
			       uint64_t val) {
	if (prop == NULL || idx >= prop->dpp_nr)
		return;
	prop->dpp_entries[idx].dpe_type = type;
	prop->dpp_entries[idx].dpe_val  = val;
}

static void set_prop_entry_str(daos_prop_t *prop, uint32_t idx, uint32_t type,
			       const char *str) {
	if (prop == NULL || idx >= prop->dpp_nr)
		return;
	prop->dpp_entries[idx].dpe_type = type;
	if (str == NULL) {
		prop->dpp_entries[idx].dpe_str = NULL;
		return;
	}
...
// testPoolProp wraps a caller-allocated C daos_prop_t for testing propsFromC.
type testPoolProp struct {
	prop *C.daos_prop_t
}

// newTestPoolProp allocates a daos_prop_t sized for nrEntries entries.
func newTestPoolProp(nrEntries int) *testPoolProp {
	return &testPoolProp{prop: C.daos_prop_alloc(C.uint32_t(nrEntries))}
}

// setEntryVal populates entry idx as a numeric (dpe_val) property.
func (p *testPoolProp) setEntryVal(idx int, propType uint32, val uint64) {
	C.set_prop_entry_val(p.prop, C.uint32_t(idx), C.uint32_t(propType), C.uint64_t(val))
}

// setEntryStr populates entry idx as a string (dpe_str) property. The string
// is copied into newly allocated C memory which daos_prop_free will release.
func (p *testPoolProp) setEntryStr(idx int, propType uint32, str string) {
	cs := C.CString(str)
	defer C.free(unsafe.Pointer(cs))
	C.set_prop_entry_str(p.prop, C.uint32_t(idx), C.uint32_t(propType), cs)
}

// ptr returns the underlying C pointer for passing to the library.
func (p *testPoolProp) ptr() *C.daos_prop_t {
	return p.prop
}

// free releases the allocation and any per-entry string storage.
func (p *testPoolProp) free() {
	if p.prop != nil {
		C.daos_prop_free(p.prop)
		p.prop = nil
	}
}

// testPoolPropType constants — re-exported from cgo so _test.go files (which
// cannot use cgo directly in a c-shared main package) can still refer to them.
const (
...
func callPoolCreate(
	...
	args := C.struct_daos_control_pool_create_args{
		...
		svc:       svc.ptr(),
		pool_uuid: poolUUID.ptr(),
	}
	return int(daos_control_pool_create(C.uintptr_t(handle), &args))
}

// validatePoolCreateArgsSpec describes a daos_control_pool_create_args
// struct for testing validatePoolCreateArgs from _test.go (which cannot
// construct C structs directly).
...
// callValidatePoolCreateArgs builds a C args struct per spec, runs the
// validator, and returns the rc errorToRC would produce.
func callValidatePoolCreateArgs(spec *validatePoolCreateArgsSpec) int {
	...
	var poolUUID *testUUID
	if !spec.omitPoolUUID {
		poolUUID = newTestUUID()
		args.pool_uuid = poolUUID.ptr()
	}

	rc := errorToRC(validatePoolCreateArgs(&args))
	runtime.KeepAlive(poolUUID)
	return rc
}
...
func testConvertRankListToC(ranks []uint32, rl *testRankList) {
	...
	// Copy back to the destination C rank list, using its preallocated
	// capacity as the bound.
	copyRankListToC(goRanks, rl.ptr(), int(rl.nr()))
}
...
// testCheckPoolInfo is a Go-friendly snapshot of one entry in dci_pools.
...
// testCheckReportInfo is a Go-friendly snapshot of one entry in dci_reports.
...
// readCheckInfo snapshots a populated daos_check_info into Go values so tests
// can assert against the result without writing cgo directly (which is not
// supported in _test.go files of a c-shared main package).
func readCheckInfo(dci *C.struct_daos_check_info) (status, phase string, pools []testCheckPoolInfo, reports []testCheckReportInfo) {
	if dci == nil {
		return
	}
	status = C.GoString(dci.dci_status)
	phase = C.GoString(dci.dci_phase)

	if dci.dci_pools != nil && dci.dci_pool_nr > 0 {
		poolSlice := unsafe.Slice(dci.dci_pools, int(dci.dci_pool_nr))
		pools = make([]testCheckPoolInfo, len(poolSlice))
		for i := range poolSlice {
			pools[i] = testCheckPoolInfo{
				UUID:   uuidFromC(&poolSlice[i].dcpi_uuid),
				Status: C.GoString(poolSlice[i].dcpi_status),
				Phase:  C.GoString(poolSlice[i].dcpi_phase),
			}
		}
	}

	if dci.dci_reports != nil && dci.dci_report_nr > 0 {
		reportSlice := unsafe.Slice(dci.dci_reports, int(dci.dci_report_nr))
		reports = make([]testCheckReportInfo, len(reportSlice))
		for i := range reportSlice {
			r := testCheckReportInfo{
				UUID:   uuidFromC(&reportSlice[i].dcri_uuid),
				Seq:    uint64(reportSlice[i].dcri_seq),
				Class:  uint32(reportSlice[i].dcri_class),
				Action: uint32(reportSlice[i].dcri_act),
				Result: int(reportSlice[i].dcri_result),
			}
			nOpts := int(reportSlice[i].dcri_option_nr)
			r.Options = make([]int, nOpts)
			for j := 0; j < nOpts; j++ {
				r.Options[j] = int(reportSlice[i].dcri_options[j])
			}
			reports[i] = r
		}
	}
...
// checkInfoCounts reports the current dci_pool_nr and dci_report_nr fields
// and whether the pool/report pointers are nil. Used to assert the post-free
// state of the struct.
func checkInfoCounts(dci *C.struct_daos_check_info) (poolNr, reportNr int, poolsNil, reportsNil, statusNil, phaseNil bool) {
	if dci == nil {
		return
	}
	return int(dci.dci_pool_nr), int(dci.dci_report_nr),
...
// testDeviceInfo is a Go-friendly snapshot of a populated C struct device_list
// entry, used by callStorageDeviceListPopulated.
...
// callStorageDeviceListPopulated is a test helper that calls
// daos_control_storage_device_list with a caller-allocated array and returns
// the contents as Go values for inspection. capacity is passed through as the
// caller-side in/out slot count so the library can detect an under-sized
// buffer and return BufTooSmall.
func callStorageDeviceListPopulated(handle cgo.Handle, capacity int) ([]testDeviceInfo, int, int) {
	devices := make([]C.struct_device_list, capacity)
	ndisks := C.int(capacity)
	...
	result := make([]testDeviceInfo, n)
	for i := 0; i < n; i++ {
		tgtCount := int(devices[i].n_tgtidx)
		result[i] = testDeviceInfo{
		...
		for j := 0; j < tgtCount; j++ {
			result[i].Targets[j] = int32(devices[i].tgtidx[j])
		}
	}
	return result, int(ndisks), rc
}
...
// callStorageQueryDeviceHealthSized is like callStorageQueryDeviceHealth but
// lets the test specify an exact output buffer size, to exercise the
// truncation contract. The returned string reflects only the bytes that the
// library wrote (up to, but not including, the terminating NUL).
func callStorageQueryDeviceHealthSized(handle cgo.Handle, host, statsKey string, devUUID *testUUID, bufSize int) (string, int) {
	var cHost *C.char
	if host != "" {
		cHost = C.CString(host)
		defer C.free(unsafe.Pointer(cHost))
...
func callPoolSetProp(handle cgo.Handle, label string, poolUUID *testUUID, name, value string) int {
	var cLabel *C.char
	if label != "" {
