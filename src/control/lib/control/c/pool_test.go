...
func TestPoolCreate(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Use the current process uid/gid: the binding resolves
			// them via passwd/group and an uncooperative host (e.g. a
			// sandbox without uid 1000) would otherwise fail the
			// lookup before ever exercising the control plane path.
			rc := callPoolCreate(
				handle,
				uint32(os.Getuid()),
				uint32(os.Getgid()),
...
func TestPoolExclude(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"failure - response error": {
			// firstRankStatus recovers the embedded status from the
			// per-rank result message so the caller sees the real
			// error (Nonexistent) rather than a generic MiscError.
			mic: &control.MockInvokerConfig{
				UnaryResponse: control.MockMSResponse("host1", nil, &mgmtpb.PoolExcludeResp{
...
func TestPoolDrain(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"failure - response error": {
			// firstRankStatus recovers the embedded Busy status
			// from the per-rank result message.
			mic: &control.MockInvokerConfig{
				UnaryResponse: control.MockMSResponse("host1", nil, &mgmtpb.PoolDrainResp{
...
// TestPoolListErrorResetsNpools verifies the contract: on any error return,
// *npools is set to 0 so the caller is never required to call
// daos_control_pool_list_free. The sentinel value 99 is used as the initial
// *npools to confirm it is overwritten on the error path.
func TestPoolListErrorResetsNpools(t *testing.T) {
	...
	// Pre-load *npools with a non-zero sentinel; on error the function must
	// reset it to 0 so a naïve caller looping on *npools doesn't iterate.
	npools := uint64(99)
	rc := callPoolList(handle, &npools, nil)

	if rc != int(daos.NoPermission) {
		t.Fatalf("expected RC %d (NoPermission), got %d", int(daos.NoPermission), rc)
	}
	if npools != 0 {
		t.Fatalf("expected *npools==0 on error, got %d", npools)
	}
}

// TestPoolListFreeIdempotent verifies that daos_control_pool_list_free is a
// no-op on a second call — the check-info sibling already pins this property
// and pool_list_free's freed pointers should read back nil so a double-free
// can't dereference a stale allocation.
func TestPoolListFreeIdempotent(t *testing.T) {
	...
	pools := newTestPoolListInfo(1)
	npools := uint64(1)
	if rc := callPoolList(handle, &npools, pools); rc != 0 {
		t.Fatalf("list rc=%d", rc)
	}
	if npools != 1 {
		t.Fatalf("npools=%d, want 1", npools)
	}

	// First free: releases label + svc rank list, NULLs the pointers.
	pools.free()

	// Second free must be a no-op: no panic, no double-free. Memcheck would
	// flag a double-free on a real allocator, but the in-code guard is that
	// daos_control_pool_list_free sets the freed pointers to nil and skips.
	pools.free()
}
