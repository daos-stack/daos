...
func TestControlC_ParsePolicies(t *testing.T) {
...
func TestControlC_CheckSwitch(t *testing.T) {
	for name, tc := range map[string]struct {
...
func TestControlC_CheckStart(t *testing.T) {
...
func TestControlC_CheckStop(t *testing.T) {
	testUUID := uuid.MustParse("12345678-1234-1234-1234-123456789abc")
...
func TestControlC_CheckQuery(t *testing.T) {
	testUUID := uuid.MustParse("12345678-1234-1234-1234-123456789abc")
...
func TestControlC_CheckQueryWithInfoAndFree(t *testing.T) {
	log, buf := logging.NewTestLogger(t.Name())
	defer test.ShowBufferOnFailure(t, buf)
	...
	info, rc := callCheckQueryWithInfo(handle, nil)
	if rc != 0 {
		t.Fatalf("rc=%d, want 0", rc)
	}
	// Free-on-zeroed path: the helper already ran the free twice internally.
	// Post-free state must be zeroed.
	if !info.PostFreeZeroed {
		t.Fatal("post-free state not zeroed on empty struct")
	}
}

func TestControlC_CheckInfoFreeNil(t *testing.T) {
	callCheckInfoFreeNil()
}
...
func TestControlC_CheckQueryWithInfoPopulated(t *testing.T) {
	...
	info, rc := callCheckQueryWithInfo(handle, []uuid.UUID{uuidA, uuidB})
	if rc != 0 {
		t.Fatalf("rc=%d, want 0", rc)
	}

	if info.Status == "" {
		t.Error("Status not populated")
	}
	if info.Phase == "" {
		t.Error("Phase not populated")
	}

	if len(info.Pools) != 2 {
		t.Fatalf("got %d pools, want 2", len(info.Pools))
	}
	gotUUIDs := map[uuid.UUID]bool{info.Pools[0].UUID: true, info.Pools[1].UUID: true}
	...
	if len(info.Reports) != 2 {
		t.Fatalf("got %d reports, want 2", len(info.Reports))
	}
	// Reports sort by class, not input order — index by UUID for robustness.
	reportsByUUID := make(map[uuid.UUID]testCheckReportInfo, len(info.Reports))
	...
	// The helper ran free twice internally; post-free state must be zeroed.
	if !info.PostFreeZeroed {
		t.Error("post-free state not zeroed")
	}
}

// TestCheckQueryWithInfoEmptyUUIDsReturnsAllPools pins that a caller passing
// zero UUIDs to daos_control_check_query receives every pool the server
// returned, not an empty pool list. The previous implementation silently
// dropped pools when the UUID filter was empty.
func TestControlC_CheckQueryWithInfoEmptyUUIDsReturnsAllPools(t *testing.T) {
	uuidA := uuid.MustParse("10000000-0000-0000-0000-000000000001")
	uuidB := uuid.MustParse("20000000-0000-0000-0000-000000000002")
	...
	if len(info.Pools) != 2 {
		t.Fatalf("got %d pools with empty UUID filter, want 2", len(info.Pools))
	}
	gotUUIDs := map[uuid.UUID]bool{}
	for _, p := range info.Pools {
		gotUUIDs[p.UUID] = true
		if p.Status == "" || p.Phase == "" {
			t.Errorf("pool %s: missing Status/Phase", p.UUID)
		}
	}
	for _, want := range []uuid.UUID{uuidA, uuidB} {
		if !gotUUIDs[want] {
			t.Errorf("missing pool %s", want)
		}
	}
}

func TestControlC_CheckRepair(t *testing.T) {
	for name, tc := range map[string]struct {
		mic    *control.MockInvokerConfig
		seq    uint64
		action uint32
...
func TestControlC_CheckSetPolicy(t *testing.T) {
	for name, tc := range map[string]struct {
		mic      *control.MockInvokerConfig
		flags    uint32
		policies string
...
// TestCheckSetPolicyInteractiveFlag pins that the TCPF_INTERACT flag (1<<1)
// is forwarded on the request as AllInteractive=true. The sibling test
// covers TCPF_RESET but leaves this branch unexercised.
func TestControlC_CheckSetPolicyInteractiveFlag(t *testing.T) {
	log, buf := logging.NewTestLogger(t.Name())
	defer test.ShowBufferOnFailure(t, buf)
	...
	const tcpfInteract = 1 << 1
	if rc := callCheckSetPolicy(handle, tcpfInteract, ""); rc != 0 {
		t.Fatalf("rc=%d, want 0", rc)
	}
	...
	req, ok := mi.SentReqs[0].(*control.SystemCheckSetPolicyReq)
	if !ok {
		t.Fatalf("SentReqs[0] type=%T, want *control.SystemCheckSetPolicyReq", mi.SentReqs[0])
	}
	if !req.AllInteractive {
		t.Fatal("AllInteractive=false, want true (TCPF_INTERACT should flip it)")
	}
	if req.ResetToDefaults {
		t.Fatal("ResetToDefaults=true, want false (TCPF_RESET was not set)")
	}
}

func TestControlC_CheckOperationsInvalidHandle(t *testing.T) {
	testUUID := uuid.MustParse("12345678-1234-1234-1234-123456789abc")
