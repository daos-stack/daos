		...
		for _, hss := range resp.HostStorage {
			...
			for _, dev := range hss.HostStorage.SmdInfo.Devices {
				...
				if devUUID, err := uuid.Parse(dev.UUID); err == nil {
					copyUUIDToC(devUUID, &deviceSlice[idx].dl_device_id)
				}
				...
				copyStringToCharArray(state, &deviceSlice[idx].dl_state[0], C.DAOS_DEV_STATE_MAX_LEN)

				deviceSlice[idx].dl_rank = C.int(dev.Rank)

				copyStringToCharArray(hostAddr, &deviceSlice[idx].dl_host[0], C.DAOS_HOSTNAME_MAX_LEN)
				...
				for i := 0; i < nTgts; i++ {
					deviceSlice[idx].dl_tgtidx[i] = C.int(dev.TargetIDs[i])
				}
				deviceSlice[idx].dl_n_tgtidx = C.int(nTgts)
