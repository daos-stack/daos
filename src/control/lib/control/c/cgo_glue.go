...
// never-allocated or freed handle; recover it here so a stale handle reads
// as DER_INVAL rather than a generic export panic.
func getContext(handle C.uintptr_t) (c *ctrlContext, rc C.int) {
	if handle == 0 {
	...
	defer func() {
		if r := recover(); r != nil {
			c, rc = nil, C.int(errorToRC(errInvalidHandle))
		}
	}()
...
func errorToRC(err error) int {
	...
	// Faults carry a typed code end-to-end; map the actionable ones
	// before falling through to string heuristics.
	var f *fault.Fault
	if errors.As(err, &f) {
		switch f.Code {
		case code.ClientRpcTimeout, code.ClientConnectionTimedOut:
			return int(daos.TimedOut)
		case code.ClientConnectionRefused, code.ClientConnectionNoRoute,
			code.ClientConnectionBadHost, code.ClientConnectionClosed,
			code.ServerDataPlaneNotStarted:
			return int(daos.Unreachable)
		case code.SystemCheckerInvalidMemberStates:
			return int(daos.TryAgain)
		case code.SystemCheckerNotEnabled:
			return int(daos.NotInit)
		case code.SystemCheckerEnabled:
...
func statusFromMsg(msg string) (daos.Status, bool) {
	for _, m := range derStatusPattern.FindAllStringSubmatch(msg, -1) {
		// Strictly negative: a DER_SUCCESS(0) token quoted inside an error
		// message must not convert the failure into rc 0.
...
// logging every errored member first so the collapsed rc doesn't lose
// which ranks failed and why.
func firstMemberStatus(ctx *ctrlContext, results system.MemberResults) error {
	if ctx != nil && ctx.log != nil {
		for _, r := range results {
			if r != nil && r.Errored {
				ctx.log.Errorf("rank %d: %s", r.Rank, r.Msg)
			}
		}
	}
