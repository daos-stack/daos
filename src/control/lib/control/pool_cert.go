...
// The cert path is derived from the key path by stripping any extension
// and appending ".crt"; the contract is that the cert and key live
// side-by-side with the same base name (e.g. foo.key / foo.crt).
