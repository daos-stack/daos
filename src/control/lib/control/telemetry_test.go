...
func mockScrapeFnSuccess(t *testing.T, metricFam ...*pclient.MetricFamily) func(context.Context, *url.URL, httpGetFn, time.Duration, bool) ([]byte, error) {
	t.Helper()

	return func(_ context.Context, _ *url.URL, _ httpGetFn, _ time.Duration, _ bool) ([]byte, error) {
...
func TestControl_scrapeMetrics(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		req       httpGetter
		scrapeFn  func(context.Context, *url.URL, httpGetFn, time.Duration, bool) ([]byte, error)
		expResult pbMetricMap
		expErr    error
	}{
		"check scrape params": {
			scrapeFn: func(_ context.Context, url *url.URL, getter httpGetFn, timeout time.Duration, allowInsecure bool) ([]byte, error) {
...
func TestControl_MetricsList(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			if tc.scrapeFn == nil {
				tc.scrapeFn = func(context.Context, *url.URL, httpGetFn, time.Duration, bool) ([]byte, error) {
					return nil, nil
				}
			}
			if tc.req != nil {
