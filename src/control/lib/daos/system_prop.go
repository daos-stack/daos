...
func SystemProperties() SystemPropertyMap {
	...
	return SystemPropertyMap{
		...
		SystemPropertySelfHeal: SystemProperty{
			...
			Value:       NewStringPropVal("exclude;pool_exclude;pool_rebuild", enumSubsets([]string{"exclude", "pool_exclude", "pool_rebuild"}, ";")...),
			Description: "Self-heal policy for the system",
		},
	}
}

// enumSubsets returns a slice of all subsets of strings, including the empty set.
func enumSubsets(strings []string, sep string) []string {
	if len(strings) == 0 {
		return []string{""}
	}

	subsets := enumSubsets(strings[1:], sep)
	n := len(subsets)
