...
func daos_pool_list_attr(poolHdl C.daos_handle_t, buf *C.char, size *C.size_t, ev *C.struct_daos_event) C.int {
	if len(daos_pool_list_attr_RCList) > 0 {
		rc := daos_pool_list_attr_RCList[daos_pool_list_attr_CallCount]
		daos_pool_list_attr_CallCount++
		if rc != 0 {
			return rc
		}
	}
	if daos_pool_list_attr_RC != 0 {
		return daos_pool_list_attr_RC
	}

	bufSize := 0
	for _, attr := range daos_pool_list_attr_AttrList {
...
func daos_pool_get_attr(poolHdl C.daos_handle_t, n C.int, names **C.char, values *unsafe.Pointer, sizes *C.size_t, ev *C.struct_daos_event) C.int {
	if len(daos_pool_get_attr_RCList) > 0 {
		rc := daos_pool_get_attr_RCList[daos_pool_get_attr_CallCount]
		daos_pool_get_attr_CallCount++
		if rc != 0 {
			return rc
		}
	}
	if daos_pool_get_attr_RC != 0 {
		return daos_pool_get_attr_RC
	}

	daos_pool_get_attr_SetN = int(n)
	daos_pool_get_attr_ReqNames = make(map[string]struct{})
	cReqNames := unsafe.Slice(names, n)
	for i := 0; i < int(n); i++ {
		daos_pool_get_attr_ReqNames[C.GoString(cReqNames[i])] = struct{}{}
	}

	if len(daos_pool_get_attr_ReqNames) > 0 && len(daos_pool_get_attr_AttrList) == 0 {
		return -C.int(daos.Nonexistent)
	}

	attrListMap := daos_pool_get_attr_AttrList.AsMap()
	reqAttrCt := 0
	for attrName := range daos_pool_get_attr_ReqNames {
	...
	idx := 0
	for _, attr := range daos_pool_get_attr_AttrList {
		if _, ok := daos_pool_get_attr_ReqNames[attr.Name]; !ok {
			continue
		}
...
func daos_pool_set_attr(poolHdl C.daos_handle_t, n C.int, names **C.char, values *unsafe.Pointer, sizes *C.size_t, ev *C.struct_daos_event) C.int {
	...
	for i := 0; i < int(n); i++ {
		...
		daos_pool_set_attr_AttrList = append(daos_pool_set_attr_AttrList, &daos.Attribute{
			Name:  C.GoString(namesSlice[i]),
			Value: make([]byte, sizesSlice[i]),
		})
		copy(daos_pool_set_attr_AttrList[len(daos_pool_set_attr_AttrList)-1].Value, valueSlice)
	}
...
func daos_pool_del_attr(poolHdl C.daos_handle_t, n C.int, name **C.char, ev *C.struct_daos_event) C.int {
	if daos_pool_del_attr_RC != 0 {
		return daos_pool_del_attr_RC
	}

	nameSlice := unsafe.Slice(name, n)
	for i := 0; i < int(n); i++ {
		daos_pool_del_attr_AttrNames = append(daos_pool_del_attr_AttrNames, C.GoString(nameSlice[i]))
	}
