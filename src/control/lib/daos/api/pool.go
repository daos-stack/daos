...
// fromCtx retrieves the PoolHandle from the supplied context, if available.
func (ph *PoolHandle) fromCtx(ctx context.Context) error {
	if ph == nil {
		return ErrInvalidPoolHandle
	}
	return ph.connHandle.fromCtx(ctx, poolHandleKey)
}
...
func (ph *PoolHandle) toCtx(ctx context.Context) context.Context {
	...
	return ph.connHandle.toCtx(ctx, poolHandleKey)
}
...
func (ph *PoolHandle) Disconnect(ctx context.Context) error {
	...
	logging.FromContext(ctx).Debugf("Disconnect(%s)", ph)
