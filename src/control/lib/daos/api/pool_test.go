...
func TestAPI_PoolListAttributes(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"daos_pool_list_attr() fails (get buf size)": {
			...
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			expErr: errors.Wrap(daos.IOError, "failed to list attributes"),
		},
...
func TestAPI_PoolGetAttributes(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"daos_pool_get_attr() fails (sizes)": {
			...
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			expErr: errors.Wrap(daos.IOError, "failed to get attribute sizes"),
		},
		"daos_pool_get_attr() fails (values)": {
			...
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			expErr: errors.Wrap(daos.IOError, "failed to get attribute values"),
		},
		"empty requested attribute name": {
			ctx:       testCtxHandle.toCtx(test.Context(t)),
			attrNames: test.JoinArgs(nil, "a", ""),
			expErr:    errors.Errorf("empty attribute name at index 1"),
		},
		"no attributes set; attributes requested": {
			...
			checkParams: func(t *testing.T) {
				test.CmpAny(t, "req attr names", test.JoinArgs(nil, "foo"), daos_pool_get_attr_ReqNames)
			},
			expErr: errors.Wrap(daos.Nonexistent, "failed to get attribute sizes"),
		},
...
func TestAPI_PoolSetAttributes(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"no attributes to set": {
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			expErr: errors.Wrap(daos.InvalidInput, "no attributes provided"),
		},
		"nil toSet attribute": {
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			toSet:  append(daos_default_AttrList, nil),
			expErr: errors.Wrap(daos.InvalidInput, "nil attribute at index 3"),
		},
		"toSet attribute with empty name": {
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			toSet:  append(daos_default_AttrList, &daos.Attribute{Name: ""}),
			expErr: errors.Wrap(daos.InvalidInput, "empty attribute name at index 3"),
		},
		"toSet attribute with empty value": {
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			toSet:  append(daos_default_AttrList, &daos.Attribute{Name: "empty"}),
			expErr: errors.Wrap(daos.InvalidInput, "empty attribute value at index 3"),
		},
		"daos_pool_set_attr() fails": {
			...
			expErr: errors.Wrap(daos.IOError, "failed to set attributes"),
		},
		"success": {
			ctx:   testCtxHandle.toCtx(test.Context(t)),
			toSet: daos_default_AttrList,
...
func TestAPI_PoolDeleteAttributes(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"no attributes to delete": {
			ctx:    testCtxHandle.toCtx(test.Context(t)),
			expErr: errors.Wrap(daos.InvalidInput, "no attribute names provided"),
		},
		"empty name in toDelete list": {
			ctx:      testCtxHandle.toCtx(test.Context(t)),
			toDelete: test.JoinArgs(nil, "foo", "", "bar"),
			expErr:   errors.Wrap(daos.InvalidInput, "empty attribute name at index 1"),
		},
		"daos_pool_det_attr() fails": {
			setup: func(t *testing.T) {
				daos_pool_del_attr_RC = -_Ctype_int(daos.IOError)
			},
			ctx:      testCtxHandle.toCtx(test.Context(t)),
			toDelete: test.JoinArgs(nil, daos_default_AttrList[0].Name),
			expErr:   errors.Wrap(daos.IOError, "failed to delete attributes"),
		},
		"success": {
			ctx:      testCtxHandle.toCtx(test.Context(t)),
			toDelete: test.JoinArgs(nil, daos_default_AttrList[0].Name),
...
func TestAPI_PoolHandleMethods(t *testing.T) {
	...
	for i := 0; i < thType.NumMethod(); i++ {
		...
		case "DeleteAttributes":
			methArgs = append(methArgs, reflect.ValueOf(daos_default_AttrList[0].Name))
			expResults = 1
		case "FillHandle", "IsValid", "String", "UUID":
