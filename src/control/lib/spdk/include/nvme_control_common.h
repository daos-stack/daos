...
struct ret_t *
_discover_vmd(prober, bool, health_getter);
...
void
_collect(struct ret_t *, data_copier, pci_getter, socket_id_getter, bool);

/**
 * Collect controller and namespace information of the NVMe devices.
 *
 * \param vmd_only flag to signify whether nvme controllers should only be
 *                 collected if of type "vmd".
 *
 * \return a pointer to a return struct (ret_t).
 */
struct ret_t *
