...
// ManagementService implements (the Go portion of) Management Service, satisfying
// pb.MgmtSvcServer.
...
func NewManagementService(h *IOServerHarness) *ManagementService {
	return &ManagementService{
		harness: h,
	}
}

func (svc *ManagementService) GetAttachInfo(ctx context.Context, req *pb.GetAttachInfoReq) (*pb.GetAttachInfoResp, error) {
	...
	svc.mutex.Lock()
	dresp, err := makeDrpcCall(mi.drpcClient, drpcClientID, getAttachInfo, req)
	svc.mutex.Unlock()
	if err != nil {
		return nil, err
	}
...
func (svc *ManagementService) Join(ctx context.Context, req *pb.JoinReq) (*pb.JoinResp, error) {
	mi, err := svc.harness.GetManagementInstance()
	if err != nil {
		return nil, err
	}
	...
	svc.mutex.Lock()
	dresp, err := makeDrpcCall(mi.drpcClient, drpcClientID, join, req)
	svc.mutex.Unlock()
	if err != nil {
		return nil, err
	}
...
func (svc *ManagementService) CreatePool(ctx context.Context, req *pb.CreatePoolReq) (*pb.CreatePoolResp, error) {
	mi, err := svc.harness.GetManagementInstance()
	if err != nil {
		return nil, err
	}
	...
	svc.mutex.Lock()
	dresp, err := makeDrpcCall(mi.drpcClient, drpcClientID, createPool, req)
	svc.mutex.Unlock()
	if err != nil {
		return nil, err
	}
...
func (svc *ManagementService) DestroyPool(ctx context.Context, req *pb.DestroyPoolReq) (*pb.DestroyPoolResp, error) {
	mi, err := svc.harness.GetManagementInstance()
	if err != nil {
		return nil, err
	}
	...
	svc.mutex.Lock()
	dresp, err := makeDrpcCall(mi.drpcClient, drpcClientID, destroyPool, req)
	svc.mutex.Unlock()
	if err != nil {
		return nil, err
	}
