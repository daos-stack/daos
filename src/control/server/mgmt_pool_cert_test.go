...
func testRootedCA(t *testing.T, cn string) (string, []byte) {
	t.Helper()
	rootPath, pems := testRootedCAs(t, cn)
	return rootPath, pems[0]
}

// testRootedCAs is testRootedCA for several CAs under one root.
func testRootedCAs(t *testing.T, cns ...string) (string, [][]byte) {
	t.Helper()
	...
	var pems [][]byte
	for _, cn := range cns {
		chained, _ := sectest.NewCA(t, cn, rootCert, rootKey)
		pems = append(pems, chained)
	}
...
func TestServer_MgmtSvc_PoolAddCA(t *testing.T) {
	rootPath, pems := testRootedCAs(t,
		security.PoolCACommonName(uuid.MustParse(mockUUID)), security.PoolCACommonName(test.MockPoolUUID(7)))
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			ms := newTestMgmtSvc(t, log)
			ms.daosCARootPath = rootPath
			ms.allowInsecure = tc.insecure
			addTestPools(t, ms.sysdb, mockUUID)
...
func TestServer_MgmtSvc_PoolAddCA_BundleCap(t *testing.T) {
	...
	capRootPath, capPEM := testRootedCA(t, security.PoolCACommonName(uuid.MustParse(mockUUID)))
	ms := newTestMgmtSvc(t, log)
	ms.daosCARootPath = capRootPath
	addTestPools(t, ms.sysdb, mockUUID)
	cfg := new(mockDrpcClientConfig)
...
func TestServer_MgmtSvc_PoolAddCA_ChainValidation(t *testing.T) {
	...
	chainedPEM, _ := sectest.NewCA(t, security.PoolCACommonName(uuid.MustParse(mockUUID)), daosCACert, daosCAKey)
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			_ = tc.insecure // transport security no longer bypasses provenance
			addTestPools(t, ms.sysdb, mockUUID)
	...
	for name, tc := range map[string]struct {
		...
		"fresh CN is dated skew ahead": {
			...
			expWatermark: func(t time.Time) error {
				...
				earliest := time.Now().Add(security.NotBeforeSkewTolerance - 2*time.Second)
