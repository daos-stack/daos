...
// WithStorageAccelProps sets the storage accelerator properties for this instance.
func (c *Config) WithStorageAccelProps(engine string, opts ...string) *Config {
	if err := c.Storage.AccelProps.Set(engine, opts...); err != nil {
		panic(err)
	}
	return c
}
