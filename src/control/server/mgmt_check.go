...
func (svc *mgmtSvc) SystemCheckSetPolicy(ctx context.Context, req *mgmtpb.CheckSetPolicyReq) (*mgmtpb.DaosResp, error) {
	...
	// 'check set-policy' only changes policy, not flags, then reuse 'Flags' to
	// indicate something internally, such as do not dRPC call for test purpose.
	if req.Flags == 0 {
		...
		resp := new(mgmtpb.CheckSetPolicyResp)
		if err = proto.Unmarshal(dResp.Body, resp); err != nil {
