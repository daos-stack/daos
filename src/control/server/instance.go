...
func (ei *EngineInstance) determineRank(ctx context.Context, ready *srvpb.NotifyReadyReq) (ranklist.Rank, bool, uint32, uint32, error) {
	superblock := ei.getSuperblock()
	if superblock == nil {
		return ranklist.NilRank, false, 0, 0, errors.New("nil superblock while determining rank")
	}
	...
	if err != nil {
		...
		return ranklist.NilRank, false, 0, 0, err
	}
	switch resp.State {
	case system.MemberStateAdminExcluded, system.MemberStateExcluded:
		return ranklist.NilRank, resp.LocalJoin, 0, 0, errors.Errorf("rank %d excluded", resp.Rank)
	case system.MemberStateCheckerStarted:
		...
		if !ready.GetCheckMode() {
			...
			return ranklist.NilRank, resp.LocalJoin, 0, 0, errors.Errorf("rank %d restarting to enable checker", resp.Rank)
		}
	}
	...
	if !superblock.ValidRank || ready.Uri != superblock.URI {
		...
		*superblock.Rank = r
		...
		if err := ei.WriteSuperblock(); err != nil {
			return ranklist.NilRank, resp.LocalJoin, 0, 0, err
		}
	}

	return r, resp.LocalJoin, resp.MapVersion, resp.DaosVersion, nil
}
...
func (ei *EngineInstance) handleReady(ctx context.Context, ready *srvpb.NotifyReadyReq) error {
	...
	r, localJoin, mapVersion, daosVersion, err := ei.determineRank(ctx, ready)
	if err != nil {
		return err
	}
	...
	return ei.SetupRank(ctx, r, mapVersion, daosVersion)
}

func (ei *EngineInstance) SetupRank(ctx context.Context, rank ranklist.Rank, map_version uint32, daos_version uint32) error {
	if ei.IsReady() {
		ei.log.Debugf("SetupRank called on an already set-up instance %d", ei.Index())
		return nil
	}

	if err := ei.callSetRank(ctx, rank, map_version, daos_version); err != nil {
...
func (ei *EngineInstance) callSetRank(ctx context.Context, rank ranklist.Rank, map_version uint32, daos_version uint32) error {
	dresp, err := ei.callDrpc(ctx, daos.MethodSetRank, &mgmtpb.SetRankReq{Rank: rank.Uint32(), MapVersion: map_version, DaosVersion: daos_version})
	if err != nil {
		return err
	}
