...
type IOServerInstance struct {
	...
	msClient      *managementServiceClient
...
func NewIOServerInstance(ext External, log logging.Logger,
	bp *storage.BdevProvider, msc *managementServiceClient, r *ioserver.Runner) *IOServerInstance {
...
func (srv *IOServerInstance) WithClient(dcli drpc.DomainSocketClient) *IOServerInstance {
	srv.drpcClient = dcli
	return srv
}
...
func (srv *IOServerInstance) Shutdown(ctx context.Context) error {
	return srv.runner.Shutdown(ctx)
}
...
func (srv *IOServerInstance) callSetRank(rank ioserver.Rank) error {
	dresp, err := makeDrpcCall(srv.drpcClient, drpcClientID, setRank, &mgmtpb.SetRankReq{Rank: uint32(rank)})
	if err != nil {
		return err
	}
...
func (srv *IOServerInstance) callCreateMS() error {
	...
	dresp, err := makeDrpcCall(srv.drpcClient, drpcClientID, createMS, req)
	if err != nil {
		return err
	}
...
func (srv *IOServerInstance) callStartMS() error {
	dresp, err := makeDrpcCall(srv.drpcClient, drpcClientID, startMS, nil)
	if err != nil {
		return err
	}
...
func (srv *IOServerInstance) callSetUp() error {
	dresp, err := makeDrpcCall(srv.drpcClient, drpcClientID, setUp, nil)
	if err != nil {
		return err
	}
