...
func (svc *mgmtSvc) getFanoutNoAdminExcluded(req systemReq) (*fanoutRequest, *fanoutResponse, error) {
	fReq, fResp, err := svc.getFanout(req)
	if err != nil {
		return nil, nil, err
	}

	// If ranks not explicitly requested, we can filter out admin-excluded ranks.
	if req.GetRanks() == "" {
...
func (svc *mgmtSvc) filterAdminExcludedRanks(req *fanoutRequest) {
	for _, r := range req.Ranks.Ranks() {
...
func (svc *mgmtSvc) SystemStart(ctx context.Context, req *mgmtpb.SystemStartReq) (*mgmtpb.SystemStartResp, error) {
	...
	fReq, fResp, err := svc.getFanoutNoAdminExcluded(req)
	if err != nil {
		return nil, err
	}
