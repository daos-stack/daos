...
func (svc *mgmtSvc) PoolSetProp(parent context.Context, req *mgmtpb.PoolSetPropReq) (*mgmtpb.PoolSetPropResp, error) {
	...
	for _, prop := range req.GetProperties() {
		...
		case daos.PoolPropertyCACert:
			return nil, errors.New("ca_cert must be set via PoolAddCA/PoolRemoveCA")
...
func (svc *mgmtSvc) PoolGetCA(ctx context.Context, req *mgmtpb.PoolGetCAReq) (*mgmtpb.PoolGetCAResp, error) {
	...
	bundle, err := svc.readPoolProperty(ctx, req.GetSys(), req.GetId(), daos.PoolPropertyCACert)
	if err != nil {
...
func (svc *mgmtSvc) PoolAddCA(parent context.Context, req *mgmtpb.PoolAddCAReq) (*mgmtpb.PoolAddCAResp, error) {
	...
	if svc.allowInsecure {
		return nil, errors.New("node authentication requires transport security (allow_insecure: false)")
	}
	...
	// A CA the cluster cannot verify would lock everyone out of the pool.
	...
	if err := security.VerifyPoolCAIdentity(cert, poolUUID); err != nil {
		return nil, err
	}

	existing, err := svc.readPoolProperty(ctx, req.GetSys(), req.GetId(), daos.PoolPropertyCACert)
	if err != nil {
	...
	if err := svc.writePoolByteProperty(ctx, req.GetSys(), req.GetId(), daos.PoolPropertyCACert, combined); err != nil {
		return nil, errors.Wrap(err, "writing pool CA bundle")
	}

	resp := &mgmtpb.PoolAddCAResp{PoolUuid: poolUUID.String()}
	// Handles opened before the requirement never proved access.
	if len(existing) == 0 && !req.GetNoEvict() {
...
func (svc *mgmtSvc) PoolRemoveCA(parent context.Context, req *mgmtpb.PoolRemoveCAReq) (*mgmtpb.PoolRemoveCAResp, error) {
	...
	if removed > 0 && security.CountPEMCerts(remaining) == 0 {
...
func (svc *mgmtSvc) PoolRevokeClient(parent context.Context, req *mgmtpb.PoolRevokeClientReq) (*mgmtpb.PoolRevokeClientResp, error) {
	...
	caBundle, err := svc.readPoolProperty(ctx, req.GetSys(), req.GetId(), daos.PoolPropertyCACert)
	if err != nil {
		return nil, errors.Wrap(err, "reading pool CA bundle")
	}
	if len(caBundle) == 0 {
	...
	// Dated skew ahead: a cert issued from a host whose clock is ahead of
	// this one, but within the tolerance, must not postdate the revocation.
	now := time.Now().Add(svc.poolCertSkew())
	committed := security.AdvanceCertWatermark(wm, cn, now)
	wm[cn] = committed
	cutoff, err := security.WatermarkCutoff(caBundle)
	if err != nil {
		return nil, err
	}
	wm, _ = security.PruneCertWatermarks(wm, cutoff)
