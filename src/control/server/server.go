...
func Start(log *logging.LeveledLogger, cfg *Configuration) error {
	...
	for i, srvCfg := range cfg.Servers {
		...
		bp, err := storage.NewBdevProvider(harness.ext, log,
			srvCfg.Storage.SCM.MountPoint, &srvCfg.Storage.Bdev)
		if err != nil {
			return err
		}

		msClient := newManagementServiceClient(ctx, log, mscConfig{
	...
	addr := fmt.Sprintf("0.0.0.0:%d", cfg.ControlPort)
	...
	if !needsRespawn {
		mgmtpb.RegisterMgmtSvcServer(grpcServer, NewManagementService(harness))
