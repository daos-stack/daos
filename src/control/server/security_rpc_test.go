...
func generateTestCertChain(t *testing.T) *testCertChain {
	...
	nodeTemplate := &x509.Certificate{
		SerialNumber: serial,
		Subject:      pkix.Name{CommonName: security.CertCNPrefixNode + testMachineName},
		NotBefore:    time.Now().Add(-time.Minute),
		NotAfter:     time.Now().Add(time.Hour),
...
func TestSrvSecurityModule_ValidateNodeCert(t *testing.T) {
	...
	stalePayload := make([]byte, security.PoPPayloadLen)
	...
	staleSig, _ := security.SignPoP(chain.nodeKey, nil, stalePayload)
	...
	for name, tc := range map[string]struct {
		...
		"empty machine name on node-prefix cert": {
			nodePEM:     chain.nodePEM,
			poolCAPEM:   chain.poolCAPEM,
			payload:     validPayload,
			pop:         validSig,
...
func TestSrvSecurityModule_ValidateNodeCert_Watermarks(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"watermark equal to cert NotBefore revokes cert": {
			watermarks: map[string]string{
...
// A PoP captured for pool A must not authenticate a connect to pool B,
// even when the node cert chains to a CA shared between the two pools.
func TestSrvSecurityModule_ValidateNodeCert_CrossPoolReplay(t *testing.T) {
	chain := generateTestCertChain(t)
	...
	payload, sig := makePoP(t, chain.nodeKey, poolA, handle)
	...
	mod := NewSecurityModule(log, &security.TransportConfig{
		CertificateConfig: security.CertificateConfig{CARootPath: caPath},
	})

	// PoolId says pool B, but the PoP payload binds pool A.
	req := &auth.ValidateNodeCertReq{
		PoolCa:          chain.poolCAPEM,
		NodeCert:        chain.nodePEM,
		NodeCertPop:     sig,
		NodeCertPayload: payload,
		PoolId:          uuid.Must(uuid.FromBytes(poolB)).String(),
		MachineName:     testMachineName,
	}
	respBytes, err := mod.HandleCall(test.Context(t), nil, daos.MethodValidateNodeCert, marshal(t, req))
	if err != nil {
	...
	if daos.Status(resp.Status) != daos.NoPermission {
		t.Errorf("cross-pool replay: expected NoPermission, got %v", daos.Status(resp.Status))
	}
}

// A malformed entry anywhere in the pool CA bundle must reject the connect
// rather than be silently skipped.
func TestSrvSecurityModule_ValidateNodeCert_MalformedCABundle(t *testing.T) {
	chain := generateTestCertChain(t)
	poolUUID := make([]byte, 16)
	handleUUID := make([]byte, 16)
	rand.Read(poolUUID)
	...
	concat := func(extra *pem.Block) []byte {
		out := make([]byte, 0, len(chain.poolCAPEM)+200)
		out = append(out, chain.poolCAPEM...)
		return append(out, pem.EncodeToMemory(extra)...)
	}
	for name, badBundle := range map[string][]byte{
		"non-CERTIFICATE PEM block":   concat(&pem.Block{Type: "PRIVATE KEY", Bytes: []byte("x")}),
		"malformed CERTIFICATE block": concat(&pem.Block{Type: "CERTIFICATE", Bytes: []byte("not der")}),
	} {
		t.Run(name, func(t *testing.T) {
			req := &auth.ValidateNodeCertReq{
				PoolCa:          badBundle,
			...
			if daos.Status(resp.Status) != daos.BadCert {
				t.Errorf("expected BadCert, got %v", daos.Status(resp.Status))
			}
		})
	}
}
