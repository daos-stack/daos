...
// handleCacheMaxSize bounds the replay cache to prevent unbounded growth
// under an adversarial sender (a peer with a valid client cert spamming
// fresh handle UUIDs). Steady-state legitimate traffic plateaus well
// below this; reaching it is itself an attack signal.
...
type SecurityModule struct {
	...
	// handleCache tracks recently-seen (pool, handle) tuples for PoP
	// replay protection. Entries expire 2*maxClockSkew after insert.
...
func (m *SecurityModule) processValidateNodeCert(body []byte) ([]byte, error) {
	...
	case strings.HasPrefix(cn, security.CertCNPrefixNode):
		// Node-prefix certs require a credential machine name to
		// cross-check against. An empty value is a missing input,
		// not a bypass; the engine's get_sec_origin_for_token also
		// rejects empty machinenames, but enforce here as well so
		// defense-in-depth isn't conditional on an upstream check.
		if req.MachineName == "" {
	...
	// Check for replay within the skew window. Key on (pool, handle) so
	// the cache contract matches the PoP payload binding (pool_uuid ||
	// handle_uuid || ts).
	...
	// Only clean up when the cap is reached — i.e., under attack.
	// Steady-state traffic doesn't trigger this branch.
	if len(m.handleCache) >= handleCacheMaxSize {
		...
		// If sweeping expired entries didn't free space, drop one
		// arbitrary entry to make room for the new insert.
		if len(m.handleCache) >= handleCacheMaxSize {
