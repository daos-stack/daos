...
func TestServer_MgmtSvc_GetAttachInfo(t *testing.T) {
	msReplica := st.MockMember(t, 0, system.MemberStateJoined)
	nonReplica := st.MockMember(t, 1, system.MemberStateJoined)
...
func mockMember(t *testing.T, r, a int32, s string) *system.Member {
	...
	return system.NewMember(system.Rank(r), test.MockUUID(r), "", test.MockHostAddr(a), state)
}
...
func mgmtSystemTestSetup(t *testing.T, l logging.Logger, mbs system.Members, r ...[]*control.HostResponse) *mgmtSvc {
	...
	db := raft.MockDatabase(t, l)
	svc.membership = st.MockMembership(t, l, db, mockResolver)
	for _, m := range mbs {
