...
func harnessAction(ctx context.Context, msClient *mgmtSvcClient, req *RemoteHarnessReq) (system.MemberResults, error) {
	...
	rpcResp, err := requestFn(ctx, req.Addr, &mgmtpb.RanksReq{Ranks: req.Ranks})
	if err != nil {
		return nil, err
	}
	...
	for _, result := range rpcResp.Results {
		...
		memberResults = append(memberResults,
			system.NewMemberResult(result.Rank, req.Action.String(), err))
	}
