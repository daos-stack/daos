...
func AnnotateFinding(f *Finding) *Finding {
	...
	case FindingCodeScannedPoolMissing:
		...
		if len(f.Resolutions) == 0 {
			f.AddResolutions("Ignore", "Remove the pool components from storage targets")
		}
	...
	case FindingCodeMsPoolMissing:
		if f.Description == "" && len(f.Details) == 2 {
			f.Description = fmt.Sprintf("MS pool %s (%q) not found in storage scan", f.Details[0], f.Details[1])
		}
		if len(f.Resolutions) == 0 {
