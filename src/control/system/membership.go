...
func (m *Membership) OnEvent(_ context.Context, evi events.Event) {
	if common.InterfaceIsNil(evi) {
		m.log.Error("nil event")
		return
	}
	// FIXME: Get rid of events.Event and switch everything to use the events.RASEvent
	evt, ok := evi.(*events.RASEvent)
	if !ok {
		return
	}
	...
	case events.RASRankExit:
		...
		m.log.Debugf("update rank %d to %+v (%s)", evt.Rank, member,
			member.Info)
	case events.RASPoolSvcRanksUpdate:
		if evt == nil {
		...
		if ei == nil {
			m.log.Error("no extended info in PoolSvcRanksUpdate event received")
			return
		}
		...
		m.log.Debugf("update pool %s (state=%s) svc ranks %v->%v",
			ps.PoolUUID, ps.State, ps.Replicas, ei.SvcRanks)

		ps.Replicas = RanksFromUint32(ei.SvcRanks)
