...
func (tly *tally) OnEvent(_ context.Context, evt *RASEvent) {
	...
	tly.rx = append(tly.rx, evt)
	if len(tly.rx) == tly.expectedRx {
		close(tly.finished)
	}
}

func (tly *tally) getRx() []*RASEvent {
	tly.Lock()
	defer tly.Unlock()
...
func TestEvents_PubSub_Basic(t *testing.T) {
	...
	common.AssertEqual(t, []*RASEvent{evt1, evt1}, tly1.getRx(),
		"tly1 unexpected slice of received events")
	common.AssertEqual(t, []*RASEvent{evt1, evt1}, tly2.getRx(),
		"tly2 unexpected slice of received events")
}
...
func TestEvents_PubSub_DisableForwarding(t *testing.T) {
	evt1 := NewRankDownEvent("foo", 1, 1, common.ExitStatus("test"))
	...
	ps.DisableForwarding(evt1.ID, evt2.ID)
	...
	common.AssertEqual(t, tly1.getRx()[0].ShouldForward(), false, "expecting should not forward")
	common.AssertEqual(t, tly1.getRx()[1].ShouldForward(), false, "expecting should not forward")

	ps.EnableForwarding(evt1.ID, evt2.ID)
