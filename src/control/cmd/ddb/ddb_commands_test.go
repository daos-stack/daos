...
func testCmdHelp(t *testing.T, cmdStr string, helpSubStr string) {
	...
	if err != nil {
		t.Fatalf("Error parsing csum_dump help option: %v\n", err)
	}

	test.AssertTrue(t, strings.Contains(string(result.Bytes()), helpSubStr),
		"Expected help message not found in output: "+string(result.Bytes()))
}
...
func TestLs(t *testing.T) {
	for name, tc := range map[string]struct {
		args    []string
		expCall func(path string, recursive bool, details bool) error
		expErr  error
	}{
		"Invalid options": {
			args:   []string{"--bar"},
