...
// captureStdout replaces os.Stdout with a pipe, runs fn, restores os.Stdout, and returns
// the captured output along with any error from fn.
...
// runCmdToStdout calls parseOpts with the given args and captures stdout output.
// errHelpRequested is treated as a non-error (consistent with main()).
// Returns the parsed options, stdout output, and error.
func runCmdToStdout(ctx *DdbContextStub, args []string) (cliOptions, string, error) {
	var opts cliOptions
...
func runMainFlow(ctx *DdbContextStub, args []string) (string, error) {
	stdout, err := captureStdout(func() error {
