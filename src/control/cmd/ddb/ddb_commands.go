...
the path must include the extent, otherwise, it must not.`,
	...
	app.AddCommand(&grumble.Command{
		Name:      "setup",
		Aliases:   nil,
		Help:      "Prepare the environment for Metadata On SSD mode",
		LongHelp:  "",
		HelpGroup: "vos",
		Flags: func(f *grumble.Flags) {
		...
		Args: func(a *grumble.Args) {
			a.String("db_path", "Path to the vos db.")
			a.String("scm_mount", "Path to the scm mountpoint. Metadata will map on this path.")
		},
		Run: func(c *grumble.Context) error {
			return ddbSetup(ctx, c.Args.String("db_path"), c.Args.String("scm_mount"), c.Flags.Uint("scm_mount_size"))
		},
		Completer: nil,
	})
}
