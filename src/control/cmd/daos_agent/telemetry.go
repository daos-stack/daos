...
func startPrometheusExporter(ctx context.Context, log logging.Logger, cfg *Config) (func(), error) {
	expCfg := &promexp.ExporterConfig{
		...
		Register: func(ctx context.Context, log logging.Logger) error {
			c, err := promexp.NewClientCollector(ctx, log, &promexp.CollectorOpts{
