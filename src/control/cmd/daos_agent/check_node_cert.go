...
func (cmd *checkNodeCertCmd) runChecks(mi *msInfo) *nodeCertChecker {
	...
	if err := security.CheckValidity(cert, now, security.NotBeforeSkewTolerance); errors.Is(err, security.ErrCertNotYetValid) {
		c.warn("validity", fmt.Sprintf("%s (%s; the server accepts up to pool_cert_max_clock_skew early)", window, err))
	} else if err != nil {
...
func (cmd *checkNodeCertCmd) checkChain(c *nodeCertChecker, mi *msInfo, cert *x509.Certificate, now time.Time) {
	...
	if err := security.VerifyNodeCertChain(cert, root, mi.caBundle, now.Add(security.NotBeforeSkewTolerance)); err != nil {
