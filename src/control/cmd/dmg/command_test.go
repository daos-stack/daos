...
func (bci *bridgeConnInvoker) InvokeUnaryRPC(ctx context.Context, uReq control.UnaryRequest) (*control.UnaryResponse, error) {
	...
	case *control.PoolGetCAReq:
		resp = control.MockMSResponse("", nil, &mgmtpb.PoolGetCAResp{})
	...
	case *control.PoolRebuildManageReq:
		if req.OpCode == control.PoolRebuildOpCodeStart {
			resp = control.MockMSResponse("", nil, &mgmtpb.DaosResp{})
		} else if req.OpCode == control.PoolRebuildOpCodeStop {
			resp = control.MockMSResponse("", nil, &mgmtpb.DaosResp{})
		}
