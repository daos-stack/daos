...
func (cmd *smdQueryCmd) makeRequest(ctx context.Context, req *control.SmdQueryReq) error {
	...
	if err := pretty.PrintSmdInfoMap(resp.HostStorage, &bld); err != nil {
		return err
	}
...
type listDevicesQueryCmd struct {
	smdQueryCmd
	rankCmd
	Health bool   `short:"b" long:"health" description:"Include BIO health in results"`
