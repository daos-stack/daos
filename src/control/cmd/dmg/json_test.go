	...
	for _, args := range cmdArgs {
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			case "storage nvme-rebind":
				testArgs = append(testArgs, "-l", "foo.com", "-a", test.MockPCIAddr())
			case "storage query target-health":
				testArgs = append(testArgs, []string{"-r", "0", "-t", "0"}...)
			case "storage query device-health":
				testArgs = append(testArgs, []string{"-u", test.MockUUID()}...)
			case "storage set nvme-faulty":
				testArgs = append(testArgs, []string{"--force", "-u", test.MockUUID()}...)
			case "storage replace nvme":
				testArgs = append(testArgs, []string{"--old-uuid", test.MockUUID(), "--new-uuid", test.MockUUID()}...)
			case "storage identify vmd":
				testArgs = append(testArgs, []string{"--uuid", test.MockUUID()}...)
			case "pool create":
				testArgs = append(testArgs, []string{"-s", "1TB"}...)
			case "pool destroy", "pool evict", "pool query", "pool get-acl":
				testArgs = append(testArgs, []string{test.MockUUID()}...)
			case "pool overwrite-acl", "pool update-acl":
				testArgs = append(testArgs, []string{test.MockUUID(), "-a", aclPath}...)
			case "pool delete-acl":
				testArgs = append(testArgs, []string{test.MockUUID(), "-p", "foo@"}...)
			case "pool set-prop":
				testArgs = append(testArgs, []string{test.MockUUID(), "label:foo"}...)
			case "pool get-prop":
				testArgs = append(testArgs, []string{test.MockUUID(), "label"}...)
			case "pool extend":
				testArgs = append(testArgs, []string{test.MockUUID(), "--ranks", "0"}...)
			case "pool exclude", "pool drain", "pool reintegrate":
				testArgs = append(testArgs, []string{test.MockUUID(), "--rank", "0"}...)
			case "container set-owner":
				testArgs = append(testArgs, []string{"--user", "foo", "--pool", test.MockUUID(), "--cont", test.MockUUID()}...)
