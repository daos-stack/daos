...
type systemRebuildCmd struct {
	Start systemRebuildStartCmd `command:"start" description:"Start an interactive system rebuild"`
	Stop  systemRebuildStopCmd  `command:"start" description:"Stop an interactive system rebuild"`
}
...
func (cmd *systemRebuildOpCmd) execute(opCode control.RebuildOpCode) (errOut error) {
	defer func() {
	...
	req := &control.SystemRebuildReq{
		RebuildOp: opCode,
	}

	resp, err := control.SystemRebuild(cmd.MustLogCtx(), cmd.ctlInvoker, req)
	if err != nil {
		return err // control api returned an error, disregard response
	}
