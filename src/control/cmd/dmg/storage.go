...
func (cmd *nvmeSetFaultyCmd) Execute(_ []string) error {
	cmd.log.Notice("WARNING: This command will permanently mark the device as unusable!")
	if !cmd.Force {
		if cmd.jsonOutputEnabled() {
...
func (cmd *nvmeReplaceCmd) Execute(_ []string) error {
	if cmd.OldDevUUID == cmd.NewDevUUID {
		cmd.log.Notice("WARNING: Attempting to reuse a previously set FAULTY device!")
	}
