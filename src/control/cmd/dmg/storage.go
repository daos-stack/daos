...
func scmModuleTable(ms storage.ScmModules) string {
	buf := bytes.NewBufferString("SCM modules:\n")
...
func scmNsTable(nss storage.ScmNamespaces) string {
	buf := bytes.NewBufferString("SCM namespaces:\n")
	...
	for _, srv := range result.Servers {
		...
		if summary {
			...
			case len(sres.Namespaces) > 0:
				fmt.Fprintf(&buf, "%s", scmNsTable(sres.Namespaces))
			default:
				fmt.Fprintf(&buf, "%s", scmModuleTable(sres.Modules))
			}
...
func scanCmdDisplaySummary(result *client.StorageScanResp) (string, error) {
	...
	formatter := NewTableFormatter([]string{hostlistTitle, scmTitle, nvmeTitle})
	var table []TableRow

	for _, result := range groups.Keys() {
		row := TableRow{hostlistTitle: groups[result].RangedString()}
