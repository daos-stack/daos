...
func calcStorageRatio(scmSize, nvmeSize uint64) (ratio float64) {
	ratio = 1.0
	if nvmeSize > 0 {
		ratio = float64(scmSize) / float64(nvmeSize)
	}
	return
}
...
func (c *PoolCreateCmd) Execute(args []string) error {
	...
	scmSize, err := parseSize(c.ScmSize)
	if err != nil {
		return errors.Wrapf(err, "failed to parse %q as SCM size", c.ScmSize)
	}
	nvmeSize, err := parseSize(c.NVMeSize)
	if err != nil {
		return errors.Wrapf(err, "failed to parse %q as NVMe size", c.NVMeSize)
	}

	ratio := calcStorageRatio(scmSize, nvmeSize)
	if ratio < minStorageRatio {
		c.log.Infof("SCM:NVMe ratio is less than %.0f%%, DAOS performance will suffer!",
			minStorageRatio*100)
	}
	c.log.Infof("Creating DAOS pool with %s SCM and %s NVMe storage (%.3f ratio)",
		humanize.Bytes(scmSize), humanize.Bytes(nvmeSize), ratio)
	...
	req := &client.PoolCreateReq{
		ScmBytes: scmSize, NvmeBytes: nvmeSize,
