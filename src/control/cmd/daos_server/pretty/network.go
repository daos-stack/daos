...
func PrintFabricScan(fs []netdetect.FabricScan, out io.Writer, onlyProviders bool, searchProvider string) error {
	if len(fs) == 0 {
		return nil
	}
	...
	for _, fi := range fs {
		providers = append(providers, fi.Provider)
	}
	providers = dedupeStringSlice(providers)
	...
	for _, fi := range fs {
		...
		if fi.Provider == searchProvider || searchProvider == "" {
