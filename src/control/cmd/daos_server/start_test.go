...
func genMinimalConfig() *config.Configuration {
...
func genDefaultExpected() *config.Configuration {
...
func TestStartOptions(t *testing.T) {
	insecureTransport := config.NewConfiguration().TransportConfig
	insecureTransport.AllowInsecure = true

	for desc, tc := range map[string]struct {
		argList  []string
		expCfgFn func(*config.Configuration) *config.Configuration
		expErr   error
	}{
		"None": {
			argList:  []string{},
			expCfgFn: func(cfg *config.Configuration) *config.Configuration { return cfg },
		},
		"Port (short)": {
			argList: []string{"-p", "42"},
			expCfgFn: func(cfg *config.Configuration) *config.Configuration {
		...
		t.Run(desc, func(t *testing.T) {
			...
			var gotConfig *config.Configuration
			var opts mainOpts
			opts.Start.start = func(log *logging.LeveledLogger, cfg *config.Configuration) error {
				gotConfig = cfg
				return nil
			}
...
func TestStartLoggingConfiguration(t *testing.T) {
	for desc, tc := range map[string]struct {
		configFn  func(*config.Configuration) *config.Configuration
		logFnName string
		input     string
		wantRe    *regexp.Regexp
	}{
		"JSON": {
			configFn: func(cfg *config.Configuration) *config.Configuration {
