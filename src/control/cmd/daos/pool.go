...
func (cmd *poolBaseCmd) connectPool(flags daos.PoolConnectFlag) error {
	...
	case cmd.PoolID().HasLabel():
		req.ID = cmd.PoolID().Label
	case cmd.PoolID().HasUUID():
		req.ID = cmd.PoolID().UUID.String()
	default:
	...
	if err := cmd.pool.FillHandle(unsafe.Pointer(&cmd.cPoolHandle)); err != nil {
		if err := cmd.pool.Disconnect(cmd.MustLogCtx()); err != nil {
			cmd.Errorf("pool disconnect failed in cleanup: %v", err)
		}
		return err
	}
...
func (cmd *poolBaseCmd) getAttr(name string) (*daos.Attribute, error) {
...
type poolCmd struct {
	...
	GetAttr      poolListAttrCmd     `command:"get-attr" alias:"getattr" description:"get pool user-defined attribute"`
...
func (cmd *poolListAttrsCmd) Execute(_ []string) error {
	...
	if !cmd.Verbose {
		attrNames, err := cmd.pool.ListAttributes(cmd.MustLogCtx())
		if err != nil {
			return errors.Wrapf(err, "failed to list attributes for pool %s", cmd.PoolID())
		}
		attrs = attrListFromNames(attrNames)
	} else {
		var err error
		attrs, err = cmd.pool.GetAttributes(cmd.MustLogCtx())
		if err != nil {
...
func (cmd *poolListAttrCmd) Execute(_ []string) error {
	cleanup, err := cmd.resolveAndConnect(daos.PoolConnectFlagReadOnly, nil)
	if err != nil {
		return err
	}
	...
	attrs, err := cmd.pool.GetAttributes(cmd.MustLogCtx(), cmd.Args.Attrs.ParsedProps.ToSlice()...)
	if err != nil {
		return errors.Wrapf(err, "failed to get attributes for pool %s", cmd.PoolID())
	}
...
func (cmd *poolSetAttrCmd) Execute(_ []string) error {
	...
	if err := cmd.pool.SetAttributes(cmd.MustLogCtx(), attrs...); err != nil {
		return errors.Wrapf(err, "failed to set attributes on pool %s", cmd.PoolID())
	}
	cmd.Infof("Attributes successfully set on pool %q", cmd.PoolID())
...
func (cmd *poolDelAttrCmd) Execute(_ []string) error {
	...
	attrNames := cmd.Args.Attrs.ParsedProps.ToSlice()
	attrsString := strings.Join(attrNames, ",")
	if err := cmd.pool.DeleteAttributes(cmd.MustLogCtx(), attrNames...); err != nil {
		return errors.Wrapf(err, "failed to delete attributes %s on pool %s", attrsString, cmd.PoolID())
	}
	cmd.Infof("Attribute(s) %s successfully deleted on pool %q", attrsString, cmd.PoolID())
