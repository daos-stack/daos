...
// validateNodeCertForUse checks that a loaded cert is usable: not expired
// and, for node-scoped certs, CN-bound to machineName. NotBefore is the
// server's call (it knows the configured skew), so it is not checked here.
func validateNodeCertForUse(cert *x509.Certificate, poolID, machineName string, now time.Time) error {
	...
	if now.After(cert.NotAfter) {
		return errors.Wrapf(ErrCertInvalid, "node cert for pool %s expired at %s",
...
type NodeCertPresentation struct {
	...
	MaxSkew     time.Duration     // tolerate clock skew on cert NotBefore and PoP timestamps; zero defaults to NotBeforeSkewTolerance
...
func (p *NodeCertPresentation) Validate() (*PoPPayload, error) {
	...
	cert, err := p.verifyCert(now, maxSkew)
	if err != nil {
		return nil, err
	}
...
// verifyCert checks that the presented cert is acceptable. NotBefore is
// tolerated up to maxSkew so a freshly issued cert is usable at once;
// the leaf's NotAfter is strict.
func (p *NodeCertPresentation) verifyCert(now time.Time, maxSkew time.Duration) (*x509.Certificate, error) {
	...
	if err := CheckValidity(cert, now, maxSkew); err != nil {
		return nil, err
	}
