...
func TestNodeCertLoader_KeyCertMismatch(t *testing.T) {
	...
	keyPath := filepath.Join(tmpDir, poolUUID.String()+".key")
...
func TestSecurity_ValidateNodeCertForUse(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"not yet valid is the server's call": {CertCNPrefixNode + testMachineName, now.Add(time.Hour), notAfter, now, false},
	}
