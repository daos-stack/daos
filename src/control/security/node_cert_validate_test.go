...
func (f *validationFixture) reissue(t *testing.T, v *NodeCertPresentation, notBefore, notAfter time.Time) {
	...
	leafPEM, leafKey := sectest.NewLeafCert(t, CertCNPrefixNode+v.MachineName, f.caCert, f.caKey, notBefore, notAfter)
	pop, err := signPoP(leafKey, v.PoPPayload)
	if err != nil {
		t.Fatal(err)
	}
	v.Cert = leafPEM
...
func TestSecurity_NodeCertPresentation(t *testing.T) {
	...
	t.Run("not yet valid beyond skew", func(t *testing.T) {
		v := f.signedPresentation(t, CertCNPrefixNode+"machine1")
		f.reissue(t, v, time.Now().Add(NotBeforeSkewTolerance+time.Minute), time.Now().Add(time.Hour))
