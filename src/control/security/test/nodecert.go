...
func WriteNodeCert(t *testing.T, dir string, poolUUID uuid.UUID, cn string, notBefore, notAfter time.Time) {
	t.Helper()
	...
	if err := os.WriteFile(filepath.Join(dir, poolUUID.String()+".crt"), certPEM, 0644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(dir, poolUUID.String()+".key"), keyPEM, 0400); err != nil {
		t.Fatal(err)
	}
}
