...
static int
init_path(daos_handle_t poh, char *path, struct dv_indexed_tree_path *itp)
{
	int rc;
...
ddb_run_ls(struct ddb_ctx *ctx, struct ls_options *opt)
{
	...
	rc = init_path(ctx->dc_poh, opt->path, &itp);
	if (!SUCCESS(rc))
		return rc;
...
ddb_run_value_load(struct ddb_ctx *ctx, struct value_load_options *opt)
{
	...
	rc = init_path(ctx->dc_poh, opt->dst, &itp);
...
process_ilog_op(struct ddb_ctx *ctx, char *path, enum ddb_ilog_op op)
{
	...
	rc = init_path(ctx->dc_poh, path, &itp);
