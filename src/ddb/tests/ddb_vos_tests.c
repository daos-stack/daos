...
path_verify(void **state)
{
	...
	itp_free(&itp);
	/* set to an oid */
	sprintf(path, "/%s/"DF_UOID, g_uuids_str[8], DP_UOID(g_oids[0]));
	assert_success(itp_parse(path, &itp));
	assert_success(dv_path_verify(tctx->dvt_poh, &itp));
	assert_true(itp_has_obj_complete(&itp));
	itp_free(&itp);
	...
	itp_free(&itp);
	/* set to key */
	sprintf(path, "/%s/"DF_UOID"/%s", g_uuids_str[8], DP_UOID(g_oids[0]),
	...
	itp_free(&itp);
	/* set to key */
	sprintf(path, "/%s/"DF_UOID"/%s/%s", g_uuids_str[8], DP_UOID(g_oids[0]),
		(char*)g_dkeys[0].iov_buf,
	 ...
	 */
	/* set to an index */
	assert_success(itp_parse("[7]/[0]/[0]/[0]/[0]", &itp));
	assert_success(dv_path_verify(tctx->dvt_poh, &itp));
	assert_true(itp_has_recx_complete(&itp));
	itp_free(&itp);
	/* set to key */
	sprintf(path, "/%s/"DF_UOID"/%s/%s/"DF_DDB_RECX, g_uuids_str[8], DP_UOID(g_oids[0]),
		(char*)g_dkeys[0].iov_buf,
		(char*)g_akeys[0].iov_buf,
