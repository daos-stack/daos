...
/* [todo-ryon]: add some documentation around how keys are parsed */
enum key_value_type {
...
key_parse_bin(const char *input, daos_key_t *key)
{
	...
	for (i = 0; i < len; i+=2) {
		...
		tmp[0] = input[i];
		tmp[1] = input[i + 1];
		uint8_t byte = strtoul(tmp, NULL, 16);
		buf[i/2] = byte;
	}
...
key_parse_typed(const char *key_str, daos_key_t *key)
{
	...
	/* have key value ... just verifying the rest */
...
/*
 * return number of chars consumed, or error
 * Will allocate the memory for the key
 */
int
