...
is_idx_set(char *str, struct indexed_tree_path_part *part)
{
	...
	if (str[0] == '[' && str[str_len - 1] == ']') {
		part->itp_part_idx = atol(str + 1);
		part->itp_has_part_idx = true;
		return true;
	}
	return false;
}
...
bool
itp_part_value_set(struct dv_indexed_tree_path *itp, path_parts_e part_key, void *part_value)
{
...
bool
itp_set(struct dv_indexed_tree_path *itp, path_parts_e part_key,
	void *part_value, uint32_t part_idx)
{
	int i;
...
bool
itp_has_part_value(struct dv_indexed_tree_path *itp, path_parts_e part_key)
{
	return itp->itp_parts[part_key].itp_has_part_value;
}
...
itp_verify(struct dv_indexed_tree_path *itp)
{
	...
	uint32_t	path_part_to_error[] = {
	    /* Must match the ordering of path_parts_e */
	    DDBER_INVALID_CONT,
	    DDBER_INVALID_OBJ,
	    DDBER_INVALID_DKEY,
	    DDBER_INVALID_AKEY,
