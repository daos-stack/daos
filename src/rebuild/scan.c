...
rebuild_obj_send_cb(struct tree_cache_root *root, struct rebuild_send_arg *arg)
{
	...
	if (rc < 0 || arg->count == 0) {
		D_DEBUG(DB_REBUILD, DF_RBF " cannot get objects: " DF_RC "\n", DP_RBF_RPT(rpt),
			DP_RC(rc));
		D_GOTO(out, rc);
	}
	...
	D_DEBUG(DB_REBUILD,
		DF_RBF " send rebuild objects to tgt %d cnt %d stable epoch " DF_U64 "\n",
		DP_RBF_RPT(rpt), arg->tgt_id, arg->count, rpt->rt_stable_epoch);
	while (1) {
		enqueue_id = 0;
		max_delay = 0;
		...
		D_DEBUG(DB_REBUILD, DF_RBF " retry send object to tgt_id %d\n", DP_RBF_RPT(rpt),
			arg->tgt_id);
...
rebuild_tgt_iter_cb(daos_handle_t ih, d_iov_t *key_iov, d_iov_t *val_iov, void *data)
{
	...
	while (!dbtree_is_empty(root->root_hdl)) {
		...
		if (rc < 0) {
			DL_ERROR(rc, DF_RBF " rebuild_obj_send_cb failed", DP_RBF_RPT(arg->rpt));
			break;
		}
	}
		     ...
		     d_iov_t *val_iov, void *data)
{
	...
	while (!dbtree_is_empty(root->root_hdl)) {
		...
		if (rc < 0) {
			DL_ERROR(rc, DF_RBF " rebuild_tgt_send_cb failed", DP_RBF_RPT(arg->rpt));
			break;
		}
	}
...
rebuild_objects_send_ult(void *data)
{
	...
	while (!tls->rebuild_pool_scan_done || !dbtree_is_empty(tls->rebuild_tree_hdl)) {
		...
		if (rc < 0) {
			DL_ERROR(rc, DF_RBF " dbtree iterate failed", DP_RBF_RPT(rpt));
			break;
		}
		dss_sleep(0);
	}

	D_DEBUG(DB_REBUILD, DF_RBF " objects send finished\n", DP_RBF_RPT(rpt));
out:
	if (oids != NULL)
		D_FREE(oids);
	if (shards != NULL)
	...
	if (rc != 0 && tls->rebuild_pool_status == 0) {
		D_DEBUG(DB_REBUILD, DF_RBF " objects send error: " DF_RC "\n", DP_RBF_RPT(rpt),
			DP_RC(rc));
	    ...
	    unsigned *acts)
{
	...
	D_DEBUG(DB_REBUILD, DF_RBF " deleting stale object " DF_UOID " oid layout %u/%u",
		DP_RBF_RPT(rpt), DP_UOID(oid), oid.id_layout_ver, new_layout_ver);
	...
	do {
		...
		*acts |= VOS_ITER_CB_DELETE;
		...
		D_DEBUG(DB_REBUILD, DF_RBF " retry by " DF_RC "/" DF_UOID "\n", DP_RBF_RPT(rpt),
	...
	if (rc != 0)
		DL_ERROR(rc, DF_RBF " Failed to delete object " DF_UOID, DP_RBF_RPT(rpt),
			 DP_UOID(oid));
		    ...
		    void *data, unsigned *acts)
{
	...
	if (map == NULL) {
		D_ERROR(DF_RBF " " DF_UOID ": Cannot find valid placement map\n", DP_RBF_RPT(rpt),
			DP_UOID(oid));
	...
	if (oc_attr == NULL) {
		D_INFO(DF_RBF " skip invalid " DF_UOID "\n", DP_RBF_RPT(rpt), DP_UOID(oid));
		D_GOTO(out, rc = 0);
	}
	...
	if (rc <= 0) {
		DL_CDEBUG(rc == 0, DB_REBUILD, DLOG_ERR, rc, DF_RBF " " DF_UOID " rebuild shards",
			  DP_RBF_RPT(rpt), DP_UOID(oid));
		D_GOTO(out, rc);
	}

	D_DEBUG(DB_REBUILD, DF_RBF " rebuild obj " DF_UOID " rebuild_nr %d\n", DP_RBF_RPT(rpt),
		DP_UOID(oid), rc);
	rebuild_nr = rc;
	rc = 0;
	for (i = 0; i < rebuild_nr; i++) {
		D_DEBUG(DB_REBUILD,
			DF_RBF " cont " DF_UUID " rebuild obj " DF_UOID " on %d for shard %d"
			       " eph " DF_U64 " visible %s\n",
			DP_RBF_RPT(rpt), DP_UUID(arg->co_uuid), DP_UOID(oid), tgts[i], shards[i],
		...
		if ((int)tgts[i] == -1 || oid.id_shard / grp_size != shards[i] / grp_size) {
			D_DEBUG(DB_REBUILD,
				DF_RBF " i %d stale object " DF_UOID " shards %u "
				       "grp_size %u tgt %d\n",
				DP_RBF_RPT(rpt), i, DP_UOID(oid), shards[i], grp_size,
				(int)tgts[i]);
			continue;
		}
...
out:
	...
	if (--arg->yield_freq == 0 || arg->obj_yield_cnt <= 0) {
		D_DEBUG(DB_REBUILD, DF_RBF " rebuild yield: %d\n", DP_RBF_RPT(rpt), rc);
			  ...
			  void *data, unsigned *acts)
{
	...
	if (uuid_compare(arg->co_uuid, entry->ie_couuid) == 0) {
		D_DEBUG(DB_REBUILD, DF_RBF " co_uuid " DF_UUID " already scanned\n",
			DP_RBF_RPT(rpt), DP_UUID(arg->co_uuid));
		return 0;
	}

	rc = vos_cont_open(iter_param->ip_hdl, entry->ie_couuid, &coh);
	if (rc == -DER_NONEXIST) {
		D_DEBUG(DB_REBUILD, DF_RBF " co_uuid " DF_UUID " already destroyed\n",
			DP_RBF_RPT(rpt), DP_UUID(arg->co_uuid));
		return 0;
	}

	if (rc != 0) {
		DL_ERROR(rc, DF_RBF " Open container " DF_UUID " failed", DP_RBF_RPT(rpt),
	...
	if (rc == -DER_NONEXIST || rc == -DER_SHUTDOWN) {
		D_DEBUG(DB_REBUILD, DF_RBF " co_uuid " DF_UUID " already destroyed or destroying\n",
			DP_RBF_RPT(rpt), DP_UUID(arg->co_uuid));
		rc = 0;
		D_GOTO(close, rc);
	}
	...
	if (rc != 0) {
		DL_ERROR(rc, DF_RBF " Container " DF_UUID ", ds_cont_child_lookup failed",
			 DP_RBF_RPT(rpt), DP_UUID(entry->ie_couuid));
		D_GOTO(close, rc);
	}
	...
	rc = ds_cont_fetch_snaps(rpt->rt_pool->sp_iv_ns, entry->ie_couuid, NULL,
				 &snapshot_cnt);
	if (rc) {
		DL_ERROR(rc, DF_RBF " Container " DF_UUID ", ds_cont_fetch_snaps failed",
			 DP_RBF_RPT(rpt), DP_UUID(entry->ie_couuid));
		D_GOTO(close, rc);
	}
	...
	if (rc) {
		DL_ERROR(rc, DF_RBF " Container " DF_UUID ", ds_cont_get_props failed",
			 DP_RBF_RPT(rpt), DP_UUID(entry->ie_couuid));
		D_GOTO(close, rc);
	}
	...
	while (cont_child->sc_ec_agg_active &&
		...
		D_INFO(DF_RBF " " DF_UUID " wait for ec agg abort.\n", DP_RBF_RPT(rpt),
		       DP_UUID(entry->ie_couuid));
		dss_sleep(1000);
		if (rpt->rt_abort || rpt->rt_finishing) {
			D_DEBUG(DB_REBUILD, DF_RBF " " DF_UUID " rebuild abort %u/%u.\n",
				DP_RBF_RPT(rpt), DP_UUID(entry->ie_couuid), rpt->rt_abort,
				rpt->rt_finishing);
...
close:
	...
	D_DEBUG(DB_REBUILD, DF_RBF " " DF_UUID " iterate cont done: " DF_RC "\n", DP_RBF_RPT(rpt),
...
rebuild_scanner(void *data)
{
	...
	if (!is_rebuild_scanning_tgt(rpt)) {
		D_DEBUG(DB_REBUILD, DF_RBF " skip scan\n", DP_RBF_RPT(rpt));
		D_GOTO(out, rc = 0);
	}
	...
	while (daos_fail_check(DAOS_REBUILD_TGT_SCAN_HANG)) {
		...
		D_DEBUG(DB_REBUILD, DF_RBF " sleep 2 seconds then retry\n", DP_RBF_RPT(rpt));
		dss_sleep(2 * 1000);
	}
	...
	if (rc != 0) {
		DL_ERROR(rc, DF_RBF " failed to create rebuild tree", DP_RBF_RPT(rpt));
		D_GOTO(out, rc);
	}
...
out:
	...
	D_DEBUG(DB_REBUILD, DF_RBF " iterate pool done: " DF_RC "\n", DP_RBF_RPT(rpt), DP_RC(rc));
	return rc;
}
...
rebuild_scan_leader(void *data)
{
	...
	D_DEBUG(DB_REBUILD, DF_RBF " check resync %u/%u < %u\n", DP_RBF_RPT(rpt),
	...
	while (rpt->rt_global_dtx_resync_version < rpt->rt_rebuild_ver) {
		if (!rpt->rt_abort && !rpt->rt_finishing) {
			...
			if (rpt->rt_global_dtx_resync_version < rpt->rt_rebuild_ver) {
				D_INFO(DF_RBF " wait for global dtx %u\n", DP_RBF_RPT(rpt),
		...
		if (rpt->rt_abort || rpt->rt_finishing) {
			DL_INFO(-DER_SHUTDOWN, DF_RBF ": shutdown rebuild", DP_RBF_RPT(rpt));
			D_GOTO(out, rc = -DER_SHUTDOWN);
		}
	}

	D_DEBUG(DB_REBUILD, DF_RBF " scan collective begin\n", DP_RBF_RPT(rpt));
	...
	D_DEBUG(DB_REBUILD, DF_RBF "rebuild scan collective done\n", DP_RBF_RPT(rpt));
	...
	ABT_mutex_unlock(rpt->rt_lock);
	if (rc) {
		DL_ERROR(rc, DF_RBF " send rebuild object list failed", DP_RBF_RPT(rpt));
		D_GOTO(out, rc);
	}

	D_DEBUG(DB_REBUILD, DF_RBF " sent objects to initiator: " DF_RC "\n", DP_RBF_RPT(rpt),
		DP_RC(rc));
out:
	tls = rebuild_pool_tls_lookup(rpt->rt_pool_uuid, rpt->rt_rebuild_ver,
				      rpt->rt_rebuild_gen);
	...
	if (tls->rebuild_pool_status == 0 && rc != 0)
		tls->rebuild_pool_status = rc;
	DL_INFO(rc, DF_RBF " scan leader done", DP_RBF_RPT(rpt));
	rpt_put(rpt);
}
