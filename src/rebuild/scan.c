		    ...
		    void *data, unsigned *acts)
{
	...
	if (rc <= 0) {
		D_ERROR(DF_UOID " rebuild checked layout, rc %d\n", DP_UOID(oid), rc);
		D_GOTO(out, rc);
	}

	if (oc_attr->ca_grp_nr == 6)
		D_ERROR("rebuild obj "DF_UOID" rebuild_nr %d\n", DP_UOID(oid), rc);

	rebuild_nr = rc;
	rc = 0;
	for (i = 0, j = 0; i < rebuild_nr; i++) {
		...
		if (rc == -DER_ALREADY)
			...
			D_ERROR("rebuild obj "DF_UOID"/"DF_UUID"/"DF_UUID" on tgt %d for "
				"shard %d, eph "DF_U64", grp_nr %u, grp_sz %u, visible %s\n",
				DP_UOID(oid), DP_UUID(rpt->rt_pool_uuid), DP_UUID(arg->co_uuid),
				tgts[i], shards[i], ent->ie_epoch, oc_attr->ca_grp_nr, grp_size,
