...
static int
rebuild_iv_ent_init(struct ds_iv_key *iv_key, void *data, struct ds_iv_entry *entry)
{
	int rc;
...
static int
rebuild_iv_ent_fetch(struct ds_iv_entry *entry, struct ds_iv_key *key, d_sg_list_t *dst,
		     void **priv)
{
	D_ASSERT(0);
	return 0;
...
static int
rebuild_iv_ent_update(struct ds_iv_entry *entry, struct ds_iv_key *key, d_sg_list_t *src,
		      void **priv)
{
	...
	rgt = rebuild_global_pool_tracker_lookup(src_iv->riv_pool_uuid, src_iv->riv_ver,
	...
	if (rgt->rgt_leader_term == src_iv->riv_leader_term) {
		...
		if (!src_iv->riv_global_done) {
			rgt->rgt_status.rs_toberb_obj_nr += src_iv->riv_toberb_obj_count;
		...
		D_DEBUG(DB_REBUILD,
			"update rebuild " DF_UUID " ver %d gen %u"
			" toberb_obj/rb_obj/rec/global state/status/rank/abort " DF_U64 "/" DF_U64
			"/" DF_U64 "/%d/%d/%d/%d\n",
			...
			rgt->rgt_status.rs_rec_nr, rgt->rgt_status.rs_state,
			rgt->rgt_status.rs_errno, src_iv->riv_rank, rgt->rgt_abort);
	}
	rgt_put(rgt);
out:
...
static int
rebuild_iv_ent_refresh(struct ds_iv_entry *entry, struct ds_iv_key *key, d_sg_list_t *src,
		       int ref_rc, void **priv)
{
	...
	rpt = rpt_lookup(src_iv->riv_pool_uuid, -1, src_iv->riv_ver, src_iv->riv_rebuild_gen);
	if (rpt == NULL)
		return 0;
...
static int
rebuild_iv_alloc(struct ds_iv_entry *entry, struct ds_iv_key *key, d_sg_list_t *sgl)
{
...
int
rebuild_iv_update(void *ns, struct rebuild_iv *iv, unsigned int shortcut, unsigned int sync_mode,
		  bool retry)
{
	d_sg_list_t      sgl;
	d_iov_t          iov;
