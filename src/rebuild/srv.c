			     ...
			     int op, unsigned int rank)
{
	...
	D_DEBUG(DB_REBUILD, "rebuild IV %u final "DF_UUID"/%u dtx %u: %d\n",
		op, DP_UUID(rgt->rgt_pool_uuid), rgt->rgt_rebuild_ver,
		iv.riv_global_dtx_resyc_version, rgt->rgt_status.rs_errno);
 ...
 */
static void
rebuild_leader_status_check(struct ds_pool *pool, uint32_t map_ver, uint32_t max_ver, uint32_t op,
			    struct rebuild_global_pool_tracker *rgt)
{
	double			last_print = 0;
	unsigned int		total;
	...
	while (1) {
		...
		for (i = 0; i < excluded.rl_nr; i++) {
			...
			if (dom->do_comp.co_status == PO_COMP_ST_UP &&
			    dom->do_comp.co_in_ver < max_ver)
				continue;

			D_DEBUG(DB_REBUILD, "exclude rank %d/%x.\n", dom->do_comp.co_rank,
		...
		snprintf(sbuf, RBLD_SBUF_LEN,
			 ...
			 "toberb_obj=" DF_U64", rb_obj="DF_U64", rec="DF_U64", size="DF_U64
			 " done %d status %d/%d epoch "DF_U64" duration=%d secs)\n",
			 RB_OP_STR(op), str, DP_UUID(pool->sp_uuid), myrank,
			 rgt->rgt_leader_term, rgt->rgt_dtx_resync_version, map_ver,
...
rebuild_scan_broadcast(struct ds_pool *pool, struct rebuild_global_pool_tracker *rgt,
		       struct pool_target_id_list *tgts_failed, uint32_t max_ver,
		       daos_rebuild_opc_t rebuild_op)
{
	...
	if (rebuild_op == RB_OP_REINT || rebuild_op == RB_OP_EXTEND ||
		...
		for (i = 0; i < up_ranks.rl_nr; i++) {
			...
			D_ASSERT(dom != NULL);
			D_DEBUG(DB_REBUILD, "rank %u ver %u max_ver %u\n",
				up_ranks.rl_ranks[i], dom->do_comp.co_in_ver, max_ver);
			if (dom->do_comp.co_in_ver <= max_ver)
				continue;
...
rebuild_debug_print_queue()
{
	...
	d_list_for_each_entry(task, &rebuild_gst.rg_queue_list, dst_list) {
		...
		D_DEBUG(DB_REBUILD, DF_UUID" op=%s ver=%u max_ver=%u tgts=%s\n",
			DP_UUID(task->dst_pool_uuid), RB_OP_STR(task->dst_rebuild_op),
			task->dst_map_ver, task->dst_max_ver, tgts_buf);
	}
}

static int
rebuild_task_add_tgts(struct ds_pool *pool, struct rebuild_task *task,
		      struct pool_target_id_list *tgts)
{
	int i;
	int rc;
	...
	for (i = 0; i < tgts->pti_number; i++) {
		...
		rc = pool_map_find_target(pool->sp_map, tgts->pti_ids[i].pti_id,
					  &target);
		if (target == NULL)
			continue;

		if (target->ta_comp.co_status & (PO_COMP_ST_DOWN | PO_COMP_ST_DOWNOUT |
						 PO_COMP_ST_DRAIN))
			task->dst_max_ver = max(task->dst_max_ver, target->ta_comp.co_fseq);
		else
			task->dst_max_ver = max(task->dst_max_ver, target->ta_comp.co_in_ver);
	}
		       ...
		       struct pool_target_id_list *tgts)
{
	...
	/* Merge the failed ranks to existing rebuild task */
	rc = rebuild_task_add_tgts(pool, merge_task, tgts);
	if (rc)
		return rc;
		     ...
		     struct rebuild_global_pool_tracker **p_rgt)
{
	...
	rc = rebuild_prepare(pool, task->dst_map_ver, task->dst_rebuild_gen,
			     leader_term, &task->dst_tgts, task->dst_rebuild_op, p_rgt);
	if (rc) {
	...
	rc = rebuild_scan_broadcast(pool, *p_rgt, &task->dst_tgts, task->dst_max_ver,
				    task->dst_rebuild_op);
	if (rc) {
...
rebuild_task_ult(void *arg)
{
	...
	rebuild_leader_status_check(pool, task->dst_map_ver, task->dst_map_ver,
...
try_reschedule:
	if (rgt == NULL || !is_rebuild_global_done(rgt) ||
		...
		ret = ds_rebuild_schedule(pool, task->dst_map_ver, ++task->dst_rebuild_gen,
					  &task->dst_tgts, opc, task->dst_max_ver, 5);
		if (ret != 0)
...
ds_rebuild_schedule(struct ds_pool *pool, uint32_t map_ver, uint32_t rebuild_gen,
		    struct pool_target_id_list *tgts, daos_rebuild_opc_t rebuild_op,
		    uint32_t orig_max_ver, uint64_t delay_sec)
{
	...
	rc = rebuild_task_add_tgts(pool, new_task, tgts);
	if (rc)
		D_GOTO(free, rc);

	/* Reclaim task needs to keep the same max version as the reintegration
	 * task to make sure the same ranks/targets for both reintegration/extend
	 * and reclaim.
	 */
	if (rebuild_op == RB_OP_RECLAIM) {
		D_ASSERT(orig_max_ver != 0);
		new_task->dst_max_ver = orig_max_ver;
	}
