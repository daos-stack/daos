...
ddb_run_dev_list(struct ddb_ctx *ctx, struct dev_list_options *opt)
{
	...
	d_list_for_each_entry_safe(dev_info, tmp, &dev_list, bdi_link) {
		ddb_printf(ctx, "Device:" DF_UUID " [inuse:%d, faulty:%d, plugged:%d]\n",
			   DP_UUID(dev_info->bdi_dev_id), dev_info->bdi_flags & NVME_DEV_FL_INUSE,
			   dev_info->bdi_flags & NVME_DEV_FL_FAULTY,
			   dev_info->bdi_flags & NVME_DEV_FL_PLUGGED);
...
ddb_run_dev_replace(struct ddb_ctx *ctx, struct dev_replace_options *opt)
{
	...
	if (uuid_is_null(opt->old_devid)) {
		ddb_error(ctx, "Old device isn't specified\n");
		return -DER_INVAL;
	}

	if (uuid_is_null(opt->new_devid)) {
		ddb_error(ctx, "New device isn't specified\n");
		return -DER_INVAL;
	}

	if (uuid_compare(opt->old_devid, opt->new_devid) == 0) {
	...
	ddb_printf(ctx,
		   ...
		   DP_UUID(opt->old_devid), DP_UUID(opt->new_devid), db_path);

	rc = dv_dev_replace(db_path, opt->old_devid, opt->new_devid);
	if (rc)
