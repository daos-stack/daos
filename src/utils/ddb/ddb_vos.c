...
#define DEFAULT_SYS_MEM_RSVD (26UL * GiB)
#define DEFAULT_ENG_MEM_RSVD (1UL * GiB)
#define DDB_SETUP_BUF_MAX    256
...
static int
auto_calculate_scm_mount_size(unsigned int *scm_mount_size)
{
	int            rc = 0;
	struct sysinfo info;
	unsigned long  sz_byte             = 0;
	unsigned long  sys_rsvd_byte       = DEFAULT_SYS_MEM_RSVD;
	unsigned long  engine_rsvd_byte    = DEFAULT_ENG_MEM_RSVD;
	 ...
	 *
	 * 'setup' only recreate one engine's scm info each once. So 'nr engines' set to 1.
...
static int
tmpfs_setup_precheck(const char *path, unsigned int size_limit, bool *need_mount)
{
	int           rc = 0;
	uint64_t      tmpfs_size;
	struct stat   st_path, st_parent;
	struct statfs fs;
	char          parent_path[DDB_SETUP_BUF_MAX];
...
static int
ddb_tmpfs_setup(const char *scm_mount)
{
	char   newborns_path[DDB_SETUP_BUF_MAX] = {0};
	char   zombies_path[DDB_SETUP_BUF_MAX]  = {0};
	mode_t stored_mode;
	int    rc;

	/** create the path string */
	snprintf(newborns_path, sizeof(newborns_path), "%s/NEWBORNS", scm_mount);
	snprintf(zombies_path, sizeof(zombies_path), "%s/ZOMBIES", scm_mount);
	...
	struct recreate_tgts_arg recreate_arg = {
	    .bind_cpu_fn         = NULL,
	    .cleanup_newborns_fn = NULL,
	    .newborns_path       = newborns_path,
	    .vos_storage_path    = scm_mount,
	};

	rc = vos_pool_recreate_tgts(&recreate_arg);
...
int
dv_run_setup(const char *db_path, const char *scm_mount, unsigned int scm_mount_size)
{
	int          rc;
	bool         md_on_ssd;
	bool         need_mount                 = true;
	unsigned int sz                         = scm_mount_size;
	char         options[DDB_SETUP_BUF_MAX] = {0};
	...
	if (!md_on_ssd) {
		D_ERROR("Not md on ssd mode, Ignore to setup.");
		goto out;
	}

	/* Fetch scm_mount_size */
	if (sz == 0) {
		rc = auto_calculate_scm_mount_size(&sz);
		if (rc) {
	...
	rc = tmpfs_setup_precheck(scm_mount, sz, &need_mount);
	if (rc) {
	...
	snprintf(options, sizeof(options) - 1, "mpol=prefer:0,size=%dg,huge=always", sz);
