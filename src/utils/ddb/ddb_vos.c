...
static int
prov_mem_setup_precheck(const char *path, unsigned int size_limit, bool *need_mount)
{
	int           rc = 0;
	uint64_t      tmpfs_size;
...
static int
ddb_prov_mem_setup(const char *scm_mount)
{
	int    rc = 0;
	mode_t stored_mode;
...
dv_run_prov_mem(const char *db_path, const char *scm_mount, unsigned int scm_mount_size)
{
	...
	rc = prov_mem_setup_precheck(scm_mount, sz, &need_mount);
	if (rc) {
	...
	/* setup vos_file */
	rc = ddb_prov_mem_setup(scm_mount);
