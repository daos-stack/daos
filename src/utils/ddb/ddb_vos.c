 ...
 */
#define DDB_VOS_FILE "vos-" /* suffixed by thread id */
#define DDB_RDB_FILE "rdb-"
...
static char *newborns_path = NULL;
/** directory for destroyed pool */
static char *zombies_path = NULL;

static int
tmpfs_path_gen(const uuid_t pool_uuid, const char *dir, const char *fname, int *idx, char **fpath)
{
	int size;
	int off;
...
tgt_vos_preallocate(uuid_t uuid, daos_size_t scm_size, int tgt_id)
{
	...
	rc = tmpfs_path_gen(uuid, newborns_path, DDB_VOS_FILE, &tgt_id, &path);
	if (rc)
		goto out;
...
static int
tgt_recreate(const char *path, uuid_t pool_uuid, daos_size_t scm_size, int tgt_nr,
	     daos_size_t rdb_blob_sz)
{
	...
	rc = tmpfs_path_gen(pool_uuid, path, NULL, NULL, &pool_path);
	if (rc) {
	...
	rc = tmpfs_path_gen(pool_uuid, newborns_path, NULL, NULL, &pool_newborn_path);
	if (rc) {
	...
	/** create VOS files */
	rc = tgt_vos_preallocate_parallel(pool_uuid, scm_size, tgt_nr, &dummy_cancel_state);
	if (rc) {
	...
	if (rdb_blob_sz) {
		rc = tmpfs_path_gen(pool_uuid, newborns_path, DDB_RDB_FILE "pool", NULL, &rdb_path);
		if (rdb_path == NULL) {
...
static int
ddb_tmpfs_setup(const char *path)
{
	struct smd_pool_info *pool_info = NULL;
	...
	D_ASPRINTF(newborns_path, "%s/NEWBORNS", path);
	if (newborns_path == NULL)
		D_GOTO(err, rc = -DER_NOMEM);
	D_ASPRINTF(zombies_path, "%s/ZOMBIES", path);
	...
	d_list_for_each_entry_safe(pool_info, tmp, &pool_list, spi_link) {
		D_INFO("recreating vos files for pool " DF_UUID "\n", DP_UUID(pool_info->spi_id));
		...
		rc = tgt_recreate(path, pool_info->spi_id, pool_info->spi_scm_sz,
...
int
dv_run_setup(const char *db_path, const char *scm_mount)
{
	int rc;
	...
	if (bio_nvme_configured(SMD_DEV_TYPE_META)) {
		...
		DL_ERROR(rc, "Not md on ssd mode, Ignore to setup.");
		rc = -DER_NOTSUPPORTED;
	}
