			 ...
			 char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, options_short, options_long, &index)) != -1) {
		switch (opt) {
		case 'o':
			rc = uuid_parse(optarg, cmd_args->old_devid);
			if (rc) {
			...
			break;
		case 'n':
			rc = uuid_parse(optarg, cmd_args->new_devid);
			if (rc) {
...
ddb_commands_help(struct ddb_ctx *ctx)
{
	...
	ddb_print(ctx, "\tList scanned devices and devices recorded in SMD\n");
	ddb_print(ctx, "    [db_path]\n");
	ddb_print(ctx, "\tPath to the vos db. (default /mnt/daos)\n");
	ddb_print(ctx, "\n");
	...
	ddb_print(ctx, "\tReplaced an old device recorded in SMD with a new unused device\n");
	ddb_print(ctx, "    [db_path]\n");
	ddb_print(ctx, "\tPath to the vos db. (default /mnt/daos)\n");
	ddb_print(ctx, "Options:\n");
...
ddb_program_help(struct ddb_ctx *ctx)
{
	...
	ddb_print(ctx, "   dev_list	     List scanned devices and devices recorded in SMD\n");
	ddb_print(ctx, "   dev_replace	     Replace an old device with new device in SMD\n");
}
