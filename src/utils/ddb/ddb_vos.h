 ...
 *
 * \param[in]  path       VOS pool file path in the format
 *                        "[/dir/]<pool-uuid>/(vos-N|rdb-pool)".
 * \param[in]  db_path    Path to the VOS metadata DB directory (SMD/NVMe).
 *                        If NULL or empty, the DB directory is derived from
 *                        the leading path component of \p path.
 * \param[out] poh        Pool handle set on success.
 * \param[in]  flags      Flags forwarded to vos_pool_open() (e.g. VOS_POF_FOR_FEATURE_FLAG
 *                        to skip VEA load when only reading/writing pool feature flags).
 * \param[in]  write_mode When false the pool is mapped copy-on-write so that
 *                        internal PMEMOBJ bookkeeping (SDS, ULOG replay) does
 *                        not persist to the storage medium.
 *
