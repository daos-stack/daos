...
pool_get_prop_hdlr(struct cmd_args_s *ap)
{
	...
	rc = daos_pool_connect(ap->p_uuid, ap->sysname,
			       ...
			       NULL /* info */, NULL /* ev */);
	if (rc != 0) {
		fprintf(stderr, "failed to connect to pool "DF_UUIDF": %s\n",
			ap->p_uuid, d_errdesc(rc));
		D_GOTO(out, rc);
	}
...
pool_set_attr_hdlr(struct cmd_args_s *ap)
{
	...
	if (rc != 0) {
		fprintf(stderr, "failed to set attribute %s for pool "DF_UUIDF
			": %s\n", ap->attrname_str, ap->p_uuid, d_errdesc(rc));
		D_GOTO(out_disconnect, rc);
	}
...
pool_query_hdlr(struct cmd_args_s *ap)
{
	...
	if (rc != 0) {
		fprintf(stderr, "failed to query pool "DF_UUIDF": %s\n",
			ap->p_uuid, d_errdesc(rc));
		D_GOTO(out_disconnect, rc);
	}
...
cont_get_prop_hdlr(struct cmd_args_s *ap)
{
	...
	if (rc) {
		fprintf(stderr, "failed to query container "DF_UUIDF": %s\n",
			ap->c_uuid, d_errdesc(rc));
		D_GOTO(err_out, rc);
	}
...
cont_destroy_hdlr(struct cmd_args_s *ap)
{
	...
	if (rc != 0)
		fprintf(stderr, "failed to destroy container "DF_UUIDF": %s\n",
			ap->c_uuid, d_errdesc(rc));
	else
...
parse_acl_file(const char *path, struct daos_acl **acl)
{
	...
	while (getline(&line, &line_len, instream) != -1) {
		...
		if (rc != 0) {
			fprintf(stderr,
				"Error parsing ACE '%s' from file: %s\n",
				trimmed, d_errstr(rc));
			D_GOTO(parse_err, rc);
		}
		...
		daos_ace_free(ace);
		if (rc != 0) {
			fprintf(stderr, "Error parsing ACL file: %s\n",
				d_errstr(rc));
			D_GOTO(parse_err, rc);
		}
