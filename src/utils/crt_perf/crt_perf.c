...
static const char   *crt_perf_short_options = "hc:d:p:H:P:k:l:bC:Z:y:z:w:x:BRvMf:g:aD";
...
crt_perf_usage(const char *execname)
{
	...
	printf("    -a, --daos_agent     Use DAOS agent (experimental)\n");
		...
		for (j = 0; j < opts->request_max; j++) {
			...
			if (opts->daos_agent)
				*(struct crt_perf_daos_rate_in *)in_req =
				    (struct crt_perf_daos_rate_in){
					.iov    = (d_iov_t){.iov_buf     = info->rpc_buf,
