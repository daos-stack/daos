...
static const char   *crt_perf_short_options = "hc:d:p:H:P:k:l:bC:Z:y:z:w:x:BRvMf:";
...
crt_perf_context_cleanup(struct crt_perf_context_info *info)
{
	...
	if (info->remote_bulk_handle_ids != NULL) {
		free(info->remote_bulk_handle_ids);
		info->remote_bulk_handle_ids = NULL;
	}
	if (info->bulk_requests != NULL) {
		free(info->bulk_requests);
		info->bulk_requests = NULL;
	}
			...
			bool init_data, bool bulk_create)
{
	...
	info->bulk_bufs = (void **)calloc(info->handle_max, sizeof(*info->bulk_bufs));
	CRT_PERF_CHECK_ERROR(info->bulk_bufs == NULL, error, rc, -DER_NOMEM, "malloc(%zu) failed",
			     info->handle_max * sizeof(*info->bulk_bufs));

	info->local_bulk_handles =
	    (crt_bulk_t *)calloc(info->handle_max, sizeof(*info->local_bulk_handles));
	CRT_PERF_CHECK_ERROR(info->local_bulk_handles == NULL, error, rc, -DER_NOMEM,
			     "calloc(%zu) failed",
			     info->handle_max * sizeof(*info->local_bulk_handles));
	...
	for (i = 0; i < info->handle_max; i++) {
		...
		CRT_PERF_CHECK_ERROR(info->bulk_bufs[i] == NULL, error, rc, -DER_NOMEM,
				     "aligned_alloc(%zu, %zu) failed", page_size,
				     info->buf_size_max);
...
crt_perf_bulk_init_cb(crt_rpc_t *rpc)
{
	...
	if (info->bulk_bufs == NULL) {
		...
		info->remote_bulk_handles =
		    (crt_bulk_t *)calloc(info->handle_max, sizeof(*info->remote_bulk_handles));
		CRT_PERF_CHECK_ERROR(info->remote_bulk_handles == NULL, error, rc, -DER_NOMEM,
				     "malloc(%zu) failed",
				     info->handle_max * sizeof(*info->remote_bulk_handles));

		info->remote_bulk_handle_ids =
		    (uint32_t *)calloc(info->handle_max, sizeof(*info->remote_bulk_handle_ids));
		...
		info->bulk_requests = (struct crt_perf_request *)calloc(
		    info->handle_max, sizeof(*info->bulk_requests));
...
crt_perf_init(int argc, char *argv[], bool listen, struct crt_perf_info *info)
{
	...
	CRT_PERF_CHECK_ERROR(info->opts.protocol == NULL, error, rc, -DER_INVAL,
			     "protocol is required, use -p or --protocol to specify");
	...
	if (info->opts.comm == NULL && strchr(info->opts.protocol, '+') != NULL)
		crt_init_options.cio_provider = info->opts.protocol;
	else if (info->opts.comm != NULL) {
		snprintf(provider, sizeof(provider), "%s+%s", info->opts.comm, info->opts.protocol);
		crt_init_options.cio_provider = provider;
	} else
		crt_init_options.cio_provider = info->opts.protocol;
	...
	if (info->opts.port != NULL) {
		...
		crt_init_options.cio_port = port_str;
	}
	...
	if (info->mpi_info.rank == 0)
		printf("# Class %d using info string: %s://%s%s%s%s%s\n", 1,
		 ...
		 size_t buf_size, size_t skip)
{
	...
	for (i = 0; i < skip + (size_t)opts->loop; i++) {
		...
		for (j = 0; j < opts->request_max; j++) {
			...
			*in_iov = (struct iovec){.iov_base = info->rpc_buf, .iov_len = buf_size};
