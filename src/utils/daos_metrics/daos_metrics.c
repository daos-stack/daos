...
static int
process_metrics(int metric_id, char *dirname, int format, int filter,
		int extra_descriptors, int delay, int num_iter,
		d_tm_iter_cb_t iter_cb, void *arg)
{
...
static void
iter_reset(struct d_tm_context *ctx, struct d_tm_node_t *node, int level,
	   char *path, int format, int opt_fields, void *arg)
{
...
main(int argc, char **argv)
{
	...
	while (1) {
		...
		opt = getopt_long_only(argc, argv, "S:cCdtsgi:p:D:MmTrj:he",
				       long_options, NULL);
		if (opt == -1)
			break;
	...
	rc = process_metrics(srv_idx, dirname, format, filter, extra_descriptors,
			     delay, num_iter, iter_cb, stdout);
	if (rc)
