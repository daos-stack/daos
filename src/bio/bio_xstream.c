	       ...
	       unsigned int *dev_state)
{
	...
	*dev_state = dev_info->sdi_state;
	...
	if (d_bdev == NULL)
		D_ERROR("Device " DF_UUID " [model:%s, serial:%s] for target %d type %d "
			"isn't plugged or the SMD table is stale/corrupted.\n",
			DP_UUID(dev_info->sdi_id), dev_info->sdi_model, dev_info->sdi_serial,
			tgt_id, st);
	smd_dev_free_info(dev_info);
