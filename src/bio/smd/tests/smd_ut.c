...
ut_device(void **state)
{
	...
	for (i = 1; i < 3; i++) {
		rc = smd_dev_add_tgt(id1, i);
		assert_int_equal(rc, 0);
	}

	rc = smd_dev_add_tgt(id2, 1);
	assert_int_equal(rc, -DER_EXIST);

	rc = smd_dev_add_tgt(id2, 3);
	assert_int_equal(rc, 0);
...
ut_pool(void **state)
{
	...
	for (i = 0; i < 4; i++) {
		rc = smd_pool_add_tgt(id1, i, i << 10);
		assert_int_equal(rc, 0);

		rc = smd_pool_add_tgt(id2, i, i << 20);
		assert_int_equal(rc, 0);
	}

	rc = smd_pool_add_tgt(id1, 0, 5000);
	assert_int_equal(rc, -DER_EXIST);
