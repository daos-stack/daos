...
get_hotplug_busid_range(const char *nvme_conf)
{
	...
	/**
	 * Attempt to capture daos object but accept that it might be missing in the case that
	 * daos_nvme.conf file was generated by a v2.0 server.
	 */
	rc = spdk_json_find(ctx->values, "daos_data", NULL, &daos_data,
			    SPDK_JSON_VAL_OBJECT_BEGIN);
	if (rc < 0) {
		D_WARN("Failed to find 'daos_data' key: %s, config may have been generated by a "
		       "v2.0 daos_server\n", strerror(-rc));
		rc = 0;
		goto out; /* non-fatal */
	}
