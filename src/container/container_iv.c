		   ...
		   d_sg_list_t *src, void **priv)
{
	...
	if (src == NULL) {
		...
		if (uuid_is_null(civ_key->cont_uuid)) {
			...
			D_DEBUG(DB_TRACE, "delete tree "DF_UUID"\n", DP_UUID(civ_key->cont_uuid));
...
int
cont_iv_entry_delete(void *ns, uuid_t cont_uuid)
{
	int rc;

	D_DEBUG(DB_MD, "delete iv entry "DF_UUID"\n", DP_UUID(cont_uuid));
	...
	if (rc != 0)
		D_DEBUG(DB_MD, "delete agg epoch boundry "DF_UUID"\n", DP_UUID(cont_uuid));
