		  ...
		  d_sg_list_t *src, void **_priv)
{
	...
	if (ns_entry->ns->iv_master_rank == myrank) {
		...
		rc = ds_cont_oid_fetch_add(key->poh_uuid, key->key_id, num_oids, &avail->oid);
		if (rc) {
...
int
oid_iv_reserve(void *ns, uuid_t poh_uuid, uuid_t co_uuid, uint64_t num_oids, d_sg_list_t *value)
{
