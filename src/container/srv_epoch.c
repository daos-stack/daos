...
#define DF_EPOCH_PROP		"GLRE="DF_U64" GHCE="DF_U64" GHPCE="DF_U64 \
				" GLHE="DF_U64
#define DP_EPOCH_PROP(prop)	prop->ep_glre, prop->ep_ghce, prop->ep_ghpce, \
				prop->ep_glhe
...
update_ghce(struct rdb_tx *tx, struct cont *cont, struct epoch_prop *prop)
{
	...
	if (ghce < prop->ep_ghce) {
		D_ERROR(DF_CONT": GHCE would decrease: "DF_EPOCH_PROP"\n",
			DP_CONT(cont->c_svc->cs_pool_uuid, cont->c_uuid),
			DP_EPOCH_PROP(prop));
		return -DER_IO;
