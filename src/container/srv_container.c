...
/* callback function for setting containers status. */
static int
set_cont_cb(daos_handle_t ih, d_iov_t *key, d_iov_t *val, void *varg)
{
	struct set_cont_iter_args	*ap = varg;
	uuid_t				 cont_uuid;
	struct cont			*cont;
	d_iov_t				 value;
	int				 rc;
...
out_free_prop:
	D_DEBUG(DB_MD, "pool/cont: "DF_CONTF" set: "DF_RC"\n",
		DP_CONT(ap->pool_uuid, cont_uuid), DP_RC(rc));
	if (prop)
		daos_prop_free(prop);
	cont_put(cont);
...
/**
 * Set all containers healthy status
 *
 * \param[in]	pool_uuid	Pool UUID.
 * \param[in]	co_status	Container status
 */
int
ds_cont_set_status(uuid_t pool_uuid, uint64_t co_status)
{
	int				 rc;
	struct rdb_tx			 tx;
	struct set_cont_iter_args	 args = { 0 };
	struct cont_svc			*svc;
...
out_svc:
	D_DEBUG(DB_MD, DF_UUID" set all containers status: rc %d\n", DP_UUID(pool_uuid), rc);
	cont_svc_put_leader(svc);
