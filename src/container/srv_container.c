		  ...
		  struct cont *cont, crt_rpc_t *rpc)
{
	...
	switch (opc_get(rpc->cr_opc)) {
	case CONT_OPEN:
		d_tm_increment_counter(metrics->op_open_ctr, 1);
		d_tm_increment_gauge(metrics->open_cont_gauge, 1);
		rc = cont_open(tx, pool_hdl, cont, rpc);
		break;
	case CONT_CLOSE:
		d_tm_increment_counter(metrics->op_close_ctr, 1);
		d_tm_decrement_gauge(metrics->open_cont_gauge, 1);
		rc = cont_close(tx, pool_hdl, cont, rpc);
		break;
	case CONT_DESTROY:
		d_tm_increment_counter(metrics->op_destroy_ctr, 1);
