		     ...
		     struct daos_llink **link)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": opening\n", DP_CONT(po_uuid, co_uuid));
...
cont_child_free_ref(struct daos_llink *llink)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": freeing\n",
		DP_CONT(cont->sc_pool->spc_uuid, cont->sc_uuid));
		  ...
		  struct ds_cont_child **cont)
{
	...
	if (rc != 0) {
		if (rc == -DER_NONEXIST)
			D_DEBUG(DB_DSMS, DF_CONT": failed to lookup%s "
				"container: "DF_RC"\n",
				DP_CONT(po_uuid, co_uuid),
...
cont_child_stop(struct ds_cont_child *cont_child)
{
	...
	while ((hdl = d_list_pop_entry(&cont_child->sc_open_hdls,
		...
		D_DEBUG(DB_DSMS, "Force closing container open handle "DF_UUID"/"DF_UUID"\n",
	...
	if (cont_child_started(cont_child)) {
		D_DEBUG(DB_DSMS, DF_CONT"[%d]: Stopping container\n",
...
ds_cont_child_stop_all(struct ds_pool_child *pool_child)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID"[%d]: Stopping all containers\n",
		 ...
		 struct ds_cont_child **cont_out)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT"[%d]: Starting container\n",
	...
	rc = cont_child_lookup(tls->dt_cont_cache, co_uuid,
			       ...
			       &cont_child);
	if (rc) {
		D_CDEBUG(rc != -DER_NONEXIST, DLOG_ERR, DB_DSMS,
...
ds_cont_child_start_all(struct ds_pool_child *pool_child)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID"[%d]: Starting all containers\n",
		DP_UUID(pool_child->spc_uuid),
		dss_get_module_info()->dmi_tgt_id);
...
cont_hdl_rec_free(struct d_hash_table *htable, d_list_t *rlink)
{
	...
	D_DEBUG(DB_DSMS, "freeing "DF_UUID"\n", DP_UUID(hdl->sch_uuid));
...
cont_destroy_wait(struct ds_pool_child *child, uuid_t co_uuid)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": wait container destroy\n",
	...
	D_DEBUG(DB_DSMS, DF_CONT": container destroy done\n",
		DP_CONT(child->spc_uuid, co_uuid));
#endif
}
...
cont_child_destroy_one(void *vin)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": destroying vos container\n",
			...
			struct ds_cont_child **cont_out)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": creating new vos container\n",
		DP_CONT(pool_uuid, cont_uuid));
	...
	if (hdl != NULL) {
		if (flags != 0) {
			if (hdl->sch_flags != flags) {
				...
				D_DEBUG(DB_DSMS, DF_CONT": found compatible"
...
err_cont:
	if (daos_handle_is_valid(poh)) {
		...
		D_DEBUG(DB_DSMS, DF_CONT": destroying new vos container\n",
			DP_CONT(pool_uuid, cont_uuid));
...
cont_close_hdl(uuid_t cont_hdl_uuid)
{
	...
	if (hdl == NULL) {
		D_DEBUG(DB_DSMS, DF_CONT": already closed: hdl="DF_UUID"\n",
	...
	if (cont_child != NULL) {
		D_DEBUG(DB_DSMS, DF_CONT": closing (%d): hdl="DF_UUID"\n",
			DP_CONT(cont_child->sc_pool->spc_uuid,
				cont_child->sc_uuid),
...
ds_cont_tgt_epoch_aggregate_handler(crt_rpc_t *rpc)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": handling rpc %p: epr (%p) [#"DF_U64"]\n",
	...
	out->tao_rc = 0;
	D_DEBUG(DB_DSMS, DF_CONT": replying rpc %p: "DF_RC"\n",
		DP_CONT(in->tai_pool_uuid, in->tai_cont_uuid), rpc,
		DP_RC(out->tao_rc));
	crt_reply_send(rpc);
	if (out->tao_rc != 0)
...
ds_cont_oid_alloc_handler(crt_rpc_t *rpc)
{
	...
	D_DEBUG(DB_DSMS, DF_CONT": processing rpc %p: hdl="DF_UUID" opc=%u\n",
	...
	D_DEBUG(DB_DSMS, DF_CONT": replying rpc %p: hdl="DF_UUID
		" opc=%u rc="DF_RC"\n",
		DP_CONT(pool_hdl->sph_pool->sp_uuid, in->ci_uuid), rpc,
