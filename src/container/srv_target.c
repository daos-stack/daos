...
static bool
cont_aggregate_runnable(struct ds_cont_child *cont, struct sched_request *req, struct agg_param *param)
{
	struct ds_pool	*pool = cont->sc_pool->spc_pool;
	...
	if (param->ap_vos_agg) {
		...
		if (pool->sp_reintegrating) {
			...
			D_DEBUG(DB_EPC, DF_CONT": skip aggregation during reintegration %d.\n",
				DP_CONT(cont->sc_pool->spc_uuid, cont->sc_uuid), pool->sp_reintegrating);
			return false;
		} else {
