...
cont_query_complete(tse_task_t *task, void *data)
{
	...
	for (i = 0; (arg->cqa_info->ci_snapshots && (i < arg->cqa_info->ci_nsnapshots)); i++) {
		D_DEBUG(DF_DSMC, DF_CONT": ci_snapshots[%u]=%"PRIu64"\n",
			DP_CONT(pool->dp_pool, cont->dc_uuid), i, arg->cqa_info->ci_snapshots[i]);
	}
	/* TODO */
...
dc_cont_query(tse_task_t *task)
{
	...
	if (args->info && args->info->ci_snapshots && (args->info->ci_nsnapshots > 0)) {
		...
		D_DEBUG(DF_DSMC, DF_CONT": created bulk xfer, iov buf=%p, buf_len=%zu, len=%zu\n",
					DP_CONT(pool->dp_pool, cont->dc_uuid), iov.iov_buf,
