		  ...
		  struct fuse_file_info *fi_out, bool is_new, fuse_req_t req)
{
	...
	/* This is the major hash table operation in dfuse and represents most operations that are
	 * not either (add|dec)ref or find.
	 *
	...
	if (rlink != &ie->ie_htl) {
		...
		/* Check for conflicts, in either the dfs or oid space.  This can happen because of
		 * the fact we squash larger identifiers into a shorter 64 bit space, but if the
		 * bitshifting is right it shouldn't happen until there are a large number of active
		...
		/* Check the OID */
		if (ie->ie_oid.lo != inode->ie_oid.lo || ie->ie_oid.hi != inode->ie_oid.hi) {
		...
		if (ie->ie_stat.st_ino == ie->ie_dfs->dfs_ino) {
			...
			DFUSE_TRA_DEBUG(inode, "File has moved from '%s to '%s'", inode->ie_name,
					ie->ie_name);
		 ...
		 daos_size_t len)
{
	...
	/* Check for an existing dfp, otherwise allocate a new one. */
...
dfuse_cb_lookup(fuse_req_t req, struct dfuse_inode_entry *parent, const char *name)
{
	...
	rc = dfs_lookupx(parent->ie_dfs->dfs_ns, parent->ie_obj, name, O_RDWR | O_NOFOLLOW,
			 &ie->ie_obj, NULL, &ie->ie_stat, 1, &duns_xattr_name, (void **)&outp,
			 &attr_len);
	if (rc) {
