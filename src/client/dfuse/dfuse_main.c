...
show_help(char *name)
{
	printf(
	    ...
	    "  as positional arguments, using either UUIDs or labels.\n"
	    ...
	    "* A special pathname can be constructed by using the pool and container UUIDs\n"
	    "  as the first and second pathname components.  TODO: Need a better explanation...\n"
	    ...
	    "By default, the (TODO: clarify total or fuse?) thread count is one per available core\n"
	    "(TODO: hyperthread core or physical core?) to allow maximum throughput.\n"
	    "This can be modified in two ways: Reducing the number of available cores\n"
	    "by running dfuse in a cpuset via numactl or similar tools,\n"
	    ...
	    "* Increasing the --eq-count option at a fixed --thread-count will reduce the number of\n"
	    "  fuse threads accordingly. The default value for --eq-count is 1.\n"
	    "* The --singlethread mode will use one thread for handling fuse requests and a second\n"
	    ...
	    "\n"
	    "The -o option can be used to run dfuse via fstab or similar and accepts standard mount\n"
	    ...
	    "* TODO: What happens if --disable-caching and --enable-wb-cache are both specified?\n"
	    "\n"
	    "version: %s\n",
	    name, DAOS_VERSION);
}
