...
discover_daos_mount(void)
{
	...
	if (pool == NULL) {
		D_WARN("DAOS_POOL is not set.");
		return;
	}

	container = getenv("DAOS_CONTAINER");
	if (container == NULL) {
		D_WARN("DAOS_CONTAINER is not set.");
		return;
	}
...
discover_dfuse(void)
{
	...
	if (fp == NULL) {
		D_ERROR("failed to open file /proc/self/mounts: %s\n", strerror(errno));
		return;
	}
...
retrieve_handles_from_fuse(int idx)
{
	...
	if (fd < 0) {
		errno_saved = errno;
		D_ERROR("failed to open dir %s: %s\n", dfs_list[idx].fs_root, strerror(errno));
		errno = errno_saved;
		return (-1);
	}
	...
	if (rc != 0) {
		errno_saved = errno;
		D_ERROR("failed to query size info from dfuse with ioctl(): %s\n", strerror(errno));
		goto err;
	}
	...
	if (rc != 0) {
		errno_saved = errno;
		D_ERROR("failed to query pool handle from dfuse with ioctl(): %s\n",
			strerror(errno));
		goto err;
	}
	...
	if (rc != 0) {
		...
		D_ERROR("failed to create pool handle in daos_pool_global2local(): %s\n",
	...
	if (rc != 0) {
		errno_saved = errno;
		D_ERROR("failed to query container handle from dfuse with ioctl(): %s\n",
			strerror(errno));
		goto err;
	}
	...
	if (rc != 0) {
		errno_saved = daos_der2errno(rc);
		D_ERROR("failed to create container handle in daos_pool_global2local(): %s\n",
			strerror(errno_saved));
		goto err;
	}
	...
	if (rc != 0) {
		errno_saved = errno;
		D_ERROR("failed to query DFS handle from dfuse with ioctl(): %s\n",
			strerror(errno));
		goto err;
	}
	...
	if (rc != 0) {
		errno_saved = daos_der2errno(rc);
		D_ERROR("failed to create DFS handle in daos_pool_global2local(): %s\n",
			strerror(errno_saved));
		goto err;
	}
	...
	if (rc) {
		errno_saved = daos_der2errno(rc);
		D_ERROR("failed to create hash table for dir: %s\n", strerror(errno_saved));
		goto err;
	}
	D_FREE(buff);
...
init_fd_list(void)
{
	...
	if (pthread_mutex_init(&lock_fd, NULL) != 0) {
		D_ERROR("mutex create_new_lock lock_fd init failed: %s\n", strerror(errno));
		exit(1);
	}
	if (pthread_mutex_init(&lock_dirfd, NULL) != 0) {
		D_ERROR("mutex create_new_lock lock_dirfd init failed: %s\n", strerror(errno));
		exit(1);
	}
	if (pthread_mutex_init(&lock_mmap, NULL) != 0) {
		D_ERROR("mutex create_new_lock lock_mmap init failed: %s\n", strerror(errno));
		exit(1);
	}
	if (pthread_mutex_init(&lock_fd_dup2ed, NULL) != 0) {
		D_ERROR("mutex create_new_lock lock_fd_dup2ed init failed: %s\n", strerror(errno));
		exit(1);
	}
...
find_next_available_dirfd(struct dir_obj *obj, int *new_dir_fd)
{
	...
	if (next_free_dirfd < 0) {
		...
		if (allocated)
			D_FREE(new_obj);
		D_ERROR("Failed to allocate dirfd: out of space.\n");
		return EMFILE;
	}
	idx = next_free_dirfd;
	if (idx >= 0) {
...
find_next_available_map(int *idx)
{
	...
	*idx = -1;
	...
	if (next_free_map < 0) {
		...
		D_ASSERT(rc == 0);
		D_ERROR("Failed to allocate space from mmap_list[]: out of space.\n");
		return EMFILE;
	}
	*idx = next_free_map;
...
close_dup_fd_src(int (*next_close)(int fd), int fd)
{
	...
	if (idx_dup < 0) {
		D_ERROR("failed to find fd %d in fd_dup2_list[].\n", fd);
		errno = EINVAL;
		return (-1);
	}
	if (fd_dest > 0)
...
pread(int fd, void *buf, size_t size, off_t offset)
{
	...
	if (rc) {
		D_ERROR("dfs_read(%p, %zu) failed: %s\n", (void *)ptr, size, strerror(rc));
		errno      = rc;
		bytes_read = -1;
	}
...
pwrite(int fd, const void *buf, size_t size, off_t offset)
{
	...
	if (rc) {
		D_ERROR("dfs_write(%p, %zu) failed: %s\n", (void *)ptr, size, strerror(rc));
		errno = rc;
		return (-1);
	}
	if (bLog)
		...
		case S_IFREG:
			...
			while (byte_left > 0) {
				...
				if (byte_read != byte_to_write) {
					close(fd);
					/* Unexpected!!! */
					D_ERROR("error to read %" PRIu64 " bytes from %s\n",
				...
				if (rc != byte_to_write) {
					close(fd);
					/* Unexpected!!! */
					D_ERROR("error to write %" PRIu64 " bytes from %s\n",
...
ioctl(int fd, unsigned long request, ...)
{
	...
	/* We output the message only if env is set. */
	if (bLog)
...
dup2(int oldfd, int newfd)
{
	...
	if (fd_Directed >= FD_FILE_BASE) {
		...
		if (fd < 0) {
			...
			errno_save = errno;
			D_ERROR("failed to get a fd from kernel: %s\n", strerror(errno));
			errno = errno_save;
			return (-1);
		} else if (fd != newfd) {
...
init_myhook(void)
{
	...
	if (pthread_mutex_init(&lock_dfs, NULL) != 0) {
		D_ERROR("mutex create_new_lock lock_dfs failed: %s\n", strerror(errno));
		/* fatal error: failed to create a mutex. */
		exit(1);
	}
	init_fd_list();
...
out_err_ht:
	...
	if (rc2 != 0)
		D_ERROR("error in dfs_umount(%s): %s\n", dfs_list[idx].fs_root, strerror(rc2));
