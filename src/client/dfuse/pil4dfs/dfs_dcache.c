		  ...
		  uint32_t gc_period, uint32_t gc_reclaim_max)
{
	...
	if (rc != 0)
		D_GOTO(error_alloc, rc = daos_der2errno(rc));
...
dcache_destroy_act(dfs_dcache_t *dcache)
{
	...
	while ((rlink = d_hash_rec_first(&dcache->dd_dir_hash)) != NULL) {
		...
		gc_del_rec(dcache, dlist2drec(rlink));
...
static int
dcache_create_dact(dfs_dcache_t **dcache, dfs_t *dfs)
{
	dfs_dcache_t *dcache_tmp = NULL;
	int           rc;
	      ...
	      uint32_t gc_period, uint32_t gc_reclaim_max)
{
	...
	if (rec_timeout == 0) {
		return dcache_create_dact(dcache, dfs);
	}

	return dcache_create_act(dcache, dfs, bits, rec_timeout, gc_period, gc_reclaim_max);
}
int
