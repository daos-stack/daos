...
entry_stat(dfs_t *dfs, daos_handle_t th, daos_handle_t oh, const char *name, size_t len,
	   struct dfs_obj *obj, struct stat *stbuf, uint64_t *obj_hlc)
{
	...
	if (dfs->layout_v > 2)
		...
		rc = fetch_entry(dfs->layout_v, oh, th, name, len, true, &exists, &entry, 0, NULL,
				 NULL, NULL);
	if (rc)
		return rc;
...
restart:
	...
	rc = fetch_entry(dfs->layout_v, parent->oh, th, file->name, len, false, &exists, entry, 0,
			 NULL, NULL, NULL);
	if (rc) {
...
fopen:
	...
	rc = daos_array_open_with_attr(dfs->coh, entry->oid, th, daos_mode, 1, entry->chunk_size,
				       &file->oh, NULL);
	if (rc != 0) {
		D_ERROR("daos_array_open_with_attr() failed, " DF_RC "\n", DP_RC(rc));
		D_GOTO(out, rc = daos_der2errno(rc));
...
static int
open_sb(daos_handle_t coh, bool create, daos_obj_id_t super_oid, dfs_attr_t *attr,
	daos_handle_t *oh, dfs_layout_ver_t *ver)
{
	...
	if (iods[2].iod_size != sizeof(layout_ver) || layout_ver > DFS_LAYOUT_VERSION) {
		...
		dfs_t **_dfs)
{
	...
	rc = daos_obj_generate_oid_by_rf(poh, rf_factor, &roots.cr_oids[0], 0, dattr.da_oclass_id,
					 0, 0);
	if (rc) {
	...
	rc = daos_obj_generate_oid_by_rf(poh, rf_factor, &roots.cr_oids[1], 0, dattr.da_oclass_id,
					 0, 0);
	if (rc) {
...
dfs_access(dfs_t *dfs, dfs_obj_t *parent, const char *name, int mask)
{
	...
	if (mask != F_OK) {
		if (dfs->layout_v <= 2)
			...
			rc =
			    check_access(entry.uid, entry.gid, getuid(), getgid(), sym->mode, mask);
	}
	dfs_release(sym);
...
dfs_listxattr(dfs_t *dfs, dfs_obj_t *obj, char *list, daos_size_t *size)
{
	...
	while (!daos_anchor_is_eof(&anchor)) {
		...
		for (ptr = enum_buf, i = 0; i < number; i++) {
			...
			len = snprintf(ptr_list, kds[i].kd_key_len - 1, "%s", ptr + 2);
