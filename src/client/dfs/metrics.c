...
csv_file_path(pid_t pid, const char *root_dir, char **file_dir, char **file_name)
{
	...
	/*
	 * NB: Default dc_jobid is $nodename-$pid, which is not very useful
	 * as an organizational scheme.
	 *
	 * If the jobid is not default, then the path should look like:
	 *	$root_dir/job/$jobid/$procname/$nodename/$now-$pid.csv
	 * If the jobid is default, then the path should look like:
	 * 	$root_dir/proc/$procname/$nodename/$now-$pid.csv
	 */
	...
	if (strstr(dc_jobid, name.nodename) != NULL)
		D_ASPRINTF(tmp_dir, "%s%sproc/%s/%s", path_prefix, path_sep, get_process_name(),
			   name.nodename);
	else
		D_ASPRINTF(tmp_dir, "%s%sjob/%s/%s/%s", path_prefix, path_sep, dc_jobid,
			   get_process_name(), name.nodename);
	...
	D_ASPRINTF(tmp_name, "%s-%d.csv", time_str, pid);
	if (tmp_name == NULL) {
