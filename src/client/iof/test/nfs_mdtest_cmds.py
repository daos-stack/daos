#!/usr/bin/env python3
"""
Please see nfs_mdtest_main.yml for prerequisite setup details.
...
nfs_mdtest_cmds.py needs to exist in the following directories:

install/Linux/TESTING/scripts/

from within the root of the iof repository.

nfs_mdtest_cmds.py does the following:
...
6) Mount the export dir from export node on to the mount dir on the mount
   node using paramiko.
7) Perform the following mdtest operations in the mount dir, on mount node
   using paramiko:
   read & write
...
class Nfs_Mdtest_Cmds():
    ...
    def __init__(self, test_info=None, log_base_path=None):
        ...
        self.test_dir = "mdtest"
        self.test_timeout = 30 * 10 # Time in seconds.

        # Build test cmd shell environment.

        ompi_oversubscribe = "OMPI_MCA_rmaps_base_oversubscribe={!s}".format(
            self.test_info.get_defaultENV('OMPI_MCA_rmaps_base_oversubscribe'))

        # Build mpi multi-task cmd.

        mpi_tasks = self.test_info.get_defaultENV("MPI_TASKS")
        mpi_cmd = "{!s}mpirun -N {!s}".format(
            self.test_info.get_defaultENV("IOF_OMPI_BIN"),
            mpi_tasks)

        # Build test cmd.

        self.test_cmd = "{!s} {!s}".format(ompi_oversubscribe, mpi_cmd)
        self.test_cmd = "{!s} /testbin/mdtest/bin/mdtest".format(self.test_cmd)

        self.test_cmd_args = "-i 1 -I 1 -u"
    ...
    def mdtest_tasks(self):
        ...
        # Launch mdtest cmd on mount node.

        cmd = "{!s} -d {!s}".format(self.test_cmd, self.mount_dir)
