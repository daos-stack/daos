...
dc_metrics_reset_tls_data()
{
	...
	memset(&metrics_tls_agg, 0, sizeof(metrics_tls_agg));
	metrics_tls_agg.update_stat.st_min = ULONG_MAX;
	metrics_tls_agg.fetch_stat.st_min = ULONG_MAX;
	D_MUTEX_UNLOCK(&metrics_tls_lock);
}
...
dc_metrics_tls_aggr(dc_metrics_tls_data_t *value)
{
	...
	/** Update stats */
	dc_metrics_aggr_stats(&metrics_tls_agg.update_stat, \
			&value->update_stat);

	/** Fetch stats */
	dc_metrics_aggr_stats(&metrics_tls_agg.fetch_stat, \
			&value->fetch_stat);
...
dc_metrics_init()
{
	...
	is_metrics_enabled = ((env != NULL) && (strncmp(env, "ON", strlen("ON")) == 0));
...
int
dc_metrics_update_iodist(int is_update, size_t size, struct daos_oclass_attr *ptype, int is_full_stripe)
{
	...
	if (is_update) {
		...
		if (ptype->ca_resil == DAOS_RES_REPL) {
			...
			if (idx == DAOS_METRICS_IO_ECU_PART) {
				ptr->idp.idp_updatecnt_bkt[DAOS_METRICS_IO_ECU_PART + is_full_stripe]++;
				ptr->idp.idp_updatesz_bkt[DAOS_METRICS_IO_ECU_PART + is_full_stripe] += size;
			} else {
...
daos_metrics_get_stats(enum daos_metrics_stats_grp ms_grp, daos_metrics_ustats_t *stats)
{
	...
	if ((ms_grp != DAOS_METRICS_OBJ_UPDATE_STATS) && \
	    (ms_grp != DAOS_METRICS_OBJ_FETCH_STATS)) {
...
dump_pool_rpccntrs(FILE *fp)
{
	...
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "pool connect", pcntrs->prc_connect_cnt.mc_inflight, \
			pcntrs->prc_connect_cnt.mc_success, pcntrs->prc_connect_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","pool disconnect", pcntrs->prc_disconnect_cnt.mc_inflight,\
			pcntrs->prc_disconnect_cnt.mc_success,pcntrs->prc_disconnect_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","pool attr(get/set)", pcntrs->prc_attr_cnt.mc_inflight, \
...
dump_cont_rpccntrs(FILE *fp)
{
	...
	fprintf(fp, "%-16s\t%12s\t%12s\t%12s\n","Name","Inflight","Success","Failure");
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont create", ccntrs->crc_create_cnt.mc_inflight, \
			ccntrs->crc_create_cnt.mc_success, ccntrs->crc_create_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont destroy", ccntrs->crc_destroy_cnt.mc_inflight, \
			ccntrs->crc_destroy_cnt.mc_success, ccntrs->crc_destroy_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont open", ccntrs->crc_open_cnt.mc_inflight, \
			ccntrs->crc_open_cnt.mc_success, ccntrs->crc_open_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont close", ccntrs->crc_close_cnt.mc_inflight, \
			ccntrs->crc_close_cnt.mc_success, ccntrs->crc_close_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont snapshot", ccntrs->crc_snapshot_cnt.mc_inflight, \
			ccntrs->crc_snapshot_cnt.mc_success, ccntrs->crc_snapshot_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont snapdestroy", ccntrs->crc_snapdel_cnt.mc_inflight, \
			ccntrs->crc_snapdel_cnt.mc_success, ccntrs->crc_snapdel_cnt.mc_failure);
	...
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont query", ccntrs->crc_query_cnt.mc_inflight, \
			ccntrs->crc_query_cnt.mc_success, ccntrs->crc_query_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont oidalloc", ccntrs->crc_oidalloc_cnt.mc_inflight, \
			ccntrs->crc_oidalloc_cnt.mc_success, ccntrs->crc_oidalloc_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n", "cont aggregate", ccntrs->crc_aggregate_cnt.mc_inflight, \
...
dump_obj_rpccntrs(FILE *fp)
{
	...
	fprintf(fp, "%-16s\t%12s\t%12s\t%12s\n","Name","Inflight","Success","Failure");
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj update", ocntrs->orc_update_cnt.mc_inflight, \
			ocntrs->orc_update_cnt.mc_success, ocntrs->orc_update_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj fetch", ocntrs->orc_fetch_cnt.mc_inflight, \
			ocntrs->orc_fetch_cnt.mc_success, ocntrs->orc_fetch_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj enum dkey", ocntrs->orc_dkey_enum_cnt.mc_inflight, \
			ocntrs->orc_dkey_enum_cnt.mc_success, ocntrs->orc_dkey_enum_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj enum akey", ocntrs->orc_akey_enum_cnt.mc_inflight, \
			ocntrs->orc_akey_enum_cnt.mc_success, ocntrs->orc_akey_enum_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj enum recx", ocntrs->orc_recx_enum_cnt.mc_inflight, \
			ocntrs->orc_recx_enum_cnt.mc_success, ocntrs->orc_recx_enum_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj enum obj", ocntrs->orc_obj_enum_cnt.mc_inflight, \
			ocntrs->orc_obj_enum_cnt.mc_success, ocntrs->orc_obj_enum_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj punch obj", ocntrs->orc_obj_punch_cnt.mc_inflight, \
			ocntrs->orc_obj_punch_cnt.mc_success, ocntrs->orc_obj_punch_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj punch dkeys", ocntrs->orc_dkey_punch_cnt.mc_inflight, \
			ocntrs->orc_dkey_punch_cnt.mc_success, ocntrs->orc_dkey_punch_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj punch akeys", ocntrs->orc_akey_punch_cnt.mc_inflight, \
			ocntrs->orc_akey_punch_cnt.mc_success, ocntrs->orc_akey_punch_cnt.mc_failure);
	fprintf(fp, "%-16s\t%12lu\t%12lu\t%12lu\n","obj query keys", ocntrs->orc_querykey_cnt.mc_inflight, \
			ocntrs->orc_querykey_cnt.mc_success, ocntrs->orc_querykey_cnt.mc_failure);
...
dump_obj_stats(FILE *fp)
{
	...
	fprintf(fp, "%-10s\t%12lu\t%16lu\t%20lu\t%12lu\t%12lu\n", "update", stats->u.st_obj_update.st_value, \
			stats->u.st_obj_update.st_sum, stats->u.st_obj_update.st_sum_of_squares, \
			stats->u.st_obj_update.st_min, stats->u.st_obj_update.st_max);
	...
	fprintf(fp, "%-10s\t%12lu\t%16lu\t%20lu\t%12lu\t%12lu\n", "fetch", stats->u.st_obj_fetch.st_value, \
			stats->u.st_obj_fetch.st_sum, stats->u.st_obj_fetch.st_sum_of_squares, \
			stats->u.st_obj_fetch.st_min, stats->u.st_obj_fetch.st_max);
	fflush(fp);
alloc_out:
...
static const char *distname_bysize[] = {
        "DM_IO_0_1K",
        "DM_IO_1K_2K",
        "DM_IO_2K_4K",
        "DM_IO_4K_8K",
...
dump_obj_iodist_bysize(FILE *fp)
{
	...
	for (i =0; i< DAOS_METRICS_IO_BYSIZE_COUNT;i++) {
		fprintf(fp,"%-16s\t%12lu\t%12lu\n",distname_bysize[i], dist->u.dt_bsz.ids_updatecnt_bkt[i], \
				dist->u.dt_bsz.ids_fetchcnt_bkt[i]);
	}
	fflush(fp);
alloc_out:
...
dump_obj_iodist_byptype(FILE *fp)
{
	...
	for (i =0; i< DAOS_METRICS_IO_BYPTYPE_COUNT;i++) {
		fprintf(fp,"%-16s\t%12lu\t%12lu\n",distname_byptype[i], dist->u.dt_bpt.idp_updatecnt_bkt[i], \
				dist->u.dt_bpt.idp_updatesz_bkt[i]);
	}
	fflush(fp);
alloc_out:
