...
/** whether actual daos_fini should be processed by destructor when program exits */
static bool module_delay_fini;
...
daos_init(void)
{
	...
	d_getenv_bool("D_CLEANUP_ON_EXIT", &module_delay_fini);
	if (module_delay_fini)
		D_INFO("Delaying libdaos cleanup to program exit\n");
...
daos_fini(void)
{
	...
	if (module_refcount == 0) {
		 ...
		 */
		module_refcount--;
	} else if (!module_delay_fini || module_destructor_called) {
