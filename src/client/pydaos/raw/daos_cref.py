...
class RankList(ctypes.Structure):
    ...
    _fields_ = [("rl_ranks", ctypes.POINTER(ctypes.c_uint32)), ("rl_nr", ctypes.c_uint)]
...
class DTgtList(ctypes.Structure):
    ...
    _fields_ = [
        ("tl_ranks", ctypes.POINTER(ctypes.c_uint32)),
...
class EpochRange(ctypes.Structure):
    ...
    _fields_ = [("epr_lo", ctypes.c_uint64), ("epr_hi", ctypes.c_uint64)]
...
class DaosSpace(ctypes.Structure):
    """Structure to represent Pool Target Space usage info
    Represents struct: daos_space"""

    _fields_ = [("s_total", ctypes.c_uint64 * 2), ("s_free", ctypes.c_uint64 * 2)]
...
class DaosProperty(ctypes.Structure):
    ...
    def __init__(self, num_structs):
        ...
        self.dpp_entries = ctypes.cast(total_prop_entries, ctypes.POINTER(DaosPropertyEntry))
...
class DaosObjAttr(ctypes.Structure):
    ...
    _fields_ = [("oa_rank", ctypes.c_int), ("oa_oa", DaosObjClassAttr)]
...
class DaosShardLoc(ctypes.Structure):
    ...
    _fields_ = [("sd_rank", ctypes.c_uint32), ("sd_tgt_idx", ctypes.c_uint32)]
...
class DaosObjShard(ctypes.Structure):
    ...
    _fields_ = [("os_replica_nr", ctypes.c_uint32), ("os_shard_loc", DaosShardLoc * 5)]
...
class DaosKeyDescriptor(ctypes.Structure):
    ...
    _fields_ = [("kd_key_len", ctypes.c_uint64), ("kd_val_type", ctypes.c_uint32)]
...
def async_worker(func_ref, param_list, context, cb_func=None, obj=None):
    ...
    _ = qfunc(ctypes.byref(qhandle))

    efunc = context.get_function('init-event')
    _ = efunc(param_list[-1], qhandle, None)

    # calling the api function here
    _ = func_ref(*param_list)
    ...
    _ = efunc(qhandle, c_wait, c_timeout, c_num, ctypes.byref(c_event_ptr))
