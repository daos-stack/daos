...
class DaosContainerInputParams(ctypes.Structure):
    # pylint: disable=too-few-public-methods
    """Defines the input parameters for a DaosContainer object.

    This is a python container properties structure used to set the type, e.g.
    posix, and enable checksum.

    NOTE: This structure can be enhanced in future for setting other container
    ...
    def __init__(self):
        """Initailize a DaosContainerInputParams object.

        Defines the default values for the container input parameters used to
        set container properties.
        """
        super(DaosContainerInputParams, self).__init__()
        self.type = "Unknown"
    ...
    def get_daos_properties(self):
        """Get the DaosProperty defined by this set of container inputs.

        Returns:
            daos_cref.DaosProperty: the container properties defined by the
                current input parameters.  Can be None.

        """
        daos_property = None

        # Define the DaosProperty for the specified inputs:
        #   1. Layout Type.
        #   2. Enable checksum,
        #   3. Server Verfiy
        #   4. Chunk Size Allocation.
        if self.type != "Unknown" and not self.enable_chksum:
            # Only type like posix, hdf5 defined.
            daos_property = daos_cref.DaosProperty(1)
        elif self.type == "Unknown" and self.enable_chksum:
            # Obly checksum enabled.
            daos_property = daos_cref.DaosProperty(3)
        elif self.type != "Unknown" and self.enable_chksum:
        ...
        if daos_property:
            ...
            # e.g.:
            #   - layer is none, checksum dpp_entries will start with index 0
            #   - layer is not none, checksum dpp_entries will start at index 1
            index = 0
            ...
            if self.enable_chksum is True:
                ...
                # Process the chksum_type input
                if self.chksum_type == 100:
                    daos_property.dpp_entries[index].dpe_val = ctypes.c_uint64(
                        1)
                else:
                    daos_property.dpp_entries[index].dpe_val = ctypes.c_uint64(
        ...
        return daos_property


class DaosInputParams(object):
    # pylint: disable=too-few-public-methods
    """Defines the input parameters for a Daos object.

    This is a helper python method which can be used to pack input parameters
    for multipe daos object create methods, e.g. container or pool (future).
    """

    def __init__(self):
        """Initailize a DaosInputParams object.

        Get the input params for setting container properties for create method.
        """
        self.container_params = DaosContainerInputParams()

    def get_container_create_properties(self):
        """Get the container create properties from the container inputs.
        ...
        """
        return self.container_params.get_daos_properties()
...
class DaosContainer(object):
    ...
    def create(self, poh, con_uuid=None, inputs=None, cb_func=None):
        ...
        if inputs is None:
            inputs = DaosInputParams()
        self.properties = inputs.get_container_create_properties()
        ...
        if cb_func is None:
            if self.properties is None:
                ...
                ret = func(self.poh, self.uuid, ctypes.byref(self.properties),
                           None)
                if ret != 0:
                    self.uuid = (ctypes.c_ubyte * 1)(0)
                    raise DaosApiError(
            ...
            if self.properties is None:
                ...
                params = [self.poh, self.uuid, ctypes.byref(self.properties),
                          None, event]
            thread = threading.Thread(target=daos_cref.AsyncWorker1,
                                      args=(func,
                                            params,
