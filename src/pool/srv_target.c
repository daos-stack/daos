		       ...
		       unsigned int map_version)
{
	...
	if (pool->sp_map_version < map_version ||
		...
		if (rc != 0) {
			ABT_rwlock_unlock(pool->sp_lock);
			D_ERROR(DF_UUID": failed to update pl_map: "DF_RC"\n",
				DP_UUID(pool->sp_uuid), DP_RC(rc));
			D_GOTO(out, rc);
		}
...
out:
	out->tuo_rc = (rc == 0 ? 0 : 1);
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %d "DF_RC"\n",
		DP_UUID(in->tui_uuid), rpc, out->tuo_rc, DP_RC(rc));
	crt_reply_send(rpc);
}
...
ds_pool_cont_iter(daos_handle_t ph, pool_iter_cb_t callback, void *arg)
{
	...
	if (rc != 0) {
		D_ERROR("prepare co iterator failed "DF_RC"\n", DP_RC(rc));
		return rc;
	}
...
dtx_resync_ult(void *data)
{
	...
	if (rc != 0)
		D_ERROR("Fail to resync some DTX(s) for the pool/cont "
			...
			"object rebuild: rc = "DF_RC".\n",
			DP_UUID(args->po_uuid), DP_UUID(args->co_uuid),
			DP_RC(rc));
	return rc;
}
