...
stop_gc_ult(struct ds_pool_child *child)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID"[%d]: Stopping GC ULT\n",
...
ds_pool_child_put(struct ds_pool_child *child)
{
	...
	child->spc_ref--;
	if (child->spc_ref == 0) {
		D_DEBUG(DB_DSMS, DF_UUID": destroying\n",
...
gc_ult(void *arg)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID"[%d]: GC ULT started\n",
...
out:
	D_DEBUG(DB_DSMS, DF_UUID"[%d]: GC ULT stopped\n",
		DP_UUID(child->spc_uuid), dmi->dmi_tgt_id);
}
...
pool_child_add_one(void *varg)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID": creating\n", DP_UUID(arg->pla_uuid));
...
pool_free_ref(struct daos_llink *llink)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID": freeing\n", DP_UUID(pool->sp_uuid));
...
pool_hdl_rec_free(struct d_hash_table *htable, d_list_t *rlink)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID": freeing "DF_UUID"\n",
...
ds_pool_tgt_connect(struct ds_pool *pool, struct pool_iv_conn *pic)
{
	...
	if (hdl != NULL) {
		if (hdl->sph_sec_capas == pic->pic_capas) {
			D_DEBUG(DB_DSMS, DF_UUID": found compatible pool "
...
out:
	...
	D_DEBUG(DB_DSMS, DF_UUID": connect "DF_RC"\n",
		DP_UUID(pool->sp_uuid), DP_RC(rc));
	return rc;
}
...
ds_pool_tgt_disconnect(uuid_t uuid)
{
	...
	if (hdl == NULL) {
		D_DEBUG(DB_DSMS, "handle "DF_UUID" does not exist\n",
			DP_UUID(uuid));
		return;
	}
...
ds_pool_tgt_disconnect_handler(crt_rpc_t *rpc)
{
	...
	D_DEBUG(DB_DSMS, DF_UUID": handling rpc %p: hdls[0]="DF_UUID" nhdls="
...
out:
	out->tdo_rc = (rc == 0 ? 0 : 1);
	D_DEBUG(DB_DSMS, DF_UUID": replying rpc %p: %d "DF_RC"\n",
