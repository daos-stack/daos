...
rechoose:
	...
	pool_create_in_set_data(rpc, POOL_CREATE, DAOS_POOL_VERSION, target_addrs, prop, ndomains,
				ntargets, domains);
...
ds_pool_create_handler(crt_rpc_t *rpc)
{
	...
	pool_create_in_get_data(rpc, POOL_CREATE, DAOS_POOL_VERSION, &tgt_ranks, &prop, &ndomains,
				&ntgts, &domains);
...
ds_pool_connect_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_connect_in_get_data(rpc, POOL_CONNECT, handler_version, &cred, &flags, &query_bits,
				 &bulk, &cli_pool_version);
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": refusing connect attempt for " DF_X64 " error: "DF_RC"\n",
			DP_UUID(in->pci_op.pi_uuid), flags, DP_RC(rc));
		D_GOTO(out_map_version, rc);
	}

	rc = ds_sec_cred_get_origin(&cred, &machine);
	...
	rc = pool_connect_iv_dist(svc, in->pci_op.pi_hdl, flags, sec_capas, &cred, global_ver,
				  obj_layout_ver);
	...
	D_ALLOC(hdl, sizeof(*hdl) + cred.iov_len);
	if (hdl == NULL)
	...
	hdl->ph_cred_len = cred.iov_len;
	memcpy(&hdl->ph_cred[0], cred.iov_buf, cred.iov_len);
...
realloc_resp:
	...
	pool_list_cont_in_set_data(rpc, POOL_LIST_CONT, DAOS_POOL_VERSION, bulk, ncont);
...
ds_pool_list_cont_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_list_cont_in_get_data(rpc, POOL_LIST_CONT, handler_version, &bulk, &ncont_in);
	...
	if (rc != 0) {
		...
		D_DEBUG(DB_MD, DF_UUID": hdl="DF_UUID": has %"PRIu64 "containers (more than client"
...
ds_pool_filter_cont_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_filter_cont_in_get_data(rpc, POOL_FILTER_CONT, handler_version, &bulk, &ncont_in,
				     &filt_in);
	...
	if (rc != 0) {
		...
		D_DEBUG(DB_MD, DF_UUID": hdl="DF_UUID": %"PRIu64" matching containers "
			...
			DP_UUID(in->pfci_op.pi_hdl), ncont, ncont_in);
		D_GOTO(out_free_cont_buf, rc = -DER_TRUNC);
	} else {
...
ds_pool_query_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_query_in_get_data(rpc, POOL_QUERY, handler_version, &bulk, &query_bits);
...
ds_pool_query_info_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_query_info_in_get_data(rpc, POOL_QUERY_INFO, handler_version, &rank, &tgt);
	...
	if (tgt_state == PO_COMP_ST_UPIN) {
		...
		if (rc)
			D_ERROR(DF_UUID": Failed to query rank:%u, tgt:%d, "
				""DF_RC"\n", DP_UUID(in->pqii_op.pi_uuid), rank, tgt, DP_RC(rc));
	} else {
...
rechoose:
	...
	pool_query_info_in_set_data(rpc, POOL_QUERY_INFO, DAOS_POOL_VERSION, rank, tgt_idx);
...
rechoose:
	...
	rc = pool_req_create(info->dmi_ctx, &ep, POOL_PROP_GET, pool_uuid, no_uuid, &req_time,
			     &rpc);
	if (rc != 0) {
...
rechoose:
	...
	pool_tgt_update_in_set_data(rpc, opc_get(rpc->cr_opc), DAOS_POOL_VERSION,
				    target_addrs->pta_addrs, (size_t)target_addrs->pta_number);
...
rechoose:
	...
	rc = pool_req_create(info->dmi_ctx, &ep, POOL_PROP_SET, pool_uuid, no_uuid, &req_time,
			     &rpc);
	if (rc != 0) {
...
ds_pool_update_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_tgt_update_in_get_data(rpc, opc_get(rpc->cr_opc), handler_version, &list.pta_addrs,
				    &list.pta_number);
...
ds_pool_attr_set_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_attr_set_in_get_data(rpc, POOL_ATTR_SET, handler_version, &count, &bulk);
...
ds_pool_attr_del_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_attr_del_in_get_data(rpc, POOL_ATTR_DEL, handler_version, &count, &bulk);
...
ds_pool_attr_get_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_attr_get_in_get_data(rpc, POOL_ATTR_GET, handler_version, &count, &key_length,
				  &bulk);
...
ds_pool_attr_list_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	pool_attr_list_in_get_data(rpc, POOL_ATTR_LIST, handler_version, &bulk);
