				...
				const char *machine)
{
	...
	ca_entry = daos_prop_entry_get(prop, DAOS_PROP_PO_CA_CERT);
...
pool_connect_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	/*
	 * Node authentication gates the connect before any user-level evaluation.
	 * Only a new handle is validated: a retry that reuses an existing handle
	 * skips this, or the control plane's anti-replay check would reject it.
	 */
	if (!skip_update) {
