...
/*
 * Read the DB for map_buf, map_version, prop and iv_hdls. If the return value is 0,
...
__ds_pool_mark_upgrade_completed(uuid_t pool_uuid, struct pool_svc *svc, int rc)
{
	...
	rc1 = rdb_tx_update(&tx, &svc->ps_root, &ds_pool_prop_upgrade_global_version, &value);
	if (rc1)
		D_GOTO(out_tx, rc1);
