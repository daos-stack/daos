	...
	for (i = 0; i < prop->dpp_nr; i++) {
		...
		case DAOS_PROP_PO_POOL_CA: {
			...
			if (bv != NULL && bv->dpb_len > 0) {
				...
				rc = rdb_tx_update(tx, kvs, &ds_pool_prop_pool_ca, &value);
			} else {
				rc = rdb_tx_delete(tx, kvs, &ds_pool_prop_pool_ca);
				if (rc == -DER_NONEXIST)
					rc = 0;
			}
			if (rc)
	       ...
	       daos_prop_t **prop_out)
{
	...
	if (bits & DAOS_PO_QUERY_PROP_POOL_CA) {
		d_iov_set(&value, NULL, 0);
		rc = rdb_tx_lookup(tx, &svc->ps_root, &ds_pool_prop_pool_ca, &value);
		D_ASSERT(idx < nr);
