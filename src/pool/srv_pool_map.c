		      ...
		      uint32_t *version)
{
	...
	rc = pool_map_refresh_tgt_dom_status(map, target->ta_comp.co_id, PO_COMP_ST_UPIN);
	       ...
	       int opc, uint32_t *version, bool print_changes)
{
	...
	case MAP_EXTEND:
		...
		case PO_COMP_ST_NEW:
			...
			D_DEBUG(DB_MD, "change "DF_TARGET" to UP %p\n",
				DP_TARGET(target), map);
			rc = pool_map_refresh_tgt_dom_status(map, target->ta_comp.co_id,
							     PO_COMP_ST_UP);
			D_ASSERT(rc != 0); /* This target must be findable */
			if (print_changes)
...
static void
update_one_dom(struct pool_domain *dom, int opc, bool exclude_rank, uint32_t *version)
{
	switch(opc) {
	case MAP_REINT:
		if (dom->do_comp.co_status == PO_COMP_ST_DOWN)
			dom->do_comp.co_flags = PO_COMPF_DOWN2UP;
		else
	...
	case MAP_FINISH_REBUILD:
		...
		if (exclude_rank &&
			...
			dom->do_comp.co_out_ver = (*version)++;
			D_DEBUG(DB_MD, "change rank %u to DOWNOUT\n", dom->do_comp.co_rank);
		}
		break;
	case MAP_REVERT_REBUILD:
		if (dom->do_comp.co_status == PO_COMP_ST_UP) {
			if (dom->do_comp.co_fseq == 1) {
				...
				if (dom->do_comp.co_flags == PO_COMPF_DOWN2UP)
					dom->do_comp.co_status = PO_COMP_ST_DOWN;
				else
					dom->do_comp.co_status = PO_COMP_ST_DOWNOUT;
				dom->do_comp.co_out_ver = (*version)++;
			}
			D_DEBUG(DB_MD, "change rank %u to status %u ver %u\n",
			...
			bool print_changes)
{
	...
	for (i = 0; i < tgts->pti_number; i++) {
		...
		update_one_dom(dom, opc, exclude_rank, &version);
	}
