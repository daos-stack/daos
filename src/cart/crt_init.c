...
dump_envariables(void)
{
	...
	char *envars[] = {"CRT_PHY_ADDR_STR",   "D_LOG_STDERR_IN_LOG",
			  "D_LOG_FILE",         "D_LOG_FILE_APPEND_PID",
			  "D_LOG_MASK",         "DD_MASK",
			  "DD_STDERR",          "DD_SUBSYS",
			  "CRT_TIMEOUT",        "CRT_ATTACH_INFO_PATH",
			  "OFI_PORT",           "OFI_INTERFACE",
			  "OFI_DOMAIN",         "CRT_CREDIT_EP_CTX",
			  "CRT_CTX_SHARE_ADDR", "CRT_CTX_NUM",
			  "D_FI_CONFIG",        "FI_UNIVERSE_SIZE",
			  "CRT_ENABLE_MEM_PIN", "FI_OFI_RXM_USE_SRX",
			  "D_LOG_FLUSH",        "CRT_MRC_ENABLE"};
...
static void
prov_data_init(struct crt_prov_gdata *prov_data, int provider, bool sep_mode, int max_ctx_num,
...
data_init(int server, crt_init_options_t *opt)
{
	...
	if (credits == 0) {
		...
		D_DEBUG(DB_ALL,
			"ENV CRT_CREDIT_EP_CTX's value %d exceed max "
			"allowed value, use %d for flow control.\n",
			credits, CRT_MAX_CREDITS_PER_EP_CTX);
		credits = CRT_MAX_CREDITS_PER_EP_CTX;
	} else {
		D_DEBUG(DB_ALL,
			"CRT_CREDIT_EP_CTX set as %d for flow "
			"control.\n",
			credits);
	}
...
crt_init_opt(crt_group_id_t grpid, uint32_t flags, crt_init_options_t *opt)
{
	...
	if (crt_gdata.cg_inited == 0) {
		...
		for (plugin_idx = 0; crt_na_dict[plugin_idx].nad_str != NULL; plugin_idx++) {
...
do_init:
		...
		prov_data_init(&crt_gdata.cg_prov_gdata[prov], prov, set_sep, max_num_ctx,
			       max_expect_size, max_unexpect_size);

		/* rxm and verbs providers only works with regular EP */
		if ((prov == CRT_NA_OFI_VERBS_RXM || prov == CRT_NA_OFI_TCP_RXM) &&
		...
		if (crt_na_type_is_ofi(prov) || crt_na_type_is_ucx(prov)) {
			...
			if (rc != 0) {
				D_ERROR("crt_na_ofi_config_init() failed, " DF_RC "\n", DP_RC(rc));
				D_GOTO(out, rc);
			}
		}
...
crt_finalize(void)
{
	...
	if (crt_gdata.cg_refcount == 0) {
		...
		if (prov_data->cpg_ctx_num > 0) {
			D_ASSERT(!crt_context_empty(crt_gdata.cg_init_prov, CRT_LOCKED));
...
int
crt_na_ofi_config_init(int provider, crt_init_options_t *opt)
{
	char                     *port_str;
	char                     *interface;
	int                       port;
	...
	if (domain == NULL) {
		...
		if (provider == CRT_NA_OFI_VERBS_RXM || provider == CRT_NA_OFI_CXI)
	...
	for (ifa = if_addrs; ifa != NULL; ifa = ifa->ifa_next) {
		...
		if (ifa->ifa_addr->sa_family == AF_INET) {
			...
			ip_str =
			    inet_ntop(AF_INET, tmp_ptr, na_ofi_cfg->noc_ip_str, INET_ADDRSTRLEN);
			if (ip_str == NULL) {
	...
	if (crt_is_service() && port_str != NULL && strlen(port_str) > 0) {
		if (!is_integer_str(port_str)) {
			...
			if (provider == CRT_NA_OFI_SOCKETS || provider == CRT_NA_OFI_VERBS_RXM ||
			...
			D_DEBUG(DB_ALL,
				"OFI_PORT %d, using it as service "
				"port.\n",
				port);
		}
...
void
crt_na_ofi_config_fini(int provider)
{
	struct crt_na_ofi_config *na_ofi_cfg;
