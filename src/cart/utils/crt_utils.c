...
crtu_agent_populate_opt(const char *name, crt_init_options_t *opt)
{
	...
	D_STRNDUP(opt->cio_provider, info.provider, DAOS_SYS_INFO_STRING_MAX);
	D_STRNDUP(opt->cio_interface, info.interface, DAOS_SYS_INFO_STRING_MAX);
	D_STRNDUP(opt->cio_domain, info.domain, DAOS_SYS_INFO_STRING_MAX);
	...
	opt->cio_crt_timeout = info.crt_timeout;
		     ...
		     crt_init_options_t *init_opt, bool use_daos_agent_env)
{
	...
	if (!use_daos_agent_env) {
		if (use_cfg) {
			...
			crt_env_get(CRT_L_GRP_CFG, &grp_cfg_file);
