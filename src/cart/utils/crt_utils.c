...
void
crtu_test_init(d_rank_t rank, int num_attach_retries, bool is_server, bool assert_on_error)
{
...
write_completion_file(void)
{
	...
	D_ASSERTF(dir != NULL, "DAOS_TEST_SHARED_DIR must be set for --write_completion_file "
			       "option.\n");
...
crtu_sync_timedwait(struct wfr_status *wfrs, int sec, int line_number)
{
	...
	if (opts.assert_on_error) {
		D_ASSERTF(rc == 0,
			  "clock_gettime() failed at line %d "
			  "rc: %d\n",
			  line_number, rc);
	} else {
		wfrs->rc = rc;
...
int
crtu_wait_for_ranks(crt_context_t ctx, crt_group_t *grp, d_rank_list_t *rank_list, int tag,
		    int total_ctx, double ping_timeout, double total_timeout)
{
...
int
crtu_dc_mgmt_net_cfg_rank_add(const char *name, crt_group_t *group, crt_context_t *context)
{
	int                               i;
	int                               rc               = 0;
	...
	rc = dc_get_attach_info(name, true /* all_ranks */, &crt_net_cfg_info, &crt_net_cfg_resp);
	if (opts.assert_on_error)
	...
	for (i = 0; i < crt_net_cfg_resp->n_rank_uris; i++) {
		...
		rc = crt_group_primary_rank_add(context, group, rank_uri->rank, rank_uri->uri);

		if (rc != 0) {
			D_ERROR("failed to add rank %u URI %s to group %s: " DF_RC "\n",
				rank_uri->rank, rank_uri->uri, name, DP_RC(rc));
			goto err_group;
		}
...
int
crtu_cli_start_basic(char *local_group_name, char *srv_group_name, crt_group_t **grp,
		     d_rank_list_t **rank_list, crt_context_t *crt_ctx, pthread_t *progress_thread,
		     unsigned int total_srv_ctx, bool use_cfg, crt_init_options_t *init_opt,
		     bool use_daos_agent_env)
{
	...
	if (!use_daos_agent_env) {
		...
		rc = crtu_dc_mgmt_net_cfg_rank_add(srv_group_name, *grp, *crt_ctx);
		if (rc != 0) {
		     ...
		     crt_group_t **grp, uint32_t *grp_size, crt_init_options_t *init_opt)
{
	...
	if (init_opt) {
		rc = crt_init_opt(srv_group_name,
				  CRT_FLAG_BIT_SERVER | CRT_FLAG_BIT_AUTO_SWIM_DISABLE, init_opt);
	} else {
		rc = crt_init(srv_group_name, CRT_FLAG_BIT_SERVER | CRT_FLAG_BIT_AUTO_SWIM_DISABLE);
	}
