...
grp_priv_set_membs(struct crt_grp_priv *priv, d_rank_list_t *list)
{
	if (!priv->gp_primary) {
		...
		return d_rank_list_dup_sort_uniq(&priv->gp_membs.cgm_linear_list, list);
	}
...
grp_priv_fini_membs(struct crt_grp_priv *priv)
{
	...
	while ((index = d_list_pop_entry(&priv->gp_membs.cgm_free_indices, struct free_index,
					 fi_link)) != NULL) {
		D_FREE(index);
	}
}
...
int
crt_grp_lc_uri_insert(struct crt_grp_priv *grp_priv, d_rank_t rank, uint32_t tag, const char *uri);
int
crt_grp_lc_addr_insert(struct crt_grp_priv *grp_priv, struct crt_context *ctx_idx, d_rank_t rank,
		       uint32_t tag, hg_addr_t *hg_addr);
int
...
static inline int
crt_grp_psr_set(struct crt_grp_priv *grp_priv, d_rank_t psr_rank, crt_phy_addr_t psr_addr,
		bool steal)
{
	int rc = 0;
	...
	if (steal) {
		...
		D_STRNDUP(grp_priv->gp_psr_phy_addr, psr_addr, CRT_ADDR_STR_MAX_LEN);
...
#define CRT_RANK_PRESENT(grp, rank) crt_rank_present(grp, rank)
