...
static int
crt_get_opx_info_string(char *provider, char *domain, char *ip, char **string, int start_port,
			int ctx_idx)
{
	int   rc  = 0;
	int   hfi = -1;
	...
	if (ip == NULL)
		D_ASPRINTF(*string, "%s://%s:%d:%d", provider, domain, hfi, start_port + ctx_idx);
	else
		D_ASPRINTF(*string, "%s://%s/%s:%d:%d", provider, domain, ip, hfi,
			   start_port + ctx_idx);
...
crt_hg_req_send(struct crt_rpc_priv *rpc_priv)
{
	...
	if (hg_ret != HG_SUCCESS) {
		RPC_ERROR(rpc_priv, "HG_Forward failed, hg_ret: " DF_HG_RC "\n", DP_HG_RC(hg_ret));
	} else {
