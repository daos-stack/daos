 ...
 */
static inline int
crt_hg_pool_enable(struct crt_hg_context *hg_ctx, int32_t max_num, int32_t prepost_num)
{
	...
	if (hg_ctx == NULL || max_num <= 0 || prepost_num < 0 || prepost_num > max_num) {
...
crt_hg_pool_init(struct crt_hg_context *hg_ctx)
{
	...
	rc = crt_hg_pool_enable(hg_ctx, CRT_HG_POOL_MAX_NUM, CRT_HG_POOL_PREPOST_NUM);
	if (rc != 0)
...
crt_hg_reg_rpcid(hg_class_t *hg_class)
{
	...
	rc =
	    crt_hg_reg(hg_class, CRT_HG_RPCID, (crt_proc_cb_t)crt_proc_in_common,
		       (crt_proc_cb_t)crt_proc_out_common, (crt_hg_rpc_cb_t)crt_rpc_handler_common);
	if (rc != 0) {
	...
	rc =
	    crt_hg_reg(hg_class, CRT_HG_ONEWAY_RPCID, (crt_proc_cb_t)crt_proc_in_common,
		       (crt_proc_cb_t)crt_proc_out_common, (crt_hg_rpc_cb_t)crt_rpc_handler_common);
	if (rc != 0) {
...
crt_get_info_string(int provider, char **string, int ctx_idx)
{
	...
	if (crt_provider_is_contig_ep(provider) && start_port != -1) {
		if (ip_str == NULL)
			...
			D_ASPRINTF(*string, "%s://%s/%s:%d", provider_str, domain_str, ip_str,
				   start_port + ctx_idx);
	} else {
		if (ip_str == NULL)
...
crt_hg_class_init(int provider, int idx, hg_class_t **ret_hg_class)
{
	...
	if (crt_provider_is_sep(provider))
		init_info.na_init_info.max_contexts = crt_provider_get_max_ctx_num(provider);
	else
...
crt_rpc_handler_common(hg_handle_t hg_hdl)
{
	...
	if (unlikely(rc != 0)) {
		RPC_ERROR(rpc_priv, "failed to invoke RPC handler, rc: " DF_RC "\n", DP_RC(rc));
...
crt_hg_req_create(struct crt_hg_context *hg_ctx, struct crt_rpc_priv *rpc_priv)
{
	...
	if (crt_provider_is_sep(hg_ctx->chc_provider)) {
		hg_ret = HG_Set_target_id(rpc_priv->crp_hg_hdl, rpc_priv->crp_pub.cr_ep.ep_tag);
		if (hg_ret != HG_SUCCESS) {
			if (hg_created)
...
crt_hg_req_destroy(struct crt_rpc_priv *rpc_priv)
{
	...
	if (rpc_priv->crp_output_got != 0) {
		hg_ret = HG_Free_output(rpc_priv->crp_hg_hdl, &rpc_priv->crp_pub.cr_output);
		if (hg_ret != HG_SUCCESS) {
			RPC_ERROR(rpc_priv, "HG_Free_output failed, hg_ret: %d\n", hg_ret);
		}
	}
	if (rpc_priv->crp_input_got != 0) {
		hg_ret = HG_Free_input(rpc_priv->crp_hg_hdl, &rpc_priv->crp_pub.cr_input);
		if (hg_ret != HG_SUCCESS)
			RPC_ERROR(rpc_priv, "HG_Free_input failed, hg_ret: %d\n", hg_ret);
	}

	crt_rpc_priv_fini(rpc_priv);

	if (!rpc_priv->crp_coll && rpc_priv->crp_hg_hdl != NULL && (rpc_priv->crp_input_got == 0)) {
		if (!rpc_priv->crp_srv && !rpc_priv->crp_opc_info->coi_no_reply) {
...
crt_hg_req_send_cb(const struct hg_cb_info *hg_cbinfo)
{
	...
	RPC_TRACE(DB_TRACE, rpc_priv, "Invoking RPC callback (rank %d tag %d) rc: " DF_RC "\n",
...
crt_hg_req_send(struct crt_rpc_priv *rpc_priv)
{
	...
	if (hg_ret != HG_SUCCESS) {
		RPC_ERROR(rpc_priv, "HG_Forward failed, hg_ret: %d\n", hg_ret);
	} else {
...
crt_hg_reply_send(struct crt_rpc_priv *rpc_priv)
{
	...
	RPC_ADDREF(rpc_priv);
	hg_ret = HG_Respond(rpc_priv->crp_hg_hdl, crt_hg_reply_send_cb, rpc_priv,
			    &rpc_priv->crp_pub.cr_output);
	if (hg_ret != HG_SUCCESS) {
		RPC_ERROR(rpc_priv, "HG_Respond failed, hg_ret: %d\n", hg_ret);
...
crt_hg_bulk_access(crt_bulk_t bulk_hdl, d_sg_list_t *sgl)
{
	...
	hg_ret = HG_Bulk_access(hg_bulk_hdl, 0, bulk_len, HG_BULK_READWRITE, bulk_sgnum, buf_ptrs,
				buf_sizes, &actual_sgnum);
	if (hg_ret != HG_SUCCESS) {
...
crt_hg_bulk_transfer_cb(const struct hg_cb_info *hg_cbinfo)
{
	...
	D_ASSERT(hg_cbinfo->info.bulk.origin_handle == bulk_desc->bd_remote_hdl);
	D_ASSERT(hg_cbinfo->info.bulk.local_handle == bulk_desc->bd_local_hdl);
		     ...
		     crt_bulk_opid_t *opid, bool bind)
{
	...
	D_ASSERT(bulk_desc != NULL);
	D_ASSERT(bulk_desc->bd_bulk_op == CRT_BULK_PUT || bulk_desc->bd_bulk_op == CRT_BULK_GET);
	...
	hg_bulk_op = (bulk_desc->bd_bulk_op == CRT_BULK_PUT) ? HG_BULK_PUSH : HG_BULK_PULL;
	rpc_priv   = container_of(bulk_desc->bd_rpc, struct crt_rpc_priv, crp_pub);
	if (bind)
		hg_ret = HG_Bulk_bind_transfer(hg_ctx->chc_bulkctx, crt_hg_bulk_transfer_cb,
					       bulk_cbinfo, hg_bulk_op, bulk_desc->bd_remote_hdl,
					       bulk_desc->bd_remote_off, bulk_desc->bd_local_hdl,
		...
		hg_ret = HG_Bulk_transfer_id(
		    ...
		    bulk_desc->bd_remote_hdl, bulk_desc->bd_remote_off, bulk_desc->bd_local_hdl,
		    bulk_desc->bd_local_off, bulk_desc->bd_len,
		    opid != NULL ? (hg_op_id_t *)opid : HG_OP_ID_IGNORE);
	if (hg_ret != HG_SUCCESS) {
