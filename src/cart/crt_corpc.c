...
static inline int
crt_corpc_info_init(struct crt_rpc_priv *rpc_priv, struct crt_grp_priv *grp_priv,
		    bool grp_ref_taken, d_rank_list_t *filter_ranks, uint32_t grp_ver,
		    crt_bulk_t co_bulk_hdl, void *priv, uint32_t flags, int tree_topo,
		    d_rank_t grp_root, bool init_hdr, bool root_excluded)
{
...
crt_corpc_initiate(struct crt_rpc_priv *rpc_priv)
{
	...
	rc =
	    crt_corpc_info_init(rpc_priv, grp_priv, grp_ref_taken, co_hdr->coh_filter_ranks,
				co_hdr->coh_grp_ver /* grp_ver */, rpc_priv->crp_pub.cr_co_bulk_hdl,
				NULL /* priv */, rpc_priv->crp_flags, co_hdr->coh_tree_topo,
				co_hdr->coh_root, false /* init_hdr */, false /* root_excluded */);
	if (rc != 0) {
...
crt_corpc_chained_bulk_cb(const struct crt_bulk_cb_info *cb_info)
{
	...
	if (rc != 0) {
		RPC_ERROR(rpc_priv, "crt_corpc_chained_bulk_cb, bulk failed: " DF_RC "\n",
			  DP_RC(rc));
		D_FREE(bulk_buf);
		D_GOTO(out, rc);
	}
...
crt_corpc_common_hdlr(struct crt_rpc_priv *rpc_priv)
{
	...
	if (parent_bulk_hdl != CRT_BULK_NULL) {
		...
		if (rc != 0 || bulk_len == 0) {
			RPC_ERROR(rpc_priv, "crt_bulk_get_len failed: " DF_RC "\n", DP_RC(rc));
			D_GOTO(out, rc);
		}
		...
		rc = crt_bulk_create(rpc_priv->crp_pub.cr_ctx, &bulk_sgl, CRT_BULK_RW,
				     &local_bulk_hdl);
		if (rc != 0) {
			RPC_ERROR(rpc_priv, "crt_bulk_create failed: " DF_RC "\n", DP_RC(rc));
		...
		rc = crt_bulk_transfer(&bulk_desc, crt_corpc_chained_bulk_cb, bulk_iov.iov_buf,
				       NULL);
		if (rc != 0) {
			RPC_ERROR(rpc_priv, "crt_bulk_transfer failed: " DF_RC "\n", DP_RC(rc));
			D_FREE(bulk_iov.iov_buf);
			RPC_DECREF(rpc_priv);
		}
		D_GOTO(out, rc);
...
out:
	if (rc != 0)
		RPC_ERROR(rpc_priv, "crt_corpc_common_hdlr failed: " DF_RC "\n", DP_RC(rc));
	return rc;
}
		     ...
		     int tree_topo, crt_rpc_t **req)
{
	...
	if ((filter_invert && !rc) || (!filter_invert && rc)) {
		...
		if (filter_invert) {
			...
			d_rank_list_filter(&tmp_rank_list, tobe_filter_ranks, true /* exclude */);
		}
	}
	...
	rc = crt_corpc_info_init(rpc_priv, grp_priv, false, tobe_filter_ranks,
				 grp_ver /* grp_ver */, co_bulk_hdl, priv, flags, tree_topo,
				 grp_root, true /* init_hdr */, root_excluded);
	if (rc != 0) {
		RPC_ERROR(rpc_priv, "crt_corpc_info_init failed: " DF_RC "\n", DP_RC(rc));
		D_GOTO(out, rc);
	}
...
static inline void
corpc_add_child_rpc(struct crt_rpc_priv *parent_rpc_priv, struct crt_rpc_priv *child_rpc_priv)
{
	...
	D_ASSERT(parent_rpc_priv->crp_coll == 1 && parent_rpc_priv->crp_corpc_info != NULL);
	...
	d_list_add_tail(&child_rpc_priv->crp_parent_link, &co_info->co_child_rpcs);
...
crt_corpc_fail_parent_rpc(struct crt_rpc_priv *parent_rpc_priv, int failed_rc)
{
	...
	if (failed_rc != -DER_GRPVER)
		RPC_ERROR(parent_rpc_priv, "CORPC failed: " DF_RC "\n", DP_RC(failed_rc));
	else
		RPC_TRACE(DB_NET, parent_rpc_priv, "CORPC failed: " DF_RC "\n", DP_RC(failed_rc));
}
...
crt_corpc_complete(struct crt_rpc_priv *rpc_priv)
{
	...
	if (am_root) {
		...
		if (rc != 0)
			RPC_ERROR(rpc_priv, "crt_hg_reply_send failed: " DF_RC "\n", DP_RC(rc));
		/*
		 * on root node, don't need to free chained bulk handle as it is
		 * created and passed in by user.
		 */
		rc = crt_corpc_free_chained_bulk(rpc_priv->crp_coreq_hdr.coh_bulk_hdl);
		if (rc != 0)
			RPC_ERROR(rpc_priv, "crt_corpc_free_chainded_bulk failed: " DF_RC "\n",
				  DP_RC(rc));
		/*
...
static inline void
crt_corpc_fail_child_rpc(struct crt_rpc_priv *parent_rpc_priv, uint32_t failed_num, int failed_rc)
{
	struct crt_corpc_info *co_info;
	uint32_t               wait_num;
	uint32_t               done_num;
...
crt_corpc_reply_hdlr(const struct crt_cb_info *cb_info)
{
	...
	if (child_rpc_priv->crp_reply_hdr.cch_rc != 0)
		crt_corpc_fail_parent_rpc(parent_rpc_priv, child_rpc_priv->crp_reply_hdr.cch_rc);
	...
	if (parent_rpc_priv == child_rpc_priv) {
		...
		d_list_for_each_entry_safe(tmp_rpc_priv, next, &co_info->co_replied_rpcs,
					   crp_parent_link) {
			D_ASSERT(tmp_rpc_priv != parent_rpc_priv);
			D_ASSERT(co_ops->co_aggregate != NULL);
			rc = co_ops->co_aggregate(&tmp_rpc_priv->crp_pub, &parent_rpc_priv->crp_pub,
						  co_info->co_priv);
			if (rc != 0) {
			...
			co_info->co_child_ack_num++;
			D_DEBUG(DB_NET,
				"parent rpc %p, child rpc %p, "
				"wait_num %d, ack_num %d.\n",
				parent_rpc_priv, child_rpc_priv, wait_num,
				co_info->co_child_ack_num);
			corpc_del_child_rpc_locked(parent_rpc_priv, tmp_rpc_priv);
		}
	}
	...
	if (co_info->co_local_done == 1) {
		if (child_rpc_priv != parent_rpc_priv) {
			if (co_info->co_root_excluded == 1 && co_info->co_child_ack_num == 0 &&
		...
		co_info->co_child_ack_num++;
		D_DEBUG(DB_NET,
			"parent rpc %p, child rpc %p, wait_num %d, "
			"ack_num %d.\n",
			parent_rpc_priv, child_rpc_priv, wait_num, co_info->co_child_ack_num);
		if (parent_rpc_priv != child_rpc_priv)
			corpc_del_child_rpc_locked(parent_rpc_priv, child_rpc_priv);
	} else {
		D_ASSERT(wait_num > co_info->co_child_ack_num);
		d_list_move_tail(&child_rpc_priv->crp_parent_link, &co_info->co_replied_rpcs);
		D_DEBUG(DB_NET,
			"parent rpc %p, child rpc %p move to "
			"replided rpcs.\n",
...
aggregate_done:
	...
	if (req_done) {
		...
		am_root = (co_info->co_grp_priv->gp_self == co_info->co_root);
		if (co_ops && co_ops->co_post_reply && !am_root)
			co_ops->co_post_reply(&parent_rpc_priv->crp_pub, co_info->co_priv);
...
crt_corpc_req_hdlr(struct crt_rpc_priv *rpc_priv)
{
	...
	if (co_ops && co_ops->co_pre_forward) {
		...
		if (rc != 0) {
			RPC_ERROR(rpc_priv, "co_pre_forward(group %s) failed: " DF_RC "\n",
	...
	rc =
	    crt_tree_get_children(co_info->co_grp_priv, co_info->co_grp_ver,
				  rpc_priv->crp_flags & CRT_RPC_FLAG_FILTER_INVERT,
				  co_info->co_filter_ranks, co_info->co_tree_topo, co_info->co_root,
				  co_info->co_grp_priv->gp_self, &children_rank_list, &ver_match);
	...
	D_DEBUG(DB_TRACE, "group %s grp_rank %d, co_info->co_child_num: %d.\n",
		co_info->co_grp_priv->gp_pub.cg_grpid, co_info->co_grp_priv->gp_self,
	...
	for (i = 0; i < co_info->co_child_num; i++) {
		...
		rc = crt_req_create_internal(rpc_priv->crp_pub.cr_ctx, &tgt_ep,
					     rpc_priv->crp_pub.cr_opc, true /* forward */,
					     &child_rpc);
		if (rc != 0) {
