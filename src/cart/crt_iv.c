...
#define IV_DBG(key, msg, ...) D_DEBUG(DB_TRACE, "[key=%p] " msg, (key)->iov_buf, ##__VA_ARGS__)
...
#define IVNS_ADDREF(xx)                                                                            \
	do {                                                                                       \
		...
		D_ASSERTF(__ivns->cii_ref_count != 0, "%p addref from zero\n", __ivns);            \
	...
	do {                                                                                       \
		...
		if (__ivns == NULL)                                                                \
			break;                                                                     \
                                                                                                   \
		D_SPIN_LOCK(&__ivns->cii_ref_lock);                                                \
		D_ASSERTF(__ivns->cii_ref_count >= (num), "%p decref(%d) from %d\n", __ivns, num,  \
			  __ivns->cii_ref_count);                                                  \
...
static int
crt_ivf_bulk_transfer(struct crt_ivns_internal *ivns_internal, uint32_t class_id, d_iov_t *iv_key,
		      d_sg_list_t *iv_value, crt_bulk_t dest_bulk, crt_rpc_t *rpc, void *user_priv);
...
crt_ivf_key_in_progress_find(struct crt_ivns_internal *ivns, struct crt_iv_ops *ops,
			     crt_iv_key_t *key)
{
	struct ivf_key_in_progress *entry;
	bool                        found = false;

	d_list_for_each_entry(entry, &ivns->cii_keys_in_progress_list, kip_link) {
...
static struct ivf_key_in_progress *
crt_ivf_key_in_progress_set(struct crt_ivns_internal *ivns, crt_iv_key_t *key)
{
	struct ivf_key_in_progress *entry;
	int                         rc;

	D_ALLOC(entry, offsetof(struct ivf_key_in_progress, payload[0]) + key->iov_buf_len);
	if (entry == NULL)
		return NULL;
 ...
 */
static bool
crt_ivf_key_in_progress_unset(struct crt_ivns_internal *ivns, struct ivf_key_in_progress *entry)
{
	if (!entry)
		return true;
...
static int
crt_ivf_pending_request_add(struct crt_ivns_internal *ivns_internal, struct crt_iv_ops *iv_ops,
			    struct ivf_key_in_progress *entry, struct iv_fetch_cb_info *iv_info)
{
 ...
 */
static int
crt_ivf_finalize(struct iv_fetch_cb_info *iv_info, crt_iv_key_t *iv_key, int output_rc)
{
	crt_rpc_t         *rpc;
	int                rc = 0;
	...
	if (rpc) {
		...
		if (output_rc == 0) {
			...
			rc = crt_ivf_bulk_transfer(
			    iv_info->ifc_ivns_internal, iv_info->ifc_class_id, iv_key, iv_value,
			    iv_info->ifc_child_bulk, rpc, iv_info->ifc_user_priv);
			if (rc != 0)
		...
		iv_info->ifc_comp_cb(iv_info->ifc_ivns_internal, iv_info->ifc_class_id, iv_key,
				     NULL, iv_value, output_rc, iv_info->ifc_comp_cb_arg);
	}

	if (need_put)
		iv_ops->ivo_on_put(iv_info->ifc_ivns_internal, iv_value, iv_info->ifc_user_priv);
...
static int
crt_ivf_pending_reqs_process(struct crt_ivns_internal *ivns_internal, uint32_t class_id,
			     struct ivf_key_in_progress *kip_entry, uint32_t rc_value)
{
	struct crt_iv_ops       *iv_ops;
	struct pending_fetch    *pending_fetch;
	...
	while ((pending_fetch = d_list_pop_entry(&kip_entry->kip_pending_fetch_list,
		...
		IV_DBG(&iv_info->ifc_iv_key, "Processing request for kip=%p\n", kip_entry);

		/* Pending remote fetch case */
		if (iv_info->ifc_child_rpc) {
			IV_DBG(&iv_info->ifc_iv_key, "pending remote fetch for kip=%p\n",
			       kip_entry);
			...
			rc = iv_ops->ivo_on_get(ivns_internal, &iv_info->ifc_iv_key, 0,
			...
			if (rc == 0) {
				put_needed = true;
				rc = iv_ops->ivo_on_fetch(ivns_internal, &iv_info->ifc_iv_key, 0x0,
							  CRT_IV_FLAG_PENDING_FETCH, &tmp_iv_value,
							  iv_info->ifc_user_priv);
			}
			...
			if (rc == 0) {
				...
				rc = crt_ivf_bulk_transfer(
				    ivns_internal, class_id, &iv_info->ifc_iv_key, &tmp_iv_value,
				    iv_info->ifc_child_bulk, iv_info->ifc_child_rpc,
				    iv_info->ifc_user_priv);
			} else {
			...
			IV_DBG(&iv_info->ifc_iv_key, "pending local fetch for kip=%p\n", kip_entry);

			if (rc_value != 0) {
				iv_info->ifc_comp_cb(ivns_internal, class_id, &iv_info->ifc_iv_key,
						     NULL, &tmp_iv_value, rc_value,
			...
			iv_info->ifc_comp_cb(ivns_internal, class_id, &iv_info->ifc_iv_key, NULL,
					     &tmp_iv_value, rc_value, iv_info->ifc_comp_cb_arg);
...
crt_ivns_internal_lookup(struct crt_ivns_id *ivns_id)
{
	...
	d_list_for_each_entry(entry, &ns_list, cii_link) {
		...
		if ((entry->cii_gns.gn_ivns_id.ii_nsid == ivns_id->ii_nsid) &&
		    (strcmp(entry->cii_gns.gn_ivns_id.ii_group_name, ivns_id->ii_group_name) ==
		     0)) {
			IVNS_ADDREF(entry);
...
crt_ivns_internal_create(crt_context_t crt_ctx, struct crt_grp_priv *grp_priv,
			 struct crt_iv_class *iv_classes, uint32_t num_class, int tree_topo,
			 uint32_t nsid, void *user_priv)
{
	struct crt_ivns_internal *ivns_internal;
	...
	D_STRNDUP(internal_ivns_id->ii_group_name, grp_priv->gp_pub.cg_grpid, CRT_GROUP_ID_MAX_LEN);
	...
	memcpy(ivns_internal->cii_iv_classes, iv_classes, sizeof(*iv_classes) * num_class);
...
crt_iv_namespace_create(crt_context_t crt_ctx, crt_group_t *grp, int tree_topo,
			...
			crt_iv_namespace_t *ivns)
{
	return crt_iv_namespace_create_priv(crt_ctx, grp, tree_topo, iv_classes, num_classes,
					    iv_ns_id, NULL, ivns);
}
			     ...
			     uint32_t iv_ns_id, void *user_priv, crt_iv_namespace_t *ivns)
{
	...
	ivns_internal = crt_ivns_internal_create(crt_ctx, grp_priv, iv_classes, num_classes,
...
int
crt_iv_namespace_destroy(crt_iv_namespace_t ivns, crt_iv_namespace_destroy_cb_t destroy_cb,
			 void *cb_arg)
{
	struct crt_ivns_internal *ivns_internal;
...
crt_ivf_bulk_transfer_done_cb(const struct crt_bulk_cb_info *info)
{
	...
	iv_ops = crt_iv_ops_get(cb_info->tci_ivns_internal, cb_info->tci_class_id);
	D_ASSERT(iv_ops != NULL);

	rc = iv_ops->ivo_on_put(cb_info->tci_ivns_internal, &cb_info->tci_iv_value,
		      ...
		      d_sg_list_t *iv_value, crt_bulk_t dest_bulk, crt_rpc_t *rpc, void *user_priv)
{
	...
	rc = crt_bulk_create(rpc->cr_ctx, iv_value, CRT_BULK_RW, &bulk_hdl);
	if (rc != 0) {
	...
	rc = crt_bulk_transfer(&bulk_desc, crt_ivf_bulk_transfer_done_cb, cb_info, &opid);
cleanup:
	if (rc != 0) {
...
handle_ivfetch_response(const struct crt_cb_info *cb_info)
{
	...
	iv_ops->ivo_on_refresh(ivns, &input->ifi_key, 0, /* TODO: iv_ver */
			       rc == 0 ? &iv_info->ifc_iv_value : NULL, false, rc,
			       iv_info->ifc_user_priv);
...
static int
crt_ivf_rpc_issue(d_rank_t dest_node, crt_iv_key_t *iv_key, d_sg_list_t *iv_value,
		  d_rank_t root_node, uint32_t grp_ver, struct iv_fetch_cb_info *cb_info)
{
	struct crt_ivns_internal   *ivns_internal;
	...
	if (entry) {
		...
		if (entry->kip_rpc_in_progress == true) {
			rc = crt_ivf_pending_request_add(ivns_internal, iv_ops, entry, cb_info);
	...
	rc = crt_bulk_create(ivns_internal->cii_ctx, iv_value, CRT_BULK_RW, &local_bulk);
	if (rc != 0) {
		D_ERROR("crt_bulk_create(): " DF_RC "\n", DP_RC(rc));
		D_GOTO(exit, rc);
	}
	...
	if (rc != 0) {
		...
		if (entry) {
			if (d_list_empty(&entry->kip_pending_fetch_list)) {
				...
				if (!crt_ivf_key_in_progress_unset(ivns_internal, entry))
					D_MUTEX_UNLOCK(&entry->kip_lock);
			} else {
				D_MUTEX_UNLOCK(&entry->kip_lock);
			}
...
static int
crt_iv_ranks_parent_get(struct crt_ivns_internal *ivns_internal, d_rank_t cur_node,
			d_rank_t root_node, d_rank_t *ret_node)
{
	...
	rc = crt_tree_get_parent(ivns_internal->cii_grp_priv, 0, NULL,
				 ivns_internal->cii_gns.gn_tree_topo, root_node, cur_node,
				 &parent_rank);
	if (rc == 0)
		*ret_node = parent_rank;

	D_DEBUG(DB_TRACE,
		"parent lookup: current=%d, root=%d, parent=%d "
		"rc=%d\n",
...
static int
crt_iv_parent_get(struct crt_ivns_internal *ivns_internal, d_rank_t root_node, d_rank_t *ret_node)
{
	...
	return crt_iv_ranks_parent_get(ivns_internal, self, root_node, ret_node);
}
...
crt_hdlr_iv_fetch_aux(void *arg)
{
	...
	rc = iv_ops->ivo_on_get(ivns_internal, &input->ifi_key, 0, CRT_IV_PERM_READ, &iv_value,
				&user_priv);
	if (rc != 0) {
	...
	rc = iv_ops->ivo_on_fetch(ivns_internal, &input->ifi_key, 0, 0x0, &iv_value, user_priv);
	if (rc == 0) {
		/* Note: This increments ref count on 'rpc_req' and ivns */
		rc = crt_ivf_bulk_transfer(ivns_internal, input->ifi_class_id, &input->ifi_key,
					   &iv_value, input->ifi_value_bulk, rpc_req, user_priv);
		if (rc != 0) {
			D_ERROR("bulk transfer failed; " DF_RC "\n", DP_RC(rc));
			D_GOTO(send_error, rc);
		}
		...
		rc = iv_ops->ivo_on_get(ivns_internal, &input->ifi_key, 0, CRT_IV_PERM_WRITE,
					&iv_value, &user_priv);
		if (rc != 0) {
			D_ERROR("ivo_on_get(): " DF_RC "\n", DP_RC(rc));
			D_GOTO(send_error, rc);
...
crt_hdlr_iv_fetch(crt_rpc_t *rpc_req)
{
	...
	if (iv_ops->ivo_pre_fetch != NULL) {
		...
		iv_ops->ivo_pre_fetch(ivns_internal, &input->ifi_key, crt_hdlr_iv_fetch_aux,
				      rpc_req);
	} else {
...
static int
get_shortcut_path(struct crt_ivns_internal *ivns, d_rank_t root_rank, crt_iv_shortcut_t shortcut,
		  d_rank_t *next_node)
{
	int rc = 0;
	     ...
	     crt_iv_shortcut_t shortcut, crt_iv_comp_cb_t fetch_comp_cb, void *cb_arg)
{
	...
	rc = iv_ops->ivo_on_get(ivns_internal, iv_key, 0, CRT_IV_PERM_READ, iv_value, &user_priv);
	if (rc != 0) {
		D_ERROR("ivo_on_get(): " DF_RC "\n", DP_RC(rc));
		D_GOTO(exit, rc);
	}
	...
	rc = iv_ops->ivo_on_fetch(ivns_internal, iv_key, 0, 0, iv_value, user_priv);

	/* The fetch info is contained on current server.  */
	if (rc == 0) {
		/* Finish up the completion call back */
		iv_ops->ivo_on_refresh(ivns_internal, iv_key, 0, iv_value, false, 0x0, user_priv);

		fetch_comp_cb(ivns_internal, class_id, iv_key, NULL, iv_value, rc, cb_arg);
		...
		iv_ops->ivo_on_refresh(ivns_internal, iv_key, 0, NULL, false, rc, user_priv);

		fetch_comp_cb(ivns_internal, class_id, iv_key, NULL, NULL, rc, cb_arg);
	...
	rc = iv_ops->ivo_on_get(ivns_internal, iv_key, 0, CRT_IV_PERM_WRITE, iv_value, &user_priv);
	if (rc != 0) {
		D_ERROR("ivo_on_get(): " DF_RC "\n", DP_RC(rc));
		D_GOTO(exit, rc);
	}
	...
	rc = crt_ivf_rpc_issue(next_node, iv_key, iv_value, root_rank, grp_ver_entry, cb_info);
exit:
	if (rc != 0) {
...
crt_hdlr_iv_sync_aux(void *arg)
{
	...
	if (rpc_req->cr_co_bulk_hdl == CRT_BULK_NULL) {
		rc = iv_ops->ivo_on_refresh(ivns_internal, &input->ivs_key, 0, NULL, true, 0x0,
					    NULL);
		D_GOTO(exit, rc);
	}
	...
	case CRT_IV_SYNC_EVENT_UPDATE: {
		...
		rc = iv_ops->ivo_on_get(ivns_internal, &input->ivs_key, 0, CRT_IV_PERM_READ,
					&iv_value, &user_priv);
		if (rc != 0) {
			D_ERROR("ivo_on_get(): " DF_RC "\n", DP_RC(rc));
			D_GOTO(exit, rc);
		...
		rc = iv_ops->ivo_on_refresh(ivns_internal, &input->ivs_key, 0, &tmp_iv, false, 0,
					    user_priv);
		D_FREE(tmp_iovs);
		if (rc != 0) {
	...
	case CRT_IV_SYNC_EVENT_NOTIFY:
		rc = iv_ops->ivo_on_refresh(ivns_internal, &input->ivs_key, 0, 0, false, 0,
					    user_priv);
		if (rc != 0) {
			D_ERROR("ivo_on_refresh(): " DF_RC "\n", DP_RC(rc));
			D_GOTO(exit, rc);
...
crt_hdlr_iv_sync(crt_rpc_t *rpc_req)
{
	...
	if (iv_ops->ivo_pre_refresh != NULL) {
		...
		iv_ops->ivo_pre_refresh(ivns_internal, &input->ivs_key, crt_hdlr_iv_sync_aux,
					rpc_req);
	} else {
...
static int
call_pre_sync_cb(struct crt_ivns_internal *ivns_internal, struct crt_iv_sync_in *input,
		 crt_rpc_t *rpc_req)
{
	struct crt_iv_ops *iv_ops;
	d_sg_list_t        iv_value;
	...
	rc = iv_ops->ivo_on_get(ivns_internal, &input->ivs_key, 0, CRT_IV_PERM_READ, &iv_value,
				&user_priv);
	if (rc != 0) {
		D_ERROR("ivo_on_get(): " DF_RC "\n", DP_RC(rc));
		D_GOTO(exit, rc);
...
handle_ivsync_response(const struct crt_cb_info *cb_info)
{
	...
	if (iv_sync->isc_do_callback) {
		...
		iv_sync->isc_update_comp_cb(iv_sync->isc_ivns_internal, iv_sync->isc_class_id,
					    &iv_sync->isc_iv_key, NULL, &iv_sync->isc_iv_value,
					    iv_sync->isc_update_rc, iv_sync->isc_cb_arg);
	...
	if (iv_sync->isc_ivns_internal) {
		iv_ops = crt_iv_ops_get(iv_sync->isc_ivns_internal, iv_sync->isc_class_id);
		D_ASSERT(iv_ops != NULL);

		iv_ops->ivo_on_put(iv_sync->isc_ivns_internal, NULL, iv_sync->isc_user_priv);
...
crt_ivsync_rpc_issue(struct crt_ivns_internal *ivns_internal, uint32_t class_id,
		     ...
		     crt_iv_sync_t *sync_type, d_rank_t src_node, d_rank_t dst_node,
		     crt_iv_comp_cb_t update_comp_cb, void *cb_arg, void *user_priv, int update_rc)
{
	crt_rpc_t              *corpc_req;
	struct crt_iv_sync_in  *input;
	int                     rc               = 0;
	...
	if (sync_type->ivs_event == CRT_IV_SYNC_EVENT_UPDATE)
		rc = iv_ops->ivo_on_refresh(ivns_internal, iv_key, 0, iv_value,
					    iv_value ? false : true, 0, user_priv);
	else if (sync_type->ivs_event == CRT_IV_SYNC_EVENT_NOTIFY)
		rc = iv_ops->ivo_on_refresh(ivns_internal, iv_key, 0, NULL, iv_value ? false : true,
					    0, user_priv);
	else {
	...
	if (iv_value != NULL) {
		...
		rc = crt_bulk_create(ivns_internal->cii_ctx, iv_value, CRT_BULK_RO, &local_bulk);
		if (rc != 0) {
	...
	rc = crt_corpc_req_create(ivns_internal->cii_ctx, &ivns_internal->cii_grp_priv->gp_pub,
				  &excluded_list, CRT_OPC_IV_SYNC, local_bulk, NULL, 0,
	...
	d_iov_set(&input->ivs_sync_type, &iv_sync_cb->isc_sync_type, sizeof(crt_iv_sync_t));
...
exit:
	if (delay_completion == false || rc != 0) {
		...
		update_comp_cb(ivns_internal, class_id, iv_key, NULL, iv_value, update_rc, cb_arg);
		if (rc == 0)
...
finalize_transfer_back(struct update_cb_info *cb_info, int rc)
{
	...
	iv_ops->ivo_on_put(ivns, &cb_info->uci_iv_value, cb_info->uci_user_priv);
...
static int
transfer_back_to_child(crt_iv_key_t *key, struct update_cb_info *cb_info, bool do_refresh,
		       int update_rc)
{
	...
	if (do_refresh)
		iv_ops->ivo_on_refresh(ivns, key, 0, &cb_info->uci_iv_value, false, update_rc,
				       cb_info->uci_user_priv);

	/* No more children -- we are the originator; call update_cb */
	if (cb_info->uci_child_rpc == NULL) {
		cb_info->uci_comp_cb(ivns, cb_info->uci_class_id, key, NULL, &cb_info->uci_iv_value,
				     update_rc, cb_info->uci_cb_arg);
	...
	rc = crt_bulk_transfer(&bulk_desc, bulk_update_transfer_back_done, cb_info, 0);
	if (rc != 0) {
...
handle_ivupdate_response(const struct crt_cb_info *cb_info)
{
	...
	if (iv_info->uci_sync_type.ivs_flags & CRT_IV_SYNC_BIDIRECTIONAL) {
		transfer_back_to_child(&input->ivu_key, iv_info, true, cb_info->cci_rc);
		D_GOTO(exit, 0);
	}

	iv_ops = crt_iv_ops_get(iv_info->uci_ivns_internal, iv_info->uci_class_id);
	D_ASSERT(iv_ops != NULL);
	...
	if (iv_info->uci_child_rpc) {
		...
		if (iv_info->uci_bulk_hdl != CRT_BULK_NULL) {
			rc = iv_ops->ivo_on_put(iv_info->uci_ivns_internal, &iv_info->uci_iv_value,
		...
		if (crt_reply_send(iv_info->uci_child_rpc) != DER_SUCCESS)
			D_ERROR("Failed to respond on rpc: %p", iv_info->uci_child_rpc);
		...
		rc = crt_ivsync_rpc_issue(iv_info->uci_ivns_internal, iv_info->uci_class_id,
					  &input->ivu_key, 0, tmp_iv_value, &iv_info->uci_sync_type,
					  input->ivu_caller_node, input->ivu_root_node,
					  iv_info->uci_comp_cb, iv_info->uci_cb_arg,
					  iv_info->uci_user_priv, rc);
		if (rc != 0) {
			rc = iv_ops->ivo_on_put(iv_info->uci_ivns_internal, tmp_iv_value,
						iv_info->uci_user_priv);
		}
	}
	...
	D_FREE(iv_info);
exit:; /* avoid compiler error: gcc 3.4 */
}

/* Helper function to issue IV UPDATE RPC*/
static int
crt_ivu_rpc_issue(d_rank_t dest_rank, crt_iv_key_t *iv_key, d_sg_list_t *iv_value,
		  crt_iv_sync_t *sync_type, d_rank_t root_rank, uint32_t grp_ver,
		  struct update_cb_info *cb_info)
{
	...
	rc = crt_req_create(ivns_internal->cii_ctx, &ep, CRT_OPC_IV_UPDATE, &rpc);
	if (rc != 0) {
		D_ERROR("crt_req_create(): " DF_RC "\n", DP_RC(rc));
		D_GOTO(exit, rc);
	}
	...
	if (grp_ver != local_grp_ver) {
		D_DEBUG(DB_ALL,
			...
			"On entry: %d: Changed to :%d\n",
			ivns_internal->cii_gns.gn_ivns_id.ii_group_name, grp_ver, local_grp_ver);
		D_GOTO(exit, rc = -DER_GRPVER);
	}
...
handle_response_cb(const struct crt_cb_info *cb_info)
{
	...
	if (crt_ctx->cc_iv_resp_cb != NULL) {
		...
		rc            = crt_ctx->cc_iv_resp_cb((crt_context_t)crt_ctx, info,
						       handle_response_cb_internal, crt_ctx->cc_rpc_cb_arg);
		if (rc) {
...
bulk_update_transfer_done_aux(const struct crt_bulk_cb_info *info)
{
	...
	update_rc = iv_ops->ivo_on_update(ivns_internal, &input->ivu_key, 0, false,
					  &cb_info->buc_iv_value, cb_info->buc_user_priv);
	...
	if (update_rc == -DER_IVCB_FORWARD) {
		...
		rc = crt_iv_parent_get(ivns_internal, input->ivu_root_node, &next_rank);
		D_RWLOCK_UNLOCK(&ivns_internal->cii_grp_priv->gp_rwlock);
		...
		rc = crt_ivu_rpc_issue(next_rank, &input->ivu_key, &cb_info->buc_iv_value,
				       sync_type, input->ivu_root_node, grp_ver, update_cb_info);
		if (rc != 0) {
		...
		if (sync_type->ivs_flags & CRT_IV_SYNC_BIDIRECTIONAL) {
			rc = transfer_back_to_child(&input->ivu_key, update_cb_info, false,
						    update_rc);
			if (rc == 0)
				rc = update_rc;
		...
		output->rc = iv_ops->ivo_on_put(ivns_internal, &cb_info->buc_iv_value,
...
bulk_update_transfer_done(const struct crt_bulk_cb_info *info)
{
	...
	if (iv_ops->ivo_pre_update != NULL) {
		...
		crt_bulk_desc_dup(info_dup->bci_bulk_desc, info->bci_bulk_desc);
		...
		iv_ops->ivo_pre_update(ivns_internal, &input->ivu_key,
				       bulk_update_transfer_done_aux_wrapper, info_dup);
	} else {
		bulk_update_transfer_done_aux(info);
...
crt_hdlr_iv_update(crt_rpc_t *rpc_req)
{
	...
	if (input->ivu_iv_value_bulk == CRT_BULK_NULL) {
		rc = iv_ops->ivo_on_refresh(ivns_internal, &input->ivu_key, 0, NULL, true, 0, NULL);
		if (rc == -DER_IVCB_FORWARD) {
			...
			rc = crt_ivu_rpc_issue(next_rank, &input->ivu_key, NULL, sync_type,
					       input->ivu_root_node, grp_ver_entry, update_cb_info);
	...
	rc = crt_bulk_transfer(&bulk_desc, bulk_update_transfer_done, cb_info, 0);
	if (rc != 0) {
...
static int
crt_iv_update_internal(crt_iv_namespace_t ivns, uint32_t class_id, crt_iv_key_t *iv_key,
		       ...
		       crt_iv_sync_t sync_type, crt_iv_comp_cb_t update_comp_cb, void *cb_arg)
{
	...
	if (iv_value != NULL)
		...
		rc = iv_ops->ivo_on_refresh(ivns, iv_key, 0, NULL, true, 0, priv);

	if (rc == 0) {
		if (sync_type.ivs_flags & CRT_IV_SYNC_BIDIRECTIONAL) {
			rc = update_comp_cb(ivns_internal, class_id, iv_key, NULL, iv_value, rc,
					    cb_arg);
		} else {
			/* issue sync. will call completion callback */
			rc = crt_ivsync_rpc_issue(ivns_internal, class_id, iv_key, iv_ver, iv_value,
						  &sync_type, ivns_internal->cii_grp_priv->gp_self,
		...
		if (grp_ver != grp_ver2) {
			D_DEBUG(DB_ALL,
				"Group (%s) version mismatch. "
				"On Entry: %d:: Changed to:%d\n",
				ivns_internal->cii_gns.gn_ivns_id.ii_group_name, grp_ver, grp_ver2);
		...
		rc = crt_ivu_rpc_issue(next_node, iv_key, iv_value, &sync_type, root_rank, grp_ver,
				       cb_info);
		...
		D_CDEBUG(rc == -DER_NONEXIST || rc == -DER_NOTLEADER, DLOG_INFO, DLOG_ERR,
	      ...
	      crt_iv_sync_t sync_type, crt_iv_comp_cb_t update_comp_cb, void *cb_arg)
{
	...
	rc = crt_iv_update_internal(ivns, class_id, iv_key, iv_ver, iv_value, shortcut, sync_type,
				    update_comp_cb, cb_arg);
exit:
	return rc;
}
...
{
	return crt_iv_update_internal(ivns, class_id, iv_key, iv_ver, NULL, shortcut, sync_type,
				      invali_comp_cb, cb_arg);
}
		     ...
		     uint32_t *nchildren)
{
	...
	if (rc != 0)
		D_ERROR("grp %s, root %d self %d failed; " DF_RC "\n",
			ivns_internal->cii_grp_priv->gp_pub.cg_grpid, root_rank, self_rank,
			DP_RC(rc));
