...
static int
verify_ctl_in_args(struct crt_ctl_ep_ls_in *in_args)
{
	...
	if (!crt_grp_id_identical(in_args->cel_grp_id, grp_priv->gp_pub.cg_grpid)) {
...
crt_hdlr_ctl_get_uri_cache(crt_rpc_t *rpc_req)
{
	...
	rc = d_hash_table_traverse(&grp_priv->gp_uri_lookup_cache, crt_ctl_fill_buffer_cb,
				   &uri_cache);
...
crt_hdlr_ctl_ls(crt_rpc_t *rpc_req)
{
	...
	d_list_for_each_entry(ctx, ctx_list, cc_link) {
		...
		D_MUTEX_LOCK(&ctx->cc_mutex);
		rc = crt_hg_get_addr(ctx->cc_hg_ctx.chc_hgcla, addr_str, &str_size);
		D_MUTEX_UNLOCK(&ctx->cc_mutex);
		...
		count += snprintf(addr_buf + count, addr_buf_len - count, "%s", addr_str);
		count += 1;
	}
