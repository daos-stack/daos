...
crt_context_req_track(struct crt_rpc_priv *rpc_priv)
{
	...
	quota_rc = crt_context_get_quota_resource(rpc_priv->crp_pub.cr_ctx, CRT_QUOTA_RPCS);
...
credits_available(struct crt_ep_inflight *epi)
{
	...
	/* TODO: inflight right now includes items queued in quota waitq, and can exceed credit
	 * limit */
...
crt_context_req_untrack(struct crt_rpc_priv *rpc_priv)
{
	...
	D_MUTEX_LOCK(&crt_ctx->cc_mutex);
	tmp_rpc =
	    d_list_pop_entry(&crt_ctx->cc_quotas.rpc_waitq, struct crt_rpc_priv, crp_waitq_link);
	D_MUTEX_UNLOCK(&crt_ctx->cc_mutex);
