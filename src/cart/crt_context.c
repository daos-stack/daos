...
crt_ctx_epi_abort(d_list_t *rlink, void *arg)
{
	...
	d_list_for_each_entry_safe(rpc_priv, rpc_next, &epi->epi_req_waitq,
		...
		D_ASSERT(epi->epi_req_wait_num > 0);
		if (msg_logged == false) {
			D_DEBUG(DB_NET, "XS(%d): destroy context (idx %d, rank %d, "
			       "req_wait_num "DF_U64").\n", id, ctx->cc_idx,
			       epi->epi_ep.ep_rank, epi->epi_req_wait_num);
			msg_logged = true;
		}
	...
	D_DEBUG(DB_NET, "begin waiting for cancelled / aborted RPCs\n");
	...
	D_DEBUG(DB_NET, "end waiting for cancelled / aborted RPCs\n");
