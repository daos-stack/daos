		     ...
		     struct swim_member_update **pupds, size_t *pnupds)
{
	...
	if (id != self_id) {
		...
		rc = ctx->sc_ops->get_member_state(ctx, self_id, &upds[n].smu_state);
		if (rc) {
			SWIM_ERROR("get_member_state(%lu): " DF_RC "\n", self_id, DP_RC(rc));
			D_GOTO(out_unlock, rc);
		}
	...
	while (item != NULL) {
		...
		if (item->si_id != id && item->si_id != self_id && item->si_id != to) {
...
swim_member_alive(struct swim_context *ctx, swim_id_t from, swim_id_t id, uint64_t nr)
{
	...
	if (id_state.sms_status == SWIM_MEMBER_DEAD || id_state.sms_status == SWIM_MEMBER_ALIVE ||
...
swim_member_dead(struct swim_context *ctx, swim_id_t from, swim_id_t id, uint64_t nr)
{
	...
	/* ignore old updates or updates for dead members */
	if (id_state.sms_status == SWIM_MEMBER_DEAD || id_state.sms_incarnation > nr)
		D_GOTO(out, rc = -DER_ALREADY);
...
swim_member_suspect(struct swim_context *ctx, swim_id_t from, swim_id_t id, uint64_t nr)
{
	...
	/* ignore old updates or updates for dead members */
	if (id_state.sms_status == SWIM_MEMBER_DEAD || id_state.sms_status == SWIM_MEMBER_SUSPECT ||
	    id_state.sms_incarnation > nr)
		D_GOTO(out, rc = -DER_ALREADY);
...
swim_ipings_update(struct swim_context *ctx, uint64_t now, uint64_t net_glitch_delay)
{
	...
	while (item != NULL) {
		...
		rc = ctx->sc_ops->send_reply(ctx, item->si_id, item->si_from, -DER_TIMEDOUT,
					     item->si_args);
		if (rc)
...
swim_ipings_reply(struct swim_context *ctx, swim_id_t to_id, int ret_rc)
{
	...
	while (item != NULL) {
		...
		rc =
		    ctx->sc_ops->send_reply(ctx, item->si_id, item->si_from, ret_rc, item->si_args);
		if (rc)
			SWIM_ERROR("send_reply(): " DF_RC "\n", DP_RC(rc));
...
swim_init(swim_id_t self_id, struct swim_ops *swim_ops, void *data)
{
	...
	if (swim_ops == NULL || swim_ops->send_request == NULL ||
	    swim_ops->get_dping_target == NULL || swim_ops->get_iping_target == NULL ||
	    swim_ops->get_member_state == NULL || swim_ops->set_member_state == NULL) {
...
swim_progress(struct swim_context *ctx, int64_t timeout_us)
{
	...
	if (now > ctx->sc_expect_progress_time && 0 != ctx->sc_expect_progress_time) {
	...
	for (; now <= end || ctx_state == SCS_TIMEDOUT; now = swim_now_ms()) {
		...
		case SCS_TIMEDOUT:
			...
			if (item == NULL) {
				...
				if (rc) {
					swim_ctx_unlock(ctx);
					SWIM_ERROR("swim_subgroup_init(): " DF_RC "\n", DP_RC(rc));
					D_GOTO(out, rc);
				}
				item = TAILQ_FIRST(&ctx->sc_subgroup);
			}
	...
	for (i = 0; i < nupds; i++) {
		...
		case SWIM_MEMBER_DEAD:
			if (upd_id == self_id) {
				...
				if (self_state.sms_incarnation > upds[i].smu_state.sms_incarnation)
					break; /* already incremented */

				SWIM_ERROR("{%lu %c %lu} self %s received "
					   "{%lu %c %lu} from %lu\n",
					   self_id, SWIM_STATUS_CHARS[self_state.sms_status],
					   self_state.sms_incarnation,
					   SWIM_STATUS_STR[upds[i].smu_state.sms_status], self_id,
					   SWIM_STATUS_CHARS[upds[i].smu_state.sms_status],
					   upds[i].smu_state.sms_incarnation, from_id);

				ctx->sc_ops->new_incarnation(ctx, self_id, &self_state);
				rc = swim_updates_notify(ctx, self_id, self_id, &self_state, 0);
				if (rc) {
					swim_ctx_unlock(ctx);
					SWIM_ERROR("swim_updates_notify(): " DF_RC "\n", DP_RC(rc));
					D_GOTO(out, rc);
				}
				break;
			}
...
{
	struct swim_member_state self_state = {
	    .sms_incarnation = self_incarnation, .sms_status = SWIM_MEMBER_ALIVE, .sms_delay = 0};
	...
	for (i = 0; i < nupds_in; i++) {
		if (upds_in[i].smu_id == self_id) {
			...
			SWIM_ERROR(
			    "{%lu %c %lu} self %s received {%lu %c %lu} from %lu\n", self_id, 'A',
			    self_incarnation, SWIM_STATUS_STR[upds_in[i].smu_state.sms_status],
			    upds_in[i].smu_id, SWIM_STATUS_CHARS[upds_in[i].smu_state.sms_status],
