...
#define RPC_TRACE(mask, rpc, fmt, ...)                                                             \
	do {                                                                                       \
		D_TRACE_DEBUG(mask, (rpc), "[opc=%#x rpcid=%#lx rank:tag=%d:%d] " fmt,             \
			      (rpc)->crp_pub.cr_opc, (rpc)->crp_req_hdr.cch_rpcid,                 \
...
#define RPC_ERROR(rpc, fmt, ...)                                                                   \
	do {                                                                                       \
		D_TRACE_ERROR((rpc), "[opc=%#x (%s) rpcid=%#lx rank:tag=%d:%d] " fmt,              \
			      (rpc)->crp_pub.cr_opc, crt_opc_to_str((rpc)->crp_pub.cr_opc),        \
...
#define CRT_ENTRY() D_DEBUG(DB_TRACE, ">>>> Entered %s: %d\n", __func__, __LINE__)

#define CRT_EXIT()  D_DEBUG(DB_TRACE, "<<<< Exit %s: %d\n", __func__, __LINE__)
#else
#define CRT_ENTRY() /* */
#define CRT_EXIT()  /* */
