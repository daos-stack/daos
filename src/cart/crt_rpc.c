...
crt_reply_send(crt_rpc_t *req)
{
	...
	ms_alloc =
	    rpc_priv->crp_ts_alloced.tv_sec * 1000 + rpc_priv->crp_ts_alloced.tv_nsec / 1000000;
	...
	if (delta_ms > 100)
		RPC_ERROR(rpc_priv,
			  "Delta between alloc (%ld:%ld) and response (%ld:%ld) = %ld ms\n",
			  rpc_priv->crp_ts_alloced.tv_sec, rpc_priv->crp_ts_alloced.tv_nsec,
			  now.tv_sec, now.tv_nsec, delta_ms);
