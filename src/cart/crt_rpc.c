...
crt_hdlr_ctl_log_add_msg(crt_rpc_t *rpc_req)
{
	...
	if (in_args->log_msg == NULL) {
		...
		D_EMIT("%.*s\n", CRT_CTL_MAX_LOG_MSG_SIZE, in_args->log_msg);
	}
...
CRT_RPC_DEFINE(crt_st_send_id_reply_iov, CRT_ISEQ_ST_SEND_ID, CRT_OSEQ_ST_REPLY_IOV)

CRT_RPC_DEFINE(crt_st_send_iov_reply_empty, CRT_ISEQ_ST_SEND_ID_IOV, CRT_OSEQ_ST_REPLY_EMPTY)

CRT_RPC_DEFINE(crt_st_both_iov, CRT_ISEQ_ST_SEND_ID_IOV, CRT_OSEQ_ST_REPLY_IOV)

CRT_RPC_DEFINE(crt_st_send_iov_reply_bulk, CRT_ISEQ_ST_SEND_ID_IOV_BULK, CRT_OSEQ_ST_REPLY_EMPTY)

CRT_RPC_DEFINE(crt_st_send_bulk_reply_iov, CRT_ISEQ_ST_SEND_ID_BULK, CRT_OSEQ_ST_REPLY_IOV)

CRT_RPC_DEFINE(crt_st_both_bulk, CRT_ISEQ_ST_SEND_ID_BULK, CRT_OSEQ_ST_REPLY_EMPTY)

CRT_RPC_DEFINE(crt_st_open_session, CRT_ISEQ_ST_SEND_SESSION, CRT_OSEQ_ST_REPLY_ID)

CRT_RPC_DEFINE(crt_st_close_session, CRT_ISEQ_ST_SEND_ID, CRT_OSEQ_ST_REPLY_EMPTY)

CRT_RPC_DEFINE(crt_st_start, CRT_ISEQ_ST_START, CRT_OSEQ_ST_START)

CRT_RPC_DEFINE(crt_st_status_req, CRT_ISEQ_ST_STATUS_REQ, CRT_OSEQ_ST_STATUS_REQ)
...
static int
crt_proc_struct_crt_grp_cache(crt_proc_t proc, crt_proc_op_t proc_op, struct crt_grp_cache *data)
{
...
CRT_RPC_DEFINE(crt_ctl_fi_attr_set, CRT_ISEQ_CTL_FI_ATTR_SET, CRT_OSEQ_CTL_FI_ATTR_SET)
CRT_RPC_DEFINE(crt_ctl_fi_toggle, CRT_ISEQ_CTL_FI_TOGGLE, CRT_OSEQ_CTL_FI_TOGGLE)

CRT_RPC_DEFINE(crt_ctl_log_set, CRT_ISEQ_CTL_LOG_SET, CRT_OSEQ_CTL_LOG_SET)
CRT_RPC_DEFINE(crt_ctl_log_add_msg, CRT_ISEQ_CTL_LOG_ADD_MSG, CRT_OSEQ_CTL_LOG_ADD_MSG)
...
static struct crt_proto_rpc_format crt_internal_rpcs[] = {CRT_INTERNAL_RPCS_LIST};

static struct crt_proto_rpc_format crt_fi_rpcs[] = {CRT_FI_RPCS_LIST};

static struct crt_proto_rpc_format crt_st_rpcs[] = {CRT_ST_RPCS_LIST};

static struct crt_proto_rpc_format crt_ctl_rpcs[] = {CRT_CTL_RPCS_LIST};

static struct crt_proto_rpc_format crt_iv_rpcs[] = {CRT_IV_RPCS_LIST};
...
crt_register_proto_fi(crt_endpoint_t *ep)
{
	...
	rc = crt_proto_query(ep, cpf.cpf_base, &cpf.cpf_ver, 1, crt_pfi_cb, &pfi);
	if (rc != -DER_SUCCESS)
		D_GOTO(out, rc);
...
crt_rpc_priv_set_ep(struct crt_rpc_priv *rpc_priv, crt_endpoint_t *tgt_ep)
{
	if (tgt_ep->ep_grp == NULL) {
		rpc_priv->crp_pub.cr_ep.ep_grp = &crt_gdata.cg_grp->gg_primary_grp->gp_pub;
	} else {
...
static int
check_ep(crt_endpoint_t *tgt_ep, struct crt_grp_priv **ret_grp_priv)
{
			...
			bool forward, crt_rpc_t **req)
{
	...
	if (rc != 0) {
		RPC_ERROR(rpc_priv, "crt_rpc_priv_init(%#x) failed, " DF_RC "\n", opc, DP_RC(rc));
...
crt_req_create(crt_context_t crt_ctx, crt_endpoint_t *tgt_ep, crt_opcode_t opc, crt_rpc_t **req)
{
	...
	rc = crt_req_create_internal(crt_ctx, tgt_ep, opc, false /* forward */, req);
	if (rc != 0) {
...
static int
crt_issue_uri_lookup(crt_context_t ctx, crt_group_t *group, d_rank_t contact_rank,
		     uint32_t contact_tag, d_rank_t query_rank, uint32_t query_tag,
		     struct crt_rpc_priv *chained_rpc);

#define MAX_URI_LOOKUP_RETRIES 10

static int
crt_issue_uri_lookup_retry(crt_context_t ctx, struct crt_grp_priv *grp_priv, d_rank_t query_rank,
			   uint32_t query_tag, struct crt_rpc_priv *rpc_priv)
{
	d_rank_list_t *membs;
	...
	rc = crt_issue_uri_lookup(ctx, &grp_priv->gp_pub, contact_rank, 0, query_rank, query_tag,
				  rpc_priv);
	return rc;
}
...
uri_lookup_cb(const struct crt_cb_info *cb_info)
{
	...
	if (cb_info->cci_rc != 0) {
		RPC_ERROR(chained_rpc_priv, "URI_LOOKUP rpc completed with rc=" DF_RC "\n",
	...
	rc = crt_grp_lc_uri_insert(grp_priv, ul_in->ul_rank, ul_out->ul_tag, ul_out->ul_uri);
	if (rc != 0) {
		RPC_ERROR(chained_rpc_priv, "URI insertion '%s' failed for %d:%d; rc=" DF_RC "\n",
			  ul_out->ul_uri, ul_in->ul_rank, ul_out->ul_tag, DP_RC(rc));
		D_GOTO(out, rc);
	}
	...
	if (ul_in->ul_tag != ul_out->ul_tag) {
		if (!crt_provider_is_contig_ep(ctx->cc_hg_ctx.chc_provider)) {
			rc = crt_issue_uri_lookup(lookup_rpc->cr_ctx, lookup_rpc->cr_ep.ep_grp,
						  ul_in->ul_rank, 0, ul_in->ul_rank, ul_in->ul_tag,
						  chained_rpc_priv);
			D_GOTO(out, rc);
		} else {
	...
	if (rc != 0) {
		RPC_ERROR(chained_rpc_priv, "crt_req_fill_tgt_uri() failed; rc=" DF_RC "\n",
			  DP_RC(rc));
		D_GOTO(out, rc);
	}
...
retry:
	...
	if (rc != 0) {
		if (chained_rpc_priv->crp_ul_retry++ < MAX_URI_LOOKUP_RETRIES) {
			rc =
			    crt_issue_uri_lookup_retry(lookup_rpc->cr_ctx, grp_priv, ul_in->ul_rank,
						       ul_in->ul_tag, chained_rpc_priv);
			D_GOTO(out, rc);
		} else {
 ...
 */
static d_rank_t
crt_client_get_contact_rank(crt_context_t crt_ctx, crt_group_t *grp, d_rank_t query_rank,
			    uint32_t query_tag, int *ret_idx)
{
	struct crt_grp_priv *grp_priv;
	...
	if (query_tag != 0) {
		crt_grp_lc_lookup(grp_priv, ctx->cc_idx, query_rank, 0, &cached_uri, NULL);
		if (cached_uri != NULL) {
			*ret_idx = -1;
...
crt_req_uri_lookup(struct crt_rpc_priv *rpc_priv)
{
	...
	if (!crt_is_service()) {
		...
		lookup_rank = crt_client_get_contact_rank(ctx, grp, tgt_ep->ep_rank, tgt_ep->ep_tag,
							  &ret_idx);
		...
		rc = crt_issue_uri_lookup(ctx, grp, lookup_rank, 0, tgt_ep->ep_rank, tgt_ep->ep_tag,
					  rpc_priv);
		D_GOTO(out, rc);
	}
	...
	rc = crt_issue_uri_lookup(ctx, grp, tgt_ep->ep_rank, 0, tgt_ep->ep_rank, tgt_ep->ep_tag,
				  rpc_priv);
out:
	return rc;
}
...
crt_lc_hg_addr_fill(struct crt_rpc_priv *rpc_priv)
{
	...
	crt_grp_lc_lookup(grp_priv, ctx->cc_idx, tgt_ep->ep_rank, tgt_ep->ep_tag, NULL,
...
crt_req_is_self(struct crt_rpc_priv *rpc_priv)
{
	...
	same_group    = (tgt_ep->ep_grp == NULL) ||
		     crt_grp_id_identical(tgt_ep->ep_grp->cg_grpid, grp_priv_self->gp_pub.cg_grpid);
...
crt_req_ep_lc_lookup(struct crt_rpc_priv *rpc_priv, bool *uri_exists)
{
	...
	*uri_exists = false;
	...
	crt_grp_lc_lookup(grp_priv, ctx->cc_idx, tgt_ep->ep_rank, tgt_ep->ep_tag, &base_addr,
			  &rpc_priv->crp_hg_addr);
	...
	if (base_addr == NULL && rpc_priv->crp_hg_addr == NULL) {
		if (crt_req_is_self(rpc_priv)) {
			...
			rc = crt_grp_lc_uri_insert(grp_priv, tgt_ep->ep_rank, tgt_ep->ep_tag,
						   base_addr);
			if (rc != 0)
				D_GOTO(out, rc);
	...
	if (base_addr == NULL && !crt_is_service()) {
		D_RWLOCK_RDLOCK(&grp_priv->gp_rwlock);
		if (tgt_ep->ep_rank == grp_priv->gp_psr_rank && tgt_ep->ep_tag == 0) {
			D_STRNDUP(uri, grp_priv->gp_psr_phy_addr, CRT_ADDR_STR_MAX_LEN);
			D_RWLOCK_UNLOCK(&grp_priv->gp_rwlock);
			if (uri == NULL)
				D_GOTO(out, rc = -DER_NOMEM);
...
crt_req_hg_addr_lookup(struct crt_rpc_priv *rpc_priv)
{
	...
	hg_ret = HG_Addr_lookup2(crt_ctx->cc_hg_ctx.chc_hgcla, rpc_priv->crp_tgt_uri, &hg_addr);
	...
	rc =
	    crt_grp_lc_addr_insert(rpc_priv->crp_grp_priv, crt_ctx, rpc_priv->crp_pub.cr_ep.ep_rank,
	...
	if (rc != 0) {
		RPC_ERROR(rpc_priv, "crt_req_send_internal() failed, rc %d\n", rc);
		D_GOTO(finish_rpc, rc);
	}
...
crt_req_send_immediately(struct crt_rpc_priv *rpc_priv)
{
	...
	if (rc != DER_SUCCESS) {
		RPC_ERROR(rpc_priv, "crt_hg_req_send failed, rc: %d\n", rc);
	}
out:
	return rc;
}
...
crt_req_send_internal(struct crt_rpc_priv *rpc_priv)
{
	...
	case RPC_STATE_INITED:
		...
		if (rc != 0) {
			RPC_ERROR(rpc_priv, "crt_grp_ep_lc_lookup() failed, " DF_RC "\n",
				  DP_RC(rc));
			D_GOTO(out, rc);
		}
		...
		if (rpc_priv->crp_hg_addr != NULL) {
			...
			if (rc != 0)
				RPC_ERROR(rpc_priv, "crt_req_uri_lookup() failed. rc " DF_RC "\n",
					  DP_RC(rc));
		}
		break;
	...
	case RPC_STATE_ADDR_LOOKUP:
		...
		break;
	default:
		RPC_ERROR(rpc_priv, "bad rpc state: %#x\n", rpc_priv->crp_state);
		rc = -DER_PROTO;
		break;
	}
...
crt_req_abort(crt_rpc_t *req)
{
	...
	if (rpc_priv->crp_state != RPC_STATE_REQ_SENT || rpc_priv->crp_on_wire != 1) {
...
crt_rpc_inout_buff_init(struct crt_rpc_priv *rpc_priv)
{
	...
	if (opc_info->coi_crf->crf_size_in > 0 && !rpc_priv->crp_forward) {
		rpc_pub->cr_input      = ((void *)rpc_priv) + opc_info->coi_input_offset;
		rpc_pub->cr_input_size = opc_info->coi_crf->crf_size_in;
	}
	if (opc_info->coi_crf->crf_size_out > 0) {
		rpc_pub->cr_output      = ((void *)rpc_priv) + opc_info->coi_output_offset;
...
crt_rpc_priv_init(struct crt_rpc_priv *rpc_priv, crt_context_t crt_ctx, bool srv_flag)
{
	...
	rpc_priv->crp_timeout_sec =
	    (ctx->cc_timeout_sec == 0 ? crt_gdata.cg_timeout : ctx->cc_timeout_sec);
...
crt_rpc_common_hdlr(struct crt_rpc_priv *rpc_priv)
{
	...
	if (rpc_priv->crp_coll) {
		...
		pri_root = crt_grp_priv_get_primary_rank(rpc_priv->crp_corpc_info->co_grp_priv,
	...
	if (crt_rpc_cb_customized(crt_ctx, &rpc_priv->crp_pub) &&
		...
		rc = crt_ctx->cc_rpc_cb((crt_context_t)crt_ctx, &rpc_priv->crp_pub, crt_handle_rpc,
...
struct d_binheap_ops crt_timeout_bh_ops = {.hop_enter   = timeout_bp_node_enter,
