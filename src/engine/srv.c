...
crt_destroy:
	if (dx->dx_comm) {
		D_INFO("XS(%d) call crt_context_destroy()\n", dx->dx_xs_id);
		/* For now, don't invoke the _debug version of the code
		 * because of a concern that the significant extra logging
		 * may prevent an instance of the issue from reoccurring.
		 * Testing with a longer dmg system stop deadline
		 * to see the effect of this when the problem occurs.
