...
static int
init_event(ras_event_t id, char *msg, ras_type_t type, ras_sev_t sev, char *hwid, d_rank_t *rank,
	   uint64_t *inc, char *jobid, uuid_t *pool, uuid_t *cont, daos_obj_id_t *objid,
	   char *ctlop, Shared__RASEvent *evt)
{
	...
	D_ASPRINTF(evt->timestamp, "%04d-%02d-%02dT%02d:%02d:%02d.%06ld%s", tm->tm_year + 1900,
...
send_event(Shared__RASEvent *evt, bool wait_for_resp)
{
	...
	rc = dss_drpc_call(DRPC_MODULE_SRV, DRPC_METHOD_SRV_CLUSTER_EVENT, reqb, reqb_size,
			   wait_for_resp ? 0 : DSS_DRPC_NO_RESP, wait_for_resp ? &dresp : NULL);
	if (rc != 0)
		goto out_reqb;
	if (wait_for_resp) {
...
static int
raise_ras(ras_event_t id, char *msg, ras_type_t type, ras_sev_t sev, char *hwid, d_rank_t *rank,
	  uint64_t *inc, char *jobid, uuid_t *pool, uuid_t *cont, daos_obj_id_t *objid, char *ctlop,
	  Shared__RASEvent *evt, bool wait_for_resp)
{
	int rc =
	    init_event(id, msg, type, sev, hwid, rank, inc, jobid, pool, cont, objid, ctlop, evt);
	if (rc != 0) {
		    ...
		    daos_obj_id_t *objid, char *ctlop, char *data)
{
	...
	raise_ras(id, msg, type, sev, hwid, rank, inc, jobid, pool, cont, objid, ctlop, &evt,
		  false /* wait_for_resp */);
}
		     ...
		     char *ctlop, char *data, const char *fmt, ...)
{
	...
	ds_notify_ras_event(id, buf, type, sev, hwid, rank, inc, jobid, pool, cont, objid, ctlop,
			    data);
}
...
{
	Shared__RASEvent                   evt  = SHARED__RASEVENT__INIT;
	Shared__RASEvent__PoolSvcEventInfo info = SHARED__RASEVENT__POOL_SVC_EVENT_INFO__INIT;
	...
	rc = raise_ras(RAS_POOL_REPS_UPDATE, "List of pool service replica ranks has been updated.",
		       RAS_TYPE_STATE_CHANGE, RAS_SEV_NOTICE, NULL /* hwid */, &rank /* rank */,
		       NULL /* inc */, NULL /* jobid */, pool, NULL /* cont */, NULL /* objid */,
		       NULL /* ctlop */, &evt, true /* wait_for_resp */);
...
ds_notify_swim_rank_dead(d_rank_t rank, uint64_t incarnation)
{
	...
	return raise_ras(RAS_SWIM_RANK_DEAD, "SWIM marked rank as dead.", RAS_TYPE_STATE_CHANGE,
