 ...
 * is specified.
 */
static const char *all_tests = "ismdlkfecp";
...
print_usage()
{
	...
	print_message("dfuse_test -p|--pil4dfs\n");
...
/*
 * Verrify the behavior of some functions intercepted by libpil4dfs.
 */

#define FD_LARGE 10000
void
do_pil4dfscheck(void **state)
{
	int   root;
	int   fd;
	int   fd_new;
	...
	/* only run this test if libpil4dfs.so is preloaded */
	...
	assert_return_code(fd, errno);
	/* the fd generated by libpil4dfs should be larger than FD_LARGE */
	assert_true(fd >= FD_LARGE);
	...
	/* flag different from O_CLOEXEC is not accepted */
	flag = ~flag;
	rc   = dup3(fd, fd_new, flag);
	assert_true(rc == -1);
...
run_specified_tests(const char *tests, int *sub_tests, int sub_tests_size)
{
	...
	while (*tests != '\0') {
		...
		case 'p':
			printf("\n\n=================");
			printf("dfuse specific functions intercepted by pil4dfs check");
			printf("=====================\n");
			const struct CMUnitTest pil4dfs_tests[] = {
			    cmocka_unit_test(do_pil4dfscheck),
			};
			nr_failed += cmocka_run_group_tests(pil4dfs_tests, NULL, NULL);
			break;
...
main(int argc, char **argv)
{
	...
	static struct option long_options[] = {{"test-dir", required_argument, NULL, 'M'},
					       ...
					       {"pil4dfs", no_argument, NULL, 'p'},
					       {NULL, 0, NULL, 0}};

	while ((opt = getopt_long(argc, argv, "aM:imsdlkfetcp", long_options, &index)) != -1) {
