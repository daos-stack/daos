...
dfs_test_readdir_internal(void **state, daos_oclass_id_t obj_class)
{
	...
	uint32_t                 num_ents = NUM_ENTS;
	struct dirent            ents[NUM_ENTS];
	struct stat              stbufs[NUM_ENTS];
	int			num_files = 0;
	int			num_dirs = 0;
	int			total_entries = 0;
	...
	/** use dfs anchor for readdir anchor stream */
	dfs_dir_anchor_t *dfs_anchor;

	rc = dfs_dir_anchor_init(dir, &dfs_anchor);
	assert_int_equal(rc, 0);
	total_entries = 0;

	while (!dfs_dir_anchor_is_eof(dfs_anchor)) {
		struct dirent ent;

		rc = dfs_readdir_s(dfs_mt, dir, dfs_anchor, &ent);
		if (rc == EOF)
			assert_true(dfs_dir_anchor_is_eof(dfs_anchor));
		else {
			assert_int_equal(rc, 0);
			total_entries++;
		}
	...
	assert_true(total_entries == 199);
	dfs_dir_anchor_destroy(dfs_anchor);
...
dfs_test_datacache(void **state)
{
	...
	/* offset is not zero (not aligned)*/
	d_iov_set(&iov, (char *)buf_rd + 3 * sizeof(int), (NUM_INT - 3) * sizeof(int));
	memset(buf_rd, 0, NUM_INT * sizeof(int));
	buf_rd[1] = 1;
	buf_rd[2] = 2;
	...
	for (block_num = 1; block_num <= 1024; block_num*=2) {
		...
		for (i = 0; i < block_num; i++) {
			d_iov_set(&iov_list[i], (char *)buf_rd + i * block_size, block_size);
		}
		sgl.sg_nr     = block_num;
		sgl.sg_nr_out = block_num;
	...
	for (range_num = 1; range_num <= MAX_RG; range_num*=2) {
		...
		block_size  = range_size / block_num;
		iod.iod_nr  = range_num;
		iod.iod_rgs = iod_rgs;

		sgl.sg_nr     = block_num * range_num;
		sgl.sg_nr_out = block_num * range_num;
		sgl.sg_iovs   = iov_list;

		block_cnt = 0;
		for (i = 0; i < range_num; i++) {
			iod_rgs[i].rg_idx = i * range_size;
		...
		rc = dfs_readx(dfs, file, &iod, &sgl, &read_size, NULL);
		assert_int_equal(rc, 0);
		assert_true(read_size == (NUM_INT * sizeof(int)));
		/* verify data integrity */
		for (i = 0; i < NUM_INT; i++) {
	...
	assert_true((perf_cached / perf_no_cache) > 3.0);
