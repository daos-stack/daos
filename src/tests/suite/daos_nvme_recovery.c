...
nvme_fault_reaction(void **state, int mode)
{
	...
	for (i = 0; i < ndisks; i++) {
		...
		print_message("UUID="DF_UUIDF" Traddr=%s State=%s Rank=%d Host=%s\n",
			       DP_UUID(devices[i].device_id), devices[i].traddr,
			       devices[i].state, devices[i].rank,
			       devices[i].host);
	...
	if (mode == 0) {
		...
		print_message("Connecting the pool for pool query\n");
...
nvme_verify_states_faulty(void **state)
{
	...
	rc = daos_mgmt_get_led_state(arg->group, devices[rank_pos].device_id,
				     devices[rank_pos].rank, &led_state,
				     NULL /*ev*/);
	assert_rc_equal(rc, 0);
	rc = verify_led_state(led_state, "FAULT");
	if (rc != 0) {
		print_message("LED not set to FAULT state\n");
		assert_rc_equal(rc, 0);
	}
	print_message("LED state is in FAULT(%d) state\n", led_state);

	/* Reset the LED */
	rc = daos_mgmt_reset_led_state(arg->group, devices[rank_pos].device_id,
				       devices[rank_pos].rank, NULL /*ev*/);
	assert_rc_equal(rc, 0);
	rc = daos_mgmt_get_led_state(arg->group, devices[rank_pos].device_id,
				     devices[rank_pos].rank, &led_state,
				     NULL /*ev*/);
	assert_rc_equal(rc, 0);
	rc = verify_led_state(led_state, "OFF");
	if (rc != 0) {
		print_message("LED not reset to OFF state\n");
		assert_rc_equal(rc, 0);
	}
	print_message("LED successfully reset to OFF(%d) state\n", led_state);
...
nvme_verify_states_faulty_reint(void **state)
{
	...
	print_message("UUID="DF_UUIDF" Traddr=%s Device State=%s\n",
		      DP_UUID(devices[faulty_disk_idx].device_id),
	...
	rc = daos_mgmt_get_led_state(arg->group,
				     devices[faulty_disk_idx].device_id,
				     devices[faulty_disk_idx].rank,
				     &led_state, NULL /*ev*/);
	assert_rc_equal(rc, 0);
	rc = verify_led_state(led_state, "FAULT");
	if (rc != 0) {
...
vmd_identify_led(void **state)
{
	...
	for (i = 0; i < ndisks; i++) {
		...
		rc = dmg_storage_identify_vmd(dmg_config_file, devices[i].host,
					      devices[i].device_id);
		assert_rc_equal(rc, 0);
		rc = daos_mgmt_get_led_state(arg->group, devices[i].device_id,
					     devices[i].rank, &led_state,
					     NULL /*ev*/);
		assert_rc_equal(rc, 0);
		rc = verify_led_state(led_state, "IDENTIFY");
		if (rc != 0) {
			print_message("LED not set to IDENTIFY state\n");
			assert_rc_equal(rc, 0);
		}
		print_message("LED state is in IDENTIFY(%d) state\n",
			      led_state);
	}
