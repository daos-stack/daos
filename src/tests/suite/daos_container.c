...
co_op_retry(void **state)
{
	...
	print_message("setting DAOS_CONT_CREATE_DESTROY_NOREPLY and creating container ... ");
	rc = daos_debug_set_params(arg->group, -1, DMG_KEY_FAIL_LOC,
				   DAOS_CONT_CREATE_DESTROY_NOREPLY | DAOS_FAIL_ONCE, 0, NULL);
	assert_rc_equal(rc, 0);
	...
	print_message("BEGIN: many containers with DAOS_CONT_CREATE_DESTROY_NOREPLY\n");
	for (i = 0; i < (20000 + 1024); i++) {
		char	cname[32];
		uuid_t	cuuid;

		assert_true(snprintf(cname, 32, "cont%d", i) < 32);

		/* Force a retry RPC for container create */
		rc = daos_debug_set_params(arg->group, -1, DMG_KEY_FAIL_LOC,
					   DAOS_CONT_CREATE_DESTROY_NOREPLY | DAOS_FAIL_ONCE,
					   0, NULL);
		assert_rc_equal(rc, 0);
	...
	print_message("END: many (%d) containers with DAOS_CONT_CREATE_DESTROY_NOREPLY\n", (i+1));
	...
	rc = daos_debug_set_params(arg->group, 0, DMG_KEY_FAIL_LOC,
				   DAOS_CONT_QUERY_FAIL_CORPC | DAOS_FAIL_ONCE, 0, NULL);
	assert_rc_equal(rc, 0);
	print_message("success\n");
	...
	rc = daos_debug_set_params(arg->group, 0, DMG_KEY_FAIL_LOC,
				   DAOS_CONT_CLOSE_FAIL_CORPC | DAOS_FAIL_ONCE, 0, NULL);
	assert_rc_equal(rc, 0);
	print_message("success\n");
	...
	rc = daos_debug_set_params(arg->group, 0, DMG_KEY_FAIL_LOC,
				  DAOS_CONT_DESTROY_FAIL_CORPC | DAOS_FAIL_ONCE, 0, NULL);
	assert_rc_equal(rc, 0);
	print_message("success\n");
