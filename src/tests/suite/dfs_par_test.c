...
file_atomicity_test_helper(test_arg_t *arg, int rf)
{
	...
	if (arg->myrank == 0) {
		...
		if (rf == 1 || rf == 2) {
			...
			if (rf == 1)
				attr.da_props->dpp_entries[0].dpe_val = DAOS_PROP_CO_REDUN_RF1;
			else if (rf == 2)
				attr.da_props->dpp_entries[0].dpe_val = DAOS_PROP_CO_REDUN_RF2;
		}
