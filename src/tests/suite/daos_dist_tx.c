...
{
	test_arg_t	*arg = *state;
	char		 dkey[DTX_KEY_LEN];
	char		 akey[DTX_KEY_LEN];
	char		 write_bufs[2][DTX_IO_SMALL];
	char		 fetch_buf[DTX_IO_SMALL * 2];
	daos_handle_t	 th = { 0 };
	daos_obj_id_t	 oid;
	...
	dts_key_gen(dkey, DTX_KEY_LEN, dts_dtx_dkey);
	dts_key_gen(akey, DTX_KEY_LEN, dts_dtx_akey);
...
dtx_10(void **state)
{
	...
	char		 dkey[DTX_KEY_LEN];
	char		 akey[DTX_KEY_LEN];
	char		 dkey2[DTX_KEY_LEN];
	char		 akey2[DTX_KEY_LEN];
	char		 write_buf[DTX_IO_SMALL];
	char		 fetch_buf[DTX_IO_SMALL];
	daos_handle_t	 th = { 0 };
	daos_obj_id_t	 oid;
	...
	dts_key_gen(dkey, DTX_KEY_LEN, dts_dtx_dkey);
	dts_key_gen(akey, DTX_KEY_LEN, dts_dtx_akey);
	dts_key_gen(dkey2, DTX_KEY_LEN, "tmp_dkey");
	dts_key_gen(akey2, DTX_KEY_LEN, "tmp_akey");
