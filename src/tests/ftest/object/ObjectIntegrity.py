...
from daos_api import DaosContext, DaosPool, DaosContainer, IORequest, DaosObj, DaosApiError, DaosLog
...
class ObjectDataValidation(avocado.Test):
    ...
    def test_single_object_validation(self):
        ...
        for dkey in range(self.no_of_dkeys):
            for akey in range(self.no_of_akeys):
                ...
                self.ioreq.single_insert(c_dkey, c_akey, c_value, c_size, new_transaction)
        ...
        for dkey in range(self.no_of_dkeys):
            for akey in range(self.no_of_akeys):
                ...
                if indata != (repr(val.value)[1:-1]):
                    self.d_log.error("ERROR:Data mismatch for dkey = {0}, akey={1}"
    ...
    def test_array_object_validation(self):
        ...
        for dkey in range(self.no_of_dkeys):
            for akey in range(self.no_of_akeys):
                c_values = []
                value = "{0}".format(str(akey)[0]) * self.record_length[record_index]
                ...
                new_transaction = self.container.get_new_tx()
                self.ioreq.insert_array(c_dkey, c_akey, c_values, new_transaction)
                self.container.commit_tx(new_transaction)
                transaction.append(new_transaction)
