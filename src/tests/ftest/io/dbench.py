...
class Dbench(DfuseTestBase):
    ...
    def test_dbench(self):
        ...
        """
#        dir_name = self.params.get("dirname", '/run/dbench/*')
#        file_name1 = self.params.get("filename1", '/run/dbench/*')
#        file_name2 = self.params.get("filename2", '/run/dbench/*')
#        dd_count = self.params.get("dd_count", '/run/dbench/*')
#        dd_blocksize = self.params.get("dd_blocksize", '/run/dbench/*')
...
#                            u"rmdir {}".format(abs_dir_path)]
        ...
        commands = ["dbench -c /home/standan/mpiio/dbench/loadfiles/client.txt -D /tmp/daos_dfuse/ -t 120 10"]

        for cmd in commands:
            try:
                # execute bash cmds
                ret_code = general_utils.pcmd(
                    self.hostlist_clients, cmd, timeout=self.timeout)
            ...
            # report error if any command fails
                ...
                self.log.error("Dbench Test Failed: %s",
                               str(error))
