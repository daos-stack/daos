...
class HotPlugNoActivityTest(IorTestBase):
    ...
    def get_total_nvme_bytes(self, dmg_command):
        ...
        Returns:
            int: total_bytes value of the first NVMe device in the server.

        """
        usage_out = dmg_command.storage_query_usage()
        hosts_to_smd_devices = {}
        # There's a hash value under HostStorage. Obtain "storage" -> "nvme_devices" and "hosts"
        # under it. There may be multiple hosts depending on the setup, so use "hosts" as key and
        # "smd_devices" list as value.
    ...
    def test_no_activity(self):
        ...
        total_bytes_orig = self.get_total_nvme_bytes(dmg_command=dmg_command)
        ...
        # self.log.debug("## Sleep for 30 sec...")
        # time.sleep(30)
        ...
        # self.log_step(msg)
        # error_msg = "Disk and LED state didn't turn to removed state. (UNPLUGGED and NA)"

        # 7. Verify that the disk space is down compared to before the remove.
        self.log_step("Verify that the disk space is down compared to before the remove.")
        total_bytes_hot_remove = self.get_total_nvme_bytes(dmg_command=dmg_command)
        ...
        self.repeat_query_list_devices(
            ...
            dmg_command=dmg_command)

        # 10. Verify that the disk space is back to the original.
        self.log_step("Verify that the disk space is back to the original.")
        total_bytes_hot_plug = self.get_total_nvme_bytes(dmg_command=dmg_command)
