...
class DaosAggregationIOSmall(IorTestBase):
    ...
    def test_aggregation_io_small(self):
        ...
        scm_index = 0
        ssd_index = 1
        block_size = human_to_bytes(self.params.get("block_size",
                                                    "/run/ior/*"))
        ...
        self.assertTrue(scm_free_space_after_ior <=
                        (initial_scm_free_space - total_ior))
        ...
        self.log.info("{} == {}".format((initial_ssd_free_space - total_ior),
                                        ssd_free_space_after_aggregate))
        self.assertTrue((initial_ssd_free_space - total_ior) ==
                        ssd_free_space_after_aggregate)
        self.log.info("Checking the SCM space is reclaimed")
        self.log.info("{} > {}".format(scm_free_space_after_aggregate,
                                       scm_free_space_after_ior))
