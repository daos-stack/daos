...
class TestWithTelemetryIODtx(IorTestBase, TestWithTelemetry):
    ...
    def test_io_dtx_telmetry_metrics(self):
        """JIRA ID: DAOS-8624.
        ...
        :avocado: tags=test_io_dtx_telemetry
        ...
        test_metrics = TelemetryUtils.ENGINE_IO_DTX_COMMITTED_METRICS + \
                       TelemetryUtils.ENGINE_IO_DTX_COMMITTABLE_METRICS
        self.container = []
        metrics_data = {}
        metrics_data["initial"] = self.telemetry.get_io_metrics(test_metrics)
        ...
        for transfer_size in transfer_sizes:
            ...
            for operation in ["write", "read"]:
                ...
                if transfers:
                    ...
                    # num_committed = ((blocks)/(transfers)) * 2
                    num_committed = "{}-{}".format(block_size, transfer_size)
                    key = "{}-{}".format(operation, num_committed)
                    self.log.info("Number of io dtx committed={}".format(num_committed))
                    self.log.info(
                    ...
                    metrics_data[key] = self.telemetry.get_io_metrics(test_metrics)
                else:
