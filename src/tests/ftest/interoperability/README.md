...
```
dnf list --repo daos-packages-v2.6.1 | grep -E '^daos|^mercury'
```
```
clush -S -B -w $DAOS_CLIENTS "sudo dnf install -y daos-2.6.1-3.el8 \
                                                  mercury-2.4.0~rc5-4.el8 \
                                                  daos-admin-2.6.1-3.el8  \
                                                  daos-client-2.6.1-3.el8 \
                                                  daos-client-tests-2.6.1-3.el8 \
                                                  ior mpich" && \
clush -S -B -w $DAOS_SERVERS "sudo dnf install -y daos-2.6.1-3.el8 \
                                                  mercury-2.4.0~rc5-4.el8 \
                                                  daos-admin-2.6.1-3.el8 \
                                                  daos-server-2.6.1-3.el8 " && \
