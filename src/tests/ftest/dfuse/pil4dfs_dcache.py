...
class Pil4dfsDcache(TestWithServers):
    ...
    _dcache_re = {
        "dcache_add": re.compile(r'^.*dcache_add\(\) .+$'),
        "dcache_del": re.compile(r'^.*dcache_rec_free\(\) .+$'),
        "dcache_hit": re.compile(r'^.*dcache_find_insert_act\(\) dcache hit:.+$'),
        "dcache_miss": re.compile(r'^.*dcache_find_insert_act\(\) dcache miss:.+$'),
        "no_dcache_new": re.compile(r'^.*dcache_find_insert_dact\(\).+$'),
        "no_dcache_del": re.compile(r'^.*drec_del_at_dact\(\) .+$'),
        "dcache_gc_add": re.compile(r'^.*gc_add_rec\(\) .+$'),
        "dcache_gc_del": re.compile(r'^.*gc_del_rec\(\) .+$'),
        "dcache_gc_rec": re.compile(r'^.*gc_reclaim\(\) remove expired .+$')
    }
