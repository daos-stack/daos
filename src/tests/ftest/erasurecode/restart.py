...
class EcodServerRestart(ErasureCodeIor):
    ...
    """
    Test Class Description: To validate Erasure code object data after restarting
                            all servers.
    :avocado: recursive
    """
    def setUp(self):
    ...
    def execution(self, agg_check=None):
        ...
        Args:
            agg_check: When to check Aggregation status.Either before
                       restarting all the servers or after.Default is None
                       so not to check aggregation, but wait for 20 seconds
        ...
        if not agg_check:
            ...
            # So wait for 20 seconds and restart all the servers.
            time.sleep(20)

        if agg_check == "Before":
            # Verify if Aggregation is getting started
            if not any(check_aggregation_status(self.pool,
                                                attempt=50).values()):
    ...
    def test_ec_restart_before_agg(self):
        ...
        Test Description: Test Erasure code object with IOR after all server
                          restart and Aggregation trigger before restart.
        Use Case: Create the pool, run IOR with supported EC object type class
                  for small and large transfer sizes.Verify aggregation starts,
                  Restart all the servers.Read and verify all IOR data.
    ...
    def test_ec_restart_after_agg(self):
        ...
        Test Description: Test Erasure code object with IOR after all server
                          restart and Aggregation trigger after restart.
        Use Case: Create the pool, run IOR with supported EC object type class
                  for small and large transfer sizes.Restart all servers.
                  Verify Aggregation trigger after it's start.
    ...
    def test_ec_restart_during_agg(self):
        ...
        Test Description: Test server restart works during aggregation and
                          IOR data is intact after restart.
        Use Case: Create the pool, disabled rebuild, run IOR with supported
                  ...
                  Restart all servers, Read and verify all IOR data.
