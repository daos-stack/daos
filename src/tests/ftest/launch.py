...
class Launch():
    ...
    def run(self, args):
        ...
        self.details["installed packages"] = find_packages(
            logger, all_hosts, "'(daos|libfabric|mercury)-'")
        ...
        for repeat in range(1, self.repeat + 1):
            ...
            for index, test in enumerate(self.tests):
                ...
                return_code |= self.process(
                    test, repeat, stop_daos, archive, rename, jenkins_xml, core_files, threshold)
    ...
    def process(self, test, repeat, stop_daos, archive, rename, jenkins_xml, core_files, threshold):
        ...
        if rename:
            return_code |= self._rename_avocado_test_dir(test, jenkins_xml)
    ...
    def _rename_avocado_test_dir(self, test, jenkins_xml):
        ...
        if jenkins_xml and not self._update_jenkins_xml(test, new_test_logs_dir):
            return 1024
    ...
    def _update_jenkins_xml(self, test, logs_dir):
        """Update the xml files for use in Jenkins.

        Args:
            test (TestInfo): the test information
            new_test_logs_dir (str): location of the test results
        ...
        xml_data = self._get_xml_data(xml_file)
        if not xml_data:
            return False
        ...
        if not self._update_xml(xml_file, pattern, replacement, xml_data):
            return False
        ...
        if not self._update_xml(launchable_xml, pattern, replacement, xml_data):
            return False
        ...
        run_local(logger, f"ls -al '{logs_dir}'/")
        run_local(logger, f"ls -al '{logs_dir}'/test-results/")
        run_local(logger, f"ls -al '{logs_dir}'/test-results/*")
        run_local(logger, f"ls -al '{logs_dir}'/test-results/*-*/data/*.xml")
    ...
    def _get_xml_data(self, xml_file):
