...
def process_the_cores(avocado_logs_dir, test_yaml, test_name, args):
    """Collect and process any core files generated by the test.

    Args:
        avocado_logs_dir (str): avocado job-results directory
        test_yaml (str): the test yaml file
        test_name (str): current running testname
        args (argparse.Namespace): command line arguments for this program

    Returns:
        bool: True if no core files were found or all core files were successfully processed; False
            otherwise

    """
    status = True
    hosts = NodeSet.fromlist(get_hosts_from_yaml(test_yaml, args))
    ...
    try:
        ...
        display(args, "Error processing core files for {}: {}".format(test_name, error), 0)
        status = False
    print("-" * 80)
