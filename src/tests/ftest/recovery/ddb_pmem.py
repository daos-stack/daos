...
def insert_objects(context, container, object_count, dkey_count, akey_count, base_dkey, base_akey,
                   base_data):
...
def copy_remote_to_local(remote_file_path, test_dir, remote):
    """Copy the given file from the server node to the local test node and retrieve the original
    name.
...
class DdbPMEMTest(TestWithServers):
    ...
    def __init__(self, *args, **kwargs):
        """Initialize a DdbPMEMTest object."""
        super().__init__(*args, **kwargs)
        # Number of objects and keys to insert/expect.
        self.object_count = 5
        self.dkey_count = 2
        self.akey_count = 1
    ...
    def test_recovery_ddb_rm(self):
        ...
        """
        self.log_step("Create a pool and a container. Insert objects, dkeys, and akeys.")
