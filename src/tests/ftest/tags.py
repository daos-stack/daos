...
def recommended_map_tags(paths):
    """Get the recommended tags based on the config mapping.

    Args:
        paths (list): paths to get tags for

    Returns:
        set: the recommended tags
    """
    all_mapping = get_tag_map()
    recommended = set()
    exclude = all_mapping['exclude']
    include = all_mapping['include']
    tags_per_file = all_mapping['map']

    def _any_match(path, expressions):
        for expression in expressions:
            if re.search(rf'{expression}', path):
                return True
        return False

    for path in paths:
        # Remove root path prefix
        path = re.sub(fr'{ROOT_DIR}{os.sep}*', '', os.path.realpath(path))
        # Exclude any that match the exclude list but aren't in the include list
        if _any_match(path, exclude) and not _any_match(path, include):
            continue
        # Get tags for all matches
        for _pattern, _tags in tags_per_file.items():
            if re.search(rf'{_pattern}', path):
                recommended.update(_tags.split(' '))
    return recommended
...
def run_linter(paths=None):
    ...
    """
    check_tags_yaml = False
    if not paths:
        check_tags_yaml = True
    ...
    # Make sure the tag map contains valid tags
    valid_tags = ftest_tag_map.unique_tags()
    map_tags = set()
    for _, tags in get_tag_map()['map'].items():
        for _tags in filter(None, tags.split(' ')):
            map_tags.update(_tags.split(','))
    invalid_tags = map_tags.difference(valid_tags)
    ...
    def _error_handler(_list, message, required=True):
        ...
        print(f'  {req_str} {len(_list)} {message}')
        if _list_len == 0:
            return None
        for _test in _list:
            print(f'    {_test}')
    ...
    if check_tags_yaml:
        errors.extend(list(filter(None, [
            _error_handler(list(invalid_tags), 'invalid tags in tags.yaml')])))
    if errors:
        raise errors[0]
...
def files_to_tags(paths):
    ...
    Args:
        paths (list): paths to get pragmas for
    ...
    # Get tags from the map for non-ftest paths
    core_tag_set = recommended_map_tags(paths)
    return ftest_tag_set | core_tag_set
