...
static int
check_status(void *arg)
{
	return (atomic_load((ATOMIC int *)arg) == STOP);
}
...
progress(void *arg)
{
	...
	atomic_fetch_add(status, -1);

	do {
		rc = crt_progress_cond(crt_ctx, 1000 * 1000, check_status, status);
		if (rc == -DER_TIMEDOUT)
			sched_yield();
		else if (rc != 0)
...
rpc_handler(crt_rpc_t *rpc)
{
	...
	for (i = 0; i < MSG_COUNT; i++) {
		if (in->msg == msg_values[i] && in->payload == MSG_IN_VALUE) {
			atomic_fetch_add(&msg_counts[i], 1);
...
main(int argc, char **argv)
{
	...
	rc = crt_init("threaded_server", CRT_FLAG_BIT_SERVER | CRT_FLAG_BIT_AUTO_SWIM_DISABLE);
	if (rc != 0) {
		printf("Could not start server, rc = %d", rc);
		goto log_fini;
	}

	grp = crt_group_lookup("threaded_server");
	...
	while (atomic_load(&status) != -NUM_THREADS)
		sched_yield();
