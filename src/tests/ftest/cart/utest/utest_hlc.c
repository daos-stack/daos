...
test_hlc_get_msg(void **state)
{
	...
	for (i = 0; i < COUNT; i++) {
		...
		if (i % 5 == 1)
			...
			time2 = time + (i % 3);
		rc = d_hlc_get_msg(time2, &time, NULL);
		assert_true(rc == 0);
...
test_hlc_conversion(void **state)
{
	...
	print_message("after: <%lld, %ld>\n", (long long)after.tv_sec,
		      after.tv_nsec);
	t = d_hlc2unixnsec(hlc) / (1000 * 1000 * 1000);
...
test_hlc_epsilon(void **state)
{
	...
	eps = 0;
	d_hlc_epsilon_set(eps);
	assert_true(d_hlc_epsilon_get() == 0);
	pt1 = (0x123ULL << shift) + 0x456;
	hlc1 = pt1 | mask;			/* max logical */
	pt2 = pt1 - eps;
	hlc2 = pt2 & ~mask;			/* min logical */
	assert_true(d_hlc_epsilon_get_bound(hlc2) >= hlc1);
	assert_true(d_hlc_epsilon_get_bound(hlc2) <= ((pt1 + mask) | mask));

	eps = 1;
	d_hlc_epsilon_set(eps);
	assert_true(d_hlc_epsilon_get() == 1ULL << shift);
	pt1 = 0x123ULL << shift;
	hlc1 = pt1 | mask;			/* max logical */
	pt2 = pt1 - eps;
	hlc2 = pt2 & ~mask;			/* min logical */
	...
	eps = (1ULL << shift) + 1;
	d_hlc_epsilon_set(eps);
	assert_true(d_hlc_epsilon_get() == 2ULL << shift);
	pt1 = 0x123ULL << shift;
	hlc1 = pt1 | mask;			/* max logical */
	pt2 = pt1 - eps;
	hlc2 = pt2 & ~mask;			/* min logical */
