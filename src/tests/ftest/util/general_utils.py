...
class MyHandler(EventHandler):
    """
    Interface to handle events generated by Clustershell
    """
    #pylint: disable=no-self-use
    def ev_read(self, _worker, node, _sname, msg):
        """
        To read the data from specific node, This event helps to print the
        command output while it's running.
        """
        print ("{}: {}".format(node, msg))
    #pylint: enable=no-self-use
# pylint: enable=too-few-public-methods

def clustershell_execute(cmd, hosts, timeout=60, debug=False):
    """
    ...
    local_task.run(cmd, nodes=','.join(hosts), timeout=timeout, handler=handler)
