...
def build_ior(basepath):
    ...
    with open(os.path.join(basepath, ".build_vars.json")) as build_file:
    ...
    try:
        ...
        configure_cmd = (cd_cmd + ' && ./configure --prefix={0} --with-daos={0}'
            .format(daos_dir))
        ...
        print("<IorBuildFailed> Exception occurred: {0}".format(str(excep)))
...
def run_ior(client_file, ior_flags, iteration, block_size, transfer_size,
    ...
    try:
        ...
        ior_cmd = (
            ...
            "-t {} -- -p {} -v {} -r {} -s {} -c {} -a {} -o {} "
            .format(slots, client_file, attach_info_path, ior_flags, seg_count,
                    iteration, filename, block_size, transfer_size, pool_uuid,
...
def run_ior_mpiio(basepath, mpichinstall, pool_uuid, svcl, num_procs, hostfile,
    ...
    """
        ...
        pool_uuid      --Daos Pool UUID
        svcl           --Daos Pool SVCL
        num_procs      --number of client processes
    ...
    try:
        ...
        run_cmd = (
            ...
            "-t {4} "
            "-b {5} "
            "-o daos:testFile".format(num_procs, hostfile, ior_flags, iteration,
