...
def srun(hostlist, cmd, srun_params=None):
    """Run srun cmd on slurm partition.

    Args:
        partition (str): partition to allocate the nodes from
        hosts (str): hosts to allocate and execute cmd on
    ...
    cmd = "srun -w {} {} {}".format(hostlist, ", ".join(params), cmd)
    try:
