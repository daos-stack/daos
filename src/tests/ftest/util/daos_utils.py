...
class DaosCommand(CommandWithSubCommand):
    ...
    class ContainerSubCommand(CommandWithSubCommand):
        ...
        def get_sub_command_class(self):
            ...
            elif self.sub_command.value == "uns-insert":
                self.sub_command_class = self.UnsInsertSubCommand()
            else:
                self.sub_command_class = None
        ...
        class UnsInsertSubCommand(CommonContainerSubCommand):
            """Defines an object for the daos container uns-insert command."""

            def __init__(self):
                """Create a daos container uns-insert command object."""
                super(
                    DaosCommand.ContainerSubCommand.UnsInsertSubCommand,
                    self).__init__("uns-insert")
...
def create_container(path, pool_uuid, svc, cont_type, env=None):
    ...
    Args:
        path (str): path to the daos executable
        pool_uuid (str): pool UUID
        svc (str): the pool service replicas, e.g. '1,2,3'
        cont_type (str): the type of container to create
        env (dict, optional): dictionary of environment variable names and
            values (EnvironmentVariables). Defaults to None.

    Returns:
        CmdResult: an avocado CmdResult object with the command's results or
            None if the command failed.

    """
    log = getLogger()

    daos = DaosCommand(path)
    daos.env = env
    daos.set_sub_command("container")
    daos.sub_command_class.set_sub_command("create")
    daos.sub_command_class.sub_command_class.pool.value = pool_uuid
    daos.sub_command_class.sub_command_class.svc.value = svc
    daos.sub_command_class.sub_command_class.type.value = cont_type

    try:
        result = daos.run()
    except CommandFailure as details:
        log.error("doas command failed: %s", details)
        return None
