...
from command_utils import \
    ExecutableCommand, FormattedParameter, EnvironmentVariables
...
class MacsioCommand(ExecutableCommand):
    ...
    def __init__(self, path=""):
        ...
        self.log = FormattedParameter("--log {}")
        ...
        self._env_names = ["DAOS_POOL", "DAOS_SVCL", "DAOS_CONT"]
    ...
    def get_environment(self, pool_uuid, pool_svcl, cont_uuid=None):
        """Get the environment variables to export for the macsio command.
        ...
        Returns:
            EnvironmentVariables: a dictionary of environment variable names and
                value to export prior to running macsio

        """
        env = EnvironmentVariables()
        for index, value in (pool_uuid, pool_svcl, cont_uuid):
            if value:
