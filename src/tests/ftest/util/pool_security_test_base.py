...
class PoolSecurityTestBase(TestWithServers):
    ...
    def setUp(self):
        ...
        # Create a DmgCommand object
        self.dmg = DmgCommand(self.bin)
        self.dmg.hostlist.update(self.hostlist_servers, "dmg.hostlist")

        # Create a DaosCommand object
        self.daos = DaosCommand(self.bin)
        self.daos.exit_status_exception = False
    ...
    def verify_pool_readwrite(self, svc, uuid, action, expect='Pass'):
        ...
        if action.lower() == "write":
            result = self.daos.container_create(pool=uuid, svc=svc)
        elif action.lower() == "read":
            result = self.daos.pool_query(pool=uuid, svc=svc)
        else:
            self.fail(
    ...
    def pool_acl_verification(self, current_user_acl, read, write,
        ...
        # (3)Create a pool with acl
        self.log.info("  (2)Create a pool with acl")
        self.dmg.exit_status_exception = False
        result = self.dmg.pool_create(scm_size=scm_size, acl_file=acl_file)
        self.dmg.exit_status_exception = True

        # (3)Display dmg command used to create a pool with acl
        self.log.info("  (3)dmg= %s", self.dmg)
