...
def cleanup_processes(log, test, result):
    ...
    mount_grep_result = run_remote(log, hosts, mount_grep_cmd)
    if mount_grep_result.passed_hosts:
        ...
        umount_result = run_remote(log, mount_grep_result.passed_hosts, umount_cmd)
...
def archive_files(log, summary, hosts, source, pattern, destination, depth, threshold, timeout,
    ...
    return_code, file_hosts = list_files(log, hosts, source, pattern, depth, test_result)
    if not file_hosts:
    ...
    if "log" in pattern:
        ...
        return_code |= remove_empty_files(log, file_hosts, source, pattern, depth, test_result)

        # Report an error if any files sizes exceed the threshold
        if threshold is not None:
            return_code |= check_log_size(
                log, file_hosts, source, pattern, depth, threshold, test_result)

        # Run cart_logtest on log files
        return_code |= cart_log_test(log, file_hosts, source, pattern, depth, test_result)

    # Remove any empty files
    return_code |= remove_empty_files(log, file_hosts, source, pattern, depth, test_result)

    # Compress any files larger than 1 MB
    return_code |= compress_files(log, file_hosts, source, pattern, depth, test_result)

    # Move the test files to the test-results directory on this host
    return_code |= move_files(
        log, file_hosts, source, pattern, destination, depth, timeout, test_result)

    if test and "core files" in summary:
        # Process the core files
        return_code |= process_core_files(log, os.path.split(destination)[0], test, test_result)

    return return_code


def list_files(log, hosts, source, pattern, depth, test_result):
...
def check_log_size(log, hosts, source, pattern, depth, threshold, test_result):
...
def cart_log_test(log, hosts, source, pattern, depth, test_result):
...
def remove_empty_files(log, hosts, source, pattern, depth, test_result):
...
def compress_files(log, hosts, source, pattern, depth, test_result):
...
def move_files(log, hosts, source, pattern, destination, depth, timeout, test_result):
    ...
    return_code = 0
    try:
        run_local(log, " ".join(command), check=True, timeout=timeout)
...
def process_core_files(log, test_job_results, test, test_result):
...
def rename_avocado_test_dir(log, test, job_results_dir, test_result, jenkins_xml, total_repeats):
    ...
    if jenkins_xml and not update_jenkins_xml(log, test, new_test_logs_dir, test_result):
        return 1024
...
def update_jenkins_xml(log, test, logs_dir, test_result):
    ...
    """
    log.info("Updating the xml test result files for use in Jenkins")

    # Read the test xml file
    xml_file = os.path.join(logs_dir, 'results.xml')
    xml_data = get_xml_data(log, xml_file, test_result)
    if not xml_data:
        return False
    ...
    if not update_test_xml(log, test, xml_file, xml_data, launchable_xml, test_result):
        return False
    ...
    if cmocka_files:
        ...
        for cmocka_xml in cmocka_files:
            ...
            cmocka_data = get_xml_data(log, cmocka_xml, test_result)
            if not cmocka_data:
                return False

            # Include the functional test directory in the class name of the cmocka xml file
            if not update_cmocka_xml(log, test, cmocka_xml, cmocka_data, test_class, test_result):
                return False
    return True
...
def update_test_xml(log, test, xml_file, xml_data, launchable_xml, test_result):
    ...
    """
    log.debug("Updating the xml data in the test %s file", xml_file)

    # Update the class name to include the functional test directory
    pattern = 'classname="'
    replacement = f'classname="FTEST_{test.directory}.'
    if not update_xml(log, xml_file, pattern, replacement, xml_data, test_result):
        return False
    ...
    return update_xml(log, launchable_xml, pattern, replacement, xml_data, test_result)
...
def update_cmocka_xml(log, test, cmocka_xml, cmocka_data, test_class, test_result):
    ...
    if 'classname' not in cmocka_data:
        ...
        return update_xml(log, cmocka_xml, pattern, replacement, cmocka_data, test_result)
...
def get_xml_data(log, xml_file, test_result):
...
def update_xml(log, xml_file, pattern, replacement, xml_data, test_result):
    ...
    for line in get_xml_data(log, xml_file, test_result).splitlines():
        log.debug("    %s", line)
    log.debug("")
...
def collect_test_result(log, test, test_result, job_results_dir, stop_daos, archive, rename,
    ...
    if stop_daos:
        if not stop_daos_agent_services(log, test):
            return_code |= 512
        if not stop_daos_server_service(log, test):
            return_code |= 512
        if not reset_server_storage(log, test):
            return_code |= 512
        if not cleanup_processes(log, test, test_result):
            return_code |= 4096
    ...
    if rename:
        return_code |= rename_avocado_test_dir(
            log, test, job_results_dir, test_result, jenkins_xml, total_repeats)
