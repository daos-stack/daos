...
def update_jenkins_xml(test, logs_dir, test_result):
    ...
    if not update_test_xml(test, xml_file, xml_data, launchable_xml, test_result):
        return False
    ...
    if cmocka_files:
        ...
        for cmocka_xml in cmocka_files:
            ...
            if not update_cmocka_xml(test, cmocka_xml, cmocka_data, test_class, test_result):
                return False
    return True


def update_test_xml(test, xml_file, xml_data, launchable_xml, test_result):
    ...
    return update_xml(launchable_xml, pattern, replacement, xml_data, test_result)


def update_cmocka_xml(test, cmocka_xml, cmocka_data, test_class, test_result):
    ...
    if 'classname' not in cmocka_data:
        ...
        return update_xml(cmocka_xml, pattern, replacement, cmocka_data, test_result)
