...
def fault_injection_enabled(log):
...
class AvocadoInfo():
    ...
    @staticmethod
    def get_setting(log, section, key, default=None):
...
class TestInfo():
    ...
    def set_yaml_info(self, log, include_local_host=False):
    ...
    def set_host_info(self, log, control_node):
        ...
        try:
            self.host_info.set_hosts(
                log, control_node, self.yaml_info[self.YAML_INFO_KEYS[0]],
...
class TestRunner():
    ...
    def prepare(self, log, test_log_file, test, repeat, user_create, slurm_setup, control_host,
                partition_hosts):
        ...
        status = self._setup_host_information(log, test, slurm_setup, control_host, partition_hosts)
        if status:
            return status

        # Setup (remove/create/list) the common test directory on each test host
        status = self._setup_test_directory(log, test)
        if status:
            return status

        # Setup additional test users
        status = self._user_setup(log, test, user_create)
        if status:
            return status

        # Generate certificate files for the test
        return self._generate_certs(log)

    def execute(self, log, test, repeat, number, sparse, fail_fast):
        ...
        try:
            return_code = run_local(
                log, " ".join(command), capture_output=False, check=False).returncode
            if return_code == 0:
                ...
                self.test_result.fail_test(log, "Execute", message)
            elif return_code & 8 == 8:
                log.debug("At least one avocado test variant was interrupted")
            if return_code:
                self._collect_crash_files(log)

        except RunException:
            message = f"Error executing {test} on repeat {repeat}"
            self.test_result.fail_test(log, "Execute", message, sys.exc_info())
            return_code = 1
    ...
    def process(self, log, job_results_dir, test, repeat, stop_daos, archive, rename,
        ...
        status = collect_test_result(
            log, test, self.test_result, job_results_dir, stop_daos, archive, rename, jenkins_xml,
        ...
        if self.test_result.status is None:
            self.test_result.pass_test(log)
    ...
    def _setup_host_information(self, log, test, slurm_setup, control_host, partition_hosts):
        ...
        if test.yaml_info["client_partition"]:
            ...
            if slurm_setup:
                ...
                if not create_partition(log, control_host, partition, hosts).passed:
        ...
        try:
            test.set_host_info(log, control_host)
        except LaunchException:
            message = "Error setting up host information"
            self.test_result.fail_test(log, "Prepare", message, sys.exc_info())
            return 128
    ...
    def _setup_test_directory(self, log, test):
    ...
    def _user_setup(self, log, test, create=False):
        ...
        for _user in users:
            ...
            if group and group not in group_gid:
                try:
                    ...
                    self.test_result.fail_test(log, "Prepare", str(error), sys.exc_info())
                    return 128
    ...
    @staticmethod
    def _query_create_group(log, hosts, group, create=False):
        ...
        group_ids = get_group_id(log, hosts, group).keys()
    ...
    @staticmethod
    def _query_create_user(log, hosts, user, gid=None, create=False):
        ...
        groups = get_user_groups(log, hosts, user)
    ...
    def _generate_certs(self, log):
        ...
        try:
            run_local(log, f"/usr/bin/rm -rf {certs_dir}")
            run_local(log, f"{command} {test_env.log_dir}")
        except RunException:
    ...
    def _collect_crash_files(self, log):
        ...
        if os.path.isdir(crash_dir):
            ...
            if crash_files:
                ...
                try:
                    run_local(log, f"mkdir -p {latest_crash_dir}", check=True)
                    for crash_file in crash_files:
                        run_local(log, f"mv {crash_file} {latest_crash_dir}", check=True)
                except RunException:
