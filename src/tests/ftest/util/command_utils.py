...
class TargetedExecutableCommand(CommandWithParameters):
    """Executable command that uses run_pcmd() to run given command on a targeted host.
    """

    def __init__(self, namespace, command, host, path=""):
        """Create a TargetedExecutableCommand object.

        Args:
            namespace (str): yaml namespace (path to parameters)
            command (str): string of the command to be executed.
            host (NodeSet): Single host to run the command on (targeted host).
        ...
        # Define an attribute to store the CmdResult from the last run() call.
        # Following properties will be filled:
        #   exit_status     - exit_status of the command
        #   stdout          - the stdout
        self.result = None
    ...
    def __str__(self):
        ...
        Add "sudo -n" if self.sudo flag is set to True.
    ...
    def run(self):
        ...
        # Run the command with run_pcmd() specifying the host to run.
        pcmd_out = run_pcmd(
            hosts=self.host, command=command, verbose=self.verbose, timeout=self.timeout)

        # Create the CmdResult object from the run_pcmd() output. Fill in "stdout" and
        # "exit_status".
        # run_pcmd() returns each line of stdout in separate element of the list, so
        # reconstruct the output by joining the lines with "\n".
        stdout = "\n".join(pcmd_out[0]["stdout"])
        # CmdResult expects stdout field as bytes.
        stdout = stdout.encode("utf-8")
        exit_status = pcmd_out[0]["exit_status"]
        self.result = process.CmdResult(
            command=command, stdout=stdout, exit_status=exit_status)
