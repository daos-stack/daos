...
class Snapshot(TestWithServers):
    ...
    def test_snapshot_negativecases(self):
        ...
        log_size = self.params.get("log_size", '/run/snapshot/*', default=200)
        try:
        ...
        self.log.info("==snapshot.epoch=  %s", snapshot.epoch)
        self.log.info("==written thedata[:%d]=%s", log_size, thedata[:log_size])
        self.log.info("==thedata2.value[:%d] =%s", log_size, thedata2.value[:log_size])
    ...
    def display_snapshot_test_data(self, test_data, ss_index):
        ...
        if len(test_data) < ss_index - 1:
            ...
            self.log.info("  ==original tst_data[:%d] =%s",
                log_size, test_data[ind]["tst_data"][:log_size])
    ...
    def test_snapshots(self):
        ...
        while ss_number < snapshot_loop:
            ...
            self.log.info("  ==thedata3.value[:%d]= %s", log_size, thedata3.value[:log_size])
        ...
        for ss_number in range(snapshot_loop - 1, 0, -1):
            ...
            try:
                ...
                self.fail("##(5.1)Error when retrieving the snapshot data: {}"
                          .format(str(error)))
            self.log.info("  ==snapshot tst_data[:%d] =%s", log_size, thedata5.value[:log_size])
        ...
        try:
            ...
            self.fail("##(7)Error when retrieving the snapshot data: {}"
                      .format(str(error)))
        self.log.info("=(7)=>thedata_after_snapshot.destroyed.value[:%d]= %s",
                      log_size, thedata7.value[:log_size])
        self.log.info("  ==>snapshot.epoch=     %s", snapshot.epoch)
