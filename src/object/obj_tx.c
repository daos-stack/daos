...
dc_tx_commit_cb(tse_task_t *task, void *data)
{
	...
	D_DEBUG(DB_IO, "TX " DF_DTI " commit with version %u get result: %d\n", DP_DTI(&tx->tx_id),
		tx->tx_pm_ver, rc);
	...
	if (tx->tx_pm_ver < oco->oco_map_version || daos_crt_network_error(rc) ||
		...
		if (tx->tx_orig_task != NULL)
			rc1 = dc_task_depend(tx->tx_orig_task, 1, &pool_task);
		else
...
out:
	if (rc == -DER_STALE) {
		...
		if (tx->tx_orig_task != NULL)
			rc = dc_task_depend(tx->tx_orig_task, 1, &pool_task);
		else
			   ...
			   tse_task_t *parent, uint32_t backoff)
{
	...
	if (tx->tx_set_resend && tx->tx_for_convert)
		parent->dt_result = 0;
