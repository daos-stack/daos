...
static void
dc_tx_close_internal(struct dc_tx *tx, bool locked)
{
	dc_tx_cleanup(tx);
	dc_tx_hdl_unlink(tx);
	if (!locked)
		D_MUTEX_LOCK(&tx->tx_lock);
	tx->tx_status = TX_CLOSED;
	if (!locked)
		D_MUTEX_UNLOCK(&tx->tx_lock);
	dc_tx_decref(tx);
}
...
int
dc_tx_local_close(daos_handle_t th, void *hint)
{
	struct dc_tx	*tx;
	int		 rc = 0;
	...
	D_ASSERTF(hint == tx->tx_hint, "Unmatched hint for tx %p: %p vs %p\n",
		  tx, hint, tx->tx_hint);

	D_ASSERTF(tx->tx_status != TX_CLOSED, "Trying to close TX %p more than once\n", tx);
...
int
dx_tx_set_hint(daos_handle_t th, void *hint)
{
	struct dc_tx	*tx;
	...
	D_MUTEX_LOCK(&tx->tx_lock);
	D_ASSERTF(tx->tx_hint == NULL, "Do not allow to overwrite TX %p hint: %p vs %p\n",
		  tx, hint, tx->tx_hint);
