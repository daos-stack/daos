		      ...
		      struct daos_csummer *csummer)
{
	...
	if (daos_oclass_is_ec(oca)) {
		...
		rc = obj_ec_req_reasb(dcu->dcu_iod_array.oia_iods,
				      ...
				      dcsr->dcsr_reasb, dcsr->dcsr_nr, true, dcu->dcu_flags);
		if (rc != 0)
			return rc;
...
dc_tx_add_update(struct dc_tx *tx, struct dc_object **obj, uint64_t flags,
		 ...
		 d_sg_list_t *sgls, uint32_t extra_flags, void *extra_arg)
{
	struct daos_cpd_sub_req *dcsr;
	...
	*obj = NULL;
	...
	if (extra_flags & DIOF_FOR_EC_AGG)
		dcu->dcu_flags |= ORF_FOR_EC_AGG;
	if (extra_flags & DIOF_EC_AGG_REMOVE) {
		dcu->dcu_flags |= ORF_EC_AGG_REMOVE;
		iod_array->oia_epoch_lo = (uintptr_t)extra_arg;
	}
...
dc_tx_per_akey_existence_parent_cb(tse_task_t *task, void *data)
{
	...
	if (rc == 0) {
		...
		rc = dc_tx_add_update(tx, &obj, args->flags, args->dkey, args->nr,
				      args->iods_or_akeys, args->sgls, 0, NULL);
		D_MUTEX_UNLOCK(&tx->tx_lock);
		obj_decref(obj);
	}
...
dc_tx_convert_cb(tse_task_t *task, void *data)
{
	...
	if (rc == -DER_TX_RESTART) {
		...
		case DAOS_OBJ_RPC_UPDATE: {
			...
			rc = dc_tx_add_update(tx, &obj, up->flags, up->dkey, up->nr, up->iods,
					      up->sgls, up->extra_flags, up->extra_arg);
			break;
		}
		case DAOS_OBJ_RPC_PUNCH: {
