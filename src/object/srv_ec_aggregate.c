...
agg_carry_over(struct ec_agg_entry *entry, struct ec_agg_extent *agg_extent)
{
	...
	daos_off_t   end_stripe =
	    (agg_extent->ae_recx.rx_idx + agg_extent->ae_recx.rx_nr - 1) / stripe_size;
	...
	if (end_stripe > start_stripe) {
		D_ASSERTF(end_stripe - start_stripe == 1,
			  DF_UOID ", recx " DF_RECX " SS=" DF_U64 ", ES=" DF_U64
				  ", EC_OCA(k=%d, p=%d, cs=%d)\n",
			  DP_UOID(entry->ae_oid), DP_RECX(agg_extent->ae_recx), start_stripe,
			  end_stripe, entry->ae_oca.u.ec.e_k, entry->ae_oca.u.ec.e_p,
			  entry->ae_oca.u.ec.e_len);

		tail_size = DAOS_RECX_END(agg_extent->ae_recx) - end_stripe * stripe_size;
...
agg_clear_extents(struct ec_agg_entry *entry)
{
	...
	d_list_for_each_entry_safe(extent, ext_tmp, &entry->ae_cur_stripe.as_dextents, ae_link) {
	...
	if (entry->ae_cur_stripe.as_extent_cnt > 0) {
		D_ERROR(DF_UOID " extent cnt %u\n", DP_UOID(entry->ae_oid),
			entry->ae_cur_stripe.as_extent_cnt);
		d_list_for_each_entry(extent, &entry->ae_cur_stripe.as_dextents, ae_link) {
 ...
 */
static int
agg_recx_iter_pre_cb(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
		     vos_iter_param_t *param, void *cb_arg, unsigned int *acts)
{
	struct ec_agg_entry *age = (struct ec_agg_entry *)cb_arg;

	D_ASSERT(type == VOS_ITER_RECX);
	D_ASSERT(entry->ie_recx.rx_idx ==
		 (PARITY_INDICATOR | (age->ae_cur_stripe.as_stripenum * ec_age2cs(age))));
 ...
 */
static int
agg_alloc_buf(d_sg_list_t *sgl, size_t ent_buf_len, unsigned int iov_entry, bool align_data)
{
	void *buf = NULL;
	int   rc  = 0;
...
agg_prep_sgl(struct ec_agg_entry *entry)
{
	...
	if (entry->ae_sgl.sg_iovs[AGG_IOV_DATA].iov_buf_len < data_buf_len) {
		rc = agg_alloc_buf(&entry->ae_sgl, data_buf_len, AGG_IOV_DATA, true);
		if (rc)
			goto out;
	}
	if (entry->ae_sgl.sg_iovs[AGG_IOV_ODATA].iov_buf_len < data_buf_len) {
		rc = agg_alloc_buf(&entry->ae_sgl, data_buf_len, AGG_IOV_ODATA, true);
		if (rc)
			goto out;
	}
	if (entry->ae_sgl.sg_iovs[AGG_IOV_DIFF].iov_buf_len < len * entry->ae_rsize) {
		rc = agg_alloc_buf(&entry->ae_sgl, len * entry->ae_rsize, AGG_IOV_DIFF, true);
		if (rc)
			goto out;
	}
	par_buf_len = len * p * entry->ae_rsize;
	if (entry->ae_sgl.sg_iovs[AGG_IOV_PARITY].iov_buf_len < par_buf_len) {
		rc = agg_alloc_buf(&entry->ae_sgl, par_buf_len, AGG_IOV_PARITY, false);
		if (rc)
			goto out;
	}
	return 0;
 ...
 */
static bool
agg_overlap(uint64_t estart, uint64_t elen, unsigned int cell_idx, unsigned int k, unsigned int len,
	    uint64_t stripenum)
{
...
static unsigned int
agg_count_cells(uint8_t *fcbit_map, uint8_t *tbit_map, uint64_t estart, uint64_t elen,
		unsigned int k, uint64_t len, uint64_t stripenum, unsigned int *full_cell_cnt)
{
	uint64_t     i;
...
agg_get_obj_handle(struct ec_agg_entry *entry)
{
	...
	rc = dsc_obj_open(agg_param->ap_pool_info.api_cont_hdl, entry->ae_oid.id_pub, DAOS_OO_RW,
			  &entry->ae_obj_hdl);
	if (rc)
		goto out;
	...
	for (i = 0; i < obj_ec_parity_tgt_nr(&entry->ae_oca);
	     i++, tgt_ec_idx = (tgt_ec_idx + 1) % entry->ae_obj_layout->ol_grp_size) {
 ...
 */
static int
agg_fetch_odata_cells(struct ec_agg_entry *entry, uint8_t *bit_map, unsigned int cell_cnt,
		      bool is_recalc)
{
	daos_iod_t            iod    = {0};
	d_sg_list_t           sgl    = {0};
	...
	epoch = is_recalc ? stripe->as_hi_epoch : entry->ae_par_extent.ape_epoch;
	rc    = dsc_obj_fetch(entry->ae_obj_hdl, epoch, &entry->ae_dkey, 1, &iod, &sgl, NULL,
			      DIOF_FOR_EC_AGG, NULL, NULL);
	if (rc)
...
agg_fetch_data_stripe(struct ec_agg_entry *entry)
{
	...
	rc        = vos_obj_fetch(agg_param->ap_cont_handle, entry->ae_oid,
				  entry->ae_cur_stripe.as_hi_epoch, 0, &entry->ae_dkey, 1, &iod,
				  &entry->ae_sgl);
	if (rc)
...
agg_encode_full_stripe_ult(void *arg)
{
	...
	ec_encode_data(cell_bytes, k, p, entry->ae_codec->ec_gftbls, data, parity_bufs);
...
agg_encode_full_stripe(struct ec_agg_entry *entry)
{
	...
	rc = dss_ult_create(agg_encode_full_stripe_ult, &stripe_ud, DSS_XS_OFFLOAD, tid, 0, NULL);
	if (rc)
		goto ev_out;
...
ec_age_data_is_newer(struct ec_agg_entry *entry)
{
	...
	d_list_for_each_entry(agg_extent, &entry->ae_cur_stripe.as_dextents, ae_link) {
 ...
 */
static int
agg_update_vos(struct ec_agg_param *agg_param, struct ec_agg_entry *entry, bool write_parity)
{
	daos_recx_t           recx        = {0};
	...
	if (write_parity) {
		...
		iod.iod_type  = DAOS_IOD_ARRAY;
		iod.iod_recxs = &recx;
		recx.rx_idx   = (entry->ae_cur_stripe.as_stripenum * len) | PARITY_INDICATOR;
		recx.rx_nr    = len;
		if (csummer != NULL) {
			rc = daos_csummer_calc_iods(csummer, &sgl, &iod, NULL, 1, false, NULL, 0,
						    &iod_csums);
			if (rc) {
		...
		rc = vos_obj_update(ap->ap_cont_handle, entry->ae_oid,
				    entry->ae_cur_stripe.as_hi_epoch, 0, 0, &entry->ae_dkey, 1,
				    &iod, iod_csums, &sgl);
	...
	d_list_for_each_entry(ext, &entry->ae_cur_stripe.as_dextents, ae_link) {
		...
		err = vos_obj_array_remove(ap->ap_cont_handle, entry->ae_oid, &epoch_range,
					   &entry->ae_dkey, &entry->ae_akey, &ext->ae_recx);
		if (err)
 ...
 */
static int
agg_fetch_local_extents(struct ec_agg_entry *entry, uint8_t *bit_map, unsigned int cell_cnt,
			bool is_recalc)
{
	daos_iod_t           iod   = {0};
	d_sg_list_t          sgl   = {0};
	...
	for (i = 0, j = 0; i < k; i++)
		if (isset(bit_map, i)) {
			recxs[j].rx_idx  = entry->ae_cur_stripe.as_stripenum * k * len + i * len;
			recxs[j++].rx_nr = len;
		}
	D_ASSERT(j == cell_cnt);
	 ...
	 */
	if (!is_recalc) {
		recxs[cell_cnt].rx_idx =
		    PARITY_INDICATOR | (entry->ae_cur_stripe.as_stripenum * len);
	...
	if (!is_recalc) {
		buf = entry->ae_sgl.sg_iovs[AGG_IOV_PARITY].iov_buf;
		d_iov_set(&sgl.sg_iovs[cell_cnt], &buf[pidx * cell_bytes], cell_bytes);
	}
	...
	agg_param     = container_of(entry, struct ec_agg_param, ap_agg_entry);
	rc            = vos_obj_fetch(agg_param->ap_cont_handle, entry->ae_oid,
				      entry->ae_cur_stripe.as_hi_epoch, 0, &entry->ae_dkey, 1, &iod, &sgl);
	if (rc)
...
static void
agg_diff_preprocess(struct ec_agg_entry *entry, unsigned char *diff, unsigned int cell_idx)
{
 ...
 */
static int
agg_update_parity(struct ec_agg_entry *entry, uint8_t *bit_map, unsigned int cell_cnt)
{
	unsigned int   k          = ec_age2k(entry);
	unsigned int   p          = ec_age2p(entry);
	unsigned int   cell_bytes = ec_age2cs_b(entry);
	...
	for (i = 0, j = 0; i < cell_cnt; i++) {
		...
		ec_encode_data_update(cell_bytes, k, p, j, entry->ae_codec->ec_gftbls, diff,
				      parity_bufs);
	}
out:
	return rc;
 ...
 */
static void
agg_recalc_parity(struct ec_agg_entry *entry, uint8_t *bit_map, unsigned cell_cnt)
{
	unsigned int   k          = ec_age2k(entry);
	unsigned int   p          = ec_age2p(entry);
	unsigned int   cell_bytes = ec_age2cs_b(entry);
...
agg_process_partial_stripe_ult(void *arg)
{
	 ...
	 */
	rc = agg_fetch_odata_cells(entry, bit_map, cell_cnt, stripe_ud->asu_recalc);
	if (rc)
		goto out;
...
agg_process_partial_stripe(struct ec_agg_entry *entry)
{
	...
	d_list_for_each_entry(extent, &entry->ae_cur_stripe.as_dextents, ae_link) {
		...
		if (extent->ae_recx.rx_idx - ss > eend) {
			cell_cnt +=
			    agg_count_cells(fcbit_map, tbit_map, estart, elen, k, len,
					    entry->ae_cur_stripe.as_stripenum, &full_cell_cnt);
			estart = extent->ae_recx.rx_idx - ss;
			elen   = 0;
		}
		elen += extent->ae_recx.rx_nr;
	...
	if (cell_cnt)
		rc = agg_fetch_local_extents(entry, bit_map, cell_cnt, stripe_ud.asu_recalc);
	if (rc)
		goto out;
	...
	tid = dss_get_module_info()->dmi_tgt_id;
	rc  = dss_ult_create(agg_process_partial_stripe_ult, &stripe_ud, DSS_XS_IOFW, tid, 0, NULL);
	if (rc)
		goto ev_out;
	rc = ABT_eventual_wait(stripe_ud.asu_eventual, (void **)&status);
	if (rc != ABT_SUCCESS) {
...
agg_peer_update_ult(void *arg)
{
	...
	for (peer = 0; peer < p; peer++) {
		...
		if (rc) {
			D_ERROR(DF_UOID " pidx %d to peer %d, obj_req_create " DF_RC "\n",
				DP_UOID(entry->ae_oid), pidx, peer, DP_RC(rc));
			goto out;
		}
		ec_agg_in = crt_req_get(rpc);
		uuid_copy(ec_agg_in->ea_pool_uuid, agg_param->ap_pool_info.api_pool_uuid);
		uuid_copy(ec_agg_in->ea_coh_uuid, agg_param->ap_pool_info.api_poh_uuid);
		uuid_copy(ec_agg_in->ea_cont_uuid, agg_param->ap_pool_info.api_cont_uuid);
		uuid_copy(ec_agg_in->ea_coh_uuid, agg_param->ap_pool_info.api_coh_uuid);
		...
		ec_agg_in->ea_map_ver            = agg_param->ap_pool_info.api_pool->sp_map_version;
		ec_agg_in->ea_iod_csums.ca_arrays = NULL;
		ec_agg_in->ea_iod_csums.ca_count  = 0;
		iod_csums                         = NULL;
		iod.iod_nr                        = 0;
		if (stripe_ud->asu_write_par) {
			recx.rx_idx   = (ec_agg_in->ea_stripenum * len) | PARITY_INDICATOR;
			recx.rx_nr    = len;
			iod.iod_nr    = 1;
			iod.iod_recxs = &recx;
		...
		rc = dss_rpc_send(rpc);
		if (rc) {
			D_ERROR(DF_UOID " pidx %d to peer %d, dss_rpc_send " DF_RC "\n",
				DP_UOID(entry->ae_oid), pidx, peer, DP_RC(rc));
			goto out_bulk;
		}
		...
		rpc = NULL;
		if (rc) {
			D_ERROR(DF_UOID " pidx %d to peer %d, ea_status " DF_RC "\n",
				DP_UOID(entry->ae_oid), pidx, peer, DP_RC(rc));
			break;
		}
	}
...
agg_process_holes_ult(void *arg)
{
	...
	uint32_t                 p               = ec_age2p(entry);
	uint64_t                 ss              = entry->ae_cur_stripe.as_stripenum * k * len;
	...
	d_list_for_each_entry(agg_extent, &entry->ae_cur_stripe.as_dextents, ae_link) {
		...
		last_ext_end = agg_extent->ae_recx.rx_idx + agg_extent->ae_recx.rx_nr - ss;
	...
	if (ext_cnt) {
		...
		rc = dsc_obj_fetch(entry->ae_obj_hdl, entry->ae_cur_stripe.as_hi_epoch,
				   &entry->ae_dkey, 1, iod, &entry->ae_sgl, NULL, DIOF_FOR_EC_AGG,
				   NULL, NULL);
		if (rc) {
			D_ERROR("dsc_obj_fetch failed: " DF_RC "\n", DP_RC(rc));
			goto out;
		...
		csummer = ec_agg_param2csummer(agg_param);
		rc = daos_csummer_calc_iods(csummer, &entry->ae_sgl, iod, NULL, 1, false, NULL, -1,
	...
	if (ext_cnt) {
		rc = crt_bulk_create(dss_get_module_info()->dmi_ctx, &entry->ae_sgl, CRT_BULK_RW,
				     &bulk_hdl);
		if (rc) {
	...
	for (peer = 0; peer < p; peer++) {
		...
		uuid_copy(ec_rep_in->er_pool_uuid, agg_param->ap_pool_info.api_pool_uuid);
		uuid_copy(ec_rep_in->er_coh_uuid, agg_param->ap_pool_info.api_poh_uuid);
		uuid_copy(ec_rep_in->er_cont_uuid, agg_param->ap_pool_info.api_cont_uuid);
		uuid_copy(ec_rep_in->er_coh_uuid, agg_param->ap_pool_info.api_coh_uuid);
		ec_rep_in->er_oid = entry->ae_oid;
		peer_shard        = obj_ec_parity_shard(
                    obj, entry->ae_dkey_hash,
...
agg_process_holes(struct ec_agg_entry *entry)
{
	...
	D_ALLOC_ARRAY(stripe_ud.asu_recxs, entry->ae_cur_stripe.as_extent_cnt + 1);
	...
	tid = dss_get_module_info()->dmi_tgt_id;
	rc  = dss_ult_create(agg_process_holes_ult, &stripe_ud, DSS_XS_IOFW, tid, 0, NULL);
	if (rc)
		goto ev_out;
	rc = ABT_eventual_wait(stripe_ud.asu_eventual, (void **)&status);
	if (rc != ABT_SUCCESS) {
	...
	if (iod->iod_nr) {
		...
		rc = vos_obj_update(agg_param->ap_cont_handle, entry->ae_oid,
				    entry->ae_cur_stripe.as_hi_epoch, 0, 0, &entry->ae_dkey, 1, iod,
				    stripe_ud.asu_iod_csums, &entry->ae_sgl);
		if (rc) {
		...
		recx.rx_idx = (entry->ae_cur_stripe.as_stripenum * recx.rx_nr) | PARITY_INDICATOR;
		rc = vos_obj_array_remove(agg_param->ap_cont_handle, entry->ae_oid, &epoch_range,
...
agg_process_stripe(struct ec_agg_param *agg_param, struct ec_agg_entry *entry)
{
	...
	iter_param.ip_recx.rx_idx =
	    PARITY_INDICATOR | (entry->ae_cur_stripe.as_stripenum * iter_param.ip_recx.rx_nr);
	ec_age_set_no_parity(entry);
	rc = vos_iterate(&iter_param, VOS_ITER_RECX, false, &anchors, agg_recx_iter_pre_cb, NULL,
			 entry, NULL);
	D_DEBUG(DB_EPC, "Querying parity for stripe: %lu, offset: " DF_X64 ", " DF_RC "\n",
		entry->ae_cur_stripe.as_stripenum, iter_param.ip_recx.rx_idx, DP_RC(rc));
	if (rc != 0)
		goto out;
	...
	if (ec_age_with_parity(entry) && ec_age_parity_higher(entry)) {
		...
		write_parity = false;
		D_DEBUG(DB_EPC,
			"delete replica for stripe: %lu," DF_U64 "/" DF_U64 " eph " DF_X64
			" >= " DF_X64 "\n",
			entry->ae_cur_stripe.as_stripenum, iter_param.ip_recx.rx_idx,
			iter_param.ip_recx.rx_nr, entry->ae_par_extent.ape_epoch,
...
static int
agg_extent_add(struct ec_agg_entry *agg_entry, vos_iter_entry_t *entry, daos_recx_t *recx)
{
	...
	d_list_add_tail(&extent->ae_link, &agg_entry->ae_cur_stripe.as_dextents);

	if (!agg_entry->ae_cur_stripe.as_extent_cnt) {
		/* first extent in stripe: save the start offset */
		agg_entry->ae_cur_stripe.as_offset =
		    extent->ae_recx.rx_idx -
		    rounddown(extent->ae_recx.rx_idx, ec_age2ss(agg_entry));
		agg_entry->ae_cur_stripe.as_stripenum = agg_stripenum(agg_entry, recx->rx_idx);
	}
	...
	if (BIO_ADDR_IS_HOLE(&entry->ie_biov.bi_addr)) {
		...
		agg_entry->ae_cur_stripe.as_stripe_fill += agg_in_stripe(agg_entry, recx);
	}
...
static int
agg_akey_post(daos_handle_t ih, struct ec_agg_param *agg_param, vos_iter_entry_t *entry,
	      struct ec_agg_entry *agg_entry, unsigned int *acts)
{
	int rc = 0;
...
agg_shard_is_leader(struct ds_pool *pool, struct ec_agg_entry *agg_entry)
{
	...
	ec_tgt_idx =
	    obj_ec_shard_idx_by_layout_ver(agg_entry->ae_oid.id_layout_ver, agg_entry->ae_dkey_hash,
					   oca, daos_oclass_grp_size(oca) - 1);
	/**
...
static int
agg_dkey(daos_handle_t ih, vos_iter_entry_t *entry, struct ec_agg_param *agg_param,
	 struct ec_agg_entry *agg_entry, unsigned int *acts)
{
	int rc;
	...
	agg_entry->ae_grp_idx =
	    agg_entry->ae_oid.id_shard / daos_oclass_grp_size(&agg_entry->ae_oca);
	...
	if (rc == 1) {
		...
		if (rc < 0) {
			D_ERROR("oid:" DF_UOID " ds_pool_check_leader failed " DF_RC "\n",
...
static int
agg_akey(daos_handle_t ih, vos_iter_entry_t *entry, struct ec_agg_entry *agg_entry,
	 unsigned int *acts)
{
...
static int
agg_iterate_post_cb(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
		    vos_iter_param_t *param, void *cb_arg, unsigned int *acts)
{
...
agg_filter(daos_handle_t ih, vos_iter_desc_t *desc, void *cb_arg, unsigned int *acts)
{
	...
	if (rc) {
		D_ERROR("Skip object(" DF_OID ") with unknown class(%u)\n",
			DP_OID(desc->id_oid.id_pub), daos_obj_id2class(desc->id_oid.id_pub));
		*acts = VOS_ITER_CB_SKIP;
...
check:
	if (desc->id_agg_write <= agg_param->ap_filter_eph) {
		if (desc->id_type == VOS_ITER_OBJ)
			D_DEBUG(
			    DB_EPC, "Skip oid:" DF_UOID " agg_epoch=" DF_X64 " filter=" DF_X64 "\n",
			    DP_UOID(desc->id_oid), desc->id_agg_write, agg_param->ap_filter_eph);
		else
			D_DEBUG(
			    DB_EPC, "Skip key:" DF_KEY " agg_epoch=" DF_X64 " filter=" DF_X64 "\n",
			    DP_KEY(&desc->id_key), desc->id_agg_write, agg_param->ap_filter_eph);
		agg_param->ap_credits++;
		*acts = VOS_ITER_CB_SKIP;
		goto done;
	}
 ...
 */
static int
agg_iterate_pre_cb(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
		   vos_iter_param_t *param, void *cb_arg, unsigned int *acts)
{
	struct ec_agg_param *agg_param = (struct ec_agg_param *)cb_arg;
	struct ec_agg_entry *agg_entry = &agg_param->ap_agg_entry;
...
ec_agg_init_ult(void *arg)
{
	...
	rc = ds_cont_ec_eph_insert(agg_param->ap_pool_info.api_pool,
				   agg_param->ap_pool_info.api_cont_uuid, ult_arg->tgt_idx,
				   &ult_arg->ec_query_p);
	if (rc)
		D_GOTO(out, rc);
...
ec_agg_param_init(struct ds_cont_child *cont, struct agg_param *param)
{
	...
	rc = dsc_pool_open(info->api_pool_uuid, info->api_poh_uuid, DAOS_PC_RW, NULL,
			   info->api_pool->sp_map, info->api_svc_list, &info->api_pool_hdl);
	if (rc) {
		D_ERROR("dsc_pool_open failed: " DF_RC "\n", DP_RC(rc));
		D_GOTO(out, rc);
	}

	rc = dsc_cont_open(info->api_pool_hdl, info->api_cont_uuid, info->api_coh_uuid, DAOS_COO_RW,
			   &info->api_cont_hdl);
	if (rc) {
 ...
 */
static int
cont_ec_aggregate_cb(struct ds_cont_child *cont, daos_epoch_range_t *epr, uint32_t flags,
		     struct agg_param *agg_param)
{
