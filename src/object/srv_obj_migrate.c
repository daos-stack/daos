			  ...
			  struct ds_cont_child *ds_cont)
{
	 ...
	 * multiple epoch, so it can only do fetch/update recx each time.
	 */
	for (i = 0; i < mrone->mo_iod_num; i++) {
		daos_iod_t	iod;
		int		j;
...
rw_iod_pack(struct migrate_one *mrone, daos_iod_t *iod, daos_epoch_t *ephs, d_sg_list_t *sgl)
{
	...
	if (iod->iod_type == DAOS_IOD_SINGLE) {
		...
		if (daos_oclass_is_ec(&mrone->mo_oca))
			boundary = obj_ec_stripe_rec_nr(&mrone->mo_oca);
...
static int
migrate_one_insert_recx(struct migrate_one *mrone, daos_iod_t *iod, daos_epoch_t *recx_ephs,
			daos_epoch_t punch_eph, d_sg_list_t *sgl)
{
	int i;
