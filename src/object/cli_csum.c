		   ...
		   struct dcs_iod_csums **iod_csums)
{
	...
	D_DEBUG(DB_CSUM,
		DF_C_OID_DKEY " UPDATE - csummer: %p, "
	...
	rc = daos_csummer_calc_iods(csummer_copy, sgls, iods, NULL, iod_nr, false, layout, -1,
				    iod_csums);
	if (rc != 0) {
		  ...
		  struct dcs_csum_info **dkey_csum, struct dcs_iod_csums **iod_csums)
{
	...
	rc = daos_csummer_calc_iods(csummer_copy, sgls, iods, NULL, iod_nr, true, layout, -1,
				    iod_csums);
	if (rc != 0) {
		D_ERROR("daos_csummer_calc_iods error: " DF_RC "\n", DP_RC(rc));
		daos_csummer_free_ci(csummer_copy, dkey_csum);
		      ...
		      struct obj_reasb_req *reasb_req)
{
	...
	for (i = 0; i < iod_nr; i++) {
		 ...
		 */
		if (obj_ec_singv_one_tgt(iod->iod_size, sgl, reasb_req->orr_oca)) {
...
static int
iod_sgl_copy(daos_iod_t *iod, d_sg_list_t *sgl, daos_iod_t *cp_iod, d_sg_list_t *cp_sgl,
	     struct obj_shard_iod *siod, uint64_t off)
{
 ...
 */
static void
store_csum(d_iov_t *csum_iov, const struct dcs_iod_csums *iod_csum)
{
	int c, rc;
...
dc_rw_cb_csum_verify(struct dc_csum_veriry_args *args)
{
	...
	D_ASSERTF(args->maps_nr == args->iod_nr, "maps_nr(%lu) == iod_nr(%d)", args->maps_nr,
		  args->iod_nr);
...
#define IOV_INLINE 8
		...
		singv_lo =
		    (singv_los == NULL || iod->iod_type == DAOS_IOD_ARRAY) ? NULL : &singv_los[i];
		if (singv_lo != NULL) {
		...
		if (rc != 0) {
			if (iod->iod_type == DAOS_IOD_SINGLE) {
				D_ERROR("Data Verification failed (object: " DF_C_UOID_DKEY
					" shard %d): " DF_RC "\n",
					DP_C_UOID_DKEY(args->oid, args->dkey), args->shard_idx,
					DP_RC(rc));
			} else if (iod->iod_type == DAOS_IOD_ARRAY) {
				D_ERROR("Data Verification failed (object: " DF_C_UOID_DKEY
					" shard %d, extent: " DF_RECX "): " DF_RC "\n",
					DP_C_UOID_DKEY(args->oid, args->dkey), args->shard_idx,
					DP_RECX(iod->iod_recxs[0]), DP_RC(rc));
			}
