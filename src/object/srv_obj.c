...
ds_tx_commit(void *arg)
{
	...
	if (!dta->dta_share) {
		if (rc > 0)
			...
			vos_dtx_aggregate(cont->sc_hdl, 0,
					  DTX_AGGREGATION_THRESHOLD_TIME + 30);
...
ds_tx_aggregate(void *arg)
{
	...
	vos_dtx_aggregate(dta->dta_cont->sc_hdl, 0,
			  DTX_AGGREGATION_THRESHOLD_TIME + 30);
		   ...
		   struct daos_tx_id *dti_cos)
{
	if (!daos_handle_is_inval(ioh)) {
		...
		rc = update ? vos_update_end(ioh, cont_hdl->sch_uuid,
				map_version, &orwi->orw_dkey, dth, status,
...
ds_obj_rw_handler(crt_rpc_t *rpc)
{
	...
	D_DEBUG(DB_TRACE, "rpc %p opc %d "DF_UOID" dkey %d %s tag %d eph "
		...
		(int)orw->orw_dkey.iov_len, (char *)orw->orw_dkey.iov_buf,
		tag, orw->orw_epoch, orw->orw_map_ver, map_ver,
		DP_DTI(&orw->orw_dti));
			...
			struct daos_tx_id *dti_cos)
{
	...
	case DAOS_OBJ_RPC_PUNCH:
		rc = vos_obj_punch(cont->sc_hdl, opi->opi_oid,
				   ...
				   dth, dti_cos_count, dti_cos);
		break;
	case DAOS_OBJ_RPC_PUNCH_DKEYS:
	case DAOS_OBJ_RPC_PUNCH_AKEYS: {
		daos_key_t *dkey;

		/* XXX: Currently, we only support punch single dkey per RPC. */
		D_ASSERT(opi->opi_dkeys.ca_count == 1);

		dkey = &((daos_key_t *)opi->opi_dkeys.ca_arrays)[0];
		rc = vos_obj_punch(cont->sc_hdl, opi->opi_oid, opi->opi_epoch,
				   cont_hdl->sch_uuid, opi->opi_map_ver, 0,
