...
again:
	...
	/* For EC object modification, ask DTX to 'sync' commit. */
	if (orw->orw_flags & ORF_DTX_SYNC ||
	    daos_oclass_is_ec(orw->orw_oid.id_pub, NULL))
		sync = true;
	else
		sync = false;
...
again:
	...
	if (opi->opi_flags & ORF_DTX_SYNC ||
	    daos_oclass_is_ec(opi->opi_oid.id_pub, NULL))
		sync = true;
	else
		sync = false;
