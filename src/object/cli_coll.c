		  ...
		  uint32_t map_ver, uint32_t idx)
{
	...
	if (coa->coa_for_modify) {
		...
		dct->dct_tgt_nr++;
		if (dcs->dcs_nr > 1)
			D_ERROR("AAAAA: Object "DF_OID" shard %u and %u reside on the target "
				"target %u/%u, total shared %u/%u\n", DP_OID(obj->cob_md.omd_id),
				dcs->dcs_buf[0], dcs->dcs_buf[dcs->dcs_nr - 1], dct->dct_rank,
				shard->do_target_idx, dcs->dcs_nr, obj->cob_shards_nr);
	}
