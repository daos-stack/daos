	      ...
	      uint32_t *grp_nr)
{
	...
	if (*shard_nr >= (obj->cob_max_rank - obj->cob_min_rank + 1) << 1)
		return true;
	...
	if (auxi->io_retry) {
		...
		if (rc == 0) {
			if (!shard->do_rebuilding && !shard->do_reintegrating) {
				leader = shard->do_target_rank - obj->cob_min_rank;
				goto gen_mbs;
			}
...
gen_mbs:
	if (leader != 0) {
		if (unlikely(leader >= coa->coa_dct_nr)) {
			D_WARN(DF_OID" layout changed during collective leader election\n",
