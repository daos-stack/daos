...
timing_obj_csum_update(void **state)
{
	...
	MEASURE_TIME(dc_obj_csum_update(st->csummer, props, oid, &st->td.dkey, st->td.td_iods,
					st->td.td_sgls, 1, NULL, &dkey_csum, &iod_csums),
		     noop(), daos_csummer_free_ci(st->csummer, &dkey_csum);
...
timing_obj_csum_fetch(void **state)
{
	...
	MEASURE_TIME(dc_obj_csum_fetch(st->csummer, &st->td.dkey, st->td.td_iods, st->td.td_sgls, 1,
				       NULL, &dkey_csum, &iod_csums),
		     noop(), daos_csummer_free_ci(st->csummer, &dkey_csum);
		     daos_csummer_free_ic(st->csummer, &iod_csums););
...
timiing_obj_csums_verify(void **state)
{
	 ...
	 */
	assert_success(daos_csummer_calc_iods(st->csummer, st->td.td_sgls, st->td.td_iods, NULL,
					      st->td.td_iods_nr, false, NULL, 0, &args.iods_csums));
...
cct_setup(void **state)
{
	...
	td_init(
	    &st->td, 10,
	    (struct td_init_args){.ca_iod_types = {DAOS_IOD_ARRAY, DAOS_IOD_SINGLE, DAOS_IOD_ARRAY,
						   DAOS_IOD_SINGLE, DAOS_IOD_ARRAY, DAOS_IOD_SINGLE,
						   DAOS_IOD_ARRAY, DAOS_IOD_SINGLE, DAOS_IOD_ARRAY,
						   DAOS_IOD_SINGLE},
