		    ...
		    struct dtx_coll_entry **p_dce)
{
	...
	/* For non-leader, the rank range should has already been appened after the bitmap. */
	if (!(ocpi->ocpi_flags & ORF_LEADER)) {
		...
		/* From new client. */
		if (ocpi->ocpi_flags & ORF_COLL_SORTED_TGT) {
			if (dct_nr > 2) {
				if (min_rank > dcts[2].dct_rank)
					min_rank = dcts[2].dct_rank;
				if (max_rank < dcts[dct_nr - 1].dct_rank)
					max_rank = dcts[dct_nr - 1].dct_rank;
			}
		} else {
			for (i = 2; i < dct_nr; i++) {
	...
	ranks = (uint32_t *)((void *)ddt + size);
	ranks[0] = dce->dce_min_rank;
	ranks[1] = dce->dce_max_rank;

	mbs->dm_data_size = size + sizeof(uint32_t) * 2;
