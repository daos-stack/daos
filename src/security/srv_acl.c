			  ...
			  d_iov_t *pop_payload)
{
	...
	/* Short-circuit before the dRPC for the common rejection case. */
	if (node_cert == NULL || node_cert->iov_len == 0)
		return -DER_NO_NODE_CERT;
	...
	rc = drpc_call_create(server_socket, DRPC_MODULE_SEC, DRPC_METHOD_SEC_VALIDATE_NODE_CERT,
			      &request);
	if (rc != DER_SUCCESS) {
		drpc_close(server_socket);
		return rc;
	...
	req.pop_sig.data   = pop_sig->iov_buf;
	req.pop_sig.len    = pop_sig->iov_len;

	req.pop_payload.data = pop_payload->iov_buf;
	req.pop_payload.len  = pop_payload->iov_len;
	if (machine != NULL)
	...
	resp = auth__validate_node_cert_resp__unpack(&alloc.alloc, response->body.len,
						     response->body.data);
