...
{
	assert_int_equal(dc_security_request_creds(NULL), -DER_INVAL);
}
...
test_request_credentials_succeeds_with_good_values(void **state)
{
	...
	assert_int_equal(dc_security_request_creds(&creds), DER_SUCCESS);
...
test_request_credentials_fails_if_drpc_connect_fails(void **state)
{
	...
	assert_int_equal(dc_security_request_creds(&creds), -DER_BADPATH);
...
test_request_credentials_connects_to_default_socket(void **state)
{
	...
	dc_security_request_creds(&creds);
...
test_request_credentials_fails_if_drpc_call_fails(void **state)
{
	...
	assert_int_equal(dc_security_request_creds(&creds),
			drpc_call_return);
...
test_request_credentials_fails_if_reply_null(void **state)
{
	...
	assert_int_equal(dc_security_request_creds(&creds), -DER_NOREPLY);
...
test_request_credentials_fails_if_reply_status_failure(void **state)
{
	...
	assert_int_equal(dc_security_request_creds(&creds), -DER_MISC);
