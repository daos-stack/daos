			  ...
			  d_iov_t *node_cert_pop, d_iov_t *node_cert_payload)
{
	...
	rc = process_credential_response(response, creds, node_cert, node_cert_pop,
					 node_cert_payload);
			    ...
			    d_iov_t *node_cert_pop, d_iov_t *node_cert_payload)
{
	...
	return get_cred_from_response(response, creds, node_cert, node_cert_pop, node_cert_payload);
}
		       ...
		       d_iov_t *node_cert_pop, d_iov_t *node_cert_payload)
{
	...
	if (node_cert_pop != NULL && cred_resp->node_cert_pop.len > 0) {
		...
		if (node_cert_pop->iov_buf == NULL)
			D_GOTO(out_partial, rc = -DER_NOMEM);
		memcpy(node_cert_pop->iov_buf, cred_resp->node_cert_pop.data,
	...
	if (node_cert_payload != NULL && cred_resp->node_cert_payload.len > 0) {
		D_ALLOC(node_cert_payload->iov_buf, cred_resp->node_cert_payload.len);
		if (node_cert_payload->iov_buf == NULL)
			D_GOTO(out_partial, rc = -DER_NOMEM);
		memcpy(node_cert_payload->iov_buf, cred_resp->node_cert_payload.data,
...
out_partial:
	...
	if (node_cert_pop != NULL)
		daos_iov_free(node_cert_pop);
	if (node_cert_payload != NULL)
