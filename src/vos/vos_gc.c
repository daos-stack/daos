	     ...
	     struct vos_gc_item *item, int *credits, bool *empty)
{
	...
	if (key->kr_bmap & KREC_BF_BTR) {
		...
		// D_ASSERT(gc->gc_type == GC_AKEY); /*Liang's fix for flat kv aggregation*/
	...
	D_DEBUG(DB_IO, "drain rc %d, gc type %d", rc, (int) gc->gc_type);
...
/* flat key*/
static int
gc_free_dkey(struct vos_gc *gc, struct vos_pool *pool, umem_off_t addr) //struct vos_gc_item *item)
