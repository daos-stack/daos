...
enum {
	...
	GC_CREDS_PRIV	= 128,	/**< credits for internal usage */
	GC_CREDS_MAX	= 1024,	/**< maximum credits for vos_gc_run */
};

/**
 * Default garbage bag size
 * - header of vos_gc_bag_df is 64 bytes
 * - PMDK allocation overhead is 16 bytes,
 * - each item consumes 16 bytes
 * - reserve 16 bytes for future use
 *
 * So a 4K GC bag can contains up to 250 GC items.
 */
	     ...
	     struct btr_root *root, int *credits, bool *empty)
{
	...
	D_DEBUG(DB_IO, "drain btree for %s, creds=%d\n", gc->gc_name, *credits);
	...
	D_DEBUG(DB_IO, "empty=%d, remainded creds=%d\n", *empty, *credits);
	return 0;
 failed:
	     ...
	     struct evt_root *root, int *credits, bool *empty)
{
	...
	D_DEBUG(DB_IO, "drain %s evtree, creds=%d\n", gc->gc_name, *credits);
	    ...
	    uint64_t args)
{
	...
	D_DEBUG(DB_IO, "Add %s addr="DF_X64"\n", gc_type2name(type), item_off);
...
gc_reclaim_pool(struct vos_pool *pool, int *credits, bool *empty_ret)
{
	...
	*empty_ret = false;
	while (creds > 0) {
		...
		D_DEBUG(DB_IO, "GC=%s credits=%d/%d\n", gc->gc_name,
			creds, *credits);

		item = gc_get_item(gc, pool);
		if (item == NULL) {
			if (gc->gc_type == GC_CONT) { /* top level GC */
				D_DEBUG(DB_IO, "Nothing to reclaim\n");
				*empty_ret = true;
				break;
			}
			D_DEBUG(DB_IO, "GC=%s is empty\n", gc->gc_name);
		...
		D_DEBUG(DB_IO, "GC=%s credits=%d empty=%d\n",
	...
	D_DEBUG(DB_IO, "credits origin=%d, current=%d, rc=%s\n",
		*credits, creds, d_errstr(rc));
...
gc_init_pool(struct umem_instance *umm, struct vos_pool_df *pd)
{
	...
	for (i = 0; i < GC_MAX; i++) {
		...
		/* allocate the first bag for each bin */
		bag = gc_bin_find_bag(umm, bin);
		if (!bag)
			return -DER_NOSPACE;
	}
...
gc_add_pool(struct vos_pool *pool)
{
	...
	D_DEBUG(DB_IO, "Register pool="DF_UUID" for GC\n",
		DP_UUID(pool->vp_id));
...
vos_gc_run(int *credits)
{
	...
	while (!d_list_empty(pools)) {
		...
		D_DEBUG(DB_IO, "GC pool="DF_UUID", creds=%d\n",
		...
		checked++;
		if (empty) {
			D_DEBUG(DB_IO, "Deregister pool="DF_UUID", empty=%d\n",
				DP_UUID(pool->vp_id), empty);
			gc_log_pool(pool);
			gc_del_pool(pool);

		} else {
			D_DEBUG(DB_IO, "Re-add pool="DF_UUID", opened=%d\n",
	...
	D_DEBUG(DB_IO, "checked %d pools, consumed %d/%d credits\n",
