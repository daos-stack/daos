...
vos_dtx_add_ptr(struct vos_pool *pool, void *ptr, size_t size)
{
	...
	if (unlikely(rc == -DER_NOSPACE)) {
		...
		if (ext_df != NULL && !UMOFF_IS_NULL(ext_df->ped_emerg_buf) &&
			...
			rc = umem_tx_set_snapbuf(umm, ext_df->ped_emerg_buf,
						 VOS_SNAPBUF_EMERG_SIZE);
			if (rc == 0)
				rc = umem_tx_add_ptr(umm, ptr, size);
			else
				rc = -DER_NOSPACE;
...
vos_dtx_reuse_cmt_blob(struct vos_container *cont)
{
	...
	if (unlikely(cont_df->cd_dtx_committed_head == cont_df->cd_dtx_committed_tail)) {
		D_ERROR("No space when extending commit DTX table.\n");
		return -DER_NOSPACE;
	}
	...
	for (i = 0, epoch = 0, count = 0; i < head->dbd_count; i++) {
		...
		if (rc == -DER_NONEXIST)
			rc = 0;
		if (unlikely(rc != 0)) {
			D_ERROR("Failed to remove DTX entry " DF_DTI " for urgent DTX aggregation "
				UMOFF_PF ": " DF_RC "\n",
...
out:
	DL_CDEBUG(rc == 0, DLOG_WARN, DLOG_ERR, rc,
		  "Reused (old) DTX blob %p (" UMOFF_PF ") under space pressure for " DF_UUID "/"
		  DF_UUID, dbd, UMOFF_P(dbd_off), DP_UUID(cont->vc_pool->vp_id),
		  DP_UUID(cont->vc_id));
	return rc;
}
