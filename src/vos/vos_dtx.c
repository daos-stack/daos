		    ...
		    umem_id_t ummid, bool abort)
{
	...
	if (UMMID_IS_NULL(obj->vo_dtx)) {
		if (!abort) { /* Current DTX is also committed. */
	...
	/* I am the DTX that is committed firstly. */
	if (!abort) {
		    ...
		    struct vos_dtx_record_df *rec, umem_id_t ummid, bool abort)
{
	...
	/* Other DTX that shares the key has committed firsly,
	 * it must be for sharing of update cases.
	 */
...
exchange:
	...
	if (!abort) { /* commit case */
		...
		/* Exchange the sub-tree between max epoch record and the give
		 * epoch record. The record with max epoch will be removed
		 * when aggregation.
		 */
...
vos_dtx_end(struct daos_tx_handle *dth, int result, bool leader)
{
	...
	if (leader) {
		if (dth->dth_sync)
			D_GOTO(out, rc = DTX_ACT_COMMIT_SYNC);

		if (cont->vc_dtx_committable_count > DTX_THRESHOLD_COUNT)
			D_GOTO(out, rc = DTX_ACT_COMMIT_ASYNC);
	...
	if (tab->tt_count > DTX_AGGREGATION_THRESHOLD_COUNT)
		D_GOTO(out, rc = DTX_ACT_AGGREGATE);
