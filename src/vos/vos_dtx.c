...
static inline bool
dtx_is_aborted(uint16_t tx_lid)
{
	return tx_lid == DTX_LID_ABORTED;
}

static void
dtx_set_aborted(uint16_t *tx_lid)
{
...
static bool
vos_dtx_is_normal_entry(uint16_t entry)
{
...
vos_dtx_alloc(struct umem_instance *umm, struct dtx_handle *dth)
{
	...
	dae = lrua_allocx(cont->vc_dtx_array, &idx, dth->dth_epoch, false);
	if (dae == NULL) {
		/** Probably need to commit some entries here */
		return -DER_NOMEM;
	}
...
vos_dtx_check_availability(struct umem_instance *umm, daos_handle_t coh,
			   uint16_t entry, daos_epoch_t epoch, uint32_t intent,
			   uint32_t type)
{
...
vos_dtx_deregister_record(struct umem_instance *umm, daos_handle_t coh,
			  uint16_t entry, daos_epoch_t epoch, umem_off_t record)
{
	struct vos_container		*cont;
	struct vos_dtx_act_ent		*dae;
	struct vos_dtx_act_ent_df	*dae_df;
...
vos_dtx_act_reindex(struct vos_container *cont)
{
	...
	while (1) {
		...
		for (i = 0; i < dbd->dbd_index; i++) {
			...
			D_ASSERT(dae_df->dae_lid >= DTX_LID_RESERVED);
