...
out:
	DL_CDEBUG(rc == 0 || rc == -DER_ALREADY, DB_IO, DLOG_ERR, rc,
...
vos_dtx_prepared(struct dtx_handle *dth, struct vos_dtx_cmt_ent **dce_p)
{
	...
	if (!dth->dth_active) {
		...
		D_ASSERTF(!vos_dtx_hit_inprogress(false),
			  "Active DTX "DF_DTI" with pending issue\n",
			  DP_DTI(&dth->dth_xid));
...
out:
	DL_CDEBUG(rc == 0, DB_IO, DLOG_ERR, rc,
		  "Preparinng DTX "DF_DTI": lid=%d",
		  DP_DTI(&dth->dth_xid), DAE_LID(dae));
...
vos_dtx_pack_mbs(struct umem_instance *umm, struct vos_dtx_act_ent *dae)
{
	...
	if (dth != NULL) {
		...
		D_ASSERTF(!UMOFF_IS_NULL(DAE_MBS_OFF(dae)),
			  "Invalid DTX entry "DF_DTI", mbs size %u\n",
			  DP_DTI(&DAE_XID(dae)), tmp->dm_data_size);
