...
def vos_test_data(request):
    class VosTestData:
        ...
        def create_akeys(self):
            akey1 = self.create_default_akey(key="A-key 1", value_type="single_value")
            akey2 = self.create_default_akey(key="A-key 2", value_type="array")

            return [akey1, akey2]

        def create_default_dkey(self, key="D-key 1", count=1, key_type="hashed", overhead="user"):
            akeys = self.create_akeys()
            dkey = {"count": count, "type": key_type, "overhead": overhead, "akeys": akeys}
        ...
        def create_default_object(self, key="Object 1", count=1):
            ...
            vos_object = {"targets": 0, "count": count, "dkeys": [dkey1, dkey2]}

            return vos_object

        def create_default_container(self, count=1, csum_size=0, csum_gran=16384):
            ...
            vos_container = {
                ...
                "csum_size": csum_size,
                "csum_gran": csum_gran,
                "objects": [self.raw_obj1, self.raw_obj2],
            }
        ...
        def _create_sb_akey(self, key, size):
            ...
            akey = AKey(key=key, overhead=Overhead.META, value_type=ValType.SINGLE)
        ...
        def get_mock_dfs_superblock_obj(self):
            ...
            dkey_sb.add_value(self._create_sb_akey(key="DFS_SB_VERSION", size=2))
            dkey_sb.add_value(self._create_sb_akey(key="DFS_LAYOUT_VERSION", size=2))
            dkey_sb.add_value(self._create_sb_akey(key="DFS_CHUNK_SIZE", size=8))
            dkey_sb.add_value(self._create_sb_akey(key="DFS_OBJ_CLASS", size=2))

            inode_value = VosValue(size=64)
            akey_inode = AKey(key="DFS_INODE", overhead=Overhead.META, value_type=ValType.ARRAY)
...
class AKeyTestCase(unittest.TestCase):
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(TypeError) as err:
            _ = AKey(value_type="rubbish")
        assert "value_type parameter must be of type" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = AKey(value_type="single_value", count="rubbish")
        assert "count parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = AKey(value_type="single_value", key_type="rubbish")
        assert "key_type parameter must be of type" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = AKey(value_type="single_value", overhead="rubbish")
        assert "overhead parameter must be of type" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = AKey(value_type="single_value", values=["rubbish"])
    ...
    def test_constructor(self):
        ...
        akey = AKey(key="A-key 1", key_type="hashed", value_type="single_value", values=values)
        want = self.test_data.create_default_akey(
            key="A-key 1", key_type="hashed", value_type="single_value"
        )
        assert want == akey.dump()  # nosec

        akey = AKey(key_type="integer", value_type="single_value", values=values)
        want = self.test_data.create_default_akey(key_type="integer", value_type="single_value")
        assert want == akey.dump()  # nosec

        akey = AKey(key_type="integer", value_type="single_value", count=20, values=values)
        want = self.test_data.create_default_akey(
            key_type="integer", value_type="single_value", count=20
        )
        assert want == akey.dump()  # nosec

        akey = AKey(key_type="integer", value_type="single_value", overhead="user", values=values)
        want = self.test_data.create_default_akey(
            key_type="integer", value_type="single_value", overhead="user"
        )
        assert want == akey.dump()  # nosec

        akey = AKey(key_type="integer", value_type="single_value", overhead="meta", values=values)
        want = self.test_data.create_default_akey(
            key_type="integer", value_type="single_value", overhead="meta"
        )
        assert want == akey.dump()  # nosec

        akey = AKey(key="A-key 1", value_type="array", overhead="user", values=values)
        want = self.test_data.create_default_akey(
            key="A-key 1", value_type="array", overhead="user"
        )
        assert want == akey.dump()  # nosec

        akey = AKey(key="A-key 1", value_type="array", overhead="meta", values=values)
        want = self.test_data.create_default_akey(
        ...
        want = self.test_data.create_default_akey(key="A-key 1", key_type=None, value_type="array")
        assert want == akey.dump()  # nosec
...
class DKeyTestCase(unittest.TestCase):
    ...
    def test_invalid_parameters(self):
        with pytest.raises(TypeError) as err:
            _ = DKey(count="rubbish")
        assert "count parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = DKey(key_type="rubbish")
        assert "key_type parameter must be of type" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = DKey(overhead="rubbish")
        assert "overhead parameter must be of type" in str(err.value)  # nosec
    ...
    def test_constructor(self):
        akey1 = AKey(
            key="A-key 1", value_type="single_value", values=self.test_data.create_values()
        )
        akey2 = AKey(key="A-key 2", value_type="array", values=self.test_data.create_values())
    ...
    def test_add_value(self):
        ...
        dkey = DKey(key="D-key 1")
        akey = AKey(key="A-key 1", value_type="single_value", values=self.test_data.create_values())
        dkey.add_value(akey)
        akey = AKey(key="A-key 2", value_type="array", values=self.test_data.create_values())
        dkey.add_value(akey)
...
class ObjectTestCase(unittest.TestCase):
    ...
    def test_invalid_parameters(self):
        with pytest.raises(TypeError) as err:
            _ = VosObject(count="rubbish")
        assert "count parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = VosObject(count=10, dkeys=["rubbish"])
        assert "must be of type" in str(err.value)  # nosec
...
class ContainerTestCase(unittest.TestCase):
    ...
    def test_invalid_parameters(self):
        with pytest.raises(TypeError) as err:
            _ = Container(count="rubbish")
        assert "count parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = Container(csum_size="rubbish")
        assert "csum_size parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = Container(csum_gran="rubbish")
        assert "csum_gran parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = Container(objects=["rubbish"])
        assert "must be of type" in str(err.value)  # nosec

    @pytest.mark.ut
    def test_constructor(self):
        container = Container(objects=[self.vos_object1, self.vos_object2])
        want = self.test_data.create_default_container()
        assert want == container.dump()  # nosec

        container = Container(
            count=300, csum_size=400, csum_gran=500, objects=[self.vos_object1, self.vos_object2]
        )
        want = self.test_data.create_default_container(count=300, csum_size=400, csum_gran=500)
        assert want == container.dump()  # nosec
...
class ContainersTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.vos_container1 = Container(csum_gran=300, objects=[self.vos_object1, self.vos_object2])
        self.vos_container2 = Container(csum_gran=400, objects=[self.vos_object1, self.vos_object2])
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(TypeError) as err:
            _ = Containers(num_shards="rubbish")
        assert "num_shards parameter must be of type int" in str(err.value)  # nosec

        with pytest.raises(TypeError) as err:
            _ = Containers(containers=["rubbish"])
        assert "must be of type" in str(err.value)  # nosec

    @pytest.mark.ut
    def test_constructor(self):
        containers = Containers(
            num_shards=200, containers=[self.vos_container1, self.vos_container2]
        )
        ...
        want = {"num_shards": 200, "containers": [raw_container1, raw_container2]}
    ...
    def test_add_value(self):
        ...
        raw_container1 = self.test_data.create_default_container(csum_gran=300)
        raw_container2 = self.test_data.create_default_container(csum_gran=400)
        want = {"num_shards": 500, "containers": [raw_container1, raw_container2]}
        assert want == containers.dump()  # nosec
...
class FSTestCase(unittest.TestCase):
    def setUp(self):
        test_files = [
            {"type": "file", "path": "data/deploy/driver.bin", "size": 5767168},
            {
                "type": "symlink",
                "path": "data/deploy/my_file",
                "dest": "../../specs/very_importan_file.txt",
            },
            {"type": "file", "path": "data/secret_plan.txt", "size": 3670016},
            {"type": "file", "path": "specs/readme.txt", "size": 1572864},
            {"type": "file", "path": "specs/very_importan_file.txt", "size": 2621440},
        ]
