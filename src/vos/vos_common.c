...
cancel:
	if (dtx_is_valid_handle(dth_in)) {
		if (unlikely(dth_in->dth_aborted)) {
			/* Aborted by race during the yield for local TX commit. */
			rc = vos_dtx_validation(dth_in);
			switch (rc) {
			case DTX_ST_INITED:
