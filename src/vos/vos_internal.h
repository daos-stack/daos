...
struct csum_recalc {
	struct vos_agg_phy_ent	*cr_phy_ent;
...
static inline void
vos_csum_add_recalcs(struct csum_recalc **recalcs_p,
		 ...
		 struct bio_sglist *bsgl, unsigned int idx)
{
	...
	recalcs[idx].cr_phy_ent         = phy_ent;
...
vos_csum_prepare_buf(struct vos_agg_lgc_seg *segs, unsigned int seg_cnt,
		 void **csum_buf, unsigned int buf_len, unsigned int add_len);
