...
struct vos_agg_param {
	...
	uint32_t               ap_flags;
	unsigned int          ap_discard : 1, ap_csum_err : 1, ap_nospc_err : 1, ap_discard_obj : 1;
...
static int
agg_del_sv(daos_handle_t ih, struct vos_agg_param *agg_param, vos_iter_entry_t *entry,
	   unsigned int *acts)
{
...
static int
vos_agg_obj(daos_handle_t ih, vos_iter_entry_t *entry, struct vos_agg_param *agg_param,
	    unsigned int *acts)
{
...
static int
vos_agg_dkey(daos_handle_t ih, vos_iter_entry_t *entry, struct vos_agg_param *agg_param,
	     unsigned int *acts)
{
...
static int
vos_agg_akey(daos_handle_t ih, vos_iter_entry_t *entry, struct vos_agg_param *agg_param,
	     unsigned int *acts)
{
...
static int
vos_agg_sv(daos_handle_t ih, vos_iter_entry_t *entry, struct vos_agg_param *agg_param,
	   unsigned int *acts)
{
	int rc;
	 ...
	 */
	if (agg_param->ap_max_epoch == 0 || agg_param->ap_max_epoch == entry->ie_epoch) {
		...
		case DTX_ST_PREPARED:
			 ...
			 */
			D_DEBUG(DB_EPC, "Hit uncommitted single value at epoch:" DF_X64 "\n",
				entry->ie_epoch);
			return -DER_TX_BUSY;
		case DTX_ST_ABORTED:
			/*
	...
	D_ASSERTF(entry->ie_epoch < agg_param->ap_max_epoch, "max:" DF_U64 ", cur:" DF_U64 "\n",
		  agg_param->ap_max_epoch, entry->ie_epoch);

	delete : rc = agg_del_sv(ih, agg_param, entry, acts);
	if (rc) {
...
static unsigned int
csum_prepare_ent(struct evt_entry_in *ent_in, unsigned int cs_type, unsigned int cs_len,
		 unsigned int chunksize)
{
	unsigned int cur_cnt = csum_chunk_count(chunksize, ent_in->ei_rect.rc_ex.ex_lo,
						ent_in->ei_rect.rc_ex.ex_hi, ent_in->ei_inob);
 ...
 */
static int
csum_prepare_buf(struct agg_lgc_seg *segs, unsigned int seg_cnt, void **csum_bufp,
		 unsigned int cur_len, unsigned int new_len)
{
		   ...
		   daos_epoch_t epoch)
{
	...
	d_list_for_each_entry(rm_ent, &mw->mw_rmv_ents, re_link) {
		struct evt_rect *rm_rect = &rm_ent->re_rect;

		if (rm_rect->rc_epc == epoch && rm_rect->rc_ex.ex_lo <= phy_ext->ex_hi &&
...
process_physical:
	...
	d_list_for_each_entry_safe(phy_ent, temp, &mw->mw_phy_ents, pe_link) {
		 ...
		 */
		if (ext.ex_hi <= mw->mw_ext.ex_hi || phy_ent->pe_ref == 0 || phy_ent->pe_remove)
			continue;
	...
	if (mw->mw_csum_type && io->ic_seg_cnt > 0) {
		...
		for (i = 0; i < io->ic_seg_cnt; i++) {
			...
			if (ent_in->ei_inob != 0)
				...
				cs_total += csum_prepare_ent(ent_in, cs_type, cs_len, chunksize);
		}
		/* Reallocates csum buffer. */
		if (cs_total) {
			rc = csum_prepare_buf(io->ic_segs, io->ic_seg_cnt, &io->ic_csum_buf,
...
static int
reserve_segment(struct vos_object *obj, struct agg_io_context *io, daos_size_t size,
		bio_addr_t *addr)
{
	uint64_t off, now;
	uint16_t media;
	...
	media =
	    vos_policy_media_select(vos_obj2pool(obj), DAOS_IOD_ARRAY, size, VOS_IOS_AGGREGATION);
...
static void
csum_widen_biov(struct bio_iov *biov, struct agg_phy_ent *phy_ent, struct evt_extent *ext,
		uint32_t rsize, daos_off_t phy_lo)
{
	...
	bio_iov_set_extra(biov, (ent.en_sel_ext.ex_lo - aligned_extent.ex_lo) * rsize,
			  (aligned_extent.ex_hi - ent.en_sel_ext.ex_hi) * rsize);
}
...
static int
verify_and_recalc(struct bio_sglist *bsgl, struct evt_entry_in *ent_in, struct csum_recalc *recalcs,
		  unsigned int recalc_seg_cnt)
{
...
static int
fill_one_segment(daos_handle_t ih, struct agg_merge_window *mw, struct agg_lgc_seg *lgc_seg,
		 unsigned int *acts)
{
	...
	if (mw->mw_csum_type && seg_count > io->ic_csum_recalc_cnt) {
		...
		D_REALLOC_ARRAY(buffer, io->ic_csum_recalcs, io->ic_csum_recalc_cnt, seg_count);
		if (buffer == NULL)
	...
	while (i <= lgc_seg->ls_idx_end) {
		...
		if (mw->mw_csum_type) {
			csum_widen_biov(&bsgl.bs_iovs[biov_idx], phy_ent, &ext, ent_in->ei_inob,
					phy_lo);
...
fill_segments(daos_handle_t ih, struct vos_agg_param *agg_param, unsigned int *acts)
{
	...
	for (i = 0; i < io->ic_seg_cnt; i++) {
		...
		D_DEBUG(DB_EPC, "Fill segment: %u-%u " DF_RECT "\n", lgc_seg->ls_idx_start,
			lgc_seg->ls_idx_end, DP_RECT(&lgc_seg->ls_ent_in.ei_rect));
...
insert_segments(daos_handle_t ih, struct agg_merge_window *mw, bool last, unsigned int *acts)
{
	...
	d_list_for_each_entry_safe(phy_ent, tmp, &mw->mw_phy_ents, pe_link) {
		...
		if (rect.rc_ex.ex_hi <= mw->mw_ext.ex_hi || phy_ent->pe_remove) {
		...
		phy_ent->pe_off        = rect.rc_ex.ex_lo - phy_ent->pe_rect.rc_ex.ex_lo;
...
need_merge(daos_handle_t ih, uint16_t src_media, int lgc_cnt, daos_size_t seg_size)
{
	...
	tgt_media = vos_policy_media_select(vos_obj2pool(obj), DAOS_IOD_ARRAY, seg_size,
					    VOS_IOS_AGGREGATION);
...
need_flush(daos_handle_t ih, struct vos_agg_param *agg_param, bool last)
{
	...
	for (i = 0; i < mw->mw_lgc_cnt; i++) {
		...
		if (lgc_ext.ex_lo != phy_ext.ex_lo || lgc_ext.ex_hi != phy_ext.ex_hi)
			return true;
...
static int
flush_merge_window(daos_handle_t ih, struct vos_agg_param *agg_param, bool last, unsigned int *acts)
{
	struct agg_merge_window *mw = &agg_param->ap_window;
	int                      rc;
	...
	D_DEBUG(DB_IO, "Flush to merge to window " DF_EXT "\n", DP_EXT(&mw->mw_ext));
...
trigger_flush(struct agg_merge_window *mw, struct evt_extent *lgc_ext)
{
	...
	if (merge_window_status(mw) == MW_CLOSED || merge_window_status(mw) == MW_FLUSHED)
		return false;
...
enqueue_phy_ent(struct agg_merge_window *mw, struct evt_extent *phy_ext,
		const vos_iter_entry_t *entry, bio_addr_t *addr, struct dcs_csum_info *csum_info,
		uint32_t ver)
{
	struct agg_phy_ent *phy_ent;
	...
	if (!d_list_empty(&mw->mw_phy_ents)) {
		...
		prev = d_list_entry(mw->mw_phy_ents.prev, struct agg_phy_ent, pe_link);
		...
		struct agg_phy_ent *phy_ent)
{
	...
	D_DEBUG(DB_EPC,
		"lgc_ext:" DF_EXT ", phy_ext:" DF_RECT ", mw:" DF_EXT ", "
		"index:%u\n",
		DP_EXT(lgc_ext), DP_RECT(&phy_ent->pe_rect), DP_EXT(&mw->mw_ext), cnt);
...
static int
join_merge_window(daos_handle_t ih, struct vos_agg_param *agg_param, vos_iter_entry_t *entry,
		  unsigned int *acts)
{
	struct vos_obj_iter     *oiter = vos_hdl2oiter(ih);
	struct agg_merge_window *mw    = &agg_param->ap_window;
	...
	if (phy_ent == NULL) {
		...
		phy_ent = enqueue_phy_ent(mw, &phy_ext, entry, &entry->ie_biov.bi_addr,
	...
	if (visible) {
		...
		if (rc) {
			D_ERROR("Enqueue lgc_ent win: " DF_EXT ", ent:" DF_EXT " "
				"error: " DF_RC "\n",
				DP_EXT(&mw->mw_ext), DP_EXT(&lgc_ext), DP_RC(rc));
			return rc;
		}
	} else {
...
set_window_size(struct agg_merge_window *mw, vos_iter_entry_t *entry)
{
	...
	if (rsize == 0) {
		D_DEBUG(DB_IO, "EV tree 0 iod_size could be caused by "
	...
	if (mw->mw_rsize == 0) {
		...
		if (DAOS_FAIL_CHECK(DAOS_VOS_AGG_MW_THRESH)) {
			...
			mw->mw_flush_thresh = (rsize < VOS_MW_FLUSH_THRESH) ? rsize * 2 : rsize;
...
static int
vos_agg_ev(daos_handle_t ih, vos_iter_entry_t *entry, struct vos_agg_param *agg_param,
	   unsigned int *acts)
{
	struct agg_merge_window *mw = &agg_param->ap_window;
	struct evt_extent        phy_ext, lgc_ext;
	...
	/* Aggregation */
	D_DEBUG(DB_EPC,
		"oid:" DF_UOID ", lgc_ext:" DF_EXT ", "
		"phy_ext:" DF_EXT ", epoch:" DF_X64 ".%d, flags: %x(%c)\n",
		DP_UOID(agg_param->ap_oid), DP_EXT(&lgc_ext), DP_EXT(&phy_ext), entry->ie_epoch,
		entry->ie_minor_epc, entry->ie_vis_flags, evt_vis2dbg(entry->ie_vis_flags));
	...
	if (rc)
		D_CDEBUG(rc == -DER_TX_RESTART || rc == -DER_TX_BUSY || rc == -DER_NOSPACE, DB_IO,
			 DLOG_ERR, "Join window " DF_EXT "/" DF_EXT " error: " DF_RC "\n",
			 DP_EXT(&mw->mw_ext), DP_EXT(&phy_ext), DP_RC(rc));
out:
	if (rc)
		close_merge_window(mw, rc);
...
static int
vos_aggregate_pre_cb(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
		     vos_iter_param_t *param, void *cb_arg, unsigned int *acts)
{
	...
	D_DEBUG(DB_EPC, DF_CONT ": Aggregate pre, type:%d, is_discard:%d\n",
		DP_CONT(cont->vc_pool->vp_id, cont->vc_id), type, agg_param->ap_discard);
...
static int
vos_aggregate_post_cb(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
		      vos_iter_param_t *param, void *cb_arg, unsigned int *acts)
{
	struct vos_agg_param *agg_param = cb_arg;
	struct vos_container *cont;
	...
	if (rc > 0) {
		...
		if (rc == -DER_TX_BUSY) {
			...
			switch (type) {
			default:
				D_ASSERTF(type == VOS_ITER_OBJ, "Invalid iter type\n");
				break;
			case VOS_ITER_AKEY:
				agg_param->ap_skip_dkey = true;
				/* fall through */
	    ...
	    int (*yield_func)(void *arg), void *yield_arg)
{
	...
	D_ASSERT(epr != NULL);
	D_ASSERTF(epr->epr_lo <= epr->epr_hi, "epr_lo:" DF_U64 ", epr_hi:" DF_U64 "\n", epr->epr_lo,
		  epr->epr_hi);
