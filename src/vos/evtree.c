...
evt_desc_free_all(struct evt_context *tcx, struct evt_node_entry *ne)
{
	...
	rc = vos_dtx_degister_record(evt_umm(tcx), desc->dc_dtx,
...
saved_rect_is_greater(struct evt_max_rect *saved, struct evt_entry *ent)
{
	...
	V_TRACE(DB_TRACE, "Comparing saved "DF_RECT" to "DF_ENT"\n",
...
evt_get_size(daos_handle_t toh, daos_epoch_t epoch, daos_size_t *size)
{
	...
	*size = 0;
	...
	V_TRACE(DB_TRACE, "Finding evt range at epoch "DF_U64"\n", epoch);
...
try_again:
	V_TRACE(DB_TRACE, "Scanning for maximum in "DF_RECT"\n",
		DP_RECT(&rect));
	...
	while (1) {
		...
		V_TRACE(DB_TRACE, "Checking mbr="DF_RECT", l=%d, a=%d\n",
			DP_RECT(evt_node_mbr_get(tcx, node)), level, at);
		...
		for (i = at; i < node->tn_nr; i++) {
			...
			rtmp = evt_node_rect_at(tcx, node, i);
			V_TRACE(DB_TRACE, "Checking rect[%d]="DF_RECT"\n",
				i, DP_RECT(rtmp));
			...
			/* Including the -DER_INPROGRESS case, then the call
			 * will retry with leader.
			 */
			if (rc < 0)
				return rc;
			...
			V_TRACE(DB_TRACE, "New saved rectangle "DF_RECT
				" punched? : %s\n",
...
again:
	...
	while (1) {
		...
		/* Skip invisible/uncertain record and error cases */
		...
		uint64_t in_off, const struct evt_entry_in *ent)
{
	...
	for (i = 0; i < nd->tn_nr; i++) {
		...
		desc = evt_off2desc(tcx, ne->ne_child);
		rc = evt_dtx_check_visibility(tcx, desc, DAOS_INTENT_PROBE);
		if (rc != 0) {
			nr = nd->tn_nr - i;
			memmove(ne + 1, ne, nr * sizeof(*ne));
		} else {
