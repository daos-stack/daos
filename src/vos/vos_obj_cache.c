...
check_object:
	...
	if (ts_set != NULL) {
		if (ts_set->ts_flags & VOS_COND_UPDATE_OP_MASK)
			cond_mask = VOS_ILOG_COND_UPDATE;
		else if (ts_set->ts_flags & VOS_COND_INSERT_OP_MASK)
			cond_mask = VOS_ILOG_COND_INSERT;
	}
	...
	if (rc == -DER_TX_RESTART)
		goto failed;
	if (rc == -DER_NONEXIST && cond_mask == VOS_ILOG_COND_UPDATE)
		goto out;
	if (rc != 0) {
