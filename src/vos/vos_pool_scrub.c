...
{
	return daos_csummer_get_rec_chunksize(sc_csummer(ctx), ctx->sc_iod.iod_size);
}
...
sc_get_ms_between_scrubs(struct scrub_ctx *ctx)
{
	...
	return get_ms_between_periods(ctx->sc_pool_start_scrub, now,
				      ctx->sc_pool->sp_scrub_freq_sec, ctx->sc_pool_last_csum_calcs,
...
sc_raise_ras(struct scrub_ctx *ctx)
{
	if (ds_notify_ras_event != NULL) {
		ds_notify_ras_event(RAS_POOL_CORRUPTION_DETECTED, "Data corruption detected",
...
{
	return vos_iter_process(ctx->sc_vos_iter_handle, VOS_ITER_PROC_OP_MARK_CORRUPT, NULL);
}
...
sc_verify_recx(struct scrub_ctx *ctx, d_iov_t *data)
{
	...
	for (i = 0; i < csum_nr; i++) {
		...
		d_iov_set(&chunk_iov, data->iov_buf + processed_bytes, rec_in_chunk * rec_len);
...
sc_verify_obj_value(struct scrub_ctx *ctx, struct bio_iov *biov, daos_handle_t ih)
{
	 ...
	 * calculation
	 */
	data_len = iod->iod_type == DAOS_IOD_ARRAY ? iod->iod_recxs[0].rx_nr * iod->iod_size
						   : iod->iod_size;
	...
	rc =
	    iod->iod_type == DAOS_IOD_ARRAY ? sc_verify_recx(ctx, &data) : sc_verify_sv(ctx, &data);
...
static void
sc_obj_val_setup(struct scrub_ctx *ctx, vos_iter_entry_t *entry, vos_iter_type_t type,
		 vos_iter_param_t *param, daos_handle_t ih)
{
	...
	ctx->sc_iod.iod_nr    = 1;
	ctx->sc_iod.iod_type  = type == VOS_ITER_RECX ? DAOS_IOD_ARRAY : DAOS_IOD_SINGLE;
...
static bool
sc_value_has_been_seen(struct scrub_ctx *ctx, vos_iter_entry_t *entry, vos_iter_type_t type)
{
	if (type == VOS_ITER_RECX && !recx_eq(ctx->sc_iod.iod_recxs, &entry->ie_recx))
		return false;
...
sc_scrub_cont(struct scrub_ctx *ctx)
{
	 ...
	 */
	rc = vos_iterate(&param, VOS_ITER_OBJ, true, &anchor, obj_iter_scrub_pre_cb, NULL, ctx,
			 NULL);
...
sc_cont_setup(struct scrub_ctx *ctx, vos_iter_entry_t *entry)
{
	...
	if (ctx->sc_cont_lookup_fn == NULL)
		return -DER_NOSYS;
	rc = ctx->sc_cont_lookup_fn(ctx->sc_pool_uuid, entry->ie_couuid, ctx->sc_sched_arg,
				    &ctx->sc_cont);
	if (rc != 0) {
...
/** vos_iter_cb_t */
static int
cont_iter_scrub_cb(daos_handle_t ih, vos_iter_entry_t *entry, vos_iter_type_t type,
		   vos_iter_param_t *param, void *cb_arg, unsigned int *acts)
{
	struct scrub_ctx *ctx = cb_arg;
	int               rc  = 0;
...
vos_scrub_pool(struct scrub_ctx *ctx)
{
	...
	param.ip_epr.epr_hi = DAOS_EPOCH_MAX;
	rc = vos_iterate(&param, VOS_ITER_COUUID, false, &anchor, NULL, cont_iter_scrub_cb, ctx,
			 NULL);
