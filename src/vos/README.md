...
It is possible to take into account the space allocated by the DAOS File System. An example file that reflects the current DFS implementation can be generated by running the below command:
```
[~/daos]$ vos_size_dfs_sample.py
  Using DAOS version: 1.1.0
  Output file: vos_dfs_sample.yaml
```

Then, we can execute vos_size.py to get a metadata estimate with the consideration of the space allocated by the DFS.
```
