...
static int
vos_tgt_preallocate(uuid_t uuid, daos_size_t scm_size, int tgt_id, const char *storage_path)
{
	char *newborns_path = NULL;
	char *path          = NULL;
	int   fd            = -1, rc;

	/** create the newborns_path string */
...
vos_tgt_preallocate_thrd_func(void *arg)
{
	...
	return (void *)(uintptr_t)vos_tgt_preallocate(tvpa->tvpa_uuid, tvpa->tvpa_scm_size,
						      tvpa->tvpa_tgt_id, tvpa->tvpa_storage_path);
}
			       ...
			       const char *storage_path)
{
	...
	for (i = 0; i < tgt_nr; i++) {
		rc = vos_tgt_preallocate(uuid, scm_size, i, storage_path);
		if (rc)
			break;
	}
	return rc;
			     ...
			     const char *storage_path, bind_cpu_fn_t bind_cpu_fn)
{
	...
	for (i = 0; i < tgt_nr; i++) {
		...
		entry->tvt_args.tvpa_storage_path = storage_path;
		      ...
		      const char *storage_path, bind_cpu_fn_t bind_cpu_fn)
{
	...
	rc = vos_path_gen(pool_uuid, storage_path, NULL, NULL, &pool_path);
	if (rc) {
	...
	rc = vos_path_gen(pool_uuid, newborns_path, NULL, NULL, &pool_newborns_path);
	if (rc) {
	...
	if (rdb_blob_sz) {
		rc = vos_path_gen(pool_uuid, newborns_path, RDB_FILE "pool", NULL, &rdb_path);
		if (rdb_path == NULL) {
