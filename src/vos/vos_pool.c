...
static int
vos_tgt_preallocate(uuid_t uuid, daos_size_t scm_size, int tgt_id, const char *newborns_path)
{
	char *path = NULL;
	int   fd   = -1, rc;

	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	rc = vos_path_gen(uuid, newborns_path, VOS_FILE, &tgt_id, &path);
	if (rc)
		goto out;
...
static void *
vos_tgt_preallocate_thrd_func(void *arg)
{
	...
	return (void *)(uintptr_t)vos_tgt_preallocate(tvpa->tvpa_uuid, tvpa->tvpa_scm_size,
						      tvpa->tvpa_tgt_id, tvpa->tvpa_newborns_path);
}

static void
vos_tgt_preallocate_thrds_cleanup(d_list_t *head)
{
			       ...
			       const char *newborns_path)
{
	...
	for (i = 0; i < tgt_nr; i++) {
		rc = vos_tgt_preallocate(uuid, scm_size, i, newborns_path);
		if (rc)
			break;
	}
	return rc;
			     ...
			     const char *newborns_path, bind_cpu_fn_t bind_cpu_fn)
{
	...
	for (i = 0; i < tgt_nr; i++) {
		...
		rc = pthread_create(&entry->tvt_tid, NULL, vos_tgt_preallocate_thrd_func,
				    &entry->tvt_args);
		if (rc) {
...
out:
	vos_tgt_preallocate_thrds_cleanup(&thrds_list_head);
	D_FREE(thrds_list);
	     ...
	     struct recreate_tgts_arg *recreate_arg)
{
	...
	rc = vos_path_gen(pool_uuid, recreate_arg->vos_storage_path, NULL, NULL, &pool_path);
	if (rc) {
	...
	rc = vos_path_gen(pool_uuid, recreate_arg->newborns_path, NULL, NULL, &pool_newborn_path);
	if (rc) {
		D_ERROR(DF_UUID ": vos_path_gen failed for: " DF_RC "\n", DP_UUID(pool_uuid),
			DP_RC(rc));
		goto out;
	}
	...
	/** create VOS files */
	rc = vos_tgt_preallocate_parallel(pool_uuid, scm_size, tgt_nr, &dummy_cancel_state,
					  recreate_arg->newborns_path, recreate_arg->bind_cpu_fn);
	if (rc) {
	...
	if (rdb_blob_sz) {
		rc = vos_path_gen(pool_uuid, recreate_arg->newborns_path, RDB_FILE "pool", NULL,
				  &rdb_path);
		if (rdb_path == NULL) {
...
vos_pool_recreate_tgts(struct recreate_tgts_arg *recreate_arg)
{
	...
	d_list_for_each_entry_safe(pool_info, tmp, &pool_list, spi_link) {
		...
		if ((pool_info->spi_blob_sz[SMD_DEV_TYPE_META] == 0) ||
			...
			if (recreate_arg->cleanup_newborns_fn)
				rc = recreate_arg->cleanup_newborns_fn(pool_info->spi_id, 0);
			if (rc) {
