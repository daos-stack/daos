...
akey_update(struct vos_io_context *ioc, uint32_t pm_ver, daos_handle_t ak_toh)
{
	...
	entry = vos_ts_set_get_entry(ioc->ic_ts_set);
	if (entry != NULL) {
		if (ioc->ic_epr.epr_hi <= entry->te_ts_rh) {
			/** TODO: Handle same transaction but for now, that
			 * shouldn't happen.   There is a read after this write
			 * so tell caller to try again.
			 */
			return -DER_AGAIN;
		}
	}
...
dkey_update(struct vos_io_context *ioc, uint32_t pm_ver, daos_key_t *dkey)
{
	...
	entry = vos_ts_set_get_entry(ioc->ic_ts_set);
	if (entry != NULL) {
		if (ioc->ic_epr.epr_hi <= entry->te_ts_rl) {
			/** TODO: Handle same transaction but for now, that
			 * shouldn't happen.   There is a read after this write
			 * so tell caller to try again.
			 */
...
update_read_timestamps(struct vos_io_context *ioc, int err)
{
	...
	if (ts_set->ts_flags & VOS_COND_DKEY_MASK)
		entry->te_ts_rl = MAX(entry->te_ts_rl, ioc->ic_epr.epr_hi);
	entry->te_ts_rh = MAX(entry->te_ts_rh, ioc->ic_epr.epr_hi);

	if ((ts_set->ts_flags & VOS_COND_AKEY_MASK) == 0)
		return;
	       ...
	       struct dtx_handle *dth)
{
	...
	/** TODO: Need to handle case where it's same transaction but this
	 *  should never happen before distributed transactions.
	 */
	if (entry && ioc->ic_epr.epr_hi <= entry->te_ts_rl) {
		err = -DER_AGAIN;
		goto out;
	}
