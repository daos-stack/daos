...
static const struct CMUnitTest iterator_tests[] = {
    {"VOS220: 100K update/fetch/verify test", io_multiple_dkey},
    {"VOS240.0: KV Iter tests (for dkey)", io_iter_test},
    {"VOS240.1: KV Iter tests with anchor (for dkey)", io_iter_test_with_anchor},
    {"VOS240.7: key2anchor iterator test", io_iter_test_key2anchor},
    {"VOS240.3: KV range Iteration tests (for dkey)", io_obj_forward_iter_test},
    {"VOS240.4: KV reverse range Iteration tests (for dkey)", io_obj_reverse_iter_test},
    {"VOS240.5 KV range iteration tests (for recx)", io_obj_forward_recx_iter_test},
    {"VOS240.6 KV reverse range iteration tests (for recx)", io_obj_reverse_recx_iter_test},
};

static const struct CMUnitTest io_tests[] = {
    {"VOS203: Simple update/fetch/verify test", io_simple_one_key},
    {"VOS204: Simple Punch test", io_simple_punch},
    {"VOS205: Simple near-epoch retrieval test", io_simple_near_epoch},
    {"VOS206: Simple scatter-gather list test, multiple update buffers", io_sgl_update},
    {"VOS207: Simple scatter-gather list test, multiple fetch buffers", io_sgl_fetch},
    {"VOS208: Extent hole test", io_fetch_hole},
    {"VOS220: 100K update/fetch/verify test", io_multiple_dkey},
    {"VOS222: overwrite test", io_idx_overwrite},
    {"VOS245.0: Object iter test (for oid)", oid_iter_test, oid_iter_test_setup},
    {"VOS245.1: Object iter test with anchor (for oid)", oid_iter_test_with_anchor,
     oid_iter_test_setup},
    {"VOS250.0: vos_iterate tests - Check single callback", vos_iterate_test},
    {
	"VOS280: Same Obj ID on two containers (obj_cache test)",
	io_simple_one_key_cross_container,
    },
    {"VOS281.0: Fetch from non existent object", io_fetch_no_exist_object},
    {"VOS281.1: Fetch from non existent object with zero-copy", io_fetch_no_exist_object_zc},
    {"VOS282.0: Fetch from non existent dkey", io_fetch_no_exist_dkey},
    {"VOS282.1: Fetch from non existent dkey with zero-copy", io_fetch_no_exist_dkey_zc},
    {"VOS282.2: Accessing pool, container with same UUID", pool_cont_same_uuid},
    {"VOS299: Space overflow negative error test", io_pool_overflow_test,
     io_pool_overflow_teardown},
};

static const struct CMUnitTest int_tests[] = {
    {"VOS201: VOS object IO index", io_oi_test},
    {"VOS202: VOS object cache test", io_obj_cache_test},
    {"VOS300.1: Test key query punch with subsequent update", io_query_key_punch_update},
    {"VOS300.2: Key query test", io_query_key},
    {"VOS300.3: Key query negative test", io_query_key_negative},
    {"VOS300.4: Return error on DMA buffer allocation failure", io_allocbuf_failure},
};
