...
#define assert_ms_eq(exp, duration, periods, curr, elapsed_ns)                                     \
	do {                                                                                       \
		...
		assert_int_equal(exp,                                                              \
				 get_ms_between_periods(start, elapsed, duration, periods, curr)); \
	} while (false)
 ...
 */
static void
setup_iod_type(daos_iod_t *iod, int iod_type, daos_size_t data_len, daos_recx_t *recx)
{
	int i;
...
sts_ctx_pool_init(struct sts_context *ctx)
{
	...
	rc = vos_pool_create(pmem_file, ctx->tsc_pool_uuid, 0, ctx->tsc_nvme_size, 0, &poh);
	assert_success(rc);
...
static int
get_cont_fn(uuid_t pool_uuid, uuid_t cont_uuid, void *arg, struct cont_scrub *cont)
{
...
sts_ctx_init(struct sts_context *ctx)
{
	...
	assert_success(daos_csummer_init_with_type(&test_csummer, HASH_TYPE_CRC16,
...
static int
sts_ctx_fetch(struct sts_context *ctx, int oid_lo, int iod_type, const char *dkey_str,
	      const char *akey_str, int epoch)
{
...
static void
sts_ctx_update(struct sts_context *ctx, int oid_lo, int iod_type, const char *dkey_str,
	       const char *akey_str, int epoch, bool corrupt_it)
{
	daos_unit_oid_t       oid = {0};
	daos_key_t            dkey;
	...
	rc = daos_csummer_calc_iods(ctx->tsc_csummer, &sgl, &iod, NULL, 1, false, NULL, 0,
				    &iod_csum);
	assert_success(rc);
	if (corrupt_it) {
		...
		if (iod.iod_type == DAOS_IOD_ARRAY) {
			/** corrupt last record */
			idx_to_corrupt = (iod.iod_recxs->rx_nr - 1) * iod.iod_size;
		}
...
static void
sts_ctx_punch_dkey(struct sts_context *ctx, int oid_lo, const char *dkey_str, int epoch)
{
	daos_unit_oid_t oid = {0};
	daos_key_t      dkey;
	int             rc;
	...
	iov_alloc_str(&dkey, dkey_str);
	rc = vos_obj_punch(ctx->tsc_coh, oid, epoch, 0, 0, &dkey, 0, NULL, NULL);
	assert_success(rc);
...
dkey_deleted_by_aggregation_with_multiple_extents(void **state)
{
	...
	sts_ctx_update(ctx, 1, TEST_IOD_ARRAY_4, "dkey", "akey", 1, false);
...
static const struct CMUnitTest scrubbing_tests[] = {
    ...
    TS("CSUM_SCRUBBING_00: Only scrub when idle", lazy_scrubbing_only_when_idle),
    TS("CSUM_SCRUBBING_01: SV with no corruption", scrubbing_with_no_corruption_sv),
    TS("CSUM_SCRUBBING_02: Array with no corruption", scrubbing_with_no_corruption_array),
    TS("CSUM_SCRUBBING_03: A single value corrupted", scrubbing_with_sv_corrupted),
    TS("CSUM_SCRUBBING_04: A corrupted extent value", corrupted_extent),
    TS("CSUM_SCRUBBING_05: Multiple corrupted extent values", scrubbing_with_arrays_corrupted),
    TS("CSUM_SCRUBBING_06: Scrub multiple epochs", scrub_multiple_epochs),
    TS("CSUM_SCRUBBING_06: Multiple keys, some corrupted values", scrubbing_with_multiple_akeys),
    ...
    TS("CSUM_SCRUBBING_09.3: Scrubbing has reference to container so "
       ...
       container_deleted),
    TS("CSUM_SCRUBBING_10: Scrubbing multiple objects", multiple_objects),
    TS("CSUM_SCRUBBING_11: When threshold is 0, no drain happens", no_drain_target),
    TS("CSUM_SCRUBBING_12: Scrubbing multiple overlapping extents", multiple_overlapping_extents),
    TS("CSUM_SCRUBBING_13: Evict pool target when threshold is exceeded", drain_target),
};
...
run_scrubbing_tests(int argc, char *argv[])
{
	...
	rc +=
	    cmocka_run_group_tests_name("Storage and retrieval of checksums for Single Value Type",
