...
pool_ref_count_test(void **state)
{
	...
	uuid_generate(uuid);
	ret = vos_pool_create(arg->fname[0], uuid, VPOOL_256M, 0, 0 /* qlc_size */, 0,
			      0 /* version */, NULL);
	for (i = 0; i < num; i++) {
		...
		for (i = 0; i < arg->seq_cnt[j]; i++) {
			...
			case CREAT:
				...
				if (arg->fcreate[j]) {
					...
					assert_int_equal(ret, 0);
					ret = vos_pool_create_ex(
					    arg->fname[j], arg->uuid[j], 0, 0, VPOOL_TEST_WAL_SZ,
...
pool_open_excl_test(void **state)
{
	...
	ret = vos_pool_create(arg->fname[0], uuid, VPOOL_256M, 0, 0, 0 /* qlc_size */,
			      0 /* version */, &arg->poh[0]);
	assert_rc_equal(ret, 0);
	...
	print_message("open EXCL shall fail upon existing EXCL create "
		      "opener\n");
	ret = vos_pool_create(arg->fname[0], uuid, VPOOL_256M, 0, 0 /* qlc_size */, VOS_POF_EXCL,
			      0 /* version */, &arg->poh[0]);
	assert_rc_equal(ret, 0);
	ret = vos_pool_open(arg->fname[0], uuid, VOS_POF_EXCL, &arg->poh[1]);
	assert_rc_equal(ret, -DER_BUSY);
