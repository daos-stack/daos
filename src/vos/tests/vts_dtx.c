		    ...
		    uint64_t *dkey_hash, uint64_t *epoch, bool ext)
{
	...
	args->ofeat = DAOS_OT_DKEY_UINT64 | DAOS_OT_AKEY_UINT64;

	*epoch = crt_hlc_get();

	vts_key_gen(dkey_buf, args->dkey_size, true, args);
	set_iov(dkey, dkey_buf, args->ofeat & DAOS_OT_DKEY_UINT64);
	*dkey_hash = d_hash_murmur64((const unsigned char *)dkey_buf,
	...
	rex->rx_idx = hash_key(dkey_iov, args->ofeat & DAOS_OT_DKEY_UINT64);

	vts_key_gen(akey_buf, args->akey_size, false, args);
	set_iov(akey, akey_buf, args->ofeat & DAOS_OT_AKEY_UINT64);
