...
static int
fake_tx_status_get(struct umem_instance *umm, uint16_t tx_id,
		   daos_epoch_t epoch, uint32_t intent, void *args)
{
	...
	found = lrua_lookupx(array, tx_id - 2, epoch, (void **)&entry);
...
static int
fake_tx_is_same_tx(struct umem_instance *umm, uint16_t tx_id,
...
static int
fake_tx_log_add(struct umem_instance *umm, umem_off_t offset, uint16_t *tx_id,
		daos_epoch_t epoch, void *args)
{
	...
	entry = lrua_allocx(array, &idx, epoch, false);
	assert_non_null(entry);
	...
	entry->tx_id = current_tx_id.id_tx_id = *tx_id = idx + 2;
...
static int
fake_tx_log_del(struct umem_instance *umm, umem_off_t offset, uint16_t tx_id,
		daos_epoch_t epoch, bool deregister, void *args)
{
	...
	lrua_evictx(array, tx_id - 2, epoch);
	return 0;
}
...
entries_init(struct entries *entries)
{
	...
	rc = lrua_array_alloc(&entries->array, DTX_ARRAY_LEN,
			      sizeof(struct fake_tx_entry), &cbs,
			      NULL);
	if (rc != 0)
