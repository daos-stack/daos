...
static const struct CMUnitTest punch_model_tests_pmdk[] = {
    {.name = "VOS860: Conditionals test", .test_func = cond_test},
    {.name = "VOS861: Multiple oid cond test", .test_func = multiple_oid_cond_test},
    {.name      = "VOS862: Punch while other akey is inprogress",
     .test_func = test_inprogress_parent_punch},
    {.name = "VOS863: Multikey conditionals", .test_func = test_multiple_key_conditionals},
    {.name      = "VOS864: Multikey conditionals with tx",
     .test_func = test_multiple_key_conditionals_tx},
    {.name = "VOS865: Many transactions", .test_func = many_tx},
    {.name = "VOS866: Uncommitted parent punch", .test_func = uncommitted_parent},
    {
	.name      = "VOS867: Aggregate committed key punch with subsequent in-flight update",
	.test_func = test_uncommitted_key,
    },
};

static const struct CMUnitTest punch_model_tests_all[] = {
    {.name          = "VOS800: VOS punch model array set/get size",
     .test_func     = array_set_get_size,
     .setup_func    = pm_setup,
     .teardown_func = pm_teardown},
    {.name          = "VOS801: VOS punch model array read/write/punch int32_t",
     .test_func     = array_1,
     .setup_func    = pm_setup,
     .teardown_func = pm_teardown},
    {.name          = "VOS802: VOS punch model array read/write/punch char",
     .test_func     = array_2,
     .setup_func    = pm_setup,
     .teardown_func = pm_teardown},
    {.name          = "VOS803: VOS punch model array read/write/punch int32_t static",
     .test_func     = array_3,
     .setup_func    = pm_setup,
     .teardown_func = pm_teardown},
    {.name          = "VOS804: VOS punch model array read/write/punch char static",
     .test_func     = array_4,
     .setup_func    = pm_setup,
     .teardown_func = pm_teardown},
    {.name = "VOS805: Simple punch model test", .test_func = punch_model_test},
    {.name = "VOS806: Multi update", .test_func = simple_multi_update},
    {.name          = "VOS807: Array Set/get size, write, punch",
     .test_func     = array_size_write,
     .setup_func    = pm_setup,
     .teardown_func = pm_teardown},
    {.name = "VOS808: Object punch and fetch", .test_func = object_punch_and_fetch},
    {.name = "VOS809: SGL test", .test_func = sgl_test},
    {.name = "VOS810: Small SGL test", .test_func = small_sgl},
    {.name = "VOS811: Test vos_obj_array_remove", .test_func = remove_test},
    {.name = "VOS812: Minor epoch punch sv", .test_func = minor_epoch_punch_sv},
    {.name = "VOS813: Minor epoch punch array", .test_func = minor_epoch_punch_array},
    {
	.name      = "VOS814: Minor epoch punch rebuild",
	.test_func = minor_epoch_punch_rebuild,
    },
    {.name = "VOS815: Many keys in one tree", .test_func = many_keys},
    {.name = "VOS816: Simulate EC array size", .test_func = ec_size},
};
