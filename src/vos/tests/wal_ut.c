...
ut_mc_init(struct bio_ut_args *args, uint64_t meta_sz, uint64_t wal_sz, uint64_t data_sz)
{
	...
	rc = bio_mc_create(args->bua_xs_ctxt, args->bua_pool_id, meta_sz, wal_sz, data_sz,
			   0 /* bulk_sz */, 0);
	if (rc) {
...
wal_ut_single(void **state)
{
	...
	rc = bio_wal_reserve(args->bua_mc, &tx->utx_id, NULL);
	assert_rc_equal(rc, 0);

	rc = bio_wal_commit(args->bua_mc, tx, NULL, NULL);
	assert_rc_equal(rc, 0);
