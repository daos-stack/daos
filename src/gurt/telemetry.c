		   ...
		   int opt_fields, FILE *stream)
{
	...
	if (format == D_TM_CSV) {
		fprintf(stream, "%s", name);
		if (opt_fields & D_TM_INCLUDE_TYPE)
			fprintf(stream, ",arena,ordblks,uordblks,fordblks,keepcost\n");
	} else {
		if (opt_fields & D_TM_INCLUDE_TYPE)
			fprintf(stream, "type: arena,ordblks,uordblks,fordblks,keepcost\n");
	}
	fprintf(stream, ",%d,%d,%d,%d,%d", meminfo->arena, meminfo->ordblks, meminfo->uordblks,
		meminfo->fordblks, meminfo->keepcost);
}
...
d_tm_record_meminfo(struct d_tm_node_t *metric)
{
	...
	metric->dtn_metric->dtm_data.meminfo.keepcost = mi.keepcost;
	d_tm_node_unlock(metric);
}
