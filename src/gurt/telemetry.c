...
static void
d_tm_print_meminfo(struct d_tm_meminfo_t *meminfo, char *name, int format, int opt_fields,
		   FILE *stream)
{
	...
	if (format == D_TM_CSV) {
		...
		fprintf(stream, "%s:%zu,%zu,%zu,%zu", name, meminfo->arena, meminfo->ordblks,
			meminfo->uordblks, meminfo->fordblks);
	}
}
...
static int
d_tm_get_meminfo(struct d_tm_context *ctx, struct d_tm_meminfo_t *meminfo, struct d_tm_node_t *node)
{
	struct d_tm_metric_t  *metric_data = NULL;
	struct d_tm_shmem_hdr *shmem       = NULL;
	int                    rc;
