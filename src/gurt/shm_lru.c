				...
				shm_lru_node_t ** new_node)
{
	...
	*new_node = NULL;
	...
	if (key_size <= sizeof(long int)) {
		...
		node->key = cache->off_keylist + idx_node * (long int)key_size;
	...
	if (data_size <= sizeof(long int)) {
		...
		node->data = cache->off_datalist + idx_node * (long int)data_size;
...
lru_remove_near_tail(shm_lru_cache_t *cache)
{
	...
	/* free dynamically allocated key/data space */
	if (node->key_size > LRU_ALLOC_SIZE_THRESHOLD || (cache->key_size == 0 &&
							  node->key_size > sizeof(long int)))
