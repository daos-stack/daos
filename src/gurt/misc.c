...
dm_free(void *ptr)
{
	...
	D_ASSERTF(hdr->mh_magic == DM_ALLOC_MAGIC, "magic "DF_X64"\n", hdr->mh_magic);
	//mtc->mtc_size -= hdr->mh_size;
	//mtc->mtc_count--;
	D_ASSERT(hdr->mh_tail != NULL);
	D_ASSERTF(((struct dm_tailer *)hdr->mh_tail)->mt_magic == DM_ALLOC_MAGIC, "magic "DF_X64"\n",
		  ((struct dm_tailer *)hdr->mh_tail)->mt_magic);
...
//		dm_tls_register(tag, mtc);
	...
	if (alignment == 0) {
		size_t alloc_size = roundup(hdr_size + size, 8);

		buf = malloc(alloc_size + sizeof(struct dm_tailer));
		if (!buf)
			return NULL;

		hdr = buf;
		hdr->mh_alignment = 0;
		hdr->mh_tail = buf + alloc_size;
		((struct dm_tailer *)hdr->mh_tail)->mt_magic = DM_ALLOC_MAGIC;
	} else {
		int	nob = alignment;
		...
		buf = aligned_alloc(alignment, roundup(alignment + size, 8) + sizeof(struct dm_tailer));
		if (!buf)
			return NULL;

		hdr = buf + alignment - hdr_size;
		hdr->mh_alignment = alignment;
		hdr->mh_tail = buf + roundup(alignment + size, 8);
		((struct dm_tailer *)hdr->mh_tail)->mt_magic = DM_ALLOC_MAGIC;
	}
