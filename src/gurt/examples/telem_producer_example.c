...
void test_function1(int count)
{
	 ...
	 */
	for (i = 0; i < count - 1; i++) {
		rc = d_tm_increment_counter(&loop, false, __FILE__, __func__,
					    "loop counter", NULL);
		if (rc != D_TM_SUCCESS) {
...
void test_open_handle(void)
{
	 ...
	 * example.
	 */
	rc = d_tm_increment_gauge(&num_open_handles, false, 1, __FILE__,
				  "open handles", NULL);
	if (rc != D_TM_SUCCESS)
...
void test_close_handle(void)
{
	 ...
	 */
	rc = d_tm_decrement_gauge(&num_open_handles, false, 1, __FILE__,
				  "open handles", NULL);
	if (rc != D_TM_SUCCESS)
...
void timer_snapshot(void)
{
	...
	rc = d_tm_take_timer_snapshot(&t1, false, D_TM_CLOCK_REALTIME, __FILE__,
	...
	rc = d_tm_take_timer_snapshot(&t2, false, D_TM_CLOCK_REALTIME, __FILE__,
	...
	rc = d_tm_take_timer_snapshot(&t3, false, D_TM_CLOCK_REALTIME, __FILE__,
	...
	rc = d_tm_take_timer_snapshot(&t4, false, D_TM_CLOCK_REALTIME, __FILE__,
	...
	rc = d_tm_take_timer_snapshot(&t5, false, D_TM_CLOCK_PROCESS_CPUTIME,
				      __FILE__, __func__, "snapshot 5", NULL);
	if (rc != D_TM_SUCCESS)
		printf("d_tm_take_timer_snapshot failed, rc = %d\n", rc);

	/** This is how to specify a high resolution thread CPU timer */
	rc = d_tm_take_timer_snapshot(&t6, false, D_TM_CLOCK_THREAD_CPUTIME,
...
struct d_tm_nodeList_t *add_metrics_manually(void)
{
	...
	rc = d_tm_add_metric(&counter1, false, path, D_TM_COUNTER,
	...
	rc = d_tm_add_metric(&counter2, false, path, D_TM_COUNTER,
...
main(int argc, char **argv)
{
	 ...
	 * value 1.
	 */
	rc = d_tm_increment_counter(&entry, false, __FILE__, __func__,
				    "sample counter", NULL);
	if (rc != D_TM_SUCCESS) {
		printf("d_tm_increment_counter failed, rc = %d\n", rc);
		goto failure;
